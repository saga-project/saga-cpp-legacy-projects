...
% Added copyright permission stuff
\CopyrightYear{2010}
...
% part defined here}\yyenote{definition moved up to 3.2}
...
\end{center}
\caption{
  Schematic illustrating the interaction of BlackOil, EnKF, S2S
  and Lazarus. Typically, sensor data is collected from field
  sensors to a sensor platform and streamed to a data spool (1). The
  available data is queried by Lazarus to check if new data is available (2).
  If new data is available, another ensemble of simulations must be launched. Alternatively,
  if no new data is available, Lazarus will ``sleep'' for 1 second and query the data
  spool again. Once all the ensemble members successfully terminate, the data is collected
  and the EnKF application is run (4). Control of the workflow is returned to Lazarus
  (5) and the cycle begins again.
%   Schematic illustrating the interaction of BlackOil, EnKF, S2S
%   and Lazarus. Assume that  we perform an analysis step every $15$ simulation days. If we have
...
%   platform. 
% \jhanote{This is another diagram I'm not comfortable
%     with}
}
...
%  With a
% data--spool we can adjust the frequency of EnKF analysis
% without losing any data from the reservoir. Updates to the models can
% be as immediate as the moment data is broadcast from the sensors in
% the field, or as late as when the reservoir is being abandoned. With
% immediate model updates, we can run forecast studies. 
%  \jhanote{I find
%   the above paragraph slightly out of place. The flow is somewhat
%   abrupt too}
...
%    \jhanote{And does this label deserve no
