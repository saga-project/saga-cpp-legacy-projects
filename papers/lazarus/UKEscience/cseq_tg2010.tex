...
\footnote{RE: Replica Exchange} simulations, viz., restricted set of
platforms have been used and consequently, only small physical systems
have been investigated so far. We have developed a general-purpose
framework (Lazarus~\cite{gmac}) that can overcome traditional
...
distributed environments~\cite{saga-royalsoc}.  \jhanote{Lazarus is
  part defined here} Interestingly, the same framework can be used to
...
\caption{Architecture of Lazarus -- a Framework for Developing
  Autonomic Computational Science Applications. \jhanote{Lazarus is
    part defined here} The ``Resource
  ...
  happens that the size of each sub-job is 16). \jhanote{1 year and we
    haven't been able to improve on this figure... I really don't know
    what to say.  Poor quality figures are a sure way to get the paper
    killed. Compare this figure and its quality/effectiveness to that
    of Bigjob architecture. It tells me a lot about how this work is
    prioritised!} \yyenote{Addressed} }
...
\jhanote{We haven't really said what Lazarus is.. nor discussed the
  figure}\yyenote{Actually it is mentioned in the first paragraph of 3.3 and given a reference}
...
(Ref.~\ref{fig:SensorRelay}). The EnKF application queries the data
...
\jhanote{Bad sentence..} \yyenote{Fixed}Initially developed using Cactus and
...
to use the SAGA advert service~\cite{saga_sc05}, which would remove the need to access
MySQL in Lazarus.
...
\caption{Schematic illustrating the interaction of BlackOil, EnKF, S2S
  ...
  data. \jhanote{Once again, I find this figure of poor quality and
    highly damaging to the paper.} }
\label{fig:irregular_execution}
\end{figure*}

\subsection{Putting the Components together: } The use of an
...
We defined the workload used for our experiments and performance tests, and define \tc as the time in seconds it 
takes to complete this workload. The workload is a reduced version of a typical history matching run. We start 
with an ensemble of one hundred members and we need to perform five stages/Kalman-filter runs. Since this sample 
workload is representative of the size of a typical large history matching run and a reduced form of its 
duration, any consistent reduction in the total time to completion will eventually have a larger impact on larger 
runs with more stages.
...
\caption[Simulation Output]{The saturation (relative proportion) of
  CO$_2$ for the reservoir characterised in Fig 6. \jhanote{This is
    Figure 6!}}
\label{}
\end{figure}
