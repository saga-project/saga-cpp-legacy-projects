...
assimilation with observational data collected every month or 30
days. With 10 iterations per day and 30 days in a month, each model
will require 2.5 hours on 16 cores. With 100 ensemble members, this
equates to 250 hours using 16 cores running one ensemble at a time, or
2.5 hours on a job of 1600 cores with all ensembles running
concurrently (the details of which will be presented later)

% \jhanote{don't toggle between hours-days-years}\yyenote{Best I can do, 
% want to give an estimate in days of wall clock time and not hours}
...
a very long time to solution and futher optimization both required and
possible.
...
Kalman filter ~\cite{JiaLi}. This being the case, whenever computational power
...
problems \cite{Evensen2003}. However, many challenges remain,
...
The EnKF we use is based on Cactus and PetSc\jhanote{reference please}
--it computes the Kalman gain matrix and updates the model parameters
of the ensembles. The Kalman filter requires live production data from
the reservoir for it to update the reservoir models in real-time, and
launch the subsequent long-term forecast, enhanced oil recovery and
CO$_2$ sequestration studies
...
\caption[Simulation Output]{Simulation output of the injection of
  CO$_2$ as a function of time. The output for 10  different models are
