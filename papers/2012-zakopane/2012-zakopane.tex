...
\label{sec:intro}
 ...
 Our project (as described below) has reached a point were we pause
 and try to look ahead almost as frequently as we glance backward on
 where we came from -- as by now we really begin to understand what we
 actually try to achieve.  This paper is a summary of that process,
 and of our consolidation as a project.

 Our project, 'The SAGA Project', is loosely defined as a set of
 ...
 development frameworks, based on those API implementations.  In the
 following, we will shortly recapture the history and current state of
 those three efforts (sec.~\ref{sec:retro}).  We will further discuss
 our external boundary conditions for our project evolution, such as
 the evolution of distributed systems and applications
 (sec.~\ref{sec:state}).   Finally attempt in sec.~\ref{sec:road} to
 draft our way over the decade to come, which we hope will allow us to
 move forward with a clearer aim than we managed so far.
...
%\subsection{SAGA: Abstractions for Distributed Programming and
  ...
  I must be said that over the last 10 years, the work on the SAGA
