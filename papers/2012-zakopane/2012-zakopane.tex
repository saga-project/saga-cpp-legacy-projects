...
\label{sec:retro}
 ...
 extrinsic influences on the SAGA API evolution, and present some
 ...
 \subsection{Production Distributed Cyber Infrastructures}
  ...
  first-class design objectives option have emerged.  Interestingly,
  over the last years, SAGA has been used more to develop (program)
...
\label{sec:state}
 ...
 \subsection{The SAGA API Scope, Implementation, Community}
  ...
  implementation, and the (slowly) increasing community of contributers
  ...
  \subsubsection{Implementation}
   ...
   % an \textit{advert service} API (which is not covered by GFD.90) are
   % just a few of the striking similarities.
   ...
   community.  This is why development efforts have mostly been focusing
   on these functionalities in respect to middleware adaptor
   development, as is the rising deployment of SAGA C++ across national
   and international cyber-infrastructure.  The remaining functionality
   of SAGA C++ has mostly been ignored in a general application context.
   Generally, the existing C++ implementation has shown to be very
   robust and performant -- existing performance bottlenecks are either
   caused by middleware adaptors, or by the lack of support for
   asynchronous API modes.  
   ...
   uptake has been minimal so far, and is mostly limited by the uptake
   of the Java implementations -- unfortunately, the Java
