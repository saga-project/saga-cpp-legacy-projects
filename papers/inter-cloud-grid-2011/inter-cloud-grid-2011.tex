...
current release of the Open Cloud Computing Interface is suitable to
serve many other models in addition to IaaS, including e.g. PaaS and
SaaS. \tmnote{REF occi-wg.org here}
...
\begin{itemize}
  \item The OCCI Core specification consist of a single document
    ...
    interacted with {\em renderings} and expanded through {\em
      extensions}.
  \item The OCCI HTTP Restful Rendering specification describs a
    particular rendering of the OCCI Core Model. Other renderings
    could interact with the same instance of the OCCI Core Model and
    be added easily. The HTTP Restful Rendering desribes a Web based
    RESTful API to interface with OCCI based Service Offerings.
  \item The OCCI Infrastructure is a extension specification which
    contains the definition of the extensions needed for the IaaS
...
specifications. Since OCCI is an RESTful Protocol or API is can
enhances from all the features the HTTP defines. This includes
mechanisms for authentication and authorization, Cache-mechnisms,
Content-Type definitions and dicovery capabilities.
...
both specifications are interoperable with each other. Whereas CMDI
can be used to create, retrieve, update and delete data elements in a
Cloud offering. CDMI, just like OCCI can be used to discover the
capabilites of the Service Offering. Meta-Data can be assigned to the
data elements using CDMI. Following the interaction of an CDMI
compatibel interface the same references to data elements can be used
when interacting with an OCCI compatible offering.
...
serialisation/rendering format is OVF.
