...
%   here.. for example equating applications and simulations!} 
RE
...
%with the ability to handle faults and heterogeneity, 
...
% deployment of a distributed, loosely-coupled application.  
...
% \jhanote{Andre L please let me know if you
...
\emph{BigJob} abstraction, which % in combination with the Glide-In mechanism,
...
into a larger job. 
%  \jhanote{I'm wondering if we should call BigJob
...
%   remainder of the paragraph focuses on adaptiveness.}  
...
% Although the tests we report here are still
...
% replica due to a node or network failure could cause intolerable levels
...
% components: the SAGA and Migol~\citep{schnorLuckow08}.  While
...
% \alnote{The scheduling and reliability sections partially already
%   described the solution.  I added the important points to
%   challenges. The description of our approach have been moved back to
%   the Implementation and Scheduling section.}
...
%           \caption{\footnotesize \bf RE-Manager Architecture: The core
%             of the framework, the RE-manager, orchestrates a set of
%             distributed replicas using SAGA \& Migol.  The
%             Replica-Agent is responsible for managing and monitoring
%             replicas on a single machine. Finally, the Migol
%             infrastructure ensures that the RE-Manager and the
%             Replica-Agent are monitored and recovered if necessary.
%             \jhanote{The caption of this figure need to changed to be
%               in harmony with the opening paragraph of ``RE-Manager
%               Architecture'', where this figure is referenced and
%               discussed. I.e., we talk about three components in that
%               paragraph, this caption should be structured accordingly
%               too.}  \alnote{Reworked caption to highlight this three
%               components.  I am not sure whether this is a good time
%               to introduce the GlideIn stuff. But, I decided to leave
%               this for the next subsection to focus initially on the
%               efficient implementation of RE} }
...
Migol/GRAM. 
% Migol ensures that the job description of
...
{\it \bf Deploying on Production Environments:} The RE framework has been successfully deployed on production
...
{\it \bf Patterns and Abstractions:} To understand the relationship between clustered job submissions,
...
% to support the BigJob and the Glide-In abstractions as well as provide
% an adaptive RE framework.
% 
...
% RE-Manager overall
{\it \bf Implementation:} The RE framework presented in section~\ref{sec:remd_impl} has been
...
% In particular, the Replica Agent has been extended
...
% The SAGA Glide-In framework provides the BigJob abstraction, which
% allows applications to allocate larger chunks of resources and to
% map these resources to a set of sub-jobs.
...
% of the RE-Manager. 
% The scheduler determines the number and size of all replicas and
...
{\it \bf Adaptive Replica Scheduling:} Distributed applications including RE simulations must be able to 
...
%   \subfloat[RE-Manager Scaling]{\includegraphics[width=0.48\textwidth]{performance/perf_re_numberresource_tc_64ex}}
%     \subfloat[Glide-In
...
%     observed significant performance improvements of up to 50\,\%.}
...
% To evaluate the different adaptivity 
...
In the first, the \emph{replica size} -- the number of MPI processes assigned
...
%A constant number of 16 replicas is used.
...
% the number of resources increases from two to three.
...
% Grids via the appropriate adaptors~\citep{saga_url}.
% % as SAGA is a community
% % specification and is soon to be standard~\citep{saga_url}.       

% LAMMPS~\citep{Plimpton:1995nx} supports replica-exchange
% natively, and has been extended to support
% distributed RE based on MPIg~\citep{repex_mpig}, which allows the
...
% arguably less general than our RE framework in that it requires
% changes to the source code; our implementation does not require any
% changes to the source code and NAMD can be easily replaced with
% another MD code.  Also, the use of MPIg, or any
% distributed computing generalization of MPI, requires the rigid
% co-allocation of all resources, i.\,e.\ in the absence of co-scheduler
% such as HARC, there is a under utilization of resources by requiring
% nodes to remain idle until all resources became available.  In
% contrast, our RE framework is able to adaptively use resources as they
% become available. However, we believe that where resources can be
% co-scheduled and where long or large systems are being simulated
% the approach outlined in Ref.~\citep{repex_mpig} is preferred.
...
% agile execution of RE simulations in dynamic environments has been
% developed. 
