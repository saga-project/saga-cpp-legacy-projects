...
\begin{abstract}{REMD, SAGA, Migol, Fault Tolerance}  
  ...
  % why our approach is unique.  SAGA~\citep{SAGA_Goodale06a} provides
...
% computers.  replica-exchange (RE) methods~\citep{SPdynamics,
...
architecture~\citep{ogf_cpr_arch}. % The SAGA-CPR package is a SAGA
% rendering of the GridCPR API.%, also defined in~\citep{ogf_cpr_arch}.
...
% highly scalable MD package, NAMD~\citep{Phillips:2005gd} is used for a
...
% strategy~\citep{Jeske:2007wj}.  For spawning of jobs on the selected
...
using this API.
% The package was inspired by the GridCPR~\citep{gridcpr} architecture and Migol.
...
% Condor-G~\citep{citeulike:291860} allows the management of multi-site
% computations.


% Several frameworks for high-throughput computing and task farming
% exist: Nimrod-G~\citep{buyya00nimrodg} is a specialized framework for
...
% The Legion~\citep{689541} Grid middleware provides a object-oriented
...
% such as Condor-G~\citep{citeulike:291860} and GridWay~\citep{Montero05},
...
% Migol~\citep{schnorLuckow08} is an adaptive Grid middleware,
...
% transparently. 
% 
% SAGA~\citep{SAGA_Goodale06a} provides a standardized, application-level API for 
...
% LONI~\citep{Allen:2003xy} Grid environment.      
...
% SAGA C++ reference implementation~\citep{Kaiser:2006qp}. 
% 
