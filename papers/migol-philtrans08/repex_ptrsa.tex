...
\jhanote{General Remark: We are using RE and Replica-Exchange interchangably?
...
with the Glide-in mechanism, allows the utilisation of distributed
...
into a larger job. \jhanote{I'm wondering if we should call BigJob
  ...
  novel?}  % The BigJob abstraction also serves as basis for
...
\jhanote{Remember this is a paragraph in the opening/introduction
  section, hence will be scrutinised more carefully} The RE framework
relies on the \emph{BigJob} abstraction not only for dispatching
...
production Grid environment~\citep{Luckow:2008la}. However, as the
number of replicas increase, the probability of an overall slowdown
caused by the synchronization required by the Replica-Exchange
...
halt caused by a single crowded or slow resource.  To address this
issue, we propose the SAGA Glide-In framework, which allows the
effective scheduling of clustered of tasks.
...
which builds upon the ability to cluster replicas (using BigJob) 
before submission.
...
\begin{figure}[t]
    ...
    \caption{\footnotesize \bf The Replica-Agent is used as
      placeholder job for all replica sub-jobs running on a single
...
simulations. In particular for loosely coupled applications, an
adaptive schema can perform better than a rigid co-allocation schema.
