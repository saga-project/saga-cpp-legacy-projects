...
\jhanote{Do you see the important difference: BigJob is the clustering
  of tasks; Glide-in is the schduling mechanism that utilises it to
...
queue at the resource management system potentially causing a slowdown
of the overall application. We overcome this slowdown using an
efficient dispatching scheme, which builds upon the ability to
clustering replicas (using BigJob) before submission.
...
Determination (D)), and the effective utilisation of an expanded or
...
%job.         
A scalable application requires the effective support for both
resource determination and selection. For resource determination, the
framework currently relies on a static, user-defined mapping of
replicas and resources.  BigJob and Glide-In are very useful
abstractions for dynamic resource utilisation.
...
simulations. In general, an implementation that is adaptive will have
a lower time-to-completion. \jhanote{This is a dangerous remark to
...
In scenario A, up to 3 resources running 4 Glide-Ins with 32 cores
each are used, i.\,e.\ if all resources (Abe, QB and Ranger) are
...
be observed with the increase of the number of resources.  \jhanote{I
...
resources. \jhanote{AndreL: Make it easy for the ready to grasp the
  ...
  glide-ins on Abe/QB/Ranger??} Each replica is started with a
constant number of 16 MPI processes. Using adaptive temperature
sampling, the number of replicas is dynamically increased as the
number of available resources increases. A maximum of 16 replicas are
used.
