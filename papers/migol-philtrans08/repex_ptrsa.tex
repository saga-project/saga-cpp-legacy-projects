...
Additional, heterogenity and dynamism of resources are important
factors making the execise more challengeing, ie if heterogenity leads
to difficulty/complexity in developing applications, the lack of a
perfect understanding of resource availability and the lack of a
simple model for determining optimal usage leads to deployment
challenges.
...
one studied in Ref.~\citet{Luckow:2008la}, wherein we used distributed
...
design of the SAGA~\citep{saga_gfd90} Checkpoint\,\&\,Recovery package
(CPR), and its implementation using the Migol adaptor. The motivation
here is address some of the challenges and performance bottle-necks
that we encountered when deploying the framework to solve real
scientific problems. In particular, we refer to the overall slow-down
due to the lack of co-scheduled resources to support the concurrent
completion of multiple sub-jobs whehter on the same machine or on
...
Additionally, we implement a general and extensible ``Glide-In''
framework using SAGA, which overcomes the need to co-schedule
resources for the many sub-jobs, and so although the adaptive
algorithm does not (and cannot) remove the need for synchronisation
amongst the replicas at the exchange step, it makes it very simple for
replicas on either the same resource or different resource.  The
implementation on the one hand supports the prevention of a slowdown
due to synchronisation on the one hand, whilst making it easy to
add/remove resources {\it dynamically} while respectinng the
...
resources of different sizes, as opposed to {\it
