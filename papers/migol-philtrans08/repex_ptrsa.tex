...
These applications are referred to as loosely coupled. Despite
the conceptual simplicity, many scientific problems, e.\,g.\ 
parameter sweeps and/or Monte Carlo simulations, can be solved using
infrastructure that supports this application class. A slightly
...
load balancing issues even on few resources. 
...
In particular, we refer to the overall slow-down
...
distinct distributed resources. The slowdown is particular to the RE
algorithm, and not a generic feature of the all loosely-coupled
...
\jhanote{(with replicas of the same generation?)}  We propose an
adaptive variation of the RE algorithm that permits a change in in the
number of replicas that are employed and thus can be opportunistic in
the resources that it can employ.

To reduce the time to solution, we further propose 
a general and extensible ``Glide-In''
framework based on SAGA, which overcomes the need to co-schedule
resources for the many sub-jobs. The enhanced job abstraction allows
the RE application to efficiently cluster sub-tasks, which are
executed on the same machine. 
The adaptive RE algorithm is build on top of this abstractions.
...
processes on available resources.
% makes it very simple for
% replicas on either the same resource or different resource.  
The implementation of adaptive RE prevents slowdowns due to synchronisation, 
...
the SAGA Glide-In framework. In the subsequent section we discuss our experience in
