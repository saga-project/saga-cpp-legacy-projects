...
optimal selection of the best resources to use (Resource Determination
(D)), and (ii) the effective utilisation of an expanded or reduced set
...
\item {\it Scenario A:} Vary the number of processes assigned to each
  replica to speed up the overall progress. By adding resources to
  slow replicas processes heterogeneous resources can efficiently be
  supported. \jhanote{AndreL: The last sentence needs more elaboration
...
%Both adaptive strategies have been implemented within the RE-Manager. 
Our framework supports and implements both strategies.  In
section~\ref{sec:exp} we demonstrate that as the number of available
...
\jhanote{We need a better place for this paragraph. It is an important
  ...
  is all about adaptivity. The paragrpah below is about
  agile-exeuction supporting adaptivity while there is no mention of
  adaptivity and MPIg jobs..}  It is useful to contrast our approach
...
linearly, the computed problem set \jhanote{not sure what computed
  problem set is...?} rapidly drops when using more than eight
