...
% Looks like buzzword parade to the reader - I prefer to skip it
%  The implementation
%  of our framework can be used in clusters and Grid supporting single-processor
%  as well as multi-processor simulations that require tightly-coupled resources.
...
specific~\citep{Woods:2005nx}, or require prior scheduling of distributed
resources~\citep{owain}. Work reported in the latter paper in
...
inherited from parallel and cluster computing; such applications will
...
Abstractions that enable effective distributed application development,
...
characteristics of distributed systems, in Ref~\citep{Luckow:2008la}
...
of a loosely-coupled application. Implementing a framework with the ability to handle
fault-tolerance and heterogeneity, was an important first-step on the
...
exchange temperatures between neighbouring replica processes
...
numbers of replica processes slowdowns occur. These slowdowns are 
...
next RE step. The replica processes are then re-started, i.\,e.\ they
...
of replica processes before submission. 
...
desired number of resources.  Further, the \texttt{sub\_job} object is
specified. This sub-job is mapped to a larger \texttt{sub\_job}
job using the job id.
%BS: Really, or better to a \texttt{big\_job}?
...
to just ignoring the dynamic attributes. A dynamic application 
may have different adaptive strategies that can be used.
...
%job.         
A scalable application requires the effective support both resource 
determination and selection. For resource determination, the framework 
currently relies on a static, user-defined mapping of replica processes 
and Grid machines
...
\begin{compactitem}         
    ...
    replica process to speed up the overall progress. By adding resources
    to slow replicas processes heterogeneous resources can effectively be supported.
    
    \item {\it Scenario B:} As further resources become available, 
    the number of replica processes is adjusted. Depending on the 
    underlying physics model, the additional replica processes can be used
...
\begin{acknowledgement}
  ...
  to thank Yaakoub el-Khamra
  %(aka Enrico Camran the Cigar-pedlar) 
  for useful discussions. This work has also been made possible thanks
  to
