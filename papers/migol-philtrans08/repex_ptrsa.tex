...
is a complex task for a myriad number of reasons. Distributed computing environments 
...
Additional, heterogeneity and dynamism of resources are important
factors making the exercise more challenging: heterogeneity leads
to an increased difficulty and complexity in developing adaptive 
applications and deploying these in the Grid. The uncertainty about
the availability of resources and the lack of a simple 
model for determining optimal usage lead to various scheduling and 
load balancing issues even when using only few resources. 
...
Grids. In pathological cases, the complete system can come to a halt 
...
A further cause for slowdowns are queueing delays, which particularly occur when 
using production resources. To address this issue, we propose 
a ``Glide-In'' framework based on SAGA, which  allows the direct dispatching
...
underlying physics of the problem at the same time. Using adaptive RE, 
we were able to improve the performance by up to 80\,\% during 
REMD runs in the TeraGrid.

In addition to overcoming the performance bottleneck outlined earlier,
the main advantages of our approach: First and foremost it is a
...
confined to gLITE/EGEE. Secondly, our approach can scale to use
...
framework. Section 6 discusses the enhanced job model abstraction and 
...
Migol has been seamlessly integrated into the SAGA C++ reference implementation
...
As described in section~\ref{sec:challenges}, there
is a severe limitation: all jobs are required to queue at the  
...
Glide-In jobs (see~\cite{citeulike:291860}). This job represents a placeholder 
...
\subsection{Adaptive Replica Sampling}
...
sampling range is increased. This adaptive sampling schema allows the sufficient utilization of all available resource. In contrast, other co-allocation schemas, as deployed e.\,g.\ by MPICH-G2, are not application aware and waste resources by requiring nodes to remain idle until all resources became available.
...
the TeraGrid and LONI clusters: QueenBee (QB), Poseidon and Eric.  
...
The objective of the experiments is the evaluation of the Glide-In schema on
...
used job scheduling mechanism. The RE-Manager was configured to run 
a simulation with 2 to 16 replica processes and 5 attempted exchange 
steps sampling a temperature range between 300 and 450\,K. Each replica 
process comprises of a NAMD simulation running on 16 cores 
for 500 time steps.      
...
Figure~\ref{fig:perf_remd_glidin}b) shows that the SAGA Glide-In framework can provide
...
The framework has also been deployed on multiple resources within LONI concurrently. 
...
represents a rather specific solution and is less general then our RE framework. 2)
LAMMPS and MPIg require the co-allocation of all resources, i.\,e.\ a single slow
...
decoupling of the Replica-Exchange orchestration logic from the
...
By only slightly extending the job model defined by SAGA, the Glide-In
...
basic programming interfaces. While the current implementation used 
...
the simulation space dynamically. Using this approach expensive idle 
...
to solution. In addition, we will refine our RE algorithm toward 
a more asynchronous solutions, as e.\,g.\ described by \citet{Gallicchio:2007yq}, 
to support a better scaling across heterogeneous resources.

While it has been shown, that scheduling of tasks can efficiently done on 
...
prediction~\citep{1254939} and advance reservation-based 
