...
resources require prior co-scheduling of distributed
resources~\citep{repex_mpig}.
...
%the RE framework 
\jhanote{I'm not convinced the reader knows what
  ``the framework'' at this stage means. Are you?}
\alnote{not quite,
  added RE to make ``the framework'' more concrete}
%A major concern that arises for example, is the overall
...
% Migol is an adaptive Grid middleware that provides several mechanisms
% for supporting the fault tolerance of distributed applications.
...
metadata with a Grid middleware, such as Migol, using this API.  The
...
% Using the powerful capabilities provided by SAGA-CPR, applications can
% easily and reliable orchestrate hundreds of checkpoint-restartable
% tasks and benefit from an adaptive infrastructure such as Migol at the
% same time.
...
% too much...}
% Implementing an approach that can scale upto very large 
...
abstractions for implementing the RE logic.%  At the same time
% applications can benefit from Migol by simply configuring the
...
\begin{figure}[t]
      ...
      \caption{\footnotesize \bf RE-Manager Abstractions:  
        The BigJob abstraction provides the capability to cluster sub-jobs 
        into a larger big-job. This feature is implemented on top of the Glide-In
        abstraction, which utilizes a place-holder job for this purpose. 
        The Glide-In abstraction is implemented using the abstractions provided by SAGA, 
...
Figure~\ref{fig:abstractions} summarizes the abstractions developed for
supporting the clustering of sub-jobs into a larger big-job. 
This capability is provided to the application via the \emph{BigJob} 
abstraction. This abstraction enhances 
the SAGA job model by the possibility to allocate larger chunks of resources 
prior to an application run. For this purpose, the \texttt{big\_job} 
and the \texttt{sub\_job} object are defined. 
The BigJob abstraction is implemented on top of 
the system-level Glide-In abstraction. For each 
\texttt{big\_job} object, a Glide-In job with the desired number 
of resources is started. In the following, \texttt{sub\_job} objects
are mapped to a \texttt{big\_job} using the jobid as reference.

% For this purpose the \texttt{big\_job} object is
...
underlying concepts, \I{patterns} and \I{abstractions}.  
\alnote{I commented this out (liked the second definition
...
multiple job to be submitted, which is supported using the 
application-level BigJob abstraction. BigJob in turn utilises another
abstraction -- the system-level Glide-In abstraction, which supports 
the commonly occurring requirement to schedule multiple sub-tasks 
relative to each other and not in competition with other jobs 
submitted to the batch queue system.
...
of upto 64 attempted exchanges\jhanote{AndreL: I don't think the numbers add up here!} 
\alnote{Yes, they do not add up. I standardized all figures to T\_c for 64 attempted exchanges}
Each replica is a parallel NAMD simulation, can use upto 24 MPI
processes and runs for 500 time steps between exchange attempts. The
metric of performance used in our tests, is the 
time-to-completion for 64 attempted exchanges.
...
\begin{figure}[th]
  ...
  \subfloat[RE-ManagerScaling]{\includegraphics[width=0.48\textwidth]{performance/perf_re_numberresource_tc_64ex}}
    \subfloat[Glide-In
    Performance]{\includegraphics[width=0.48\textwidth]{performance/perf_glidein_tc_64ex}}
...
Obviously, with the increase in the number of replicas the time required
for 64 exchange attempts decreases (Figure~\ref{fig:perf_remd_glidin}a). 
% allows the
% computation of a greater problem set, i.\,e.\ the sampling of a wider
% parameter range (Figure~\ref{fig:perf_remd_glidin}a).  
\jhanote{Once
  again, it is not obvious if the number of Glide-Ins is being
  increased on one machine?}  \alnote{I hope this becomes clear now
  with the new introduction section.}  
However, the figure also shows the limitations of the regular job 
submission mechanism: The more
processes are spawned, the more likely queueing delays will be.  With
the SAGA Glide-In framework the time-to-completion almost scales
linearly; in contrast, without SAGA Glide-In the time-to-completion
increases again when using more than eight replicas.
\jhanote{not sure what computed problem set is...?}  \alnote{should
  ...
  defined earlier?}\alnote{ok changed.}
This slowdown was caused by the delay of some replicas.  
In particular on QB, usually only 12 replicas became active concurrently,
while the remaining 4 replicas were queued at the local scheduler 
causing this slowdown. 
\jhanote{AndreL: Do you mean ``at the same time'', ie
  concurrently? If so, let us use concurrently please.}\alnote{rephrased this sentence}  
The remaining four replicas had to wait in the queue and produced a significant
...
$T_{c}$ from 50\,minutes to about 26 minutes per average, which corresponds to a
decrease of about 50\,\% runtime.  In the best case \jhanote{Do you
  mean single machine scenarios?} \alnote{I was referring to the best case 
  in contrast to the average case} 
improvements of upto 70\,\% could be observed. As described, this effect is mainly
...
\jhanote{We need to say something about the location of the kink when
  ...
  erratic.}        
\alnote{I updated the data. This were still the old graph from MTAGS. I 
added data from some additional runs, which reduced the benefit of Glide-In a little, 
but also the stdev for the without Glide-In case was reduced. Unfortunately, I do not have too many
data points for the 2, 4, 8 replica processes to really say whether there are earlier kinks. 
The position of the kink really depends on the length of the PBS queue. In this scenario the 
cluster was not too crowded, so that many jobs could be backfilled (but not all jobs could be 
backfilled.)} 
\alnote{I describe the slowdown with more than 12 replicas above when referring to Fig. 4a. Is this
...
% general setup                     
In scenario A, upto 3 resources, each running 4 Glide-Ins with a
constant size of 32 cores each are used. If all resources 
(Abe, QB and Ranger) are available a total of 384 cores (12 Glide-Ins
with 32 cores each) will be utilised. A constant number
of 16 replicas is used. The number of Glide-In jobs
on a resource is fixed at 4 for simplicity reasons; our results 
will hold for general values of this parameter.
...
\alnote{moved up:
We mention explicitly that for
...
\alnote{ok refined the two paragraphs above: 1) general setup 
2) experiment procedure. The points you mention in your comment should be reflected
in 2)}  
...
best possible performance, is only about 0.5. But, this is rather a
...
making this overhead negligible.  
...
adjust the number of replicas simulated -- in effect adjusting
the either the range of temperature or the specific temperatures 
simulated -- is evaluated. For this purpose, 4 Glide-Ins with 64 cores each are
...
\jhanote{AndreL: Make it easy for the ready to grasp the number of
  ...
  your question: yes} 
Each replica is started with a constant number of 16 MPI processes, i.\,e.\
every Glide-In can run 4 replicas.
              
%  procedure
At the beginning of the experiment, the 4 Glide-Ins are submitted 
to 1, 2 respectively 4 statically configured resources. 
Similarly to scenario A, different queueing delays
usually occur at the different resources. Using the adaptive temperature sampling 
algorithm, the number of replicas is dynamically varied with
the number of active Glide-Ins. Depending on the number of running Glide-Ins,
the ensemble consists of 4 to 16 replicas.
...
% to complete could be reduced by upto 80\,\%.                                      
Using the BigJob abstraction, RE sub-jobs could efficiently be dispatched 
reducing the time-to-completion upto 80\,\%. The use of different 
