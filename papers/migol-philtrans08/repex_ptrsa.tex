...
\jhanote{I think we need just a paragraph or two at most, outlining
  what SAGA Migol does and why it is required criticially for the
...
Migol provides several mechanisms for supporting the fault tolerance
...
\jhanote{all the challenges}

Running REMD simulation across a distributed environment is a great challenge. Distributed environments 
are inherently error-prone. Especially in a REMD simulation the loss of a single replica process due
to a node or network failure usually causes the abort of the entire simulations.

REMD involves the running of multiple replica jobs in parallel. At the end of each run the
results must be obtained and exchange is attempted. The process of running the replica processes
is then repeated. The implementation of such an application with running replicas across
multiple resource without a well-defined programing abstraction and middleware can be very complex.
...
\subsection{Efficiently Replica Job Scheduling}

A particular problem that became evident in earlier experiments%~\citep{Luckow:2008la}
is the effect of the queuing delay on the overall completion time. 
Various systems, such as Falkon~\citep{1362680} or the Condor
Glide-In~\citep{citeulike:291860} for avoiding this issue have been proposed.
These frameworks provide mechanisms to acquire chunks of 
resources from the resource management systems,
which can then be used to dispatch short tasks.  However, there are different 
limitations. For example, both system impose some different overheads: Condor Glidin starts
a complete set of Condor daemons within the allocated set of resources. For Falcon, \citet{citeulike:3169002} noted
a startup time of over 2 minutes on a Blue Gene/P machine. 


However, such a glidin mechanism is a very attractive model for minimizing queuing times.
For the REMD-Manager, we demonstrated the simplicity and extensible of the SAGA framework by
providing a well-defined abstraction for glidin based applications based on the SAGA Job API.
  
\begin{figure}[th]
    \centering
        \includegraphics[width=0.9\textwidth]{remdmanager_v11}
    \caption{\footnotesize \bf REMD Programing Abstractions and Resource Management: The Replica-Launcher is used as placeholder job for all 
    replica processes running on a single cluster. The REMD-Manager can control both the placeholder job and the replica jobs
    using an enhanced Job API. By using this efficient way to allocate resources, queuing times are minimized and the time
    to completion can be dramatically reduced.}
    \label{fig:remdmanager_v1.1}
\end{figure}

Figure~\ref{fig:remdmanager_v1.1} illustrates the architecture. For each cluster machine
a {\it Replica-Launcher} process is started using the SAGA CPR API. This agent is assigned the resources required 
for all replica processes on this machine. Using an enhanced Job API the REMD-Manager can submit
different small replica tasks with reference to the Replica-Launcher job. In a sense the Replica-Launcher job represents a 
resource reservation for all replica processes. It is then the responsibility of the 
Replica-Launcher to assign resources to these ``small'' tasks, monitor the tasks and free the resources after the tasks 
has been finished.

The enhanced job API behaves similar to the SAGA Job API. The handling of such jobs is exactly the same as with the normal
SAGA jobs. The API has been implemented entirely in application space using Python and the SAGA Advert API. Using the Python 
duck typing mechanism such an enhanced job object can be used as replacement to a SAGA job object -- no further source modifications 
needed. The implementation is based on a simple protocol using the Advert Service, a central key/value store. 
For each new job an an advert entry is created. The Replica-Launcher periodically polls for new jobs. 
If a new job is found and  resources are available, the job is launched otherwise it is queued. 

                                                                
In addition the Replica-Launcher servers as client for the Migol backend. It is responsible for
updating the metadata of the application, i.\,e.\ the state, monitoring endpoint and new checkpoint URLs, at the
Migol backend. During the entire runtime the replica process is monitored by
Migol using the monitoring endpoint of the Replica-Launcher. 
...
monitoring and the transparent recovery of failed tasks. By only slightly extending
the Job abstraction defined by SAGA a powerful programing abstraction for glidin jobs
has been defined and successfully deployed within the REMD application. While the current
implementation used an custom protocol to implement this mechanisms, the approach can
also be applied to other middleware platforms, such as Condor Glidin or Falcon. Currently,
we are actively working on a Condor adaptor for SAGA.




power of programatic interface
working on condor adaptor
we could easily map this to condor glidin 
...
\jhanote{This section should focus on both just the ``algorithmic''
  ...
  advances/related work around  implementation in distributd
  environments}
