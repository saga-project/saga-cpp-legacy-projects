...
%  time-to-solution. However, 
%try to avoid redundancy within the abstract - BS
The ability to orchestrate many
  ...
  which ensures the reliable execution of applications even in
...
Despite their mostly loosely coupled nature, Replica-Exchange
simulations require a small level of coupling.  
The central master, the RE-Manager, must
periodically obtain the results of all replicas to determine the 
new configuration of the next RE step.  
% skip it cause of redundancy - BS
...
which will highlight the unique features of our implementation.  
...
parallel replica dynamics as implemented in Folding@home (\citep{folding}) involves
...
\begin{figure}[th]
    ...
    \caption{\footnotesize \bf SAGA Glide-In and RE Performance (QueenBee): 
...
Figure~\ref{fig:performance_perf_distributed} shows the results of this evaluation. For this evaluation,
the RE-Manager is configured to run with 16 replica processes a 16 cores each. The figure shows
the runtime for 40 attempted pair-wise exchanges with respect to the number 
of TeraGrid resources utilized. The RE-Manager was configured to run with 4 Glide-Ins, i.\,e.\ every
Glide-In manages 4 RE processes and a total of 64 cores.  This is the optimal Glide-In size determined
in earlier experiments (see Figure~\ref{fig:perf_remd_glidin}b)).

The time to solution obviously decreases with the number of the machines used. The main 
reason for this behaviour is that with the distribution of the simulation to multiple resources
the likelihood that a single crowded resources delays the simulation decreases. This is also reflected in
the standard deviation, which clearly decreases with the degree of distribution. The results show the benefits
of the adaptive RE schema: Even if only a few resources are available, it is better to opportunistically 
use these resources instead of waiting for the complete set of nodes to become available. 

\begin{figure}[t]
    \centering
       \includegraphics[width=0.7\textwidth]{performance/perf_distributed.pdf}
    \caption{\footnotesize \bf Performance of the RE-Manager in the TeraGrid: The figure 
    shows the runtime of the RE-Manager  
    for 40 pair-wise exchanges using 16 replica processes with 16 cores each. 
    The utilization of more machines leads to a shorter time to solution.
    }
...
when deployed on a single machines. 
2) The distribution of the replica processes across multiple resources can further reduce
the time to solution -- the effect of single crowded resources on the overall runtime
