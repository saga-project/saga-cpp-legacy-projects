...
Running REMD simulation across a distributed environment is a great
challenge. Distributed environments are inherently
error-prone. Especially in a REMD simulation the loss of a single
replica process due to a node or network failure usually causes the
abort of the entire simulations.

REMD involves the running of multiple replica jobs in parallel. At the
end of each run the results must be obtained and exchange is
attempted. The process of running the replica processes is then
repeated. The implementation of such an application with running
replicas across multiple resource without a well-defined programing
abstraction and middleware can be very complex.
...
experiments%~\citep{Luckow:2008la}
...
\glidein~\citep{citeulike:291860} for avoiding this issue have been
proposed.  These frameworks provide mechanisms to acquire chunks of
resources from the resource management systems, which can then be used
to dispatch short tasks.  However, there are different
limitations. For example, both system impose some different overheads:
Condor \glidein starts a complete set of Condor daemons within the
allocated set of resources. For Falcon, \citet{citeulike:3169002}
noted a startup time of over 2 minutes on a Blue Gene/P machine.


However, such a \glidein mechanism is a very attractive model for
minimizing queuing times.  For the REMD-Manager, we demonstrated the
simplicity and extensible of the SAGA framework by providing a
well-defined abstraction for glidin based applications based on the
SAGA Job API.
...
\begin{figure}[th]
    ...
    \caption{\footnotesize \bf REMD Programing Abstractions and
      Resource Management: The Replica-Launcher is used as placeholder
      job for all replica processes running on a single cluster. The
      REMD-Manager can control both the placeholder job and the
      replica jobs using an enhanced Job API. By using this efficient
...
Figure~\ref{fig:remdmanager_v1.1} illustrates the architecture. For
each cluster machine a {\it Replica-Launcher} process is started using
the SAGA CPR API. This agent is assigned the resources required for
all replica processes on this machine. Using an enhanced Job API the
REMD-Manager can submit different small replica tasks with reference
to the Replica-Launcher job. In a sense the Replica-Launcher job
represents a resource reservation for all replica processes. It is
then the responsibility of the Replica-Launcher to assign resources to
these ``small'' tasks, monitor the tasks and free the resources after
the tasks has been finished.

\jhanote{Let us reach consistency on, BigJob/LittleJob vs MetaJob vs
  Enhanced Job terminology}

The enhanced job API behaves similar to the SAGA Job API. The handling
of such jobs is exactly the same as with the normal SAGA jobs. The API
has been implemented entirely in application space using Python and
the SAGA Advert API. Using the Python duck typing mechanism such an
...
For each new job an an advert entry is created. The Replica-Launcher
...
In addition the Replica-Launcher acts as a client for the Migol
...
% adaptor we could easily map this to condor glidin
{\bf This is the first known instance of creating a system
  (deployment) level abstraction from basic programming insterfaces.}
...
\jhanote{Mention BQP and co-scheduling as opposed to opportunisitic
