...
\jhanote{do we want \emph for bigjob?}
...
classes.  Before running regular jobs, an application must initialize
a big-job object.  The BigJob Manager then queues a Pilot-Job
\jhanote{are we confusing the reader by toggling between Pilot-Job and
  BigJob?}, which actually runs a BigJob Agent on the respective
