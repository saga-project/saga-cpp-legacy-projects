...
Condor pool. Because of problems with a support for paralle universe in the
original condor_glidein and glidein binaries a dedicated solution was prepared.
To run master and startd deamons on requested remote nodes a bash script was
...
universe and prevent a situation that one job run across clusters. The NUM_CPUS=1
assures that whole nodes are assigned to jobs.
To obtained comparable results no other parameters, especialy deciding about
timing, were changed although approppriate tuning would decrease a total
