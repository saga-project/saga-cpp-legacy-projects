...
infrastructure are very broad, and most often differ from regular, HPC
...
window.  Somewhat related to the above, but equally important,
distributed applications have a much broader range of usage modes.
...
El-Khamra et\,al.~\cite{enkf-gmac09} the BigJob has been used to support a very
...
useful since the usage of multiple clusters can reduce the time-to-completion \tc 
...
A tutorial that describes the BigJob API can be found at~\cite{bigjob_cloud_tutorial}.
...
infrastructure configurations, and determining the \tc. We compute
...
$T_{max}$. Each scenario has several sub-scenarios,
of which we discuss selected sub-scenarios.  
...
performance is and can be preserved.  In the following section we show
that the SAGA BigJob abstraction is suitable for running Pilot-Jobs on
different kinds of distributed infrastructures supporting different
usage modes.
