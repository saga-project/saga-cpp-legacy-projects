...
but in general programming systems and tools need to support the
following different aspects of of extensibility as fundamental
objectives: (ii) New application domains and usage-modes, (ii) Support
for extended functionality, (iii) Extension to work on new
infrastructure, (iv) Operation at extended scales, and (v) Uptake by
communities other than the developer (community usage).  The level of
success of these efforts depended not only on the resulting
programming system, but also on the suitability of the underlying
paradigm for Grid environments and availability of usable and
extension abstractions on given production
...
or usable across different systems, e.g., there is no such “unifying”
and ``extensible'' tools on the TeraGrid.
...
or as tools in interoperable and extensible fashion.  The SAGA-based
Pilot-Job (BigJob) is such an example; unlike other common Pilot-Job
systems, SAGA BigJob natively supports MPI job and works on a variety
of back-end systems, generally reflecting the advantage of using a
SAGA-based approach. But in addition, to the above and as we will
show, it has been designed to be extensible in a number of ways.
...
disruption. The idea of Advert Service as a central point of
...
Grids) are used.  However the approach presented here shows how to
...
BigJob can be lauched in a shorter period of time, it is probably true
