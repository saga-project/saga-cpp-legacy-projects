...
Tools (PS\&T) used to develop distributed applications need 
to account for {\it fundamental} characteristics. \alnote{What are these
...
\alnote{Rewrote that sentence:Although it will not be possible to support all of the following, and that too always,
...
objectives:}
Although it will not be possible to support all of the following properties,
a PS\&T should at least address some of these aspects:
(ii) new application domains and usage-modes, (ii) support
for extended functionalities, \alnote{e.g.??} (iii) extension to work on new
infrastructures, (iv) operation at extended scales, and (v) uptake by
communities other than the developer (community usage).  The level of
success of these efforts \alnote{which efforts?} depends not only on the resulting
programming system, but also on the suitability of the underlying
paradigm for Grid environments and on the availability of usable and
extendable abstractions on given production infrastructures. 
Interestingly, the Pilot-Job abstraction has been
widely used across several different PGIs. 
However, the existing Pilot-Job frameworks are all heavily customized and often
tightly coupled to a specific infrastructure, and not extensible
or usable across different systems, e.g. there is no such ``unifying''
and ``extensible'' tools on the TeraGrid. \alnote{that works across different PGIs?}
...
used to design and develop end-user applications, but it is also commonly
utilized to develop tools and infrastructures~\cite{saga-tools} to support
distributed applications. In a nutshell it provides the building
blocks to develop the abstractions and functionalities to support the
...
systems, SAGA BigJob natively supports MPI jobs and works on a variety
...
\alnote{Should we briefly mention the extension for Clouds and Condor here?}
...
The Grid BigJob is a SAGA-based framework and heavily utilizes the SAGA job, file
and advert API. The BigJob Agent is started using the SAGA job API.
Communication between the BigJob Agent and BigJob Manager
...
original condor\_glidein and Condor Glide-In binaries a dedicated solution was prepared.
To run master and startd daemons on requested remote nodes a shell script staged-in
from a user's desktop is submitted as a executable file of Condor-G Pilot-Job.
The script reads appropriate environmental variables (\$PBS\_NODEFILE) and files to detect
which nodes were reserved by the resource manager for the Pilot-Job. Then Condor master and startd
deamons are executed via ssh on all of these nodes. Both the master and startd are from the
standard Condor 7.3.2 package. The only necessary changes were introduced to
Condor configuration files both on user's desktop and remote resources.
All of these changes are needed to run jobs in the parallel
universe and prevent a situation that one job runs across clusters. Additionally NUM\_CPUS=1
assures that whole nodes are assigned to jobs.
To obtaine comparable results no other parameters, especially deciding about
timing, were changed although appropriate tuning would decrease a total
startup time from 331 sec on average to less 60 sec.
