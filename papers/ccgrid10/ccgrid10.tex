...
Distributed infrastructure have been used by many applications to
...
distributed applications are very broad and most often differ from
...
broader range of usage modes.  Not only do Production Grids
infrastructure, but so do the programming systems and tools used to
develop distributed applications need to account for {\it fundamental}
characteristics.
...
failed to use distributed infrastructure effectively, or have had to
implement new capabilities at one or more levels -- application,
programming system, middleware, and/or infrastructure
level~\cite{dpagrid2009, dpa-paper}.  It is generally accepted that
...
infrastructure available to end-suers~\cite{dpagrid2009}.  For
...
solutions work well in small or controlled environment, but not
...
be built as fundamental design objective of PS\&T. Although it will
not be possible to support all of the following, and that too always,
but in general PS\&T need to support the following different aspects
of of extensibility as fundamental objectives: (ii) New application
domains and usage-modes, (ii) Support for extended functionality,
(iii) Extension to work on new infrastructure, (iv) Operation at
extended scales, and (v) Uptake by communities other than the
...
underlying paradigm for Grid environments and availability of usable
and extension abstractions on given production
infrastructure. Interestingly, the Pilot-Job abstraction has been
widely used and interestingly used across several different
...
or usable across different systems, e.g., there is no “unifying” such
tool on the TeraGrid.
...
used to design and develop end-user applications, but it also commonly
used to develop tools and infrastructure~ \cite{saga-tools} to support
distributed applications. In a nutshell it provides the building
blocks to develop the abstractions and functionality to support the
characteristics required by distributed applications whether directly,
or as tools. The SAGA-based Pilot-Job (BigJob) is such an example;
unlike other common Pilot-Job systems, SAGA BigJob natively supports
MPI job and works on a variety of back-end systems, generally
reflecting the advantage of using a SAGA-based approach. But in
addition, to the above and as we will show, it has been designed to be
extensible in a number of ways.
...
infrastructure. Although the focus of this paper is on outline the
...
such applications. A Pilot-Job allows the execution of jobs without
...
of resources. Falkon does not support MPI applications, and thus not
usable for our application scenario.

\jhanote{This is now out of place I think}
...
Nimbus. However, in contrast to the SAGA-based Pilot Job,
both frameworks are strongly coupled to a particular backend
infrastructure -- Globus in the case of Nimbus and gLite in the case
of PanDA. The advantage of the SAGA Pilot-Job is that it allows
applications to seamlessly utilize different backend infrastructure,
e.\,g.\ Condor, Globus and different kinds of Clouds, at the same
time.
...
Ref.~\cite{sagamontage09} we discussed how SAGA was used to implement
a higher-level API to support workflows. In this paper, we will
discuss how SAGA can be used to implement runtime frameworks to
...
\jhanote{I think the first two paragraphs should be moved to Section
  II?}
...
advantage of using a SAGA-based approach.  \jhanote{We have talked
  about interoperabiltiy; need to address extensibility}
...
three different backends, while exposing the same user-level interface
and semantics:
...
In the following the overheads resulting from the usage of BigJob
within Grid applications is analyzed. Typically, the main overhead
when using BigJob results from the queueing time at individual
resources on a Grids respectively from the virtual machine creation
time in Cloud environments.  Figure~\ref{fig:performance_setup_time}
compares the startup time of the different Pilot-Job backends for
Grids, Condor pools and Clouds.
\begin{figure}[htbp]
    \centering
...
% Maximum  Allowed Time to Solution}
...
while addressing the bad heterogeneity The SAGA-based approach
outlined here, specifically in the context of Pilot-Jobs is being used
to integrate otherwise disparate PGI such as the TeraGrid and the OSG,
as well as the TeraGrid and EGEE; application-level interoperability
constitutes a different but an important approach towards the
interoperability of PGI.
...
extend capabilities to the existing framework will play important
roles.
