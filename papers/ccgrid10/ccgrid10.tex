...
\title{SAGA BigJob: A Pervasive, General Purpose Pilot-Job Abstraction
...
BigJob is a SAGA-based Pilot-Job implementation. In contrast to other
...
standard BigJob implementation for Grid backends, we highlight how
the BigJob framework was extended to support Condor and Cloud
resources.
...
\begin{figure}[htbp]
    ...
    \caption{BigJob Architecture: The core of the framework, the BigJob-Manager,
     orchestrates a set of Sub-Jobs via a BigJob-Agents using the SAGA job and file APIs. 
...
\begin{figure}[!ht]
 \begin{center}
     \includegraphics[width=0.75\textwidth]{figures/bigjob_condor}
 \end{center}
...
integration of heteregenous resources to facilitate distributed
Scale-out.
...
\subsection*{Hepatatis-C Virus (RNA) Using Replica-Exchange}
...
ways to accelerate sampling and to enhance the ``effective''
...
of loosely-coupled shorter-running simulations, make these ideal
...
candidate for new drugs targeting Hepatitis-C virus.  Our aim is to
use REMD to enhance the sampling of the conformational flexibility of
the internal loop referred to as {\it HCV IRES IIIb CA
  variant}~\cite{Collier:2002wd} as well as the equilibrium
...
\begin{acknowledgement}
  \footnotesize{Important funding for SAGA has been provided by the UK
    ...
    with the Condor infrastructure.}
\end{acknowledgement}
