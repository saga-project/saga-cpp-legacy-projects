...
\jhanote{Aren't we calling it middleware for building science
  gateways?}\yyenote{Yes and we say this in paragraph 2}
...
\jhanote{please do so! :)}\yyenote{Done} The Simple API for Grid Applications
...
applications (see figure~\ref{fig:saga-overview}). The functional areas that are supported by SAGA include
...
\yyenote{I do not understand the previous comment. Honestly I have no idea
why we have this paragraph or subsection here. Is this part of the capabilities
in DARE? if so why not put it in 2.1, why here?}
...
volumes.~\yyenote{I am keeping the previous sentence but do we
  ...
  protocols in addition to GO}\yyenote{I will be honest with you,
  globus online looks like a gateway to do file transfer and that is
  pretty much it. Why do you need an adaptor for globus online when you
  have adaptors to the underlying transfer mechanisms it uses: gridftp
  and scp. Globus online is a webpage that uses gridftp to some places
  and scp to others, you might as well use filezilla. Why interface to it?}
...
\yyenote{what is this? we do not do virtualized services underneath
the web interface. The virtual env you get is for the VM, not the
webserver. The webserver is fixed. I suggest deleting the following section}
...
demonstration web interfaces: DARE-NGS and DARE-HTHP.\yyenote{The following is repeated in the last paragraph. Move to delete the last sentence} If a third party
user (or gateway developer) wants to use DARE, all users have to do is
run the DARE image on a VM capable system (Quarry or something similar)
and setup their credentials.
...
sense, this is a one-off custom solution that every group that needs a
...
\yyenote{Said this before, still needs revisiting. Why is this
part of deployment? We do not talk about deployment here.}
