...
distributed modes.
\yyenote{The following is a confusing sentence}
  Last but not least, it is always challenging
...
implementations of novel execution patterns, which is often eased by
...
\yyenote{The following sentence needs improvement: what do we mean by maximizing the capacity of DARE?}
Our main design strategy of the gateway development suggested in Fig.~\ref{fig:dare-arch} is to maximize the 
capacity of DARE for gateway applications. The Middleware layers, L2 and L3, are decoupled from access level, L1, and 
...
The two main features required for any science gateway are data management and task management systems. The development 
of DARE is motivated by the need to provide a solution to the many challenges that arise from the complexity of the utilization of 
distributed heterogeneous in terms of management. As mentioned earlier, the pilot job abstraction, SAGA-BigJob has been 
successfully developed and integrated into the current DARE framework. For data management, SAGA-BigData is being actively developed in parallel to BigJob. Nonetheless, the current 
implementation with DARE relies on the legacy approach for data management using the distributed file-filed \yyenote{what does file-filed mean?}, 
application-dependent implementation and file transfer protocols using GridFTP and SCP are two main methods. Some 
...
We have been developing a gateway, DARE-RFOLD to support nc-RNA research
and serve the RNA structure prediction community. Through the DARE-RFOLD,
...
popular application, Autodock\cite{autodock}. \yyenote{The following sentence is five sentences rolled into one. Needs work} The recently announced
...
\yyenote{The following is painfully obvious: of course we want to use XSEDE compute resources as compute resources, that is the entire point of a gateway. Way too redundant, I propose we remove.}
