...
\caption{\small Overall architecture of a
 DARE-based gateway comprising five levels. 
 }
 \label{fig:dare-arch} 
\end{figure}
...
\jhanote{This section is very unsmooth to read. Changes contexts/flow
  too many times}
User interface level for DARE  are of two types DARE-WRAPPER 
(command line interface)  and DARE-Gateways (web interface).
DARE-API is a framework which provides the capabilities of runtime environment 
for executing application over distributed resources. More details about 
DARE-API are discussed in the DARE-API Level(L3).

DARE-WRAPPER provides a simple command line interface to DARE-API with an executable 
named ``textbf{dare-run}''.  This executable takes a configuration file 
as an argument. It also instantiates DARE-API with the give configuration file. 
...
\jhanote{Please structure the discussion with some flow.  Define what,
  then how!} 
  %DARE-API is available for user installation and one can
%interact with DARE using the command line executable which comes with
%its installation.  DARE-API needs three key input configuration files
\jhanote{what are these configurations of?} 
% which are necessary to execute a workflow on distributed machines.
%Key features of DARE-API include a command line interface, generating
%the necessary objects required by the P-* API (Pilot data and Pilot
%Job)
 \jhanote{P* undefined!?} 
...
Other type of interface is DARE-GATEWAYS. It which provides a
simple web interface for the user to utilize the capabilities of  DARE-API.
Every Gateway also contains user authentication to submit jobs. 
Different DARE-GATEWAYS have different web forms and workflow behind them.
...
files and tools. For example in  DARE-NGS(a DARE-GATEWAY) we have form for CHiP-SEQ
application where user can choose parameters like reference genome,  input files like control and treat data
and the mapping tools like BWA, Bowtie, and peak calling tools like MACS, PeakSeq and submits the form. 
After submission these parameters will be stored in a database. 


The database for DARE-GATEWAYS is defined so that it could hold information about the job submitted by the user as well
as the user authentication information for registration and login. 
Another important component in DARE-GATEWAYS is the job monitoring thread.  
Its functionality is to continuously ping the database for new jobs. Whenever it finds a new 
job in the database it spawns a DARE-WRAPPER after it creates the necessary configuration files. 

The DARE-GATEWAYS web server is built using Pylons~\cite{pylons_website} web framework . 
The Pylons project is a collection of Python based web frameworks that
are lightweight and easy to use. Pylons uses the common
Model-Viewer-Controller design pattern and is written in Python. 
Pylons handles everything from user authentication, database
creation/management, and data storage/retrieval. Creating a user
interface is straightforward and ``connecting'' the user interface to
the server-side  DARE-API services is surprisingly simple with job monitoring thread. 

First step to create a custom Gateway is to create configuration files for the 
targeted application using new tools in that applications workflow and
test the new configuration with the standalone DARE-WRAPPER. 
Next step is to create web forms with necessary input fields and then a simple adaptor
to convert the user submitted web form in to a configuration file that DARE-WRAPPER
can understand and instantiate DARE-API.

%\jhanote{What is them?} 
% \jhanote{have we defined what  the DARE components are?} 
\jhanote{what is a DARE-API  workflow? Do you mean a workflow written using DARE-API?} 
...
and have their Gateways write DARE configuration files.  All the required
components to run a workflow written using DARE-API
are described in a configuration file. This configuration file is then
submitted to the DARE-API's executable 
that launches executes the workflow by instantiating the P-* API. 
...
L3 represents the DARE-API. The important feature of DARE is 
to utilize different distributed resources and this complexity involving the execution of 
tasks and file transfers on distributed resources will be handled by the P*-API . 

The Pilot-API(P*-API) provides a common access layer to Pilot Job (Compute) and Pilot Data(File transfers) frameworks over
distributed production cyberinfrastructure, such as XSEDE, OSG, EGI and FutureGrid. 
Thus it provides the ability to distribute workloads in a scalable manner~\ref{pilot-api} 
P*-API hides the complexity involved in remote job executions so it is used inside DARE-API.

DARE-API features include a generating the necessary objects required by the 
P-* API by combining the user customized configuration files. 
These are categorized in to three different types,
one with resource information like job submission url (ConfTypeI),
other which holds information  like location of a tool on different resources and arguments that should be used for tools(ConfTypeII),
Final one with workflow information like tools need to executed, location of input files, ConfTypeI and ConfTypeII etc.,(ConfTypeIII)
There can be multiple ConfTypeI, ConfTypeII files for a single instance of DARE-API but only one file with ConfTypeIII.
DARE-API comes with some predefined configuration files.
After instantiation of DARE-API first loads ConfTypeIII  after that it loads ConfTypeI, ConfTypeII based on the location specified in ConfTypeIII 
...
More information about the DARE-API configuration files can be found in the DARE wiki~\cite{dare_api_web}. 

DARE-API comes with capability to schedule tasks and movement 
of files between resources when it is required during the runtime. 
Using the information provided in the configuration files DARE-API understands
which tasks need to submitted at the right time for successful completion.  
The tasks and input/output files will be distributed over different machines and
 DARE needs to keep track of them. 

Before DARE-API submits the next tasks to the P*-API,  
DARE-API should check for the status of its dependent task.
More information about the management and scheduling can be found the DARE wiki.
This automatic scheduling feature of DARE-API is very helpful 
when the user has multiple tools in the pipeline for execution
where the output files of one step become the input files for the next((TYPE II)). 

Another important feature integrated into DARE-API  is the
ability to update the recent status of the job to a specified database. 
This  updated status can then be used to show status of user submitted jobs in gateways. 
