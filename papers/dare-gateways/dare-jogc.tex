...
DARE user interface layer is comprised of three major components DARE-API, DARE-WRAPPER, DARE-GATEWAYS.

DARE-API is something which provides the capabilities of Runtime Environment. 
DARE-WRAPPER provides command line interface to DARE-API.
DARE-GATEWAYS is something which provides web interface to DARE utilizing the DARE-WRAPPER.
...
named ``textbf{dare-run}''.  This executable takes only ConfTypeIII configuration file 
as an argument and hands it over to DARE-API instance.
...
job in the database it spawns the DARE-WRAPPER by creating the necessary configuration files. 
...
\jhanote{what is a pattern? Has that been defined?}  execution. In
other words it implements the logic necessary to create jobs of single
