...
access level (L1) and resource layer (L5). Development is therefore
modular and layered. Adding support for a new resource, for example
OpenStack based clouds in L5, is as simple as adding the required SAGA
adaptors in L4. No changes need to be made at L1 or L2. Similarly,
adding more execution patterns and workflows in L2 is decoupled
from the runtime system in L4 and the resources involved in L5.
...
generating the necessary objects required
by the Pilot-API by combining the user customized configuration files.

These configuration files are carefully categorized into three different types: 
...
ConfTypeIII is critical because it has the information 
for that specific application's instance. Since the same application may run faster or slower 
based on the input  file size. So it important to let user decide the runtime different 
tools, which resources and their size based on input files.
Further user may also want to change one tool in the various 
tools he has current workflow. This can be achieved just by
...
but only one file with ConfTypeIII.  More information
about different parameters in configuration files can be found in the DARE-wiki ~\cite{dare_api_web}.
...
With these details it creates objects necessary for P*-API.
...
information provided in the configuration files DARE understands
which tasks need to submitted at the right time for successful
completion. The tasks and input/output files
will be distributed over different machines and DARE needs to keep
track of them.

Before a next task in submitted inside DARE to the Pilot-API, it
should check for the status of its dependent task.  More information
about the management and scheduling can be found the DARE wiki.  This
...
named ``\textbf{dare-run}'' to interact with DARE framework.  This tool takes a
...
The database in DARE-Gateways is defined in such a way it could hold
...
Its functionality is to continuously ping the database for new
jobs and whenever it finds a new job it spawns a
DARE-Wrapper after it creates the necessary configuration files.
Further, connecting the web interface to the server-side DARE-API services
...
and have their Gateways write DARE configuration files.  All the
required components to run a workflow written using DARE-API are
described in a configuration file(ConfTypeIII) and have this configuration 
submitted to the DARE-Wrapper.
...
The application layer implements the logic necessary for pattern of
%\jhanote{what is a pattern? Has that been defined?}  
execution. In other words it implements the logic necessary to create jobs of single
standalone applications, pipelines and dynamic workflows. It does not
run scripts on resources but is a necessary abstraction over the
actual runtime systems. \jhanote{I do not know what ``.. is a
  necessary abstraction over the actual runtime systems'' means}
...
\subsection{Runtime System Level ($L4$)}

L4 represents the core of the DARE framework. In a nutshell, it
integrates the application-level capabilities (L2, support for Type
I/II/III services) with the HPDC infrastructure (resources/L5) layer. The DARE
...
To cement support for different resources in L5, SAGA was deployed
