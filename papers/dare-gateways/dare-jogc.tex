...
In the following two subsections, we describe some general perspectives that an end-user or a developer are expected to hold. In other words, in order to provide a more specific picture on how DARE prepares and executes a target group of tasks comprising a workflow, we explain a general sequence of underlying tasks for an end-user or a developer point of view.
...
\subsection{DARE: user's perspective}
 ...
 Some new terms that are introduced in this section and the next are as follows

\textbf{dare-run}: This the name of the executable which instantiates DARE with certain configuration files as the input.
 
\textbf{dare-manager}:  This is a module inside DARE package which takes control over the execution from dare-run. It is responsible for managing several  
functions with in dare like preparing configuration files, failure, termination.  

\textbf{job}: The where a user submits to our Gateway. It generally contains several information about kind of tools 

\textbf{pilot unit}: This is an object  where it is supposed to have information about remote resources a user tries to utilize. This is utilized to launch pilot agents.   It contains attributes like authentication, working directory, data transfer url. 

\textbf{step unit}: This is also an object inside DARE which holds the information about certain  computation and data transfers that are needed to be performed. For example, in bfast first we have to do 
indexing, then mapping and then alignment. In DARE we consider indexing as step 1, mapping as step 2 and alignment as step 3. Now, Step 1 i.e. mapping step has information about the input files that need to transferred to remote resource(data unit) and a computation that needs to be performed.

For more information about pilot agent, compute unit and data unit please refer to P*.
...
at least one from the files which go by ".cu" and ".pilot" extension. The following is the description about the three kinds of configuration files.
...
When dare-run command is triggered with the dare configuration file with ".dare" extension, it starts a dare-manager module.  
...
For preparing a workflow, the dare-manager utilizes a customized configuration file parser which is packaged inside DARE.
First, the parser reads a section called 'main' from the main it goes onto preparing pilot units based on the 
parameters mentioned. Also, appropriate pilot configuration file or as a default default the one
available in the DARE package.'.dare' configuration file are also read for the information about resource names, wall time, and size of required computation.  The rest of the job description details should be available in the job configuration file
...
Now the dare-manager starts to create the pilot service with required a data service agent and a compute service agent as necessary on the remote resources with help of pilot units description we prepared in the above step.

Once pilot agents were up and running then manager dispatches a thread for each step. This thread first checks if there are any dependent steps. If it has one or more it waits until all those steps are finished and update their status as done else none it starts running the steps right away. once the step execution is started it submits the input data units in this steps to pilot data service agent and waits for all the data transfers to complete. After the transfers are successful it submits the compute units for the steps. after these compute units were done it goes onto submitting the output\textunderscore data\textunderscore units. Here it keeps updating gateway's database about the status of these steps.
