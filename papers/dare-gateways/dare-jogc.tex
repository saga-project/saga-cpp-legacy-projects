...
\begin{figure}
 \centering
 \includegraphics[scale=0.55]{figures/new_DARE-gateway-arch.pdf}
...
\jhanote{This section is very unsmooth to read. Changes contexts/flow
  too many times} User interface level for DARE are of two types
DARE-WRAPPER (command line interface) and DARE-Gateways (web
interface).  DARE-API \jhanote{sorry, is this DARE-WRAPPER or
  something different?} is a framework which provides the capabilities
of runtime environment for executing application over distributed
resources. More details about DARE-API are discussed in the DARE-API
Level(L3).

\jhanote{I see both DARE-GATEWAYS and DARE-gateways. Which is correct?}

\jhanote{I still don't understand how one can instantiate an API?}

DARE-WRAPPER provides a simple command line interface to DARE-API with
an executable named ``textbf{dare-run}''.  This executable takes a
configuration file as an argument. It also instantiates DARE-API with
the give configuration file.

%There are two interfaces to access DARE functionality: DARE-API and
%DARE-GATEWAYS.  
...
The other type of interface is DARE-GATEWAYS, which provides a simple
web interface for the user to utilize the capabilities of DARE-API.
Every Gateway also contains user authentication to submit jobs.
...
input files and tools. For example in DARE-NGS(a DARE-GATEWAY) we have
form for CHiP-SEQ application where user can choose parameters like
...
The database for DARE-GATEWAYS is defined so that it could hold
information about the job submitted by the user as well as the user
authentication information for registration and login.  Another
important component in DARE-GATEWAYS is the job monitoring thread.
Its functionality is to continuously ping the database for new
jobs. Whenever it finds a new job in the database it spawns a
DARE-WRAPPER after it creates the necessary configuration files.

The DARE-GATEWAYS web server is built using
Pylons~\cite{pylons_website} web framework .  The Pylons project is a
...
pattern and is written in Python.  Pylons handles everything from user
authentication, database creation/management, and data
storage/retrieval. Creating a user interface is straightforward and
``connecting'' the user interface to the server-side DARE-API services
is surprisingly simple with job monitoring thread.

First step to create a custom Gateway is to create configuration files
for the targeted application using new tools in that applications
workflow and test the new configuration with the standalone
DARE-WRAPPER.  Next step is to create web forms with necessary input
fields and then a simple adaptor to convert the user submitted web
form in to a configuration file that DARE-WRAPPER can understand and
instantiate DARE-API.
...
\jhanote{what is a DARE-API workflow? Do you mean a workflow written
  ...
  an executable?}  \jhanote{How do you instantiate an API so as to
  execute a workflow!}

Further, gateway developers can use DARE-API directly (Levels 2 and 3)
...
described in a configuration file. This configuration file is then
submitted to the DARE-API's executable that launches executes the
workflow by instantiating the P-* API. \jhanote{BTW, its the
  Pilot-API}
...
\subsection{DARE-API ($L3$)}

L3 represents the DARE-API. The important feature of DARE is to
utilize different distributed resources and this complexity involving
the execution of tasks and file transfers on distributed resources
will be handled by the Pilot-API.

The Pilot-API provides a common access layer to Pilot Job (Compute)
and Pilot Data (File transfers) frameworks over distributed production
cyberinfrastructure, such as XSEDE, OSG, EGI and FutureGrid.  Thus it
provides the ability to distribute workloads in a scalable
manner~\ref{pilot-api}. Pilot-API hides the complexity involved in
remote job executions so it is used inside DARE-API.

DARE-API features include generating the necessary objects required
by the Pilot-API by combining the user customized configuration files.
These are categorized in to three different types: (i) one with resource
information like job submission url (ConfTypeI), (ii) second holds
information like location of a tool on different resources and
arguments that should be used for tools(ConfTypeII), and (iii) finally with
workflow information like tools need to executed, location of input
files, ConfTypeI and ConfTypeII etc.,(ConfTypeIII). \jhanote{previous
sentence difficult to understand}. There can be
multiple ConfTypeI, ConfTypeII files for a single instance of DARE-API
but only one file with ConfTypeIII.  

DARE-API comes with some predefined configuration files.  After
instantiation of DARE-API first loads ConfTypeIII after that it loads
ConfTypeI, ConfTypeII based on the location specified in ConfTypeIII
other wise it looks for default files. and these are customizable
based on the requirements as they are stored in simple text files.
\jhanote{Not sure what the above is trying to say} More information
about the DARE-API configuration files can be found in the DARE
wiki~\cite{dare_api_web}.

DARE-API comes with capability to schedule tasks and movement of files
between resources when it is required during the runtime.  Using the
information provided in the configuration files DARE-API understands
which tasks need to submitted at the right time for successful
completion.  \jhanote{DARE-API does not come with the ability, it can
  be used to implement the ability!}  The tasks and input/output files
will be distributed over different machines and DARE needs to keep
track of them.

Before DARE-API submits the next tasks to the Pilot-API, DARE-API
...
automatic scheduling feature of DARE-API is very helpful when the user
has multiple tools in the pipeline for execution where the output
files of one step become the input files for the next.

Another important feature integrated into DARE-API is the ability to
