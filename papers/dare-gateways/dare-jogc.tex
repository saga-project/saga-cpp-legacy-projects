...
DARE user interface layer are categorized into two types DARE-WRAPPER (command line interface)  and DARE-Gateways web interface for DARE-API. DARE-API is a
framework which provides the capabilities of runtime environment for executing application over distributed resources. More details about 
DARE-API are discussed in next section.

DARE-WRAPPER provides command line interface to DARE-API with an executable 
named ``textbf{dare-run}''.  This executable take a configuration file 
as an argument and hands it over to DARE-API.
