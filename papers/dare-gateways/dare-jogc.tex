...
DARE-API is something which provides the capabilities of DARE using command line.
DARE-GATEWAYS is something which provides web interface to DARE utilizing the DARE-API

DARE-API uses P*-API. The Pilot-API provides a common access layer to Pilot Job (compute)and Pilot Data(File transfers) frameworks over distributed production cyberinfrastructure, such as XSEDE, OSG, EGI and FutureGrid. Thus it provides the ability to distribute workloads in a scalable manner~\ref{pilot-api} which is used in full extent  inside DARE.

DARE-API comes with some predefined resource, compute and data information and the user will be able to customize them based on the requirements and they are stored in simple text files. More information about the DARE-API configuration files can be found in the DARE wiki~\cite{dare_api_web}.

Key features of DARE-API include a command line interface, generating the necessary objects required by the P-* API (Pilot data and Pilot Job) by combining the user customized configuration files.
...
DARE-API acts just like file reader for P*-API when the tasks are not dependent on each other (TYPE I).
DARE-API comes in hand when the scheduling of tasks and movement of files
between resources is required. Using the configuration files DARE-API understands what tasks
need to submitted at the right time for successfull completion.
This management feature inside DARE-API is useful when the user has multiple tools in the pipeline for execution
 where the output files of one step become the input files for the next((TYPE II)).  
Since the tasks and files will be distributed over different machines DARE-API should continuously monitor 
the status of each task before submitting the next tasks to the P*-API. More information about
the management and scheduling can be found the DARE wiki.
Another important feature integrated into DARE-API  is the
ability to update the recent status of the job to a specified database. This  updated status can then be used to
show status of user submitted jobs in gateways. 
...
thus easily interfaces with DARE \jhanote{Or DARE-GATEWAY?}. Pylons
handles everything from user authentication, database
...
it could hold information about them \jhanote{What is them?} and
retrieved when DARE-API instance is started for the execution. Every
...
Further, gateway developers can use DARE \jhanote{have we defined what
  the DARE components are?} components directly (Levels 2 and 3) and
have their Gateways write DARE configuration files.  All the required
components to run a DARE-API workflow \jhanote{what is a DARE-API
  workflow? Do you mean a workflow written using DARE-API?} are
described in a configuration file. This configuration file is then
submitted to the DARE-API's executable \jhanote{What does this mean?
  How can an API have an executable?} that launches executes the
workflow by instantiating the P-* API.  \jhanote{How do you
  instantiate an API so as to execute a workflow!} 
