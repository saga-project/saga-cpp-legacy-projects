...
\begin{abstract}
  ...
  not scalable.  We introduce the SAGA-based, Distributed Application
...
and utilizes SAGA to achieve the IDEAS objective.
...
SAGA-based Pilot-Job capability, the SAGA-BigJob\cite{saga_bigjob_condor_cloud}.
...
\jhanote{Merge...} Currently all of the above requires scientists to
...
\subsection{DARE: An Extensible Software Framework for Distributed
...
fluctuation across platforms, and to the availability of dynamic data.
...
abstractions, and run-time decision making.

The use of SAGA to build the DARE framework overcomes a traditional
limitation of a frameworks: the lack of generality and coupling to
specific infrastructure.

\jhanote{merge} The DARE framework enables the simple utilization of
these additional capabilities -- either as execution patterns or
actual functional capabilities.  L2 provides application level support
at the programmatic level to exploit identified abstractions and
cross-cutting concerns; L3 provides runtime mechanisms that manage the
execution of the application.  For example, the proposed runtime
framework will be designed to support both dynamically .... Different
methods and mechanisms to be used for each..., however these methods
will be integrated in the same runtime framework.  At the AF level,
taking data-access patterns as an example, there exist multiple
data-access patterns that can be used to express computations for
data-intensive scientific applications. Often several of them can be
used simultaneously, thus there is need for the implementations of
these patterns to be necessarily extensible, general, and
multiplatform.
...
with the flexibility that scientific applications demand.  DARE
represents the first-steps towards developing an integrated runtime
and application framework that provide abstractions to
...
complexity of the reference genome and the data-size of short reads, but mostly algorithms\cite{dare-ecmls11}.
...
FutureGrid were also conducted\cite{dare-ecmls11}. Using the results obtained from such an
...
\jhanote{merge} Table~\ref{table:NGS-Distributed-file} highlights some
data volumes that typically need to be transferred with data analytics for
DARE-NGS with a model system (human genome).

\jhanote{merge} As an example of L2 for DARE-NGS,
...
% full-functional access layer. 

DARE facilitates the quick implementation of a users scientific logic
in the application layer with python scripts, whilst enabling the easy
integration with the HPDC infrastructure/resources.  The objective --
scalability ({\it distributed scale-out}) is illustrated in
Table~\ref{table:NGS-Distributed}, where two large TG/XSEDE systems --
QB and Ranger, were used for the match step in BFAST pipeline. As
...
design objectives. Each of these science applications has been
supported via both {\it command-line} as well as {\it science gateway}
interfaces. 
...
data\cite{dare-ecmls11}. We have also used DARE-based gateways for
