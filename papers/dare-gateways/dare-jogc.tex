...
extensible capability were provided this {\it gap} could be overcome.
 ...
 \jkimnote{the following is from Nancy Wilkins-Diehr paper and her observation is needed to be combined with the above} With TG, in 2006, 10 gateways used fewer than 100,000 CPU hours. In 2010, 22 gateways used 40 million hours.  By 2011, nearly 40 \% of users on TG came through gateways. In particular, the CIPRES phylogenetic gateway accounts for 25 \% of all TG users. 
...
% \jkimnote{I suggest this section should be revised for the overview on the need of gateway, in particular with XSEDE}.
...
\begin{figure}
  ...
  \caption{\small \jhanote{Use updated figure}}
  \label{fig:dare-arch} 
\end{figure}
...
\subsection{User Interface Level ($L_1$)}
The combination of the Access Layer
...
\subsection{Applications, Capabilities and Services Level ($L_2$)}
...
\subsection{Runtime System Level ($L_3$)}
\jkimnote{will revise this}
The challenges in building a scalable
...
\jkimnote{any kind of overview along with the current implementation
  or troy-based direction?}
...
Nationwide CI infrastructures such as TG and its successor XSEDE are increasingly playing an important roles for large scale scientific computation and science gateways have been making substantial contributions.  With DARE, our approaches to such a federated grid system are two folds. One is the use of large distributed HPC systems of XSEDE program as resources for a gateway, and the other is to provide an entire gateway via XSEDE.  The latter is based on interesting progress with XSEDE program.  XSEDE has Indiana University's Quarry
system, which is a gateway hosting resource consisting of geographically
