...
\begin{abstract}
  ...
  cyberinfrastructure such as TeraGrid/XSEDE. In particular, they have
  been very successful as a shared/community access models, and thus
  have been able to ``address the needs of the masses''.  However,
  there are missing capabilities and abstractions that enable the use
  of the collective capacity of distributed cyberinfrastructure such
  as TeraGrid/XSEDE, especially those that can be used to develop
  gateways in an easy, extensible and scalable fashion for both
  compute and data-intensive applications.  This is a serious barrier,
  as Gateways often require customization for specific user groups and
  infrastructure usage modes.  So whereas a single science gateway is
  a scalable access mechanism in terms of number of people, that can
  be supported the development of, the development of multiple similar
  gateways but with differing infrastructure usage and requirements is
  not scalable.  We introduce the SAGA-based, Dynamic Application
  ...
  distributed infrastructure can be built. We analyze a range of
  life-science applications, and establish that DARE provides an
  extensible and simple framework which applications can use.  DARE
  along with SAGA provides effective interoperable middleware upon
  which to build such gateways.  We discuss the architecture of the
  DARE framework, discuss how gateways can utilize the framework, and
  present representative life-science gateways -- DARE-HTHP, DARE-NGS.
\end{abstract}
...
% An interesting corollary is that the rate-of-increase of
...
phylogenetics.  % MD simulations have
% mostly relied on scaling-up to high core-counts, whilst virtual
% docking calculations benefits from high-throughput calculations.
...
% with the exception of a few common applications, e.g., large-scale
...
A primary reason is the additional and significant complexity in
providing the right abstractions so as to enable an application to be
able to seamlessly utilize distributed cyberinfrastructure.  We
observe with the advent of federated grids such as TG and its
transition to eXtreme Science and Engineering Discovery Environment
(XSEDE)~\cite{XSEDE}, such requirements become harder to achieve as
the number of connected sites and heterogeneity grows.
...
objective of Science Gateways is to capture the common requirements,
and provide the right abstractions to support them, whilst making the
application/usage-mode specific parts easy to support. This objective
imposes an important constraint / requirement on the tools \&
frameworks used to develop Science Gateways.

Viewed from a different perspective, there are hard parts (ie
integrating with HPDC infrastructure) and there are specific parts,
viz., customization for specific applications. Effective middleware
...
Thus building effective GWs has two major tracks: the first is the
development of effective UI which are cutomizable and extensible to
the end-user communities need and specific scientific
requirements. The second is the integration of these UI to a broad
range of infrastructure to support the requirement but in a way that
is scalable and flexible. These require different kinds/types of often
non-overlapping expertise. The role of effective middleware for
Gateways is to support the effective integration of these two
different tracks, viz., enable different application-specific UIs to
utilize a range of high-performanc and distributed computational
(HPDC) infrastructure.


%middleware are about providing capturing 

Additionally, it is always challenging to integrate effectively new
applications, to implement distributed data management with newly
added infrastructure, whilst supporting existing and novel execution
...
The contribution of this work addresses the following problem
statement: Although, Science Gateways have proven to be very effective
...
We illustrate and demonstrate the development of a runtime environment
that supports as a primary design objective the distributed execution
of applications on HPDC infrastructure.

The runtime environment -- DARE, supports the general goals of
distributed applications: interoperability, distributed scale-out,
extensibility, adaptivity, simplicity -- referred to as
IDEAS\cite{ideas}. For instance, previously, as a modest step towards
addressing point (iii), we have shown that a flexible Pilot-Job
...
applications. We introduce the Dynamic Application Runtime Environment
(DARE) framework that is built upon SAGA and establish its
effectiveness as an integral component for life-science gateways by
demonstrating that it can effectively utilize the collective
capabilities of distributed cyberinfrastructure such as the TG/XSEDE
and LONI.
...
\jhanote{Need to discuss before closing the introductary section: What
  do we mean by Dynamic?}

A primary contribution of this work is the DARE framework. Before we
introduce the DARE framework we discuss the ``D'' in DARE. The
Dynamics aspects come from both an understanding of
applications/analytical requirements as well the
runtime/infrastructure characteristics.

The fact that applications can cahange as a conequence of the results
...
The HPDC infrastructure utilized is in often variable/varying, and
thus runtime decisions related to scheduling and resource aggregation
need to be taken. Pilot-Jobs have been shown to be an excellent
...
applications as well~\cite{pstar11}.

As we will discuss in the next section, the DARE framework makes
extensive use of Pilot-Jobs and is also designed to support dynamic
applications as a primary/fundamental attribute.
...
% distributed applications enables the effective and quick
% construction of lightweight, extensible, gateways that can utilize
...
% DARE: THE CASE FOR COMMON FRAMEWORK TO BUILD SCIENCE GATEWAY FOR  LIFE-SCIENCE APPLICATIONS}
...
%\subsection{DARE}
% A high-level abstraction to support execution
...
\jhanote{of what?} has been determined, a gateway developer can easily
...
\begin{figure}
 ...
 \fbox{
   ...
   \includegraphics[scale=0.2]{figures/screenshot-ngs.png}
   \includegraphics[scale=0.2]{figures/screenshot-hthp.png}
