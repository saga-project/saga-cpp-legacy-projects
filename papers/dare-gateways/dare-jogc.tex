...
DARE-API comes with some predefined resource information, compute and data information and the user will be able to customize them based on his needs. These customizable parts are divided into three different configuration files


Key features of DARE-API include a command line interface, generating the necessary objects required by the P-* API (Pilot data and Pilot Job) by combining user customized configuration files.
...
Another important feature \jhanote{feature of what -- DARE-API or
  Pilot-API? Why can't we just use Pilot-API?} is it also updates the
recent status of the job to a specified database which can be used to
show status of a job in gateways. The difference between DARE-API
\jhanote{Difference between DARE-API and what? Unclear...}  is obvious
that DARE-API generate objects including pilot/resource information,
compute information and the data transfers information and hands them
to P-* API for execution. In other words DARE creates those objects on
the fly for each instance of DARE and hands them to P-* API for
execution.  \jhanote{Isn't that a usage mode of an API rather than a
  different API?} More information about the DARE-API configuration
files can be found in the DARE wiki~\cite{dare_api_web}.

Other type \jhanote{type of what?} is DARE-GATEWAYS which provides a
web interface and uses DARE API \jhanote{need to be consistent: DARE
  API or DARE-API?} executable with customized configuration files in
