...
\jhanote{Isn't that a usage mode of an API rather than a
  different API?} 

Other type of interface 
%\jhanote{type of what?}
 is DARE-GATEWAYS which provides a
web interface and uses DARE-API 
%\jhanote{need to be consistent: DARE API or DARE-API?} 
executable with customized configuration files in
the backend.  The DARE-GATEWAYS user interface is built on top of
Pylons~\cite{pylons_website}. \jhanote{This doesn't explain what
  DARE-GATEWAYS is?}
...
Model-Viewer-Controller design pattern and is written in Python and
thus easily interfaces with DARE-GATEWAY  
%\jhanote{Or DARE-GATEWAY?}
...
the server-side services in DARE was surprisingly simple.

Most Gateway commonly contain a well defined problem set where user
want to just push a submit button with some minor changes in input
files and tools. So a database inside DARE-GATEWAYS is defined so that
it could hold information about the job for example inout files, tools, size and runtime of job 
%\jhanote{What is them?} 
and retrieved when DARE-API instance is started for the execution. Every
gateway also contains some kind of user authentication to submit
jobs. Different DARE-GATEWAYS have different web forms and workflow
behind them.

Further, gateway developers can use DARE-API 
 \jhanote{have we defined what  the DARE components are?} 
 components directly (Levels 2 and 3) and
have their Gateways write DARE configuration files.  All the required
