...
(i) ConfTypeI: This holds the details about resources/machines for example job submission url, file transfer url,  authentication type, 
(ii) ConfTypeII:  With information about a tool like location of a tool on different resources and
arguments that should be used for tools, and 
(iii)ConfTypeIII:  Application instance specific information like location of input
files, which series of  tools (ConfTypeII) to use and resources as well as their size, wall time (ConfTypeI) etc..
...
First step to create a custom Gateway is to create/modify configuration files(ConfTypeII)
...
form into a configuration file(ConfTypeIII). 
...
introduced the functionality to create/write a  a configuration file(ConfTypeIII) 
...
ConfTypeIII holds the critical part of a job(instance of application) as it has the information 
...
changing a parameter with a  different ConfTypeII file in ConfTypeIII. 
There can be multiple ConfTypeI, ConfTypeII files for a single instance of DARE-API
but only one file with ConfTypeIII.  Additonal information
about other parameters in configuration files can be found in the DARE-wiki ~\cite{dare_api_web}.
...
During the DARE's runtime first it loads ConfTypeIII after that it loads
various ConfTypeI, ConfTypeII based on the location specified in ConfTypeIII.

DARE framework's scheduling mechanism entirely depends on the
parameters in  ConfTypeIII. For example, there are two tools(Tool A, Tool B) 
...
parameter in a ConfTypeIII file. DARE scheduling system waits until
...
is a combination of various configuration file (ConfTypeII, ConfTypeIII) to form a pattern of
service.
...
sophisticated execution\jhanote{what does sophisticated execution
  mean?}. In contrast Type I and Type II services aim to serve
