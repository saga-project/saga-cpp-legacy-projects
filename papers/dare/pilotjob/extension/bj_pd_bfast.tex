...
data. \alnote{Communication inside/outside BigJob; Who communicates? How?}
	...
	\item \textbf{Dynamic Resources:}
		...
		\end{itemize}
		\alnote{TODO: define atomic pilot, multiple pilot model}
...
\begin{enumerate}[A.]
	...
	\end{itemize}
	\item \textbf{Unit of Work:} A unit of work is a workload that is independent form a 
	resource. A unit of work can be assigned to a sub-job/job.
...
\hline
 &BigJob &Diane\\
\hline
MPI &yes &yes (Customer implementation of ApplicationWorker required)\\
\hline
Advanced Scheduling &no &yes (ITaskScheduler)\\
\hline
Dynamic Resources &yes (Dynamic BigJob) &yes (AgentFactories)\\
\hline
API for Agent Submission &yes &no\\
\hline
Application Interfaces &Sub-job Execution & Application-level API (\texttt{ITaskScheduler}, \texttt{IApplicationManager}, \texttt{IApplicationWorker}) \\
\hline
...
\begin{tabular}{|l|p{4cm}|p{4cm}|}
\hline
\textbf{Term} &\textbf{BJ} &\textbf{Diane}  \\
\hline
Central Coordinator &BJ Manager & Master/RunMaster \\ 
\hline
Resource Agent &BJ Agent  & Worker Agent \\
\hline
Capacity of Agent &1 BJ Agent per resource & 1 Worker Agent per core \\
\hline
