...
\jhanote{Mark to put in a para about how the API implements different
  Pilot-Jobs} \msnote{In hindsight not sure why it was at this place, and what you expected but here is an attempt:}
...
SWIFT~\cite{} relies on its scripting language to describe abstract workflows
and computations. The language provides among many things capabilities for
executing external application as well as the implicit management of data flows
between application tasks. For this purpose, SWIFT formalizes the way that
...
pilot-jobs for which the term Coaster is used. For the implementation of the 
Coaster mechanism SWIFT relies on Condor-G.
...
Internal Coordination &Master/Worker (push) &Master/Worker (pull/push) &Master/Worker &\\ \hline
