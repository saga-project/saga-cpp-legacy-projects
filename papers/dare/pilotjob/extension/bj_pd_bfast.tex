...
data.
	...
	\begin{itemize}
     ...
     \item \textbf{Dispatching / Distribution of agent} (we could call it resource agent or sub-agent):
        \begin{itemize}
    	    \item No distribution: BigJob-Cloud -- direct dispatching of sub-jobs from manager
            \item Lightweight resource agent: Diane -- Agent on resource is used as a dispatcher (capacity = 1 sub-job)
            \item BigJob heavy resource agent -- full blown: resource management, sub-job dispatching, state management...
    \end{itemize}
	    \end{itemize}
	\item How many resources are managed by an Agent (Diane Worker)? How many 
	tasks can be assigned to a agent? Single core worker, partial node worker, 
	whole node worker, multi node worker (BigJob
...
\begin{itemize}
	...
	\end{itemize}
	\item B. Sub-Job / Task / Job = the Unit of Work that is sent down to "A" to 
	be executed, is an instance of "I": Resources assigned to a single task. 
	\begin{itemize}
	   \item Task is a workload that is independent form a resource. A task will be assigned to a sub-job.
       \item Sub-job is a schedule-able unit (you don't need to have an agent
        per sub-jobs).    	
	\end{itemize}
