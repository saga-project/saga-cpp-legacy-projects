...
default). This characteristic influences: 
	...
	\item \textbf{Security}
	\item \textbf{Push vs Pull} We define the terms push and pull based upon the 
	determinism of the binding. If the task is centrally explicitly addressed to 
	a specific pilot, and there is thus no freedom for a pilot to select the 
	task, we speak of a push model. In all other situations, when there is a 
	degree of freedom for the pilots to select a task, we speak of pull.
...
\begin{enumerate}[A.]
	...
	\end{itemize}
      \item \textbf{Atomic vs. Dynamic Pilot-Jobs:} \jhanote{Is this a
          ...
          a model of understanding}\alnote{No, that's why it is in definitions and not in the model. Once we all know what we're talking about then we can remove it again.}
	\begin{itemize}
