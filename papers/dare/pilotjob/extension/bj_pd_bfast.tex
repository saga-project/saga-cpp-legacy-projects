...
\item\textbf{Pilot-Job:} A PJ is a container for multiple
  ...
  scheduled on a resource (\ref{resource}).  PJ provides a mechanism
  to decouple “tasks'' from being hard-coded to a specific
  “resource''.  \alnote{ the "thing" that is sent out to
    \ref{resource}} \label{pilot-agent}
...
\item \textbf{Unit of Work (UW):} A unit of work is a workload that
  will be assigned to a resource indirectly via a Pilot-Job.  The PJ
  in turn has flexibility in determining when and how-many resources
  the UW will receive.
...
\item \textbf{Unit of Scheduling (US) entities:} are entities that get
  scheduled to the resource, and to which UW are assigned (inside
  the PJ).
...
%   by a task.
  ...
  kernel~\ref{application-program}. \alnote{Should sub-job be a separate definition?}
...
\begin{itemize}
	...
	\item The \textbf{End-user abstraction} is the external API (respectively interface) that is exposed by the P*-framework to the applicaiton.
	\item \textbf{Task Binding} describes the 
	assignment of a UW to a pilot-job.
    %   \begin{itemize}
    ...
    %       \end{itemize}
    % \end{itemize}
	\item \textbf{Task Scheduling:} Task scheduling describes 1) the process of  
...
\end{itemize}
	...
	\textbf{Shantenu's simple model: }
     Pilot is executed on a certain resource\\
     UW gets bound to a pilot\\
     UW gets scheduled to a US  (either one or multiple UW can be 
     scheduled to a US)\\
     Properties of the US are dependent on the pilot job which they 
     are running in.\\
     Scheduling has temporal and spatial component (not only which, 
     but also when)\\
     US gets scheduled to a physical resource on which the pilot is 
     operating\\
...
\subsection{Authentication, Authorization en Accounting}
