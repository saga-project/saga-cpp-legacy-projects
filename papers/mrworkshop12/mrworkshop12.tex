...
\maketitle
\begin{abstract}
In recent years, there has been a large increase in the size of computational data, which is widely distributed across different geographic locations. Timing and cost-effective processing of these large-distributed datasets on distributed computational resources requires effective management of data and compute resources. MapReduce is an effective programming model for processing large distributed datasets. The productiveness of MapReduce on widely distributed data depends on effective management of computation(map and reduce) and intermediate data(shuffle) units.  Hierarchical MapReduce is suggested as a potential distributed solution only for high data aggregation applications leaving single cluster-MapReduce as a optimal solution for zero or ballooning data applications~\cite {weissman-mr-11}, which is still costly as it involves initial huge data transfers of source data to the single cluster. To optimize the runtime of the zero and ballooning data applications, we provide an infrastructure independent MapReduce framework using SAGA implementations of Pilot abstractions; Pilot-Jobs and Pilot-Data. Pilot-MapReduce framework utilizes Pilot-Jobs to couple the map phase computation to the nearby source data, and Pilot-Data to move intermediate data using parallel data transfers to the reduce computation phase. Our experimental evaluation using Wordcount on Random Data over Pilot-MapReduce(PMR) shows that Pilot abstraction for data movement across multiple clusters is promising, and thus minimizes overall execution time span of the entire MapReduce execution. We also show the scalability that can be achieved using PMR on multiple clusters by comparing it with SEQAL which is a Hadoop based MapReduce application.
\end{abstract}
