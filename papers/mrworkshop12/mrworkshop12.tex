...
is smaller than the output data (such as in the natural language case). If the
data is distributed, the performance of Hadoop is impacted by the time necessary
to move the input files to the single Hadoop resource. However, generally the
map and reduce phase runtime of PMR are longer than for Hadoop mainly due to the
resource heterogeneity and the resulting scheduling overhead: the slowest node
...
needs to be moved from Sierra to Hotel is 15.5\,GB, and data related to each
