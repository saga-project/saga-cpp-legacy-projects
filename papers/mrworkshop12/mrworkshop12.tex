...
datasets. The effectiveness of MapReduce on widely distributed data
depends on effective management of computation (map and reduce) and
intermediate data (shuffle) units, for applications with different
ratios of data aggregation (between map and reduce phase).
%To optimize the runtime of the zero and ballooning data applications,
In order to provide a flexible runtime environment, we design and
implement an infrastructure independent MapReduce framework using
SAGA-based Pilot abstractions for both compute (Pilot-Jobs) and data
(Pilot-Data). This framework -- called Pilot-MapReduce (PMR) utilizes
Pilot-Jobs to couple the map phase computation to the nearby source
data, and Pilot-Data to move intermediate data using parallel data
transfers to the reduce computation phase. Our experimental evaluation
using Wordcount on Random Data over Pilot-MapReduce(PMR) shows that
Pilot abstraction for data movement across multiple clusters is
promising, and thus minimizes overall execution time span of the
entire MapReduce execution. We also show the scalability that can be
achieved using PMR on multiple clusters by comparing it with SEQAL
which is a Hadoop based MapReduce application. We analyze the
effectiveness of PMR for different applications based upon the
ratio of data-aggregation between phases as well as single
...
distributed data processing based on the data aggregation 
~\cite{weissman-mr-11}. In high-data aggregation application the MapReduce
...
A critical aspect of MapReduce, is the management of data and compute localities as well as the management of data movements, e.\,g.\ between the map and the reduce phase. 
The efficient support of these capabilities via the Pilot abstractions as well as the limitations of existing MapReduce implementations motivated this work. 
Pilot abstractions are classified into two types; Pilot-Jobs and Pilot-Data. Pilot-Jobs have been notable in their ability to manage large numbers of 
...
presents related works. Section 3 gives an overview of \pilot abstractions. 
Section 4 gives an overview of our \pilotmapreduce framework. Section 5 gives 
experiment setup and result analysis. The conclusion and future work are given 
in Section 6.
...
Dryad provides distributed execution of coarse-grain data-parallel applications.
The entire execution is represented in the form of data flow graphs, where the
vertices represent the computational tasks that can be paralleled on a set of
computational resources and the links between vertices tell Dryad what other
vertices need to complete before a particular vertex can start.
...
Pilot-abstractions provide effective management of compute and data units and the relationships between them(affinites).  They liberate 
...
PMR can be easily scaled to multiple clusters. A {\it local-PMR}  performs the map and reduce computations on a single cluster. 
A {\it distributed-PMR} provides an opportunity to utlize multiple distributed producton cyberinfrastructure clusters to run the map and reduce computations.
...
Similarly, Hadoop uses mapred.reduce.parallel.copies parameter to set the number of parallel transfers run by reduce during the copy(shuffle) phase. 
and use key names for pieces of data which inform Hadoop how to send related bits of information to a common destination node.[http://developer.yahoo.com/hadoop/tutorial/module1.html]
...
\pilotmapreduce utilizes a set of compute and data pilots for this application
workflow:
\begin{enumerate}
...
\begin{figure*}
	\centering
	\subfigure[Distributed PMR]{
		\includegraphics[width=0.4\textwidth]{figures/distributed-mapreduce.pdf}
