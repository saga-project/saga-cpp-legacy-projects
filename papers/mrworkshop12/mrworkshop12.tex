...
Word Count application on random data. In the distributed scenario (i) \pilotmapreduce map CUs are distributed across 
...
For the distributed data processing of random data using the Word Count application, 
we try to compare the Distributed PMR with with Hadoop MapReduce on a single cluster. We consider that 
the initial source data is equally distributed on Sierra and Hotel.
We utilize two machines:sierra and hotel. The number of workers for Word Count is set to 8 on Hotel machine.
In case of distributed PMR, the number of workers is divided equally between Sierra and Hotel. In both the cases,
the chunk size is set to 128\,MB, number of reduces to 8. As the input data is varied the runtime for both the MapReduce increased.
...
Figure~\ref{fig:hadoopvsdistp} shows the results.
The runtime of Hadoop MR is affected by the time taken for initial amount of data transfer. \alnote{What is transferred from where to where?} For
...
But the map phase and reduce phase time of the  distributed PMR are high because of scheduling overhead of CUs involved in PMR.
...
For Genome Sequencing application we utilize two machines: india and hotel; the number of
...
Figure~\ref{fig:hmrvsdist} shows the results. In both scenarios the
