...
data pilots for this application workflow: \jhanote{Worth noting that
  ...
  reader doesn't look to figure 2 to understand these steps..}\alnote{changed enumeration list to A., B. referencing steps in fig.}
\begin{enumerate}[A.]
...
\pilotmapreduce supports different distributed MapReduce topologies: (i)
\emph{local}, (ii) \emph{distributed} and (iii) \emph{hierarchical}. A local PMR
(i) performs all map and reduce computations on a single resource.
Figure~\ref{fid:distributed-mapreduce-overview} shows options (ii) and (iii): A
distributed PMR (ii) utilizes multiple resources often to run map tasks close to
the data to avoid costly data transfers; the intermediate data is then moved to
another resource for running the reduce tasks. BigJob and BigData are used for
managing CUs and DUs and the necessary data movements. In contrast, in a
hierarchical PMR (iii) the outputs of the first complete MapReduce run are moved
to a central aggregation resource. A complete MapReduce run is then executed on
...
on top of distributed production cyberinfrastructures to form a single larger
...
larger for a random input than for an input text in English. 
