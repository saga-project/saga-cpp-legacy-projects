...
\begin{abstract}
  ...
  increasing exponentially. Often this data is widely distributed;
  inevitably, the ability to analyze data by localizing it will yield
  limited returns. Thus, efficient processing of large distributed
  datasets are required, whilst yet not introducting fundamentally new
  programming models or methods. For example, MapReduce is a proven
  effective programming model for processing large datasets; there are
  reasons to assume that it will be effective in a distributed data
  too. A desirable approach would entail an effective and effecient
  runtime environment that would manage the transtion from local to
  distributed data, without refactoring MapReduce itself. The
  ...
  of intermediate data units).  In this paper, we present the design
  and implementation of \pilotmapreduce (PMR) -- a flexible,
...
Before we proceed further, it is critical to emphasise that it is not
...
%and a reduce script which tries to remove duplicates based on the
%Picard rmdup alogrithm. We were not able to mimic the exact implementation of SEQAL reduce phase as it is a blackbox for us to implement.
...
\begin{figure}[ht]
	\centering
		\includegraphics[width=0.47\textwidth]{figures/wordcount_pmr_hadoopmr.png}
...
input data in a different format (prq instead of fastq); thus, the data was 
previously converted to meet the SEQAL requirements. The fastq files from sequencing 
machines are directly used as input to the PMR. A custom chunk script is used to chunk the
fastq files based on the number of reads. We make sure that the chunk size in both SEQAL and
PMR is equal. We measure the runtime of PMR (including the time for the chunking, mapping, shuffling
...
\begin{figure}[ht]
	\centering
		\includegraphics[width=0.47\textwidth]{figures/seqalvspmr.png}
...
\label{fig:seqalvspmr}
\end{figure}		

Figure~\ref{fig:seqalvspmr} shows the results of both applications. In the
...
% Figure~\ref{fig:seqalvspmr_map} shows the results. Notably, SEQAL requires a longer
...
% from the different implementation between SEQAL and ours.
...
section~\ref{sec:pmr-distributed}) using the genome sequencing application and a 
wordcount application on random data. In the distributed scenario (i) \pilotmapreduce map CUs are distributed across 
...
For the distributed data processing of random datausing WordCount application, 
we try to compare the Distributed PMR with with Hadoop MapReduce on a single cluster. We consider that 
the initial souce data is equally distributed on Sierra and Hotel.
We utilize two machines:sierra and hotel. The number of workers for WordCount is set to 8 on Hotel machine.
In case of distributed PMR, the number of workers is divided equally between Sierra and Hotel. In both the cases,
the chunk size is set to 128MB, number of reduces to 8. As the input data is varied the runtime for both the MapReduce increased.
The runtime for Hadoop MapReduce on Hotel, includes moving data from Sierra to Hotel and loading the entire data into HDFS and  MapReduce 
Job time.

The runtime of Hadoop MR is affected by the time taken for intitial amount of data transfer. For
2\,GB, 4\,GB and 8\,GB, 16\,GB the distributed PMR performs better than the
Local PMR, because the effective amount of data to be transferred is
much less than the initial amount of data transfer in Hadoop MR.
But the Map phase and reduce phase time of Distributed PMR are high because of scheduling overhead of CUs involved in PMR.
\pnote{observed when running distributed PMR; Still need to be modified, But checking in as time is up..}


\begin{figure}[ht]
	\centering
		\includegraphics[width=0.47\textwidth]{figures/wc_hmrvspmr_random.png}
\caption{WordCount on Random Data, Hadoop MR on 1 machine vs Distributed PMR on 2 machines} 	
\label{fig:hadoopvsdistp}
\end{figure}		


For Genome Sequncing application we utilize two machines: india and hotel; the number of
...
\caption{GS/PMR: Hierarchical MapReduce vs Distributed PMR on 2 machines} 	
\label{fig:hmrvsdist}
\end{figure}		
...
% Even though the initial data transfers in case of
% Local-MapReduce \jhanote{this is a term; please define a term before using it}
% are costly, they are needed because of lack of an abstraction which manages the
% relationships between the data and compute units effectively across wide-area
% systems.
...
including HDFS and various other daemons is required. Pilot-MapReduce provides 
more flexibility and extensible building block, which allows e.\,g.\ the  
flexible usage of sorting in the shuffle, more fine-grained control of data 
localities and transferred as well as support for different MapReduce 
topologies. Using this capabilities, applications with different 
characteristics, e.\,g.\ compute/IO and data aggregation ratios, can be 
...
In the future we will extend the capabilities of BigData to support use cases 
such as data streaming, data caching as well as different data/compute 
scheduling heuristics. Further, we will explore scenarios and applications with 
dynamic data and execution.
