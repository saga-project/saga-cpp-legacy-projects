...
\hline
GS/PMR 		&20\,GB &18.7\,GB		 &6.2\,GB\\
\hline
Word Count (English) &16\,GB&29.5\,GB&20\,MB\\
\hline
...
both Hadoop and PMR show a comparable performance. 

A major performance factor for Hadoop in the case of distributed data is the
necessity to move parts of the data (half of the input data) to the central
Hadoop cluster. Both the hierarchical Hadoop and PMR perform better than the
...
comparison to ~29.5\,GB -- the performance in the hierarchical case is
significant better.
...
In general, the distributed PMR
has a slight advantage in favor of the hierarchical PMR if the intermediate data 
is small (such as in the natural language case).  If the data is distributed, 
the performance of Hadoop is impacted by the time necessary to move the input
files to the single Hadoop resource. However, the map and reduce phase runtime
of PMR are longer than for Hadoop mainly due to the resource heterogeneity and
the resulting scheduling overhead: the slowest node determines the overall
runtime of both the map and reduce phase. 
