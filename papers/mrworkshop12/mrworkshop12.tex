...
the resource heterogeneity and the resulting scheduling overhead: the slowest node determines the overall runtime of each computation phase.
...
\caption{WordCount on Random Data, Hadoop MR on 1 machine vs Distributed PMR on 
2 machines} 	
...
For the Genome Sequencing application we utilize India and Hotel and a total of
32 nodes. For both distributed and hierarchical PMR, Hotel is used for running
the reduce tasks respectively the combining MR run. This setup ensures that the
necessary data transfers are minimized.
...
\caption{GS/PMR Hierarchical MapReduce vs Distributed PMR: The hierachic on 2 machines} 	
\label{fig:hmrvsdist}
\end{figure}		
...
\alnote{The following parFigure~\ref{fig:hmrvsdist} shows the results. In both scenarios the runtime
...
agraph needs refinement after data is in}
