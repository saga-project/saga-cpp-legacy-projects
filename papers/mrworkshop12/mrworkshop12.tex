...
\section{PilotMapReduce -- A Pilot-based SAGA-MapReduce Implementation}
\alnote{TODO: Pradeep}
The main aim of Pilot based SAGA-MapReduce is to provide a framwork

provide an implementation of Pilot based SAGA-MapReduce provides decoupling of  MapReduce programming model
with the complex task of assigning map and reduce compute units to the resources. SAGA Pilot-Job is no longer bound to run on a system providing the appropriate semantics orginally required by MapReduce, and is extensible to distributed dynamic cyber infrastructure. SAGA-MapReduce exposes a simple interface which provides the complete functionality needed by any MapReduce algorithm, while hiding the more complex functionality, such as chunking of the input, sorting the intermediate results,  managing and coordinating the workers, etc. — these are generically implemented by the framework.


 The Pilot based SAGA-MapReduce uses SAGA Pilot-API, which provides flexibilty to manage map and reduce compute units and define relationship between the intermediate data and reduce tasks across distributed infrastructure. The SAGA MapReduce framwork is responsible for:
• Chunking the input data, whether data is local/distributed. The SAGA MapReduce framwork uses SAGA Job package to launch user defined chunk scripts on all the machines, which perform the chunking of input data remotely.
• Launching Pilot-Jobs on all the clusters, where the data is located. Once the Pilot-Job becomes active on that particular machine, it starts executing map compute units on the local data. This avoids huge transfer of input data before starting actual MapReduce which is in the case of LMR~\cite{weissman-mr-11}
...still needs to be refined..................
• Collecting and grouping of partitioned and sorted intermediate data output files related to a reduce. The framework prepares 
•  
• assigning the input data to the workers of the map step;
• handling the intermediate data files produced by the map step;
and
• passing the location of the sorted output files to the workers of
the reduce step.
...
On FutureGrid, we used 8 nodes for Hadoop and Local-Pilot MapReduce on India cluster. In case of distributed-Pilot MapReduce, we use a total of 16 nodes in two widely seperated clusers - 4 nodes on India, 4 nodes on Hotel. In case of Hadoop MR, data is already present on India cluster, but needs to be pushed into HDFS before Hadoop MapReduce is started. For local-PMR, data is already present on India cluster which is stored on a shared file system. For distributed-PMR, the total data is divided between India and Hotel cluster and is already present on shared file sytems of each cluster. We set the number of reduces to 8 and chunk size is set to Hadoop default block size 128MB.

\subsection{Measuing Job completion time}
