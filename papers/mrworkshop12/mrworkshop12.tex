...
part of the sequencing workflow i.e, the read alignment %and (ii) the duplicate
removal. The read alignment is implemented in the mapping phase of the
application using BWA aligner ~\cite{Li:2010:FAL:1741823.1741825}. %The read
outputs are filtered for duplicates in the reduce %phase using Picard rmdup
program [ref: Picard.http://picard.sourceforge.net.]. We use
SEQAL~\cite{seal-2011}, a Hadoop implementation of this workflow for performance
comparison with \pilotmapreduce based implementation. SEQAL provides a workflow
to align the reads in the map phase and remove duplicates in reduce phase. Since
we use SEQAL to compare scalability of PMR across clusters, we are interested in
only in the execution of map phase. SEQAL provides an option to turnoff the
reduce phase and just perform the map phase. For PMR a custom mapper script is
developed in python, which uses core BWA aligner.
