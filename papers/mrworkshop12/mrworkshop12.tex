...
\begin{abstract}
  ...
  There continues to be significant increase in 
  ...
  lower execution time span of the entire MapReduce execution. We also
  show the scalability that can be achieved using PMR on multiple
  clusters by comparing it with SEQAL which is a Hadoop based
...
and moving data to the scheduled compute location, which is inevitable
...
increasing rapidly. The distributed processing of this data requires a sophisticated 
infrastructure. For this purpose, we utilize MapReduce to model an important 
part of the sequencing workflow i.e, the read alignment 
%and (ii) the  duplicate removal. 
The read alignment is implemented in the mapping phase of 
the application using BWA aligner ~\cite{Li:2010:FAL:1741823.1741825}. 
%The read outputs are filtered for duplicates in the reduce 
%phase using Picard rmdup program [ref:  Picard.http://picard.sourceforge.net.]. 
We use SEQAL~\cite{seal-2011}, a hadoop implementation of this workflow for performance comparision with \pilotmapreduce based implementation.
SEQAL provides a workflow to align the reads in the map phase and remove duplicates in reduce phase. Since we use  SEQAL to compare scalability of PMR across clusters, we are interestd in 
only in the execution of map phase. SEQAL provides an option to turnoff the reduce phase and just perform the map phase.
For PMR a custom mapper script is developed in python, which uses core BWA aligner.
...
simple Word Count application on a single resource.
...
The runtime of local PMR include time to chunk input data, 
running the mapping CUs, shuffling (which again comprises of sorting and the intermediate 
data transfer, and finally running the reduce CUs. Figure~\ref{fig:figures_wordcount_hadoopmr_pmr} shows the results.. The runtime of Hadoop MapReduce include 
...
\caption{Performance comparision: Local PMR versus Hadoop MapReduce using wordcount application on tex data: configuration - 128MB chunk size, 8 reduces, 8 nodes, 1worker/node} 	\label{fig:figures_wordcount_hadoopmr_pmr}
\end{figure}		
...
and reducing phase). For both frameworks, a total of 4 nodes on Futuregrid sierra machine,8 reduces, 2 workers/node, default chunk size of 128\,MB is used.
For Hadoop based SEQAL, the replication factor of two is used.


\begin{figure}[ht]
	\centering
		\includegraphics[width=0.47\textwidth]{figures/seqalvspmr_map.pdf}
\caption{Scalability comparision: SEQAL and local-PMR on sierra machines,  : configuration - 128MB chunk size, 8 reduces, 4 nodes, 2worker/node} 	
\label{fig:seqalvspmr_map}
\end{figure}		

Figure~\ref{fig:comp_with_seqal_1} shows the results of both applications. The
...
Running MapReduce on widely distributed data using a traditional single resource
MapReduce may not yield into an optimal runtime. Depending on the application's
characteristics a distributed deployment may show a better performance. In
high-data aggregation application the MapReduce output is multiple orders of
magnitude smaller than input. Hierarchical MapReduce configuration is considered
as a optimal choice since it involves less amount of output data to be
transferred to the combine MapReduce phase. In zero or ballooning data
aggregation applications, the intermediate and output data generated is larger
than or equal to input data; and a Local-MapReduce is considered as an optimal
solution, which involves initial data transfer of source data from all the
clusters to a single compute cluster. Even though the initial data transfers in
case of Local-MapReduce \jhanote{this is a term; please define a term before
using it} are costly, they are needed because of lack of an abstraction which
manages the relationships between the data and compute units effectively across
wide-area systems.
...
with respect to supporting different kind of resources and topologies.
...
including HDFS and various other daemons is required.
