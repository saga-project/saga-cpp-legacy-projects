...
deteriorates. In particular, we observe a decline in performance after
the 64 MB data, in that there is an increase in the gradient of the
plot, after chunk sizes have reached 64 MB.
...
illustrated in Fig.~\ref{fig:sphere_varying_workers}.  For the
...
Sector and Sphere on one physical node per SPE.  We observe the good
performance from 4 to 6 distributed workers, before which the
coordination costs due to the number of SPEs starts to get high, with
...
number of workers spawned.  For the local-local configuration we
launch workers on the machine running the master as separate processes
and use the local file system for file operations.  As can be seen in
Fig.~\ref{fig:sagamr_varying_workers} \sagamapreduce scales as
expected in the distribute-distribute configuration.  However, in the
...
as many files as the number of input chunks in turns, concurrent disk
I/O can cause the bottleneck when performing computations on one
pysical node.  According to Fig.~\ref{fig:sagamr_varying_workers} the
...
the linear scale. \miklosnote{Shall we note why we don't include
  results for 10 workers in the local-local configuration (takes
  toooooo much time)?}\jhanote{done}
