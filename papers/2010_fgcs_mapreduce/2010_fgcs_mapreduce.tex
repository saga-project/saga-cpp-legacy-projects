...
\begin{abstract}
  ...
  of \MR which is interoperable across different distributed
  ...
  and validating an enhanced version of MapReduce that scales-out
...
% {\it The Case for Interoperability:} Interoperability amongst clouds
...
interoperability, while service-level interoperability is weak
...
this can be easily achieved.  \amnote{we know not much about enterprise
...
% It can be asked if the emphasis on utilizing multiple clouds or clouds
...
% on interoperability is an appreciation and acknowledgment of an
...
seen in \F{Figure}, the SAGA overhead is, if compared to the overall
...
like HDFS or KFS. \miklosnote{KFS citation needed}
...
\section{Demonstrating 3 Types of Interoperability}
\label{sec:interop}

\subsection{Interoperability Types}

There are several aspects to interoperability. In this section, we
outline 3 different levels/types of interoperability.\amnote{expand}
...
interoperable when it can \I{concurrently} a programming model over
...
\subsubsection{Application: Wordcount}

We implemented two kernel functions to do a word count on four
gigabytes text data.  The first kernel function is responsible for
hashing the words in the data set into different hashing "buckets",
depending on the word's starting letter.  The standard C++ collate
hashing function was used for this purpose.  The second kernel
function reads each hash bucket, sorts the words in memory and outputs
the final count value of the words in the data set.  For example, a
file containing the words ("bread" "bee" "bee" "honey") would be
hashed into buckets as - ("bread" "bee" "bee") and ("honey").  The
second kernel function would read these intermediate bucket files,
sort the words, and produce the following result .  (.bread 1., .bee
2., .honey 1.).  The Sphere system is responsible for assigning files
for processing, synchronization, and writing output results back to
Sector.  
\amnote{check}
...
presented in \F{Figure}. As evident from the results we collected, a
...
outperforming Sphere at the 256 MB data point. As it can be seen in \F{Fig.xx}
...
configuration. These results are illustrated in Figure \F{xx} and F{xx}.  For
...
% There is a fundamental need for support  for a programming
% system/model for clouds.


% however we re-emphasize that the primary strength of SAGA in addition
