...
For example, a file containing the words \T{('bread' 'bee' 'bee'
'honey')} would be hashed into buckets as \T{('bread' 'bee' 'bee')} and
\T{('honey')}.  The second kernel function would read these
intermediate bucket files, sort the words, and produce the following
result .  \T{(.bread 1., .bee 2., .honey 1.)}.  The Sphere system is
...
$t_{pre}$ does not includes the time required to start VM instances.
...
presented above) physicaly chunked the input data files.  Our evolved
version, however, performs creates logical chunks (i.e., no file
...
early-version's job completion time.  Fig.~\ref{sagamr_comparison}
