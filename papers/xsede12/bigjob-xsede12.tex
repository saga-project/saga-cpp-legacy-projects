...
Kraken and Trestles use PBS\pmnote{ job scheduling systems}. While SAGA retains the ability to submit jobs
through the GRAM~\cite{gram}\pmnote{should be globus} job adaptor, it is an unnecessary burden on
...
successful job submission, whereas Ranger and Lonestar do not. \pmnote{the same still persist with bigjob} These small  but
...
\mrnote{Yaakoub, took your advice and made footnote... did you want it 
to be mentioned directly in the text?}
...
impact users...}
\mrnote{Could you elaborate on what you had in mind in addition to what was said already? 
Here's some random stuff that I don't know if it is along the lines of what you were looking for.}
...
~\yyenote{sentence needsbetter tie-in}. \mrnote{I tried restructuring the paragraph a bit. Let 
me know if it flows a little better now}
...
of BigJob as quickly as possible. \mrnote{I don't like this paragraph yet. It needs work}
...
Machine & Number of BigJobs & Size & Duration (hours) & Number of SubJobs & SubJob Size\\
...
These were all free energy calculations investigating the binding of HIV-1 protease to FDA approved inhibitors. Each BigJob here performs analysis on 50 simulations. Very much along the lines of those we described in last years Teragrid paper. You might want to wrap into the take some of the simulations we performed around that time (or just after). In which case you could add ~ 12 * 256 * 12 hours BigJobs - these being 4 * 64 processor NAMD simulation jobs.
...
aims of both projects. \jhanote{This is inconsistent with the
  following remark about familiarity! please fix}\yyenote{No it is
  not. We are familiar with the middleware, not the scientific application
  of amber or impact. Hence strong collaboration is separated between
  sys admins and tool devs on one hand and sys admins and scientists on another}
