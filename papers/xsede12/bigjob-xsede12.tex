...
\begin{abstract}
  The Extended Collaborative Support Service (ECSS) of \\
  XSEDE is a program to provide support for advanced user requirements that
  ...
  preliminary results obtained.  We believe the structure of the ECSS
  program enables targeted projects that address missing gaps in
  making distributed cyberinfrastructure systems more productive.  We
  also describe how SAGA has been deployed on XSEDE in Community
  Software Area as a necessary precursor for these projects.
\end{abstract}
...
\category{J.3}{Computer Applications}{Computational Biology}

\section*{General Terms}{Design,Measurement,Theory}

\keywords{Distributed Computing, High Performance, High Throughput, SAGA, ECSS Projects}
...
computing), are installed on a small subset of XSEDE (only Ranger and Lonestar,
not Kraken or Trestles) and do not support workflows or mulitple contianer
