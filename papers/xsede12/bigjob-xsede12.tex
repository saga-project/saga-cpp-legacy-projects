...
of providing support for advanced user requirements that cannot and
...
simulations; both of these ECSS projects are using a SAGA-based
...
providers. In this paper, we describe the aims and objectives of these
...
\mrnote{Tenses are being flipflopped... lead is past tense yet next sentence
...
quite welcome. \mrnote{too conversational?} 
...
users to enable launching NAMD\mrnote{Citation for NAMD needed?}, 
...
%on track to generate approximately 140,000ns of molecular dynamics simulation
%data for at least 150 different realizations of the nucleosome.
\mrnote{Needs better tie in now that redundancy is deleted}
...
\yyenote{So first off, this pilot-job definition does not explain why
  we chose to use pilot jobs to overcome the computational
  challenges. Furthermore, the use of the word speed in relation to
  calculations means FLOPS, this is wrong.  The use of the word
  ALLOCATION is also wrong. In XSEDE, allocation means one and only
  one thing and this is NOT it. We need a clear, layman's definition
  of what pilot jobs are in relation to the computational
  challenge. The following is NOT.}  Workload management and resource
...
\mrnote{This is the point where we should first try to define a pilot
  job in a clear manner} A pilot-job is a mechanism by which a proxy
...
application-level scheduling.  \mrnote{Perhaps this excerpt from the
  pstar paper is of value? ``The abstraction of a Pilot-Job
...
\jhanote{YYE valid points. Removed/addressed}
...
The strong collaboration between system administrators 
\jhanote{Yaakoub, Can we not call you a system admin! You're much more
  than that. 
and tool
...
aims of both projects. The most effective aspect of this project is
undoubtedly the overlap of the two ECSS projects in terms of tools and
infrastructure (software and hardware). While the ECSS staff's
research interests are not aligned with MD simulations, they are
however familiar with the particular MD simulation tools and packages
used.
...
award TG-MCB090174 and LONI resources.  \jhanote{Is there an ECSS
  award number that we should reference?}
