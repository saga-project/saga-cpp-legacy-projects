...
endeavor and computational challenges. The succes of these two projects is
...
from middleware develoment. The successful distribution of labor translates
...
chokepoints, scaling issues, un-necessary increases in file system load and so
...
only one thing and this is NOT it. We need a lear, layman's definition of what
pilot jobs are in relation to the computat onal challenge. The following is
NOT.}
...
and speed of the desired calculations. A common  approach fo r decoupling 
these competing allocation problems is the use of \emph{pilot-jobs (PJ)}. The  PJ
abstraction is also a promising route to address additional requirements of distributed scientific applications~\cite{ko-efficient,bigjob_cloudcom10}, suc
...
workflows~\cite{async_repex11,saga-royalsoc}. For example, SAGA-BigJob was used
...
\yyenote{Why are we repeating the definition of saga and its architecturehere?
...
machines~\ref{table:CSA-Deployments} (ranger,
