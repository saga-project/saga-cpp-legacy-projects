...
\author{\authorblockN{Shantenu Jha\authorrefmark{1}\authorrefmark{2},
    ...
    Kaiser\authorrefmark{1}, Ole Weidner\authorrefmark{1}, Andre
    Merzky\authorrefmark{1}} \authorblockA{\authorrefmark{1} Center
...
% The situation gets significantly more complicated when there is
% horizontal coupling between the distributed jobs, say for example, a
% randomly occuring global (or even local) exchange points or global
...
%SAGA provides the capability for the the distributed aspects.
...
% Varying parameters
%sometimes also lead to varying systems of equations and entirely new
%scenarios. This can change the computational size
%requirements as well as memory requirements. 
%For example, as a
%consequence of the variation in size the underlying matrix might
%become too large or even effectively doubling the number of the system
%of equations, which could more than doubles the memory required to
...
%    data-flow independent scheduling techniques difficult to use.
