...
%    applications on Grids is difficult in general.  
...
\begin{figure}[t]
      ...
      \caption{\footnotesize The {\it BigJob} abstraction originally
        ...
        applications.}\up\up\up \up\up\up
...
dynamically, i.e., mapping the sub-tasks requirement to the resources
available at the instant the sub-tasks become available and ready to
run, as opposed to {\it a priori} static method of job submission.  In
other words, an advantage of the Lazarus framework (currently based
upon the BigJob abstraction but soon to use FAUST) is the flexibility
it provides -- which can be referred to either as late binding or
loose-coupling between application resource requirement and resource
assignment.
...
After losing many jobs to various errors such as running out of
allocated diskspace, hardware failure and file-copy failure, we
quickly came to the conclusion that autonomy and self healing are
important features that we needed to incorporate in Lazarus to ensure
all large scale history matching runs complete successfully. With
fault tolerance, we no longer wasted thousands of SUs running
simulations with missing/corrupt ensemble members.  On the other hand,
the autonomy incorporated in Lazarus, while basic, ensures that we
optimize the job sizes to have a higher chance of getting through the
queue without delay and that simulations that ran into issues or
simply failed to run, are resurrected.
...
a typical production run will involve hundreds of stages.
...
\begin{figure} \begin{center} \includegraphics[scale=0.5]{./figures/Architecture.png} \caption{Block diagrams outlining the basic structure of Lazarus. Lazarus is built on several components: BigJob abstraction, SAGA file adaptor, BQP python wrappers and fault tolerance functions that are based on SAGA python bindings, BQP command line tools and HDF5 tools respectively }
\end{center}
...
\caption{Schematic diagram illustrating how a Cactus-based Reservoir
  simulation utilize Lazarus.}\up\up\up \up\up\up \up\up\up \up\up\up \up\up\up \up\up\up
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.25]{./figures/Figure7.png}
\end{center}
...
increase. % This is a reconfirmation of the use of higher-level
...
\caption{
  ...
  with the fourth experiment generating different results from the previou two. In contrast, when BQP is not used, 
...
as a functionality in FAUST~\cite{faust_url}.  Similar failures, but
different machines and different solutions, thus there is a case to be
made that FT should be provided as a general-service.
