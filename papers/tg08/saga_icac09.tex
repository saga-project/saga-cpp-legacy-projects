...
In the first stage of experiments, we determine the \tc for a single TG machine, but repeat the experiment for different TG machines and repeat 10 times at different over a few weeks, so as to get uncorrelated data (or more precisely less-correlated data).  This provides a measure of base-line performance.  We then, repeat experiments using more than one computational resource on the TG. To be precise, we employ more than one computational resource towards the solution of a single instance of the problem.  Given that are experiments are capable of utilizing three different TG resources, using two-resources at a time, gives us three different combinations.

Our results are plotted in Figure.~\ref{fig:results}. Ranger is the flagship machine of the Texas Advanced Computing Centre; QueenBee (QB) the flagship machine of LONI and Abe a large machine at NCSA. Ranger, QB and Abe are all TG resources. As the results from the single-resource configuration show, the individual resources have somewhat different intrinsic performance, which should be factored when taking into account the combined performance. As a logical extension, we perform an experiment wherein we utilize all three different TG resources. An important result, that holds in general, but also for the specific instances investigated here, is that on average, as the number of resources increases \tc to solution decreases. This is evidenced from the fact \tc (RQ) is lower than \tc (R) or \tc (Q).  \tc (RQA) is less than \tc (RQ), but the change is less significant than when going from one machine to two; however, performing many more experiments would lead 
clearer distinction in \tc.
...
\caption{A table showing  the selected configuration of the resources, when 
  the decision is guided by BQP. Data in this table corresponds to
   RQA-BQP, that is all three machines are available and corresponds to identical experiments involving machines Ranger, QB and Abe, but repeated
   ten different times.  As can be seen, the use of BQP results in a variable choice of resource  configuration on different machines.  In contrast, when BQP is not used,  the default choice (on all machines) for the BigJob size is always 64 with a duration fixed at 2:00 hours (although the chosen queue varies). So although there is not  much change between different the BQP-guided runs,  there is significant difference between BQP-guided and manual (unassisted) resource configurations}\up\up\up\up\up\up
\end{table}
