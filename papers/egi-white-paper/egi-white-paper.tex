...
\section{The SAGA Landscape}
 ...
 \subsection{SAGA Implementations}

 \jhanote{I replaced Development Efforts with Implementations}
...
\subsection{SAGA usage modes}
  ...
  of distributed applications.  As described earlier, that is achieved
  by providing a library which implements the SAGA API.  However,
  distributed applications have to {\it wrestle} with distributed
  ...
  \subsubsection*{Application Development}
   ...
   required by the applications of our target usvaste majority of distributed operations, as
   required by the applications of our target useer community
   (scientific application and tool developers).  Further, as the API
   specification and implementation is \I{standardized}, and thus
   stable, it allows for a 'write once, run anywhere' approach, which
   is in general not available otherwise (or at least not without
   \I{significantly} increase of application complexity).
  ...
  \jhanote{I don't understand why these two are here.. Andre lets
    discuss}
...
Access Layer to EMI - ARC, gLite and Unicore.
