...
\label{sec:standards}
 ...
 multiple DCIs on middleware and operational level, and to allow
 applications to use the combined set of resources as if it were
 ...
 toward system level interoperability for a variety of
 infrastructures (for example, BES is available on XSEDE, FutureGrid,
 EGI, NorduGrid, Naregi and others), the level of SLI varies widely
 and is often not the default modus operandi (BES again is not the
 default interface on XSEDE [yet] and FutureGrid, and not available on
 all resources).  

 As interoperability is a driving motivator of the P* research efforts
 described in the previous sections, we see the ultimate need for
 application level interoperability, to complement SLI.  On
 architecture and implementation level, we largely base ALI on the
 'Simple API for Grid Applications' (SAGA~\cite{gfd.90}), which we use
 as generic abstraction layer toward the distributed infrastructures.
 SAGA in turn interfaces to a wide set of other standards based
 (GridFTP, BES, DRMAA) and non-standards bases (EC2/AWS/Eucalyptus,
 Condor, Globus/GRAM), and thus utilizes SLI where available.
...
\label{ssec:std_gin}
 ...
 target infrastructures of our research efforts, OGF related standards
 like BES, JSDL, GLUE, OCCI and SAGA are the most relevant ones, and
 these are largely still evolving and are still being implemented and
 deployed.  The OGF Community Group 'GIN' (Grid Interoperation Now)
 is, for the scope of OGF, the umbrella group to host and coordinate
 efforts for interop testing and deployment.  Resources on FG have
 been used both to develop and harden such standards implementations
 as Genesis, Unicore and (from our end) SAGA, and also to test and
 demonstrate the SLI and ALI capabilities across those
 implementations, and across a wide set of infrastructures.
 ...
 Mandelbrot set), which is able to concurrently utilize more than 30
 systems, running 12 different system stacks, for a single application
 instance.  On FutureGrid alone, we have been using ~10 submission
 endpoints on 4 different systems stacks for those experiments.  It is
 obvious, that FutureGrid's capacity as research environment is
 greatly supporting that type of endeavours.
...
\label{ssec:std_xsede}
 ...
 the deployment challenges.  In fact, in our experiences the efforts
 required to provide clean and stable deployment processes for
 a variety of environments can well exceed the actual development
 efforts.
 ...
 interfering with production systems, and without exposing the process
 to the end users on those systems.  

 In detail: SAGA is deployed as community software on TeraGrid/XSEDE.
 We are using the same model on FutureGrid, and can test deployment
 and testing on the FutureGrid machines.  That provides SAGA
 installations to our (mostly CS experiment friendly) users on
 FutureGrid itself, and also mimics perfectly the procedures we use to
 deploy stable versions on XSEDE.  Further, we have been using
 FutureGrid resources to perform certification and packaging processes
 toward other production infrastructures, most notably toward the SAGA
 enrolment on the European EGI software repositories (UMD).
