...
We can generate as many number of read files as we want in the first step but the second step
could be scaled with more number of tasks. Because the number tasks in the second step is determined 
by the number read files we prepare in the first step. We can generate as many number of read files as 
we want in the first step. We can use any available resources in this step as the tasks are not coupled 
to each other but the problem is we need the transfer the appropriate read files to the respective resource.

N read files are prepared in the first step by setting the parameters accordingly. N tasks for the second step
show the scaling and to find the performance difference with the concurrent running tasks.

We use SAGA-BigJob to implement the above two steps on grids and clouds and 
the architecture of BigJob remains same for both grids and clouds. All the first 
step and second step jobs are defined as subjobs in the BigJob. At first only 
one job is submitted ie preparing the read files and the jobs in the second step are not submitted
until the first subjob is done. Once the Read files are prepared the 37 subjobs are submitted in to 
job queue of the Bigjob. Number of concurrent subjobs that can be processed depend 
...
directory where we want to start the BigJob. Bfast was installed in the home directory and the
executables path for job description of subjobs were given accordingly.
