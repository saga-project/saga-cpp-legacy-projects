...
\section{Mapping with BFAST Tool and Two Genome Datasets}
\subsection{Background}
Our target genome analysis tool is a mapping tool, BFAST, which represents a class of tools that comprises diverse genome-wide analysis applications but shares similar characteristic computational features for their execution\cite{bfast2009,bfast2009b}.  Such features include i) a requirement of input files containing sequences of a reference genome or short reads from NGS platforms ii) a production of output files that is generally written with a format that another successive tool can take as an input.  These aspects that are accompanied with a huge variation in the data volume of input, output, or temporary files often require that a tool should support parallel or multi-threading executions, which is also true with BFAST\cite{bfast2009}.  In Fig.~\ref{fig:workflow-bfast} and Table~\ref{table:bfast-summary}, the summary of BFAST is presented.

In brief, BFAST takes a reference genome sequence with a fasta format and NGS platform generated short reads files with a fastq format, and processes indexing of the reference genome, finding Candidate Alignment Locations (CALs), local alignment of CALs, and post-processing of alignment results, which taken together produces the mapping of billions of short reads onto the target reference genome.  Among the steps, three steps (index, match, loalalign) demands most of computing times that also vary depending upon the size of a reference genome as well as volumes of short reads.  As stated, BFAST can be executed by carrying out parallel or concurrent tasks by splitting the input data, short reads and a reference genome.  Short reads files can be prepared with multiple files and short reads in each file are mapped independently against a reference genome.  Meanwhile, a reference genome is also split into many files by considering the existence of multiple contigs in an entire genome, for example, multiple chromosomes and plasmids.  

Once a reference genome is decided, the following step is to create index of the reference genome with multiple index files and resulting number of the index files are decided by the number of masks times by $4^d$ where d is the option for the low memory consumption such that as $d$ increases each index files is split into $4^d$ files and processed one by one.  On the other hand, NGS short reads can be split into as many as separate files with which parallel or concurrent tasks are executed for each file.  Note that while each reads file can be processed independently, an entire reference genome sequence should be used altogether for the next step, "bfast match". 
...
The time-to-solution for "match" step depends upon several parameters that determine how to carry out parallel tasks given computational resource configurations such as the size of memory, accessible volume of disk space, the number of cores in a node, and the total number of cores.   According to the results shown in Fig.~\ref{fig:parallel-execution}, we found that when such computational configurations do not operate as limitations for overall performance, among three options, i.e., the low memory option, multiple reads files, and multi-threading support, of BFAST for concurrent execution patterns, multi-threading support offered the marginal gain.  While multiple reads file option suggests the best strategy for scaling out, i.e. the short execution time with more cores and smaller file size, the low memory option that creates more index files require longer calculation time.   
