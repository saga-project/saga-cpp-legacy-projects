...
\caption{\small The multi-threading capability measurement.  The
  ...
  genome. The low memory option with 40 index files is used for this measurement. \jhanote{Increase y-axis resolution} }
  \label{fig:parallel-execution} 
 \end{figure}
...
heterogeneous computing resources, we carried out the mapping with BFAST on a HPC grid and a Cloud environment (see Table~\ref{table:two-systems}  \jhanote{Before we say what we are
  presenting, please present a ``why and how'', i.e., outline the
  experiments}
 Our experiments with two different distributed computing environments aim to understand the differences and the commonality that are critically need for a design of an infrastructure targeting genome-wide analysis.
