...
\jhanote{This paragraph is ONE single sentence!!! Break up into simple
sentences. I do not understand what is trying to be said here. }

An understanding of computational requirements for carrying out BFAST
steps is critically important for an infrastructure development that provides a way to execute the target genome-wide
analysis tool with scalable distributed architectures. As a matter of fact, the overall execution of BFAST, or any other short mapping applications aligning short reads onto a reference genome is affected by biology such as types of target reference genomes and high-throughput sequencing techniques and protocols, as well as computational characteristics such as data structure and formats and parallelism support like multi-threading.   Here, we present our investigation to characterize the target application and more importantly the utilization of such aspects with available computing architectures with respect to parallel execution capabilities and data management.

In Table~\ref{table:two-genomes}, we summarize the typical values of
the parameters upon which the time-to-completion, $T_C$, typically depends.
Values for two genome datasets that are representative of biological
diversity, an eucaryote system, human, and a microbe, Burkerholderia
Glumae\cite{kim2011}. The two genomes differ in the size and the
genome structure of reference genomes, and types of sequencing
protocols.  Importantly, the two different genomes display widely
different data sizes in terms of the reference genome sequence, short
reads data, and required disk space for carrying out the mapping with
BFAST.
...
Fig.~\ref{fig:parallel-execution}; there exists a linear scaling in
time-to-completion as the size of read file is increased.  This
suggests that if the read files can be broken up into smaller
fragments and which can be processed concurrently, there is the
possibilty of a speed-up. This points to the potential of task-level
concurrency.
...
difference in the values, and thus the slope is also 4 times different.
...
%\end{itemize*}
Most of those solutions generated meaningful progresses in particular focusing on the utilization emerging technologies such as Cloud computing and algorithms such as MapReduce.  Notably, a general purpose infrastructure or services were also announced providing a variety of tools for genome-wide analysis\cite{galaxy}. 

In spite of interesting progress made by such efforts, some important questions remains to be answered. For example,
how to utilize heterogeneous computing resources efficiently and effectively is not simple due to major efforts for building, maintaining, and extending an infrastructure.  In that sense, it is important to make the infrastructure with a framework that is agile for dealing with heterogeneous computing and data management resources that are distributed geographically.  The agility should address a quick development cycle for an incorporation of a new bioinformatics application, an efficient maintenance of existing applications, and an easiness of capability addition for applications or the infrastructure itself.  One notable aspect, which is largely not considered in many existing solutions, is that a target application is enhanced with the utilization of heterogeneous scalable resources, which is greatly facilitated if the infrastructure supports a non (or minimal) -intrusive modification of a target application and existing data management structure.
...
To determine how the performance of the bfast match step with the different number of threads, $N_t$, and 
cores, $N_c$, we vary the number of threads for a task of the step on a single machine having multiple cores. 
The results in Fig.~\ref{fig:parallel-execution} show that the time-to-completion, $T_{C}$, decreases until the number 
of threads becomes equal to the maximum number of cores.  Interestingly, the overall performance gain is about 30 \% which is somewhat marginal, and the scaling tendency does not show any significant pattern.\jkimnote{more discussions if possible?}
...
To execute a scientific application with heterogeneous distributed
...
heterogeneous computing resources, we carried out the mapping with
BFAST on a HPC grid and a Cloud environment (see
Table~\ref{table:two-systems} % \jhanote{Before we say what we are
%   presenting, please present a ``why and how'', i.e., outline the
%   experiments}
Our experiments with two different distributed computing environments
aim to understand the differences and the commonality that are
critically need for a design of an infrastructure targeting
genome-wide analysis.

First of all, in Table~\ref{table:bigjob-loni}, we presented the
measured the time-to-completion, $T_C$ for the step 'bfast match' with different
...
along with the observation of a marginal performance
gain with the option for increasing number of threading (see Fig.~\ref{fig:parallel-execution}), suggests that using sufficient cores available from distributed resources and implementing a dynamically adapted task distribution would lead to achieve an
efficient execution of a target calculation is a given time.
...
SAGA bigjob.
 ...
 \begin{tabular}{|c|c|c|c|c|c|} 
 \hline 
 Case  &  VM's &  VM type & Tasks & Time to  \\ 
      &   & (Cores) & & completion\\ 
\hline
c1 &  4 &  2 & 4 & 1080 s \\
c2 & 1 & 8 & 4 &1020 s \\
