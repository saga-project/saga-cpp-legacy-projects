...
% \begin{lstlisting}[style=myListing, caption={\small \bf SAGA-CPR:
...
REMD-Manager can be found in Ref~\cite{remd-manager_url}.
...
The more replica processes, the more exchanges the REMD-Manager must
handle concurrently. Each exchange requires the re-staging of 6 files
with the size of 12\,MB -- this transfer alone typically requires
5-10\,s within LONI. Due to the small problem set computed by each
...
%The results also show that the overhead caused by Migol is well acceptable. 
Per average, the SAGA/Migol adaptor showed a 15\,seconds runtime overhead. 
However, it must noted that in particular in the distributed setting a high variance
of results occurred. Still this overhead is well acceptable and  
...
https://svn.cct.lsu.edu/repos/saga-projects/applications/REMDgManager/src/