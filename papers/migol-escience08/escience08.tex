...
In addition to the submission overhead, the Migol adaptor has an
initialization overhead. The overall runtime of this simple REMD
submission task including the initialization is 6.45\,sec about,
4.5\,sec slower than the GRAM launcher. In particular, the
initialization of a Migol job requires the setup of the HTTP server as
well as several metadata updates at the AIS, which accounts for the
additional costs.  

In addition to job submission overheads, it is interesting and
important to determine how the runtime of the replica is effected by
the active monitoring and the replication of checkpoints.  To evaluate
this, a NAMD job with 1000 steps was started with and without Migol
support.  During this run a very eager monitoring interval of 1 minute
was used. Checkpoint metadata was updated every five minutes -- a
single checkpoint update required approximately 1.3\,seconds on
average as shown in Figure~~\ref{fig:performance_perf_submission}.
During this 21\,minute NAMD run the overhead average is 20\,s and thus
only amounts to about 2\%.
