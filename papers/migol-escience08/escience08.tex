...
\title{
  ~\\[-3em]
  Replica-Exchange Simulations on Production Distributed
...
examples are those that involve an ensemble of decoupled tasks -- {\it
  pleasingly-distributed} applications. In spite of its conceptual
simplicity, many scientific problems based on parameter sweeps and/or
Monte Carlo simulations, can be solved using infrastructure that
supports this common application class. A slightly more complicated
and challenging class of distributed applications are those that have
a small level of coupling between the sub-tasks.  An important and
interesting type of applications that belongs to this category are
those based on \emph{Replica-Exchange
...
on it. Thus over time, due to the fact that in principle every replica
will ultimately interact/exchange with another replica, a single
failure, if left uncorrected can ultimately cause the simulation to
encounter exponentially increasing slow-down.
...
middleware and supports the transparent starting, monitoring and
recovery of tasks.
...
Checkpoint\,\&\,Recovery package (CPR) of SAGA, and its implementation
using the Migol adaptor. SAGA-CPR represents the first extension to
the core SAGA API specification, and is thus a validation of the
extensibility of the specification.  In particular, Migol as an
element of the Grid middleware, can be regarded as a reference
implementation of the GridCPR architecture~\cite{ogf_cpr_arch}.  The
SAGA-CPR package is a SAGA rendering of the GridCPR
API.%, also defined in~\cite{ogf_cpr_arch}.
...
\up Before outlining the structure of the paper, we highlight the main
advantages of our approach: Firstly and foremost it is a general
purpose framework that can is usable over varied distributed
environments such as the TeraGrid, DEISA, etc., as opposed to
WISDOM~\cite{wisdom}, that is confined to gLITE/EGEE. Secondly it is
scalable to resources of different sizes, as opposed to {\it
...
effectively.  \alnote{Should we add citations to Folding@home and
  wisdom here?}  Thirdly, it is extensible -- it can be used to
implement Replica-Exchange simulations, but also any other
application.  The ability to do all of the above arises from the use
of standard interfaces and the correct programmatic and system
abstractions: enable users to do what they do best, i.e. provide the
simulation and orchestration logic, and have the middleware provide
generic services, such as checkpoint management, application
...
\up The remainder of the paper is structured as follows: In the next
