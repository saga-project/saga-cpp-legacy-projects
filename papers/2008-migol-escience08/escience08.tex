...
\begin{abstract}
  ...
  time-to-solution.   In this paper, we discuss a specific class of applications 
  -- Replica-Exchange simulations -- where the orchestration of many
  distributed jobs in a dynamic and inherently unreliable distributed
  environment is essential for a successful completion.
  % However, the ability to orchestrate many
  %   distributed resources in a dynamic and inherently unreliable distributed
  %   environments is a major challenge.
  % The more resources and components
  %   involved, %i.e., degrees-of-freedom,
  %   the more complicated and error-prone the system becomes. 
  % We discuss a specific class of applications -- Replica-Exchange simulations
  ...
  % scientific problem. 
  % Such applications require effective mechanisms
  % to handle the unreliability inherent in dynamic distributed systems.
  We describe the design, development and deployment of
  ...
  SAGA and Migol. 
  %, is scalable, general purpose and extensible.  
  ...
  required for application development. We present details of a newly
  developed functionality in SAGA -- the Checkpoint and Recovery (CPR)
  API.  Migol is an adaptive middleware, which supports the fault-tolerance 
  of distributed applications by providing the
  ...
  with running a large scale, general-purpose
  %, SAGA-CPR based
...
Several types of applications are well
suited for distributed environments. Probably the best known and most powerful
...
% is the class of distributed applications 
...
% For example, for every replica that encounters a failure there is at
% least one replica that will wait on it.
...
architecture~\cite{ogf_cpr_arch}. % The SAGA-CPR package is a SAGA
% rendering of the GridCPR API.%, also defined in~\cite{ogf_cpr_arch}.
% as an element of the Grid middleware,
...
%   -- usable over TG, DEISA, etc. unlike the DOM approach which is
...
irregular~\cite{PhysRevLett.86.4983,pande_bj03}. An example of the latter --
...
% Well known examples of the former are; parallel replica dynamics as
% implemented in Folding@Home, also involves coordination between the
% replicas every time an event occurs, can be classified in the
% latter. 
% \alnote{References still missing - I do not understand this
%   sentence: Is the thing we want to say that [SPdynamics/pandebj03]
%   are examples for irregular exchanges?}  \jhanote{Yes, parallel
%   replica dynamics is a type of irregular replica exchange} Parallel
% replica dynamics involves coordination amongst the replicas
% (implemented either using the master-worker or the peer-to-peer) at
% irregular intervals, e.\,g.\ everytime a folding event occurs.
% Various topics have been successfully studied with this
% approach.
...
applications.  %The framework provides an standardized,
...
Since each replica exchange step involves the
...
the initialization was 8.3\,sec (bar 3 in
Fig.~\ref{fig:performance_perf_submission}), about 6.8\,sec slower
...
above the standard deviation of the measured runtimes.
% During
...
REMD-Manager was configured to run a simulation with 2 to 16 replica
...
Figure~\ref{fig:performance_perf_runtime} illustrates the results of
...
different files,  we transferred six files with the total size
of about 10\,MB before each exchange step. 
This transfer requires approximately 5-10\,s
...
% However, it must be noted that in particular when distributing jobs
% across multiple clusters a high variance of results occurred.
...
tasks. 
%The more resources involved, the more likely are negative effects caused by 
%a single slow machine. % For example, in our
% test scenario, the response times rapidly increased when we allocated
...
% This is an essential pre-requisite for running larger REMD runs with more replica processes
...
Legion~\cite{689541} to name a few. These provide basic fault-tolerance 
support by automatic re-scheduling failed tasks. Advanced
...
%for molecular docking simulations (for the evaluation of drugs).
Although WISDOM has similar application characteristics as discussed
here, the project is currently tied to the gLite~\cite{glite}
...
specification and is soon to be standard~\cite{saga_url}
...
%   scenarios?  sentence 1 and 3 are partly repetetetetive.}  In
% contrast to Wisdom and Folding@home, our approach is not restricted to
% a specific distributed environment; SAGA being a community
% specification, SAGA based job-launching and file-handling is supported
% on most general-purpose Grids via appropriate adaptors.
