...
\begin{abstract}
  ...
  % data-intensive computing. The ability to decide if data should be
  % moved to compute, or compute units be moved to where the data is
  % located -- either manually or automatically using heurisitcs, is
  % required for
  ...
  the monetary cost of computing the solution low.  Using data-sets of
...
reminiscent of Globus, which became a disastrous
by-word for everything wrong with the complexity of Grids.
...
Any {\it effective} abstraction will be cognizant and provide at least the
above features, viz., relative compute-data placement,
...
% MapReduce is a programming framework pioneered by
...
% Instead, a general purpose grid provides a much larger set of lower
% level operations.
% Some semantics,
% such as again the atomic file rename, is provided by the SAGA API
% layer, others, such as data/compute allocation are not.  
 %  Note that some
% of the required capabilities can be provided by higher level grid
% services -- those are, however, often not standardized, and often not
% available on general purpose Grids.
...
%             There are performance issues with
...
%             resources over which these chunks are distributed.
            \jhanote{I think there should be something between the
              ...
              back to the Master, non?}} \vspace{-2em}
...
%  input data to use and the output file names to write the results to)
\item Coordinating the executed workers, including the
...
in the way jobs are run and how the data are stored.  In
\sagamapreduce the final data is stored on many machines -- if there
is a DFS available, whereas SAGA All-pairs uses the database to also
store information about the job.  We decided to do this because all
data must be available to be useful. We demonstrate the SAGA All-Pairs
abstraction using the HDFS and GridFTP to not only show that SAGA
allows for many different configurations, but also to see how these
different configurations behave. We have also used a distributed
data-store -- specifically HBase (Yahoo's implementation of Bigtable)
in lieu of the traditional Advert Service to store the end-results.
...
% itself. 
An important problem in Bioinformatics -- Multiple Sequence
...
generally agreed upon~\cite{buyya_hpcc}. % A minimal set of these
% attributes can be considered the use of virtualized machines, support
% for distributed file and structured data storage.
...
machines in a traditional hierarchical file
organization.  % It has been highly successful and is
...
high-performance distributed FS implemented in C++, with many of the
same design features as HDFS.
...
increases. In general: %  For the same number of chunks assigned to a larger number
% of worker, the value should increase.
...
% \caption{this plot will probably have to be eliminated}
% \label{grids2}
% \end{figure}
...
% See Fig.~\ref{grids2}.
% MapReduce\_local\_executiontimeperworkerdatasize.png
...
there is only one chunk, $t_{pp}$ should be effectively zero (more
likely a constant), and \tc will be dominated by the other two
components --$t_{comp}$ and $t_{coord}$.  Interestingly for
1GB and 5GB, the ratio of \tc for this boundary case is very close to
...
%(2 workers each): $T_c = $ 372s, $T_c = $ 629s, $T_c = $ 1176s for 
%1, 2 and 4GB respectively.  

% \begin{tabular}{cccc}
%   \hline
%  Num of Workers  &  1GB   & 2GB & 4GB  \\
%  2 &  372 & 629 & 2051 \\
%   \hline
%  2  &  & 0.5 & 372 \\
%   \hline
...
by about a factor of 2. This is not surprising, as \sagamapreduce
...
\upp
      \centering
          ...
          \caption{\tc for \sagamapreduce using one worker (local to
            the master) for different configurations.  Hadoop
            represents Yahoo's MapReduce implementation; figures for
            Hadoop show data without chunking, which takes several
            hundred sec for Larger data-sets.  The ``SAGA
...
all localized, or are split evenly between two (similar) machines. It
shows that to get the fast \tc it is often required to both distribute
the compute, and lower the workload per worker; just lowering the
workload per worker is not good enough as there is still a point of
serialization (usually local I/O).  It shows that when workload per
worker gets to a certain point, it is beneficial to distribute the
workers, as the machine I/0 becomes the bottleneck.  When coupled with
the advantages of a distributed FS, the ability to both distribute
compute and data provides additional performance advantage.
...
We have also architected \sagamapreduce to work on Cloud platforms.
It is critical to mention that the \sagamapreduce code did not undergo
...
can be implemented using the All-Pairs and MapReduce patterns. This
...
by demonstrating using SAGA how they can also decouple the development
of applications from the deployment and run-time.
