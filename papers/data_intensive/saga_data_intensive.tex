...
\end{eqnarray}
The total time to completion can be decomposed into three primary
components: $t_{pp}$ defined as the time for pre-processing -- which
in this case is the time to chunk into fixed size data units, and to
possibly distribute them. This is in some ways the overhead of the
process.  $t_{comp}$ is the time to actually compute the map and
...
Observation: The efficiency of \sagamapreduce is pretty close to
1. Like all good (data) parallel applications. For 1Gb \tc = 659 and
\tc for 10GB = 6286s.  This is true even when the compute is
distributed over two machines: 1 worker at each site: \tc = 672s, $T_c
= $ 1081s, $T_c = $ 2051s for
1, 2 and 4GB respectively. \\
(2 workers each): $T_c = $ 372s, $T_c = $ 629s, $T_c = $ 1176s for 1,
2 and 4GB respectively.


(1 worker at each site): $T_c = $ 672s, $T_c = $ 1081s, $T_c = $ 2051s for
1, 2 and 4GB respectively. \\
(2 workers each): $T_c = $ 372s, $T_c = $ 629s, $T_c = $ 1176s for 1, 2
...
occuring for smaller data-set sizes). See Fig MapReduce\_local\_executiontimeperworkerdatasize.png
...
a point of serialization (usually local I/O).  

local:       1GB dataset, workload of 0.5GB/W \tc = 372s \\       
distributed: 1GB dataset, workload of 0.25GB/W \tc = 372s \\

local:       1GB dataset, workload of 0.1GB/W \tc = 466s\\
distributed: 1GB dataset, workload of 0.1GB/W \tc = 320s \\

local:       2GB dataset, workload of 0.2GB/W \tc = 709s\\
distributed: 2GB dataset, workload of 0.2GB/W \tc = 454s \\

local:       4GB dataset, workload of 0.5GB/W \tc =  1083s\\
distributed: 4GB dataset, workload of 0.5GB/W \tc = 912s \\

It shows that when workload per worker gets to a certain point, point,
it is beneficial to distribute the workers, as the machine I/0 becomes
the bottleneck.
