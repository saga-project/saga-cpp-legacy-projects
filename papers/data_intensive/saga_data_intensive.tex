...
With the force-of-industry behind the development (and not just hype)
if Clouds, although they are a nascent infrastructure, their impact
can not be ignored.  Specifically, with emergence of Clouds as
important distributed computing infrastructure, we need abstractions
that can support the dominant programming models for Clouds. Issues of
scale aside, ideally the transition from existing programming models
and styles (primarily Grids), must be as seamless and as least
distruptive as possible, else it risks the technical and political
horror stories engendered by Globus -- which became an unfortunate and
...
implementations on the ground. For example, there are already multiple
implementations of Google's BigTable, such as Hbase etc. This is
reminiscent of the plethora of grid middleware distributions; there is
bound to be a proliferation of such Cloud-like infrastructure.

Application level programming pattens and data-access remain
essentially invariant on differerent infrastructure; thus being able
to support application specific data-access patterns is both useful
and important~\cite{dpa-paper}.

Having said that, there are infrastructure specific -- technical and
policy, features that need to be addressed. For example, Amazon, the
archetypical Cloud System charges for data transfer across {\it its}
network. Hence Cloud Programming Models must be cognizant of the
requirement to programmatically control data and compute placement.
Not that traditional Grids applications do not have this interesting
requirement, but that such explicit support is required only for very
large-scale and high-performing applications, whereas here for Cloud
application it is required for basic cost minimization, i.e., the same
computational task can be priced very differently possibly for the
...
applications for both Clouds and Grids, which is the primary
motivation of this work.

Any {\it effective} abstraction will provide most, if not all of the
above features I a nutshell, primary aim of this work is to establish
that SAGA is a {\it effective} abstraction that can support different
programming models and is usable on traditional (Grids) and emerging
(Clouds) distributed infrastructure.  Our approach is to begin with a
well understood data-parallel programming model (MapReduce), implement
it using a standard programming interface -- the Simple API for Grid
Applications (SAGA).  SAGA has been demonstrated to support
...
aim is to use the {\it same} implementation of MapReduce on a range of
Cloud systems, and test for inter-operability between different
...
Fig.~\cite{saga_figure} provide a view of the SAGA landscape, and the
main functional areas that SAGA provides an interface for. Based upon
an analysis of more than twenty applications, the most commonly
required functionality involves job submission across different
distributed platforms, support for file access and transfer, as well
as logical file support. Less common, but somewhat more critical where
required, is the support for Checkpoint and Recovery (CPR) and Service
Discovery (SD).  The API is written in C++ with Python, C and Java
language support. The {\it engine} is the main library, which provides
...
as there exist several well documented sources~\cite{saga_url}.
...
Specifically, we have implemented MapReduce and the All-Pairs, and
have used implementations in SAGA to to solve commonly encountered
genomic tasks.  Specifically, multiple sequence alignment can be
orchestrated using the SAGA-All-pairs implementation, and genome
searching can be implemented using SAGA-MapReduce.
...
\jhanote{This needs to be shortened significantly}
...
\subsection{SAGA-MapReduce Implementation}
...
\subsection{All-Pairs} \jhanote{this will probably have to go/reduced
  drastically}
...
machines~\cite{bigtable_small}.  Another advantage is the ability to be
...
provided by the UK EPSRC grant number GR/D0766171/1.  SJ acknowledges
the e-Science Institute, Edinburgh for supporting the research theme,
...
computer resources provided by LONI. Michael Miceli was supported
by a Google Summer of Code grant.
