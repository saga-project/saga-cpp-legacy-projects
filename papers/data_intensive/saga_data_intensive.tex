...
pattern (MapReduce) and implement it using a standard programming interface
...
{\bf MapReduce: } MapReduce is a programming framework pioneered by
...
Additionally, they coordinate this effort with Bigtable.  % We will
...
features implemented, and not a fundamental limitation of the design
...
\item Coordinating all the executed workers, this includes the
  ...
  map step, handling the intermediate data files produced by the map step and
  passing the names of the sorted output files to
...
{\bf SAGA-MapReduce on Grids:}
%(Experiments 1 and 5)
%Figures~\ref{grids1}, \ref{grids2}, \ref{grids3}
...
occurring for smaller data-set sizes). See Fig.~\ref{grids2}.
...
SAGA based implementations have not been optimized, for example,
...
%   \hline \hline
  ...
  \hline \hline
  local \& local-FS & 2 & 0.2 & 709 \\
  \hline 
  distributed \& local-FS & 2 & 0.2 & 454 \\
  \hline \hline
  local \& local-FS &  4 & 0.5 & 1083s\\
  \hline
  distributed \& local-FS &  4 &  0.5&  912s \\
\hline \hline
\end{tabular}
\caption{Table showing \tc for different configurations of compute  
  and data. The two compute configurations are: (all workers are either
  placed locally or  workers are distributed across two different resources. The data configurations are: either using a single local FS   or distributed FS with 2 data-servers.}
\end{table}
% and thus not
...
boost).  To us, a mixed approach seemed most favourable, where the bulk
