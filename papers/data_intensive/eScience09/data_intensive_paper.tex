...
% A simple but illustrative example is an ensemble of an
...
%  DAG-based workflows are probably the most common
...
framework %is a generic name for a development tool that
...
% If transferring the set to the compute
...
% have a lower time to completion by having small transfer times. 
...
%Here we define the \textit{heuristic framework} method, 
...
% Is a DFS slower than manual
% placement of data? When manually handling data, what are the
% advantages of being able to move work to data, or data to the work?
...
% These variabilities listed above (tuple \ref{Eq:tuple}) are
% manipulated to determine causes for different I/O complexities
% observed in an attempt to build an understanding of issues that
% arise when utilising data-intensive applications.
...
% the All-Pairs application is aware of the data location before
% determining whether or not to assign a certain set of data-dependent
% computation to an idle worker.
