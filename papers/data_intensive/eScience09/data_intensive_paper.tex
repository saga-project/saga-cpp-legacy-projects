...
\begin{center}
    ...
    \end{tabular}
\end{center}
    \caption{\textit{Here we show the configurations of $D$ (tuple \ref{Eq:tuple}) that we use in our experiments, for two and three machines. Both $c$ and $d_a$ can have yes/no (Y/N) values. A $c = Y$ means the machine $X_i$ does computation, and a $d_a = Y$ means the machine has data stored. For C4 and C5, we divide the data equally among the machines.}}
    \label{Tab:Configs}
\end{table}
...
\fixme{Mention for CloudStore we also need to include a replication factor.}
