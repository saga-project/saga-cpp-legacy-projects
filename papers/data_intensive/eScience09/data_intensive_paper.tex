...
We also compare our results with $t_{compute}=0$ for two different data set sizes, one with $c_s = 287MB$, and the other one with half the size, $c_s = 144MB$ (rounded value). Both use CloudStore, and have eight assignments. We define two quantities, $\Delta t_c^d = t_c(c_s = 287MB) -  t_c(c_s = 144MB)$, and $t_{common} = 2 \times t_c(c_s = 144MB) - t_c(c_s = 287MB)$. 
...
\subfigure[$t_{common}=2*t_c(144MB)-t_c( 287MB)$]
{
...
%%}
\caption{\textit{$(N_c, fs, m) = (\mbox{8, CloudStore, Direct})$}}
