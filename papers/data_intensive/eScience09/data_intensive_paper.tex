...
$D$ is a comma separated list of the following
...
The first one is $C1=X_1(Y, Y), X_2(N, N)$, where $X_1$ has the data and runs all the jobs, while $X_2$ is added in our xml file, but does not participate. In the local case, this is equivalent to our one machine case, but not when we use CloudStore, as it also contributes to $t_x$. In the second one, $C2=X_1(Y, N), X_2(N, Y)$,  $X_1$ computes with the data stored at $X_2$, here, $X_2$ does no computation. The third one, $C3=X_1(Y, Y), X_2(Y, N)$, has the data in $X_1$, but computes in both $X_1$ and $X_2$ machines. In the fourth configuration, $C4=X_1(Y, Y), X_2(Y, Y)$, we distribute the data into two machines, half of the files go to $X_1$, and half go to $X_2$, both machines compute. For three machines, we have a unique configuration of  $X_1(Y, Y), X_2(Y, N), X_3(Y, Y)$, where the data is distributed in all three machines, and all of them compute. We run our experiments in the grid LONI (Louisiana Optical Network Initiative).
