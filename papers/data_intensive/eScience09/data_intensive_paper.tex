...
replication degree $r$ was larger than 1. In all of our approaches,
the local configuration showed the least $t_c$. \jhanote{although if
  volume of data increased a lot this would not be the case surely.}
The remote configuration showed the most increase in the time to
completion, while $t_c$ decreased with the mixed
configuration. \jhanote{the following sentence is not proper and needs
  refinement} Having all the data in each of machines, and computing
in all, decreased the time even further, but not to the point of the
local computations; perhaps, all the approaches are not guaranteed to
use the data in the machine the job is being run, even thought the
data is in both machines. \jhanote{next sentence could do with
  refinement too} For all the configurations, $t_c$ may be bounded and
decreased to $t_c(\mbox{local})$ as the number of workers $N_w$
increases, up to a critical $N^c_w$, after which $t_c$ will increase
due to worker coordination overhead. With $t_c(\mbox{local})$ we hit
an I/O bound probably caused by the network. It should be noted that
...
addition, we did not utilise our entire distributed system, using at
most 3 distinct machines to handle our work to keep the number of
experiments low.  \jhanote{The previous sentence does not make sense
  to me. Please fix} Overall, the use of CloudStore lowers $t_c$
