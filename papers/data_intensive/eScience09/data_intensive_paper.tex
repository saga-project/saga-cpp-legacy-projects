...
we enocde a simple heuristic to determine the {\it workload} placement
...
applications besides ones based upon the All-Pairs construct. This
abstraction applies an operation on two data-sets such that every
possible pair containing one element from the first set and one
element from the second set has some operation applied to
it.~\citep{Interop, AllPairs}. Essentially, All-Pairs is a function of
...
{\it Infrastructure Used: } In our experiments, we use the stable open
...
did not approach the DFS's performance levels. Perhaps CloudStore's
