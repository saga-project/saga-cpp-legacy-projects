...
We also compare our results with $t_{compute}=0$ for two different data set sizes, one with $c_s = 287MB$, and the other one with half the size, $c_s = 144MB$ (rounded value). Both use CloudStore, and have eight assignments. We define two quantities, $\Delta t_c^d = t_c(c_s = 287MB) -  t_c(c_s = 144MB)$, and $t_{O/H} = 2 \times t_c(c_s = 144MB) - t_c(c_s = 287MB)$. In figure \ref{Fig:CloudStore287minus144:a}, we see that the difference does not scale linearly with the number of workers. For most of our configurations, there is an overhead  which increases with the number of workers. This overhead is larger in the remote data case (C2). It is worth noticing that $\Delta t_c$ is almost zero (and negative) for eight workers when all the resources have workload and data stored (C4); this is, it took about 10 seconds less for a 2.3GB set vs. a 1.15 GB set. \betynote {Why?}.  Figure \ref{Fig:CloudStore287minus144:b} shows us that there are multiple factors that can alter $t_c$.
...
%%}
\caption{\textit{$(N_c, fs, m) = (\mbox{8, CloudStore, Direct})$. Here we compare $t_c$ for data set sizes 1.15 GB and 2.3 GB, this is, for $c_s$'s of 144 MB and 287 MB. For this, we define two quantities, $\Delta t_c^d$, which is the difference between $t_c$ for both $c_s$'s, and $t_{O/H}$, which is the overhead time of working with a larger chunk vs. 2 half-size chunks. }}
