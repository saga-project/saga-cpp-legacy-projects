...
\newcommand{\betynote}[1]{ {\textcolor{green} { ***Bety: #1 }}}
\else
...
(d, c, dc, fs, m,r),
\end{equation}
where $d$ is the total amount of data in each element of a set (\textit{chunk} size); 
$c $ is the number of elements in each set; \betynote{none of the graphs talk about $c$, either we say that before hand, and erase $c$ in here, or we add c to all graphs, most of the cases  $c= 8$}
$dc$ is a comma separated list of elements of the following
...
$r$ is the amount of replication utilized in the experiment. For example, 
 \begin{equation}
(287 \mbox{MB}, E(Y, N), P(N, Y), \mbox{CloudStore, Direct}, 1),
\end{equation}
\betynote{We have to specify replication factor for the local, gridftp case}
shows that each element of a set is 287 MB in size; the computational resource $E$ does calculations, but does not have data stored, while computational resource $P$ does not calculate, but stores the data; the filesystem used is CloudStore, it directly access the files, and we have a replication factor of 1 for our data.
