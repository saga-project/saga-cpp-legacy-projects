...
where $c_s$ is the total amount of data in each element (file) of a set
(i.e., $c_s=\mbox{\textit{chunk} size}$); $N_c$ is the number of
elements in each set (i.e., number of chunks per set); \betynote{most of
the graphs don't talk about $N_c$, either we say that before hand, and
erase $N_c$ in here, or we add $N_c$ to all graphs, most of the cases
$N_c= 8$} $D$ is a comma separated list of the following form: $X(c,
d_a)$ where $X$ is a shorthand for the computational resource, $c$
shows if the computational resource $X$ assisted calculations, and $d_a$
if the computational resource $X$ assisted in data storage, both have a
yes/no $(Y/N)$ value.; $fs$ is the type of filesystem used; $m$ is the
method used to access that filesystem; and $r$ is the amount of
replication utilized in the experiment. For example, 
 \begin{equation}
(287 \mbox{MB}, 8, E(Y, N), P(N, Y), \mbox{CloudStore, Direct}, 1),
\end{equation}
shows that each element of a set is 287 MB in size; we have 8 elements
in each set; the computational resource $E$ does calculations, but does
not have data stored, while computational resource $P$ does not
...
The first one is $C1=X_1(Y, Y), X_2(N, N)$, where $X_1$ has the data and
runs all the jobs, while $X_2$ is added in our xml file, but does not
participate. In the local case, this is equivalent to our one machine
case, but not when we use CloudStore, as it also contributes to $t_x$.
In the second one, $C2=X_1(Y, N), X_2(N, Y)$, $X_1$ computes with the
data stored at $X_2$, here, $X_2$ does no computation. The third one,
$C3=X_1(Y, Y), X_2(Y, N)$, has the data in $X_1$, but computes in both
$X_1$ and $X_2$ machines. In the fourth configuration, $C4=X_1(Y, Y),
X_2(Y, Y)$, we distribute the data into two machines, half of the files
go to $X_1$, and half go to $X_2$, both machines compute. For three
machines, we have a unique configuration of $X_1(Y, Y), X_2(Y, N),
X_3(Y, Y)$, where the data is distributed in all three machines, and all
of them compute. We run our experiments in the grid LONI (Louisiana
...
The overhead of intelligence includes all time spent pinging hosts and
...
\begin{center}
\begin{figure}
\subfigure{\includegraphics[scale=.55]{data/graphs/CloudStoreCompute}}
\subfigure{\includegraphics[scale=0.55]{data/graphs/CloudStoreNoComputeSmallerDataSet}}
\caption{\textit{Comparison of CloudStore using the All-Pairs
