...
We define Application Level Interoperability (ALI) as a feature
that arises, when other than say compiling, there
are no further changes required of the application to utilize a new
platform.  If service-level \index{interoperability} can be considered as weak
...
be supported.  Currently, many \index{programming models} and
\index{abstractions} are tied to a specific back-end infrastructure.
For example, Google's MapReduce, which is tied to Google's file
system, or Sphere\cite{sectorsphere09} which is linked to the Sector
file system. Thus there is a need to investigate interoperability of
...
the canonical \wc application.  We use \index{SAGA} --- “Simple API for Grid
...
concurrently and cooperatively towards a solution of the same
  problem instance.  Our approach was to % take \sagamapreduce and to
use the same %instance of \sagamapreduce to solve the same
...
solution of \index{ensemble-based simulations}. This provides the
motivation for our work in \S6. In \S7, we discuss \index{Azure} in
the context of ensemble-based bio-molecular
simulations. Section~\ref{sec:discuss} concludes the paper with a
...
\begin{figure}[t]
 \dnnn
 \includegraphics[width=1.0\textwidth]{figures/saga-figure02-gray}
...
Python supported as a wrapper. \textit{saga\_core} is the main library,
...
when it executes multiple programming models \textit{concurrently} over
...
\begin{figure}[htb!]
% \includegraphics[width=0.5\textwidth]{sagamr_comparison.pdf}
 \includegraphics[width=1.0\textwidth]{figures/sagamr_comparison-gray}
 \caption{
...
\begin{figure}[htb!]
 \dnnn\dnnn
 \includegraphics[width=1.0\textwidth]{figures/sphere_mr_varying_chunksize-gray}
 \caption{
...
\begin{figure}[htb!]
 \includegraphics[width=1.0\textwidth]{figures/sphere_varying_workers-gray}
 \caption{
...
\begin{figure}[htb!]
 \dnnn\dnnn
 \includegraphics[width=1.0\textwidth]{figures/sagamr_varying_workers-gray}
 \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/distributed_pilot_job}
    \caption{
      \footnotesize
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.45\textwidth]{figures/namd_run-gray}
  \label{tbd}
...
\bibitem{jha_ccpe09}
S Jha, A Merzky and G Fox, Clouds Provide Grids With Higher Levels of
  Abstractions and Support for Explicit Usage Modes, Concurrency and
  Computation: Practise and Engineering Vol 21, no 8, 1087-1108 (2009).

\bibitem{saga_escience07}
S Jha, {\it et al}, Design and Implementation of Network Performance Aware
  Applications Using SAGA and Cactus, pp 143-150, IEEE Conference on e-Science
  2007, Bangalore, ISBN 978-0-7695-3064-2.

\bibitem{saga_tg08}
S Jha {\it et al}, Developing Adaptive Scientific Applications with Hard to
  Predict Runtime Resource Requirements, Proceedings of TeraGrid 2008
  Conference (Performance Challenge Award).

\bibitem{saga_url}
{SAGA Web-Page: \url{http://saga.cct.lsu.edu}}.
...
\bibitem{occ_testbed}
{\url{http://opencloudconsortium.org/testbed/}}.

\bibitem{saga_dic_royalsoc09}
C Miceli, M Miceli, B. Rodgriguez-Milla and S Jha$^*$, Understanding
  Performance Implications of Distributed Data for Data-Intensive Applications,
  Accepted for Philosophical Transactions of the Royal Society A.

\bibitem{Begin:2008rq}
Marc-Ellian B\'egin.
\newblock Grids and clouds -- evolution or revolution.
\newblock \url{https://edms.cern.ch/file/925013/3/EGEE-Grid-Cloud.pdf}, 2008.

\bibitem{HDFS}
Dhruba Borthaku.
\newblock {The Hadoop Distributed File System: Architecture and Design}.
\newblock Retrieved from \url{http://hadoop.apache.org/common/}, 2010.

\bibitem{1239909}
Henri Casanova, Graziano Obertelli, Francine Berman, and Richard Wolski.
\newblock The apples parameter sweep template: User-level middleware for the
  grid.
\newblock {\em Sci. Program.}, 8(3):111--126, 2000.

\bibitem{cheatham-5}
D.~A. Case, III Cheatham, T.~E., T.~A. Darden, H.~Gohlker, R.~Luo, Jr. Merz,
  K.~M., A.V. Onufriev, C.~Simmerling, B.~Wang, and R.~Woods.
\newblock The amber biomolecular simulation programs.
\newblock {\em J. Comp. Chem.}, 26:1668--1688, 2005.

\bibitem{bigtable2006}
...
\newblock In {\em OSDI '06: Proceedings of the 7th USENIX Symposium on
  Operating Systems Design and Implementation}, pages 15--15, Berkeley, CA,
  USA, 2006. USENIX Association.

\bibitem{KFS}
Cloudstore.
\newblock {Cloudstore distributed file system (formerly, Kosmos file system).
...
\newblock {\em SIGOPS Oper. Syst. Rev.}, 41(6):205--220, 2007.

\bibitem{1413421}
Ewa Deelman, Gurmeet Singh, Miron Livny, Bruce Berriman, and John Good.
\newblock The cost of doing science on the cloud: the montage example.
\newblock In {\em SC '08: Proceedings of the 2008 ACM/IEEE conference on
  Supercomputing}, pages 1--12. IEEE Press, 2008.

\bibitem{eucalyptus}
Daniel~Nurmi {\it et al}.
\newblock {The Eucalyptus Open-source Cloud-computing System}.
\newblock October 2008.

\bibitem{Hill}
Constantinos Evangelinos and Chris~N. Hill.
\newblock Cloud computing for parallel scientific {HPC} applications:
  Feasibility of running coupled atmosphere-ocean climate models on {Amazon's
  EC2}.
\newblock In {\em Cloud Computing and its Applications (CCA-08)}, 2008.

\bibitem{GFS}
S.~Ghemawat, H.~Gobioff, and S.T. Leung.
\newblock {The Google file system}.
\newblock {\em ACM SIGOPS Operating Systems Review}, 37(5):43, 2003.

\bibitem{sectorsphere09}
Y.~{Gu} and R.~L. {Grossman}.
\newblock {Sector and Sphere: the design and implementation of a
  high-performance data cloud}.
\newblock {\em Royal Society of London Philosophical Transactions Series A},
  367:2429--2445, May 2009.

\bibitem{Jha:2010kx}
Shantenu Jha, Daniel~S. Katz, Andre Luckow, Andre Merzky, and Katerina Stamou.
\newblock {\em Cloud Computing: Principles and Paradigms}, chapter
  {Understanding Scientific Applications for Cloud Environments}.
\newblock Wiley, 2010.

\bibitem{Kaiser:2006qp}
H.~Kaiser, A.~Merzky, S.~Hirmer, and G.~Allen.
\newblock {The SAGA C++ Reference Implementation}.
\newblock In {\em {Object-Oriented Programming, Systems, Languages and
  Applications (OOPSLA'06) - Library-Centric Software Design (LCSD'06)}},
  Portland, OR, USA, October, 22-26 2006.

\bibitem{enkf_hybrid_grid_cloud}
Hyunjoo Kim, Yaakoub el~Khamra, S.~Jha, and Manish Parashar.
\newblock Exploring application and infrastructure adaptations on hybrid
  grid-cloud infrastructure.
\newblock In {\em First Workshop on Scientific Cloud Computing (Science Cloud
  2010)}. ACM, 2010.

\bibitem{Krishnan:2010nx}
Sriram Krishnan.
\newblock {\em {Programming Windows Azure}}.
\newblock O'Reilly Media, 2010.

\bibitem{azure_blast}
Wei Lu, Jared Jackson, and Roger Barga.
\newblock {AzureBlast: A Case Study of Developing Science Applications on the
  Cloud}.
\newblock In {\em First Workshop on Scientific Cloud Computing (Science Cloud
  2010)}. ACM, 2010.

\bibitem{Luckow:2008xy}
A.~Luckow, S.~Jha, A.~Merzky, B.~Schnor, and J.~Kim.
\newblock {Reliable Replica Exchange Molecular Dynamics Simulation in the Grid
  using SAGA CPR and Migol}.
\newblock In {\em Proceedings of UK e-Science 2008 All Hands Meeting},
  Edinburgh, UK, 2008.

\bibitem{saga_bigjob_condor_cloud}
A.~Luckow, L.~Lacinski, and S.~Jha.
\newblock Saga bigjob: An extensible and interoperable pilot-job abstraction
  for distributed applications and systems.
\newblock In {\em The 10th IEEE/ACM International Symposium on Cluster, Cloud
  and Grid Computing}, 2010.

\bibitem{saga_grid_cloud_interop}
A.~Merzky, K.~Stamou, and S.~Jha.
\newblock Application level interoperability between clouds and grids.
\newblock In {\em Grid and Pervasive Computing Conference, 2009. GPC '09.
  Workshops at the}, pages 143--150, May 2009.

\bibitem{saga_ccgrid09}
C.~Miceli, M.~Miceli, S.~Jha, H.~Kaiser, and A.~Merzky.
\newblock Programming abstractions for data intensive computing on clouds and
  grids.
\newblock In {\em Cloud 2009, Cluster Computing and the Grid, 2009. CCGRID'09.
  9th IEEE/ACM International Symposium on}, pages 478--483, May 2009.

\bibitem{Phillips:2005gd}
J.~Phillips, R.~Braun, W.~Wang, J.~Gumbart, E.~Tajkhorshid, E.~Villa,
  C.~Chipot, R.~Skeel, L.~Kale, and K.~Schulten.
\newblock {Scalable molecular dynamics with NAMD}.
\newblock {\em Journal of Computational Chemistry}, 26:1781--1802, 2005.

\bibitem{saga-core}
{T Goodale {\it et al} }.
\newblock {A Simple API for Grid Applications (SAGA)}.
\newblock http://www.ogf.org/documents/GFD.90.pdf.
