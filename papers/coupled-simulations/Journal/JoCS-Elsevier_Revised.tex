...
layer from the solid obstacle. The sampling duration ${\Delta}t$ is denoted as
the time length over which spatially-sampled instantaneous solutions are
accumulated. The sampling interval ${\Delta}{t_s}$ is defined as the time
...
%%%%%
%The layer size is free to be stretched
%along the periodic direction, while it is confined along the non-periodic
%direction to secure the sufficient space for pure CFD and MD domains.
%On the other hand, stretching the computational domain along the periodic
%direction results in the increase of computational cost.
...
${\Delta}t_{s}$ to get rid of the time-lagging event in CFD boundary condition.
...
from $n{\Delta}t+0.5{\Delta}t_{s}$ to $(n+1){\Delta}t+0.5{\Delta}t_{s}$.
Except when the velocity gradient is linear in time, the extrapolated property
is prone to provide the incorrect value from the start. The only way to reduce
this extrapolation error is to reduce the sampling duration, which is
contradictory to the condition for reducing the statistical error.
...
\includegraphics[width=0.7\linewidth]{Sequential_Coupling.pdf}}
\subfloat[Synchronized Coupling]{\label{synchro_couple}
\includegraphics[width=0.7\linewidth]{Synchro_Coupling.pdf}}
...
to the next communication point and the other code follows. One code stays
idle while another code evolves.
...
CFD solution at $n{\Delta}t$ is directly applied as the boundary condition
for MD simulation and backward time-averaged molecular dynamic properties
over $\Delta{t_{S}}$ sampling durations are imposed as CFD boundary conditions. 
\subref{synchro_mod} MD time axis is shifted forward by half of sampling
duration and extrapolation is employed on MD boundary condition to avoid
time-delay patterns.}
...
extrapolation error by introducing the \textit{predicted} solution at
...
the CFD code additionally evolves by half of sampling interval after
it approaches $n{\Delta}t$. The CFD code sends these predicted properties
at $n{\Delta}t + 0.5{\Delta}{t_s}$ to the MD site and receives the
...
applies the recieved molecular dynamic solution to advance to the next
communication point.
...
from $(n-1){\Delta}t + 0.5{\Delta}{t_s}$ to $n{\Delta}t + 0.5{\Delta}{t_s}$,
the CFD code runs the actual evolution from $(n-1){\Delta}t$ to $n{\Delta}t$
and adds the predictive evolution for ${\Delta}t + 0.5{\Delta}{t_s}$ time
interval. Predicted CFD solution at $(n+1){\Delta}t + 0.5{\Delta}{t_s}$
is applied as the interpolated hybrid boundary condition when MD solver
evolve
from $n{\Delta}t + 0.5{\Delta}{t_s}$ to $(n+1){\Delta}t + 0.5{\Delta}{t_s}$.
...
is scheduled as $2{\Delta}t + 0.5{\Delta}{t_s}$.
...
\subref{predcorr_extra} CFD code runs the prediction step by $0.5{\Delta}{t_s}$.
CFD and MD hybrid boundary conditions are extrapolated properties from the
current solution.
\subref{predcorr_inter} CFD code runs the prediction step by
${\Delta}t + 0.5{\Delta}{t_s}$.
Hybrid MD boundary conditions are interpolated from predicted CFD solutions.
...
Both CFD and MD boundary conditions become implicit.}
...
\subref{cfd_md_couette} Pure CFD solution is exactly the same as
...
\subref{hybrid_couette} The steady result by a hybrid approach produces
...
\vskip-0.2cm
\caption[]{\small \subref{hybrid_scale16} {\bf A Couette flow profile in increased system size.} 
...
\subref{hybrid_scale16} {\bf The variation of the velocity in the middle of 
...
hybrid simulations. Figure~\ref{Stokes_Sol}-\subref{vel_stokes} shows that
...
\subref{vel_stokes} Difference between hybrid and pure CFD solutions in
...
\subref{vel_stokes_history} History of the velocity field reveals that the
...
\subref{stokes_org} With conventional synchronized coupling model,
...
\subref{stokes_both} Overshoot/undershoot phenomena in the conventional
