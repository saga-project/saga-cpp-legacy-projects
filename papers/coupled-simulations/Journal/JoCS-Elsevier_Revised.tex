...
\begin{abstract}
Reduction of statistical error in molecular dynamic samples and
time-accurate temporal coupling scheme are important numerical issues
associated with hybrid computational fluid dynamics (CFD) - 
molecular dynamics (MD) simulation methodologies. 
Also, effective scheduling of distinct-yet-coupled codes
on computing resources is necessary for the efficiency of
this computationally-intensive simulation.
In this paper, we introduce our developments and implementations to
resolve above-stated issues.
We build our hybrid simulation package by deploying a constrained
Lagrangian dynamics scheme with its interface on distinct CFD and MD codes.
Statistical error in molecular dynamic sample is minimized by the delicate
determination of coupling parameters along with the adoption of
replica sampling approach.
We introduce a temporal coupling scheme based on a 'prediction-correction' way
together with interpolation in time, which provides a more time-accurate
hybrid constraint/boundary condition.
With respect to computational performance, we employ a Pilot-job abstraction
and implement the load-balancing function to provide co-scheduling and
load-balancing between logically distributed tasks on the batch queue.
We apply our hybrid simulation framework to solving
two classical flow problems of the impulsively started Couette flow and
an oscillating boundary Stokes flow.
Results verify that the multiple replica sampling contribute much on
suppressing the sampling noise.
Also, the accuracy of unsteady flow solution is increased by applying
a 'prediction-correction' temporal coupling scheme.
Finally, a Pilot-job abstraction contributes to saving computational cost
by eliminating idle waiting of first-started task and
efficiently distributing allocated resources to coupled tasks.
...
%two classical flow problems under Stokes (no inertial effects) conditions. 
