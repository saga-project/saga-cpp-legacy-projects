...
Also, the equation is applied on molecules instead of individual atom, to preserve the linear momentum between continuum and particle domain without encountering the unfavorable numerical break-down of molecular structure.
...
These two applications verify the accuracy of the proposed model and describe the effect of particle-level mass variation on the macroscopic flow evolution.
\end{abstract}
%%%%% End Abstract %%%%%
...
\cite{Thompson,Nie,Nie_cavity,Cui,Wang,Yen,Liu,ECCOMAS10}, alternating 
...
the long-range interaction (i.e., Coulomb force) is not considered. We 
...
two monatomic molecule species varying in mass. Chemical properties of
each artificially designed molecule are set to be the same as that of
the above liquid argon system, except the particle mass. Yet,
the systematic density and total number of particles are preserved,
so as to provide the reliability on preserving other characteristic properties.

Three different compositions have been simulated to verify
...
The second system contains two species of 0.5 and 2.0 non-dimensional
molecular mass whose quantities are 2:1 ratio. Likewise, the third system
consists of 0.4 and 4.0 non-dimensional mass molecules, with the quantitative
ratio of 5:1. 

Figure~\ref{Fig:Multi_species} presents the transient Couette flow profile
from three different composition of molecules. In comparison to the analytic
solution, all simulations show the same flow evolution. It verifies
the accuracy of the multi-species Lagrangian dynamics model.
Hybrid steady-state velocity solutions at time of 1500 $\tau$ globally shows
a little faster than the analytic solution. This behavior is due to 
the slight slip between wall and liquid particles, which is the response of
the inter-molecular characteristc energy. 
...
%\begin{figure}
 \begin{subfigmatrix}{1}% number of columns
  \subfigure[Mixture of 0.5 and 1.5 Mass Atoms]{\includegraphics{050150.jpg}}
  \subfigure[Mixture of 0.5 and 2.0 Mass Atoms]{\includegraphics{050200.jpg}}
  \subfigure[Mixture of 0.4 and 4.0 Mass Atoms]{\includegraphics{040400.jpg}}
 \end{subfigmatrix}
 ...
 All simulations from different particle compositions accurately describe
...
of mass of each molecule is advantageous in preserving the molecular
...
field (e.g., Coulomb force) as well as the short-range Lennard-Jones
...
modeled as the vacuum state in view of particle domain. Mathematically
