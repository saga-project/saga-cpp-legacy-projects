...
\jhanote{I don't understand the next sentence:} Based on using file-based information exchange on CFD-MD coupling simulation, any CFD or MD toolkit is replaceable with file-based appropriate interface information.  \jhanote{I would rather have us to introduce SAGA in this way: load-balancing $\rightarrow$ dynamic eresource allocation $\rightarrow$ BigJob like solution which is then provided by SAGA} In order to increase performance and stability of coupled simulation, dynamic resource allocation concept is employed using SAGA and BigJob frame work which is referred to previous section \jhanote{we have not introducted SAGA and BigJob yet.}
...
\begin{figure}
\centering
\includegraphics[scale=0.45]{figbj_m1.eps}
