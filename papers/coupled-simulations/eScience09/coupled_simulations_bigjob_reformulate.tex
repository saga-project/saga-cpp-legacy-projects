...
%Because of co-scheduling of sub-jobs, a BigJob is free from
inactive mode in
...
In Table~\ref{table:TwoBigJobs}, results from the four test runs representing S2 and S3 are presented. 
The general scenario for these cases is that one bigjob is launched first and the other bigjob becomes available later in the same system (S2) or the remote system (S3).  
When the first bigjob is available, two applications, MD and CFD, are submitted as two subjobs by requesting a half of the cpus of the BigJob. Once the other BigJob becomes available, each application is immediately configured to run in each BigJob by consuming the entire cpus.  Therefore, more available cpus allow each application to speed up.  For simple demonstration, we confine our experimental runs for such a simple configuration without load balancing or other advanced scheme to distribute available cpus.   

As shown in the table, the results with these four test cases demonstrate clearly performance gains with the additionally available cpus when two bigjobs begin to run.  Particularly, the performance gain is examined with respect to two components ${T_{comm}}$ and
    ${T_{compute}}$ of the total time-to-solution.  The major performance gain is indicated with a
    shorten ${T_{compute}}$.  ${T_{compute}}$ represents max( ${T _{MD,compute}}$, ${T_{CFD,compute}}$), a longer compute time of two applications. Each $T_{app,compute}$, where ${app}$ is MD or CFD, is measured by monitoring the start time and the end time between the data exchange.  More cpus are available with the incoming BigJob, more gain in the performance is achieved as shown the case of test 4.  On the other hand, the time for communication, ${T_{comm}}$
...
\begin{table}[!h]
\begin{center}
  \caption{\small Performance measures for Scenarios S2 and S3.  Here, we present the time to
...
According to our results, the communication time is insignificant even for cases with S3 in part thanks to the small size of the data required to be exchanged.  Of course, the file transfer depends heavily on the network condition, but it is not expected to be a major issue considering the different time scales between ${T_{compute}}$ and ${T_{compute}}$.  Furthermore, we expect that the difference would be larger as the size of material systems to be simulated increases.  Considering the scaling of computing costs for MD or CFD as the number of particles in MD or the number of mesh points increase, the scaling of the data to be exchanged, consequently, the time for transferring files is expected to remains as insignificant.   Indeed, the development of an efficient runtime environment for hybrid CFD/MD simulation lies in finding a way to decrease ${T_{compute}}$.  Arguably, the results in the table  ~\ref{table:TwoBigJobs} suggests that an BigJob-based runtime environment is able to provide an compelling idea for that.   
