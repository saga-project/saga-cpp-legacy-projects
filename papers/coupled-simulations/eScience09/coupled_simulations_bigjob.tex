...
We wanted to see if we could come up with an analysis that would tell us if running larger or longer simulations is better or worse with respect to the actual waiting time on the queue.What we did was the following. We chose two machines witch are more than 5000 processors, Ranger and Queenbee. We submitted jobs of different sizes, requiring different walltime limits. The load factor of each machine were gathered on each submitting jobs and the actual waiting times on the queue also measured every time. Lots of factors effect the waiting times. Most important of them is the load at the moment on the machine, the requested walltime limit and the number of processors requested. Two more factor that affects this are the backfilling when other irrelevant job finished and readjusting the priority of job when other isolated higher priority jobs newly on the queue. 
The waiting time has decreasing tendency toward the number of processors of job is increasing as can be seen from Table~\ref{table:waitingtime}.
%The waiting time is most acutely effected due the load as can be seen from Table~\ref{table:waitingtime} . But even then, some of the jobs start running immediately due the backfilling policy.% This may part of due to its~Considering only the data from the jobs which did not go through backfilling, it can be seen that larger processor jobs start earlier based on the priority of jobs.We cannot claim this just by running some tests on a couple of machines. It may part of due to its policies and loads under which a particular machine is run. The waiting time have a little relationship with walltime limit of jobs. But from experience, we feel that running larger processor jobs is better than running two smaller sized jobs. We say that the sum total of the waiting times of two smaller jobs is going to be larger than the waiting time of a single large processor job.

\Nkimnote{Will be added some two small job Vs. BigJob table here}

\begin{table}[t]
\caption{\small  Comparison of waiting time with requesting various resources. The tables show the actual waiting time on the queue as a second with respect of changing the number of processors and different Walltime limit when jobs were submitted on Ranger and Queenbee. 
Comparing the actual waiting times with number of processor on each walltime limit, quantitative tendency of waiting times are somewhat decreased when requesting number of processor increased. But the relationship between the waiting time and walltime limit was hardly observed.
