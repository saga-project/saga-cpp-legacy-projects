...
%   not possible using, or at least not easily, using MPI, 
 (iv) What is the largest system size that we can simulate using BigJob and/or
  ...
  % (vi) Yaakoub: We have also discussed what is required here in
  % terms
  ...
  % BigJob-Glidein runs with the same systems, so we can compare like
  % to like.
}
...
% Additional efficiency is provided via application scenario specific
...
% We investigated if it is possible to determine an optimal
% configuration using BQP on the resources used; we found that at the
% load-factors encountered, and for the wall-clock times of relevance,
% BQP did not have the sensitivity to distinguish between jobs of
% slightly different sizes and requested lengths and predicted the same
% wait times. However as the plots show, ``real data'' established that
% the wait-time was sensitive to the job-size and requested duration.
