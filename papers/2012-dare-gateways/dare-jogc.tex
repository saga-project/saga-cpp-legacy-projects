...
TG/XSEDE science-gateway program~\cite{gce11-nancy}, in 2006
10 gateways consumed 100,000 CPU hours and in 2010, 22 gateways provided
...
gateway, which was notably absent a few years ago, accounted for 25 \%
...
accessed via
gateways.  % additionally, the dominant growth in computing is
% coming from novel application areas such as ngs which can benefit from
% community provided gateways.
Given the overall success and uptake of the gateways approach,
...
for the life-sciences so as bridge the missing capability to utilize
...
and extensible to the end-user communities' needs and specific
...
The contribution of this work is to address the former
...
these challenges. Any framework used to build science gateways
...
and significant complexity in enabling applications to seamlessly utilize distributed
...
DARE provides fundamental support for dynamic execution via
...
utilizing a placeholder job or a container for a set of compute tasks.
They provide an effective abstraction for dynamic execution and
...
effectively across multiple resources on the TG/XSEDE~\cite{saga-royalsoc,saga_bigjob_condor_cloud}.  
...
(i) via template support for execution patterns (such as
...
would result in end-user communities customising
base-capabilities to utilize a range of HPDC infrastructure with low
overhead.
...
% All user side aspects of BigJob can be provided to end-users
...
%  \pilotjob + Patterns (MR) + L2 (Application Capabilities)}
...
Two features make DARE unique. The first is the support for a range of
...
common execution scenarios as {\it execution patterns}. They provide
...
machines and need to be tracked.  DARE supports simple dependencies
...
classification provides a natural and convenient breakdown of
...
configuration file. DARE-Gateway is a web interface, which provides a
...
submission these parameters will be stored in a database. Furthermore users
can look-up the status of submitted jobs and download the
output files once jobs terminate. A job monitoring thread is an important
component inside DARE-Gateways. Its function is to continuously
poll the database for new tasks, and whenever it finds a new task it
spawns it after creating the necessary configuration files.
Connecting the web interface to the server-side DARE services
is made simple with the job monitoring thread. DARE uses
...
frameworks. The SAGA-runtime system is responsible for making
...
tightly-coupled applications. Once the desired configuration has
...
It is important to consider data movement requirements,
...
currently supports essential computational tasks for NGS data analytics, including multiple mapping tools such as  
BFAST, BWA, and Bowtie and pipelines for RNA-Seq and ChIP-Seq\cite{mardis2008-arghg,ecmls_ccpe10}. Mapping 
(alignment) is the first step, along with de novo assembly, in scientific discovery employing NGS sequencing-based 
protocols, i.e., the whole genome resequencing, RNA-Seq, and ChIP-Seq. The number of available services and 
the capacity to handle large volume of data are still growing.

In the previous work~\cite{dare-ecmls11}, using DARE-NGS, we have analyzed the performance of BFAST, a mapping 
program representing the latest generation of mappers. The tool stands out with its development goal for providing 
high sensitivity in analysis, whereas the design objective of most other popular mappers has been to achieve 
computational efficiency, i.e. less memory, computing time, computing resource, etc. In fact, the computational 
complexity of the analysis (e.g. mapping) depends upon various elements such as the size and complexity of the 
reference genome and the data-size of short reads, but mostly algorithms. We characterized the computational 
requirements, I/O bottlenecks and degree of concurrency with BFAST in Ref.~\cite{dare-ecmls11}, and concluded that 
an efficient, scalable and extensible analytical approach must be supported by any framework supporting NGS-
analytics.

An examination of various execution scenarios with multiple
grid and cloud resources was conducted and a part of obtained results are presented in Table~\ref{table:NGS-Distributed}.  Not surprisingly, challenges are more involved than effective distribution of computational tasks 
...
%\subsection{\pilotjobs for Life-Sciences For Distributed Applications}
%\jhanote{SJ}

% \pilotjobs are about the separation of resource assignment and
% workload management.
...
% }
  ...
  % Application (SAGA) and the SAGA-based \pilotjob (SAGA-BigJob)
  % capability.
