...
Adding to the progress we made in understanding the molecular mechanisms of the SAM-I riboswitch illustrating one of remarkable functions of non-coding RNA gene regulatory systems, we continue and expand our efforts to investigate the riboswitch in this cycle of an allocation period. Presumably, our aims during coming years with the riboswitch can be explained with the pipeline illustrated in Fig.~\ref{fig:ribo-pipeline}. 

The big change, compared to our simulation study of the previous years, is during this period, that the required computational works are directed by the holistic approach for understanding the riboswitches. The key idea to pursue this is to combine multiple computational approaches that differ in its physical principles.  For example, the entire folding configuration space is efficiently explored by RNA secondary structure sampling as indicated in the first box of the flow chart in Fig.~\ref{fig:ribo-pipeline}.  We explored the bioinformatics approaches for RNA secondary structure prediction for riboswitch folding and our works were presented in "Emerging Computational Methodologies in Life Science"~\cite{ecmls10}.  Then, successful outcomes are injected to the second layer that carries out the 3D modeling from the 2D secondary structures sampled from the first step.  Finally, atomistic molecular dynamics simulations are carried out as the third step indicated as "3-D structure sampling" in Fig~\ref{fig:ribo-pipeline}.  

Therefore, the coming year allocation will be used for many different simulation strategies beyond MD simulations and we believe this provides us the holistic understanding of the metabolite-assisted RNA folding dynamics.  Also, we ant to emphasize that this kind of strategy is only feasible via Teragrid like federated, distributed and massively parallel resources.  
...
\hline
Type of Calculation &   Method or Package  &    SUs required \\
\hline
RNA secondary structure sampling and analysis &
Sfold/RNAfold/our developed program &  100K\\
