...
\yyenote{This probably should not be the first item in the list of accomplishments}
...
Grid and Cloud environments. This work has resulted in several publications \yyenote{Shantenu how do you want to count this total or just mine? Also does any of UK e-science stuff count?} and presentations
as well as a Masters degree \yyenote{Mine}.
...
As illustrated by the pipeline, we aim the holistic approach for understanding of folding dynamics of ribsowtich RNAs  and closely related RNA-ligand binding affinity calculations, which is contrast to our strategy of the previous years mostly aiming straightforward atomistic Molecular Dynamics simulations.  The key idea is to combine multiple computational approaches that differ in its physical principles but taken together we have much better understanding of the complex biological process carried out by riboswitch RNAs.  As a matter of fact, the primary physics principle behind this pipeline-based approach is the energy landscape perspective that considers the folding of a RNA as statistical treatment. The entire pipeline comprises three levels representing the sampling step of RNA secondary structures, 3D modeling from 2D structure information, and sampling of 3D conformation respectively.   In other words, the entire folding configuration space is efficiently explored by RNA secondary structure sampling as indicated in the first box of the flow chart in Fig.~\ref{fig:ribo-pipeline} and later atomistic MD simulations explore the relevant basins of attraction starting from the configurations sampled. Our work that demonstrated the sampling of RNA secondary structures satisfying the Boltzmann ensemble with scalable HPC resources was presented in "Emerging Computational Methodologies in Life Science"~\cite{ecmls10}. 
...
The binding affinity calculation is carried out to employ the MM-PBSA protocol.  Our preliminary results are summarized in Fig~\ref{fig:mm-pbsa-mg-table}.
...
\begin{center}
  ...
  \subfigure[]{\includegraphics[scale=0.4]{ss-schema}}
\end{center}
...
\begin{figure}
\begin{center}
  \includegraphics[scale=0.5]{n-dep.jpg}
\end{center}
...
\hline
RNA secondary structure sampling and analysis &
Sfold/RNAfold/our developed program & Ranger/QB/Abe & 100K\\
atomistic MD simulation & NAMD  & Ranger/QB/Abe/Kraken & 850K \\
RNA secondary structure sampling and analysis & in house CUDA programs & Lincorn & 50K \\
