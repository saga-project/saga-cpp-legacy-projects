...
\author{Shantenu Jha\inst{1}, Yaakoub El-Khamra\inst{1}, Joohyun
  Kim\inst{1}, Andre Luckow\inst{3}}

\institute{Center for Computation and Technology, Louisiana State University, Baton Rouge LA 70803, USA \and
...
\begin{abstract}
  The aim of this paper is to demonstrate how the Simple API for Grid
  Applications (SAGA) can be used to develop a range of
  applications which are in turn composed of multiple sub-tasks.
  %with agile execution models. 
  In particular SAGA is an effective tool for coordinating and
  orchestrating such applications, whilst keeping the application
  agnostic to the details of the infrastructure used. Although
  developed primarily in the context of distributed applications, SAGA
  provides an equally valid approach for applications with many
  sub-tasks on single high-end
  supercomputers. % dynamic applications with many sub-tasks,
  %possibly distributed, 
  Specifically, in this paper we describe how SAGA has been used to
  develop applications from the following two types of applications:
  applications with loosely-coupled homogeneous sub-tasks and,
  applications with loosely-coupled heterogeneous sub-tasks. We also
  analyse and contrast the scheduling requirements and coupling of the
  sub-tasks for these two applications.  As we will show, applications
  with multiple sub-tasks often have dynamic characteristics, and thus
...
parameter sweeps. However, there exist a large class of
scientific problems that involve applications that can either be
decomposed into smaller coupled sub-tasks via the of choice an
algoirthm, or are naturally composed of coupled sub-tasks.  In this
paper, we will discuss Replica-Exchange (RE) and Ensemble
Kalman-Filter (EnKF) based applications, as representative prototypes
of of applications with coupled sub-tasks. In addition to a level of
similarity between application characteristics, what binds the two
together, is our adopted approach of developing distributed
applications.  Although similar at some levels, they possses important
differences too. For RE based simulations, the sub-tasks are identical
(ie. replicas), whereas for the EnKF the sub-tasks are
heterogenous. Additionally the nature of coupling between the
sub-tasks in the former (regular intervals and pair-wise) is very
different from the latter (irregular and a global-synchronisation
point).
...
\begin{figure}[!h]
  \begin{center}
    \subfigure{\label{sagalayer1}
      \includegraphics[width=5.0in]{saga_comprehensive}}     
  \end{center}
...
%       \includegraphics[width=3.33in]{saga_layered_application}}
%   \end{center}
%   \caption{Schematic diagram showing how SAGA supports the development
%     of three simple, but important ways of developing distributed
%     applications. \up \up}
...
\up \alnote{Are we referring to sub-jobs as sub-tasks or sub-job? Just
...
replica-exchange itself.  The Replica-Agent %is launched by a Grid job
%and 
...
\begin{figure}[htbp]
  \subfigure{\includegraphics[width=0.5\textwidth]{perf_glidein.pdf}}
  \subfigure{\includegraphics[width=0.5\textwidth]{perf_repex2.pdf}}
...
machine by avoiding queuing time delays and fluctuations for every
sub-task and allowing the efficient dispatching of RE tasks solely
through the Replica-Agent. During our experiments we were able to
measure speedups of up to 80\,\% compared to the non Glide-In
approach.
...
framework can efficiently dispatch RE sub-tasks providing an enhanced
...
\section{Loosely-Coupled Heterogenous Sub-Tasks: Kalman-Filter
  Simulations}

\up

Ensemble Kalman filters (EnKF) are widely used in science and
engineering~\cite{KalmanPaper}.
...
data ~\cite{KalmanPaper}, to effectively solve the inverse
problem.
