...
&\multicolumn{2}{c}{$^{3}$CAC/ECE}\\
\multicolumn{2}{c}{Louisiana State University} &\multicolumn{2}{c}{Bioinformatics Laboratory} &\multicolumn{2}{c}{Rutgers University}\\
\multicolumn{2}{c}{Baton Rouge, LA} &\multicolumn{2}{c}{University of Amsterdam} &\multicolumn{2}{c}{Piscataway, NJ}\\
\end{tabular}
\end{minipage}
...
\begin{abstract} 
  ...
  for the P* Model (Pilot-API), and (iv) validate the implementation
  of the Pilot-API by concurrently using multiple {\it distinct}
  pilot-job frameworks on distinct production distributed
  cyberinfrastructures. \jhanote{We should be careful as to whether to
    say if this paper addresss (iii)? We don't discuss Pilot-API and
    we don't validate using the implementation using concurrent
    pilot-job frameworks? Maybe rephrase (iii) and (iv)?}
\end{abstract}
