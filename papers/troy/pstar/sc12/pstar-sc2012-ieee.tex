...
infeasible.\pmnote{ The last statement is a little complex to
...
%Multiple DUs can be aggregated    within a Data Unit Set.
   ...
   it needed for this paper?}\alnote{ok}
...
\begin{figure}[t]
	...
	\caption{\I{\textbf{BigData Architecture and Interactions:}} Similar to BJ, a BD-Manager orchestrates a set of \pilots, i.\,e.\ BigData-Agent. Coordination is carried out via a distributed coordination service. \upp\upp }
	\label{fig:bigdata}
\end{figure}
...
Pilot-API entities.\amnote{rather between pilot API components, right? P*
interactions is fig.1, me thinks}\alnote{fixed.} The Pilot-API decouples
...
collective usage via the Pilot-API. \onote{Haven't we established
that just because a PJ maps to the model doesn't mean that 
it is 'collectively' usable via the Pilot-API, e.g., in the 
glidein case?}  We begin by understanding the
...
\onote{We don't really 'analyze' anything here. We claim that there is C-and-C
and extrinsic factors. I also don't know if a discussion of the C-n-C
properties of DIANE and BigJob are sufficient to claim that these are 'typical
overheads for PJ frameworks'.} The overhead of a PJ framework, like many
...
\cus on Alamo/FG concurrently. \pmnote{what figure does it refer
...
are mostly insensitive to the number of jobs/tasks coordinated.\onote{Strong
claim -- data doesn't show any significant scale-up} 
\alnote{\textbf{This probably should go without data:} Although we do not
provide a detailed discussion of the dependency between coordination overhead
and the number of \cus, it is worth mentioning that the runtime increases only
slightly when going from 64 to 2048 \cus.}

\jhanote{Is this c\&c overhead?} \pmnote{what machines are used to run BigJob, are all these compute units executed concurrently using BigJob and Diane?} Figure~\ref{fig:perf_bigjob-varying-cores} illustrates the performance
...
Cray XT5 / 9,408\,nodes / 112,896\,cores / Torque)\pmnote{can remove kraken}, Trestles 
(100\,TFlop / 324\,nodes / 10,368\,cores / Torque) and QueenBee
(Linux Cluster / 668\,nodes / 5,344\,cores / PBS); FutureGrid:
India (108\,nodes / 864\,cores / PBS); \pmnote{sierra needs to be included} European Grid
...
time to transfer input files ($T_X$), and the actual \jhanote{please
  fix!! we have runtime and actual runtime? does actual just mean
  total runtime?} runtime $T_C$. $T_C$ also includes the overhead of
the pilot.
...
adaptors. \pmnote{don't we have a interoperability at compute level; since we addressed that having same compute unit descripiton for all infrastrcutres} The second is the usage of distinct PJ frameworks via the
...
\onote{Merged from above... not sure what do do with this: As we will
  see further in \S V (Experiments), in spite of a common Pilot-API
  each PJ framework has a rather different usage modality; this is
  reflective of the fact that typically, PJ frameworks ``evolve in''
  and are ``native to'' specific infrastructure. Native infrastructure
  refers to the infrastructure for which a PJ framework has been
  primarily developed for and on which infrastructures it is mainly
  used, e.g., Condor-G/Glide-in is the native PJ framework of the Open
  Science Grid and its use is heavily coupled with WMS; that does not
  mean, however, that these frameworks do not work on other
  infrastructures.}
