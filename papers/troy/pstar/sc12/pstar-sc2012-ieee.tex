...
the Pilot-API -- see section~\ref{sec:experiment-interop}.  In 
section~\ref{sec:experiment-pilotdata} we demonstrate the capabilities of
\pilotdata, which enables applications (i) to minimize the volume of the 
transferred data and (ii) to utilize different transfer protocols
...
BigJob/Futuregrid, (B4) DIANE/EGI, (B5) Condor/OSG.  \jhanote{We've
  ...
  above we are using only one! Misleading!}\alnote{removed first sentence giving this 3 vars: we don't do it in other subsec in V and I think the conf is still understandable as layout in the next 2 para} As discussed in
...
accessing these infrastructures using the native PJ \jhanote{We
  haven't defined native PJ }\alnote{added intro to nativeness to sec II discussion} capabilities for all three PJ
...
for each \cu several cross-atlantic roundtrips (with latencies $>$100\,msec) 
