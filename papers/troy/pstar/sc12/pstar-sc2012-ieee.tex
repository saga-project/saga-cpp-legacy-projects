...
\small\hfill Binding &\small\hfill Early/Late &\small\hfill Late &\small\hfill Late &\small\hfill Late\\
...
End User Environment &API &API and\newline Master/Worker Framework &CLI Tools &Swift 
script\\ 
...
Resource Abstraction &SAGA &GANGA/SAGA &Globus &Resource Provider\newline API/Globus CoG \\ 

\hline

Security &Multiple\newline (GSI, Advert DB Login) &Multiple (GSI) &Multiple\newline (GSI, 
Kerberos) &GSI\\ 
...
Finally, C3 (right bar in figure~\ref{fig:perf_interop_128-bfast-interop}) shows
the result of the OSG and XSEDE::QB run. Since QueenBee is an older XSEDE
...
scalability limitations of certain infrastructure caused e.\,g.\ by IO, memory,
and/or bandwidth bottlenecks. By using the capabilities of the Pilot-API,
i.\,e.\ the decoupling of workload submission from resource assignment,
applications can  scale-out on multiple and possibly heterogeneous
resources in a flexible way. While scale-out introduces some overheads (e.\,g.\
in particular for data-intensive applications, large amounts data need to be
moved), it enables application to improve performance, e.\,g.\ by offloading
parts of the computation to a faster resource (see C1), and to scale its problem
set, e.\,g.\ by utilizing more resources at the same time.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.4\textwidth]{../perf/sc/download-concurrent-cus.pdf}
	\caption{Download and BFAST Runtime with increasing number of concurrent CUs}
	\label{fig:perf_sc_download-concurrent-cus}
\end{figure}

Figure~\ref{fig:perf_sc_download-concurrent-cus} shows that with an increasing
number of CUs, the stage-in of the files becomes a bottleneck.

Either scale-out or optimize data transfers

Challenges:

disk space issues: we are forced to scale-out even smaller problem sets 
io issues


Future Work:

optimizations (parallel queuing and transfering)
