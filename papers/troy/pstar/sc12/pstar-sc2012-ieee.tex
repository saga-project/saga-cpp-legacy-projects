...
EU EGI and others; they are used to support hundreds of thousands of
...
\jhanote{Is it about management or is it transport we're referring
  to?}  \alnote{actually both. Just refined.}  An important challenge
...
without PD, (D2) the usage of PD with the SSH plugin, \jhanote{what
  ...
  transfer and data management} and (D3) the usage of PD with Globus
...
transfers using \pilotdata.  The optimization via \pilotdata in turn
...
can be reduced (if not eliminated); using \pilotdata the overall
...
\alnote{The intial data that I had for GO was not that good, thus, the
