...
required in distributed environments. A property
of scheduling is \emph{binding}. Binding is defined as the time at
which a WU \alnote{or SU?} is assigned to a resource. For example, a
WU can be bound to a pilot either before the pilot has in turn been
scheduled (early binding), whereas late binding can occur if the WU is
bound after the pilot has been scheduled.  \jhanote{I propose the
  following sentence be eliminated:} \jwave{Furthermore, the time of
  binding influences the point at which the binding decision is made,
  e.\,g.\ in the case of early binding, the decision could be made by
  the application, while in late binding mode the decision is made by
  the PJ framework.}
