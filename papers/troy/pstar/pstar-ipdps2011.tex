...
user-space (i.e. application-level scheduling) are \emph{pilot-jobs
  (PJ)}.  The PJ abstraction is a also a promising route to address
...
scientific applications~\cite{dpagrid2009}.  
...
just to compare and contrast them.  Different projects and users have
rolled-out their own PJ abstractions. The fact that users have voted
with their feet for PJs reinforces that the Pilot-Job is both a useful
and correct abstraction for distributed CI; the fact that it has
become an ``unregulated cottage industry'' reaffirms the lack of
common nomenclature, integration, interoperability and extension.
...
TROY is an implementation of the P* Model (\S\ref{sec:pilot-model}). It consists
of the TROY API~\cite{troy_api}, a runtime system and a set of adaptors for
various PJ frameworks (see figure~\ref{fig:figures_pstar_troy}). TROY is
consistent with the SAGA API. SAGA~\cite{saga_url,saga_gfd90} provides a simple,
...
appearance and philosophy: it re-uses many of the well defined and standardized
semantics and syntax. A further design consideration of TROY was a minimal but
complete API.

The pilot-job capabilities in TROY are provided by different PJ frameworks that
are integrated into the TROY runtime system via an adaptor. For this purpose,
TROY defines a Capability Provider Interface (CPI) that must be implemented by
the adaptors of the underlying pilot-job frameworks. This architecture also 
enables the concurrent usage of multiple PJ frameworks.
...
Figure~\ref{fig:figures_troy_classes} displays the main classes
of the TROY implementation. \alnote{remove Python implementation for now since
it implies that there might be another implementation. We should however
elaborate on why python} Figure~\ref{fig:figures_troy_flow} shows the
interactions between the TROY elements.
The TROY API decouples workload management and resource scheduling by exposing
two separate services: The \texttt{PilotJobService} (PJS) and
\texttt{WorkUnitService} (WUS). The PJS serves as factory for pilots. Also, the
PJS can be used to query the PJS for currently active pilots. A
...
Having instantiated a WorkUnitService object, PJS objects can be added and 
removed at any time. This enables applications to respond to dynamic resource 
requirements at runtime, i.\,e.\ if peak demands arise an application can 
request additional resources; if resources are not required anymore, they can be 
released. 
...
% Regardless of the state of the PJS, the application can submit a WU to the
...
% responsible for a WU, the WU transitions to an SU. The \texttt{WorkUnitService}
% can decide on an aggregation of SUs based on a set of defined
% policies or heuristics. The SU is then forwarded to the Scheduler (step 5),
% which selects an appropriate pilot. The SU is then dispatch to the selected 
% pilot using the respective adaptor (step 6 and 7). The PJ framework is 
% responsible for the actual execution of the SU on a resource. 
% Note that multiple levels of (hierarchical) scheduling can be present: commonly 
% a SU is scheduled inside of TROY runtime system as well as in the underlying PJ 
% framework.
...
state. The state model is based on the SAGA job state model. Applications can
query the state using the \texttt{get\_state()} method or they can subscribe to
state update notifications.

\alnote{should we say something about PJ and WU states?}
...
\begin{figure}[t]
	\centering
		...
		\alnote{should we rename scheduler to engine? Would be consistent w/ 
		figure 2 and reflect that there is more than scheduling (e.g. adaptor 
		loading...)}
	\label{fig:figures_troy_classes}
\end{figure}

% \alnote{not sure whether the next is much of help...}
% Also shown in figure~\ref{fig:figures_troy_classes}, are the cardinalities of
...
% can only be managed by one Work Unit Service. So can a Work Unit Service 
% manage
...
\begin{figure}[t]
	...
	pilot-job and the execution of a work unit. \alnote{Not consistent with 
	source examples, e.g. create\_pilot}
	}
...
\hline
\end{tabular}
\caption{Mapping of P* and TROY}\label{table:pstar_elements}
\end{table}

The TROY API classes and interactions are designed to reflect the P* elements
and characteristics. Table~\ref{table:pstar_elements} summarizes the mapping of
P* elements and TROY. As defined by P*, a WU represents a primary
self-containing piece of work that is submitted to TROY. At the API boundary a 
WU transitions to an SU, which functions as internal unit of scheduling.
...
\texttt{WorkUnitService} for the management of WUs. Further, the framework
utilizes a set of internal classes for implementing different P* 
characteristics, e.\,g.\
the \texttt{Scheduler} for scheduling and the adaptors implementations for 
providing the actual PJ capabilities. TROY is based on the M/W coordination 
model: The TROY runtime system functions as master for a set of PJ frameworks 
that are encapsulated within a TROY adaptor. Inside the PJ framework itself 
another coordination model can be used; most commonly this is also M/W. The TROY 
runtime system only utilizes local communication between the application, the 
runtime system and the adaptors. Again PJ frameworks can rely on their own 
communication model.
...
particular we focus on BigJob, DIANE, Condor-G and SWIFT. As summarized in
table~\ref{table:bigjob-saga-diane}, we show that the elements P* model can be
mapped to these frameworks. Further, we compare the characteristics of the four
PJ frameworks in table~\ref{table:pilot-job-comparison}. In addition to applying
the P* model, we integrated BigJob, DIANE and Condor into TROY 
in order to validate i) the suitability of the TROY abstractions and ii) the
extensibility of the TROY framework.
...
TROY provides a unified API that can be used to expose various PJ
frameworks (e.\,g.\ BigJob, DIANE and Condor-G). Further, it enables
the concurrent usage of multiple PJ frameworks (see
...
to the pilot-job user community. Also, the chosen designs enables the
easy exchange of PJ implementations and the concurrent use of multiple
PJ frameworks. TROY thus functions as common access layer for
different PJ frameworks, providing interoperability and portability of
PJ applications. To some extent, the TROY API can be considered to be
a prototype of a PJ-like API extension to SAGA (see Discussion \&
future work, \S\ref{sec:discussion-future-work}).
...
\end{tabular}
\caption{P* Characteristics and Pilot-Job Frameworks\up}\label{table:pilot-job-comparison}
\end{table*}
...
of P* elements to PJ frameworks such as DIANE and Condor-G. There are
potential for immense practical implications of our work too, for
these PJ frameworks collectively support millions of tasks yearly;
however as alluded, to these capabilities were not engineered with
...
potential to ameliorate this situtation.  % While much of our work has
% taken place at a conceptual level, o
Our experience with fragile grid middleware and distributions, here
and elsewhere has taught us to appreciate the significance of details
that come into play while deploying software on heterogeneous
distributed infrastructures, and which in spite of theoretical and
architectural advances have the potential to limit impact.
