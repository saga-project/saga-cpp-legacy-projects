...
arrives. In the following we discuss different coordination \& communication 
architectures. For this purpose, we conducted several experiments on FutureGrid~\cite{fg}.
...
the coordination overhead imposed on a single WU is minimal in particular with 
the Redis and ZMQ backend. The Advert backend currently has some limitations 
which will be discussed later. When the number of WUs is low, BJ can batch WUs optimizing the
...
overall performance is slightly worse than for Redis. In summary, at the 
investigated scenario does not show a significant bottleneck at the manager 
side. If permitted by the communication backend, a single manager is also able 
to handle larger WU counts. BigJob particularly benefits from the performance of 
alternative backends such as Redis and ZeroMQ. Further, the TROY manager introduces another hierarchy-level which can further enhance the overall 
scalability. 

Figure~\ref{fig:perf_bigjob-varying-cores} illustrates the scalability of
BigJob. Despite the performance differences in the different PJ frameworks and
c\&c backends, the overall scalability is good. Naturally both the manager and
agent exhibit certain limitations in the number of WUs and cores they can
support.
