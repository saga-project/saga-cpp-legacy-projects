...
are integrated into the TROY runtime engine via an adaptor. For this purpose,
TROY defines a Capability Provider Interface (CPI) that must be implemented by
the adaptors of the underlying pilot-job frameworks.

\begin{figure}[t]
	\centering
		\includegraphics[width=0.45\textwidth]{figures/TROY_arch.pdf}
                \caption{\textbf{TROY -- An API and Runtime Engine for
...
characteristics. Figure~\ref{fig:figures_troy_classes} displays the main classes
...
\texttt{PilotJob} object is returned as result of the \texttt{create\_pilotjob()} method
of the PJS (step 1). As in SAGA, the instantiation of PJ is done by using a
...
\lstset{
language=Python,
caption={\textbf{PilotJob Creation:} Instantiation of a Pilot Job Service using 
a Pilot Job Description.\label{lst:pjs_creation}},
frame=single,
captionpos=b,
stringstyle=\ttfamily,
...
\begin{lstlisting}
pjs = PilotJobService()
pjd = PilotJobDescription()
pjd.total_core_count = 8
pj = pjs.create_pilotjob('gram://queenbee',pjd,'bigjob')
\end{lstlisting}
\end{minipage}

The PJ object represents a pilot instance and allows the application to interact
with a pilot, e.\,g.\ to query its state or to cancel it. The process of PJ
creation is depicted in step 1-3 of figure~\ref{fig:figures_pstar} and as code
example in listing~\ref{lst:pjs_creation}.

Listing~\ref{lst:wus_reation} shows the creation of a WorkUnitService. The TROY 
API offers to the application that resources can be added and removed
at any time (i.e. at runtime) so the that available resources can be grown and
shrunk dynamically to the active application workload.

\lstset{
caption={\textbf{WorkUnitService Creation:} Instantiation of a \texttt{WorkUnitService} using a reference to the Pilot Job Service\label{lst:wus_reation}},
}
\noindent\begin{minipage}{0.47 \textwidth}
\begin{lstlisting}
wus = WorkUnitService()
...
\lstset{
caption={\textbf{WorkUnit Submission:} Instantiation and Submission of a WorkUnitDescription.\label{lst:wu_submission}}}
\noindent\begin{minipage}{0.47 \textwidth}
\begin{lstlisting}
...
The \texttt{WorkUnitService} is responsible for managing the execution
of Work Units for the application. Regardless of the state of the pilot-job, the
application can submit a work unit to the WUS (listing~\ref{lst:wu_submission}
and step 4). The WUS remains responsible for keeping track of the WUs during its
lifetime. Based on the heuristics or policies the WUS decides on a potential
aggregation of SUs. The SU is then forwarded to the Scheduler (step 5). The
Scheduler will schedule SUs to an appropriate adaptor that represents a capable
resource (step 6). Finally, the adaptor will submit the SU to the PJ framework
which will take care of the actual execution on a resource. Note that multiple
levels of (hierarchical) scheduling can be present: e.\,g.\ inside of TROY in
the scheduler of the runtime engine and in the underlying PJ framework.
...
In the current implementation the P* characteristics are as follows. The
coordination model is hierarchical as the TROY framework delegates parts of the
coordination to the adaptors but maintains the global overview itself. Regarding
the communication characteristic, TROY does not provide its own communication
channels but fully depends on the adaptors for the communication with the
pilot-jobs.
...
TROY provides a unified API that can be used to expose various PJ frameworks
(e.\,g.\ BigJob, DIANE and Condor-G). Further, it enables the concurrent
usage of multiple PJ frameworks (see figure~\ref{fig:figures_distributed_pilot_job}). The SAGA inspired
approach to TROY's API design, and its also SAGA inspired adaptor
based architecture, leverage on the design experiences of SAGA, and
seems to appeal our pilot-job user community. Also, the chosen designs
allow to very easily exchange the actual PJ implementation and to
concurrently use multiple PJ frameworks, as will be shown below. TROY
thus functions as common access layer for different PJ frameworks,
providing interoperability and portability of PJ applications. To some
extent, the TROY API can be considered to be a prototype of a PJ-like
API extension to SAGA (see future work, \S\ref{sec:discussion-future-work}.
...
submission of the WU to the PJ manager, the TROY engine takes care of binding
