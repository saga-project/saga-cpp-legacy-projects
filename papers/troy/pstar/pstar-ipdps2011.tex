...
Figure~\ref{fig:figures_troy_classes} displays the main classes of the TROY
implementation. \alnote{remove Python implementation for now since it implies
that there might be another implementation. We should however elaborate on why
python} Although there is no one-to-one mapping of elements of the P* model to
...
interface (see figure~\ref{fig:figures_distributed_pilot_job}).

As demanded by P*, the TROY API aims to decouple workload management and
resource scheduling by exposing two separate services to the application. The
Pilot Job Service is used for creating, managing, querying and canceling of
pilot-jobs while the Work Unit Service is responsible for managing the execution
of Work Units for the application. In the spirit of SAGA, the instantiation of
pilot-jobs and work units is done by creating a description and passing that to
a service responsible for the creation of the instance. The description can be
reused and has no state, while the instance has state and is a reference for
further usage.
...
together form a complete Python application 
(listings~\ref{lst:pjs_creation}-\ref{lst:wu_submission}).
...
The TROY API offers to the application that resources can be added and removed
