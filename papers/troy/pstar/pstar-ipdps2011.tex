...
the pilot-manager where it transitions to an SU. The PM is then responsible for 
scheduling the SU onto a pilot and then onto a physical resource.
...
\jhanote{AndreL: This could be a good place to describe (not define)
  ...
  implementation of coordination/commmunication/scheduling. OK?}
...
(M/W) is utilized. \jwave{A metric for describing coordination is the
  point and process of decision making} (e.\,g.\ about binding and
...
\alwave{agents/entities}. \jhanote{The previous sentence does not
  really help understand hierarchichal any better..} the
...
A DU correspond to a WU in the PJ model. A particular critical
requirement for data-intensive application, is the management of affinity
between WUs and/or DUs. Thus, Pilot-Data introduces the PD container object,
which provides a higher level abstraction for expressing relationships between
DUs. A PD is a logical container that describes the properties of a group
of DUs. Currently, there is no equivalent for PDs in the PJ model. Users of a PJ
...
introducing a SU to PD, we assume that a PD is the unit of internal scheduling.
Having instantiated a PD, it can be assigned to a PS via the PD manager. A PS is
a placeholder reserving a certain amount of storage, i.\,e.\ it corresponds to a
pilot in the pilot-job model. By associating a PD to a PS the data is actually
moved to the physical location associated with the PS. The PD manager
facilitates the creation of a PS, schedules data movement (with respect to
specified affinities) and manages data accesses.
