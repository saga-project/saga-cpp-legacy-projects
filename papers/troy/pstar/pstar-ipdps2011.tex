...
\jhanote{pilot job, pilot-job and variations therein are used. need
  consistency!}

\jhanote{We need care in the use of ``agent''!}
\alnote{We use the term agent to denote a component inside the pilot that has
some decision making capability. In most cases we could replace agent just with
pilot. However, when talking about decentral coordination with autonomic
decision making this will be difficult. Should we explicitly define an agent in
sec II under Pilot-Job (aka pilot)?}
...
implementation of the P* model. TROY provides a P* API, which exposes the
semantics of PJs in a P* conform way, and a runtime environment. Implementations
of the actual PJ functionalities -- PJ frameworks, such as BigJob and DIANE, --
are integrated into TROY via an adaptor mechanism. TROY is also able to manage
multiple, potentially different PJ instances concurrently. Further, we present
the BigJob pilot-job and BigData pilot-data implementation. Both PJ/PD
frameworks rely on SAGA for implementation of the actual PJ/PD functionality.
Using the adaptor mechanism, TROY can further be extended to other PJ
implementations. We posit that the TROY framework and API can be used for most,
if not all PJ frameworks.\jhanote{Not sure what this means, ``can be used''}
Before validating this claim, in \S{IV} of this paper, we analyze and discuss
several well-known pilot-job frameworks (DIANE, Swift-Coaster and Condor-G)
using the analytical framework provided by the P* model. \jhanote{Note the 
  distinction between
  pilot-jobs and pilot-job framework: DIANE, Swift are PJ
  frameworks. Specific instances of these tools/usage give rise to
  pilot-jobs.}\alnote{added PJ framework definition to terms and usage.}


\alnote{OLD: As we will discuss, consistent with
the goals and aims of SAGA, there can be multiple instances of BigJob
for different backends.}\jhanote{Have we not gone to multiple instances
  of the Pilot-Job for TROY and not multiple instances of BigJob?}\alnote{refined}
...
DIANE~\cite{Moscicki:908910} -- an existing and widely used pilot-job framework,
can be given a well defined API via the TROY framework. To further substantiate
the impact of TROY, we will demonstrate interoperability between different PJ
frameworks -- BigJob and DIANE -- using TROY. We believe this is also the first
demonstration of interoperation of different pilot-job implementations.
...
framework} for pilot-jobs. A \emph{PJ framework} refers to a specific 
instance of a PJ implementation that provides the complete pilot-job 
functionality (e.\,g.\ Condor-G and DIANE).

\emph{Other Terms:}
%
\emph{ \#~Resource:} An entity which provides a capability.  For
example, a compute resource provides computing capabilities: it can
execute compute jobs.  
%
\emph{ \#~Resource Slice:} A physical or logical part (i.e.  subset)
of a resource.\alnote{do we need the resource slide def.?}
%
\emph{ \#~Resource-Manager (RM):} A system component which manages the
access and utilization of a resource.  In particular, the RM maps
resource requests to slices of the resource -- that process is called
\emph{scheduling}. \alnote{In sec II we use a different kind of wording for scheduling} Note that scheduling can combine resource requests
and map them onto the same resource slice, and can also break resource
requests into smaller parts, and map those individually.  A hierarchy
of RMs can be used to manage sub-slices of a resource.  \alnote{I think talking
about a hierarchy of RMs is not useful at this point.}


\jhanote{Need to introduce pilot-job framework in contrast to
  pilot-job} \alnote{introduced PJ framework}

%
% \emph{Scheduler:} A scheduler applies scheduling constraints to an
% unconstraint resource request, thereby transforming the resource
% request into a (complete or partially) constraint request.  For
% example, a Grid meta scheduler may constring a job description (i.e.
% a resource request) to a specific resource subset, e.g. to a
% specific cluster.  Downstream schedulers can further constrain the
% resource request, for example the cluster queue manager may add
% spatial and temporal constrains, by assigning it to a specific
...
% \amnote{That is on purpose really -- an external RM does not do anything
% different than an internal RM, only that one operated (in our case) in system 
% space, and the other one in user space, on a subset of system resources
% (slice).}
% 
% 
...
% request into a (complete or partially) constraint request.  For
% example, a Grid meta scheduler may constraining a job description (i.e. a
% resource request) to a specific resource subset, e.g. to a specific
% cluster.   Downstream schedulers can further constrain the resource
% request, for example the cluster queue manager may add spatial and
% temporal constrains, by assigning it to a specific compute node and
% timeslot, so that it is completely fixed to a resource slice, i.e. it
% is \emph{assigned}.  A \emph{Scheduler} is part of the
% \emph{Resource-Manager}, i.e. it operates on a well defined set of
% resources or resource slices.\\
...
% 
% \alnote{a job description has no state and is not scheduled only WUs
% are.} \amnote{I don't think state plays a role here.  Further, a WU is
% IMHO a job description -- only that the term 'job' is overloaded in
% our environment (i.e.  an instance of an executable application), and
% thus not used here.} \alnote{ Further, there can be spatial and
% temporal constraints.}\amnote{Sure. The above is an example of a
% partial constraint -- I adjusted it to make that more clear.  There
% can also be constraints on internal metrics, like on memory usage
% etc.} 
% 
% \end{compactitem}
...
\item \textbf{Pilot-Job (PJ) or Pilot:} The PJ (or `pilot') is the entity that
actually gets submitted and scheduled on a resource using the resource's RM
system. The PJ manages the set of allocated resources and is responsible for 
the execution of \alwave{SUs/tasks} onto the resource.
\alnote{\textbf{The RM assigns a slice of the Resource to the pilot -- that
pilot then acts as RM for that resource slice.} I think simply equating a pilot
with a RM is bit too simplistic. In a sense it is a application-level resource
...
\item \textbf{Work Unit (WU):} A WU is a resource request which
  encapsulates a self-contained piece of work that is submitted to the
  \alwave{pilot-manager}. \alnote{not introduced yet} The PM has the flexibility 
  to combine and schedule WUs, e.\,g.\ it can determine when a WU is run and 
  what as well as how many resources it will receive.
...
SUs are the units of scheduling used internally. As SU is created
during the submission of a WU, i.\,e.\ once a WU has been passed into the
control of the PM, it becomes an SU. SUs are scheduled and executed on a
resource via the pilot. An SU can consists of one or more WUs. Further, they can 
be combined and aggregated. Note that that this can either be a simple
grouping or a smart aggregation of WUs (e.\,g.\ two sets of parameters merged
into one).

\item \textbf{Pilot-Manager (PM):} The PM is responsible for
  implementing the different characteristics of the P* model. It
  facilitates the coordination between the different P* elements,
  i.\,e.\ the pilots, WUs and SUs. All implementation share these
  elements, but differ in specific attributes and characteristics. A
  PM can e.\,g.\ manage solely one or multiple pilots.
...
kernel as well as other parameters. This WU is then be submitted to
the pilot-manager where it transitions to an SU. The PM is then
...
use.  \jhanote{we toggle between PM and pilot-manager}
...
\jhanote{AndreL: This could be a good place to describe (not define)
  an agent -- call it out as a ``proxy process'' that aids the
  implementation of coordination/communication/scheduling. OK?}
\jhanote{One strategy could be to not define the different types, but
  just list/enumerate? Akin to Communication. i.e. explain what
  coordination is for, what is being coordinated, how (the 3 types)}

\textbf{Coordination:} The coordination characteristic describes how
various elements of the P* Model interact and are orchestrated.
Commonly, a distributed coordination mechanism such as master/worker
(M/W) is utilized. \jwave{A metric for describing coordination is the
  point and process of decision making} (e.\,g.\ about 
scheduling of compute tasks): in the \textbf{central} model,
information about available resources (pilots) \jhanote{why limit to
  only resources? could be SU/WU too?}  are collected centrally, and
decisions are centrally made by a manager process.  \jhanote{Why not
  call this Pilot-Manager explicitly? After all, we've just defined PM
  to say, ``facilitates'' the coordination between the different P*
  elements?}  In the \textbf{hierarchical} model the decision making
process is divided up into a hierarchy of distributed
\alwave{agents/entities}. \jhanote{The previous sentence does not
  really help understand hierarchical any better..} the
Each of them coordinates a defined aspect (e.\,g.\ a certain set of
resources). In the \textbf{decentral} model, control is distributed
among the different components. \jhanote{We are using
  agents/entities/components!}  

% \jhanote{I think the previous sentence is what we would like it to
%   be.. not sure it is the case right now?}
% \begin{compactitem}
...
% \alnote{repeat, thus removed: Furthermore, the time
%   of binding influences who makes the binding decision, i.\,e.\
%   application or framework.}

% \textbf{submit/assign:} A resource request is \emph{assigned}
%   to a resource slice when its constraints allow it to run on that,
%   and only that, resource. A resource request is passed to a resource
%   manager. \jhanote{fix please}
...
%   case both terms are used very synonymous.}
 % \jhanote{I propose the
%   following sentence be eliminated:}\alnote{ok} 
...
\textbf{Scheduling:} The scheduling characteristic describes the
...
the pilot has in turn been scheduled (early binding), whereas late
binding can occur if the SU is bound after the pilot has been
scheduled.  In general, there are multiple-levels at which both
resource selection and binding decisions have to made.

% is defined as the time at which a WU \alnote{or
%   SU?}\jhanote{yes!} is assigned to a resource.


% A property of scheduling is \emph{binding}. 
...
Commonly, the M/W coordination model is used on multiple levels: On
...
\alnote{This paragraph describes a generalization to multiple levels
  of scheduling. Do we want to keep it?:} \jhanote{I think not; needs
  simplification} \alnote{refined ``Putting it all together''} 
...
\subsubsection*{Application of P* Model Elements to Data}
...
\begin{compactenum}[A.]
\item \textbf{Data Unit (DU):} DU is the base unit of data used by the 
framework,  e.\,g.\ a data file or chunk. 
...
\item \textbf{Pilot-Data Manager} is analogous to the PJ-Manager responsible for 
  managing DU, PD and PS elements. It implements the different characteristics 
  of the P* model.
\end{compactenum}
...
A DU correspond to a WU in the PJ model. A particular critical requirement for
data-intensive application, is the management of affinity between DUs and also
between WUs and DUs. Thus, Pilot-Data introduces the PD container object for
expressing relationships between DUs. A PD corresponds to an SU in the PJ model,
i.\,e.\ it is used as scheduling unit for internal optimizations, e.\,g.\ the
grouping of DUs. Having instantiated a PD, it can be assigned to a PS via the PD
manager. A PS is a placeholder reserving a certain amount of storage, i.\,e.\ it
corresponds to a pilot in the pilot-job model. By associating a PD to a PS the
data is actually moved to the physical location associated with the PS. The PD
manager facilitates the creation of PSs, schedules data movements (with respect
to specified affinities) and manages data accesses.
...
the future. 
% In the following, we discuss further implementation
...
% To implement the P* model, there are additional considerations
% necessary, e.\,g.\ the definition of the exposed end-user abstraction
% and usage model etc.  The \jhanote{API usage model.. have we defined
%   it or is it clear what it is in this context? Why note just API?}
% \jwave{API usage model} defines how resources are allocated as well as
% how WUs are described, submitted and monitored. Further,
% non-functional properties such as fault tolerance and security must be
% considered. PJ implementations differ in their support for
% authorization, authentication and accounting. Common grid technologies
% e.\,g.\ GSI, VOMS, MyProxy, etc. are used. \jhanote{for what? what are
%   we trying to say?} Further, it \jhanote{What is ``it''?} can be
% differentiated between single- and multi-user: The former
% \jhanote{What is the former?} PJ runs under the identity of a single
% user and is only able to accept jobs from this user, while the \jwave{latter}
% PJ is able to accept jobs from different users.

% \jhanote{We should discuss subsection D in the call today. It might be
%   best to move it out of P* and into TROY, or change the focus of the
%   subsection to lay-out why the API is just the first step towards a
...
implementations. Further, we show in section~\ref{sec:bigdata} how TROY is
...
restricted to single resource use (e.\,g.\ BJ), the concept of a
...
our P* Model can be used to explain/understand some of these PJ frameworks,
...
\hline
\textbf{P* Element} &\textbf{BigJob} &\textbf{DIANE} &\textbf{Condor 
...
In this section we analyze the performance and scalability of different TROY
\jwave{configurations}\jhanote{Mark: Is configurations the word we want to
use?}. In particular we investigate the performance and scalability of the
Pilot-Job implementations BigJob and DIANE. Further, we investigate the
interoperability of the TROY backends: BigJob (with SAGA/PBS and SAGA/Condor)
\jhanote{What is a TROY backend? Isn't backend used inconsistently
here?}\alnote{haven't converged on the naming yet: TROY backend vs. TROY
adaptor?} and DIANE using a genome sequencing application scenario.


\subsection{Understanding Pilot-Job Implementations}

Each P* implementation is associated with various degrees of freedom, e.\,g.\ in 
the design of the communication \& coordination (c\&c) sub-system. The primary 
barrier for performance and scalability is not the WU submission, but the 
internal coordination of the distributed components of a P* implementation.
Depending on the degree of distribution, it can be differentiated between local 
communication, i.\,e.\ LAN communication within a cluster, and distributed 
communication, i.\,e.\ communication between multiple resources. In the 
following we investigate the impact of different c\&c related factors on the 
...
this concept is also referred to as tuple
space~\cite{Gelernter:1985:GCL:2363.2433}. The data space decouples manager and
agent, i.\,e.\ both entities can operate at their own pace. Depending on the
setup this data space can be locally deployed, i.\,e.\ on the same resource, or
remotely, e.\,g.\ during a distributed run. A particular challenge during
distributed runs is the latency between the application and the Advert database.
Another challenge is that this design introduces a potential 
single-point-of-failure and scalability bottleneck if
the centralized data space is not carefully designed and operated. BigJob provides
various alternative c\&c sub-systems: Redis~\cite{redis} is e.\,g.\ is a
key/value store, which can also be deployed in a distributed, fault tolerant
way. Redis is used in a similar way as the Advert database, i.\,e.\ all
communication between BJ agent and manager is done through it. The
ZeroMQ~\cite{zmq} c\&c sub-system in contrast utilizes a client-server
architecture, which is similar to the CORBA-based c\&c sub-system of DIANE. In
this architecture, the PM maintains the overall state. Clients connect to the PM
to request new SUs or to report state updates. Both the data space and
client-server c\&c sub-system can be combined with a publish/subscribe
mechanisms, i.\,e.\ instead of polling an agent or client can receive
notifications when a new SU arrives. In the following, we evaluate different BJ
configurations and compare and contrast them with DIANE. For this purpose, we
conducted several experiments on FutureGrid~\cite{fg}. To evaluate the overall
performance and throughput, we execute a different number of WUs on the
FutureGrid resources Alamo and Sierra utilizing up to 128 cores concurrently.
...
Figure~\ref{fig:perf_bigjob-varying-wus} and \ref{fig:perf_bigjob-varying-cores}
illustrate the performance and scalability of the different BigJob c\&c
sub-systems and DIANE. Clearly, the used c\&c sub-system has a great impact of
the overall performance. The Redis backend shows the best performance for small
WUs counts providing a very natural decoupling between manager and agent. A
difference between local and distributed coordination is very small and only
observable for larger core counts. While ZeroMQ is very fast and lightweight by
itself, it requires a very careful implementation in particular concerning
synchronization and throughput optimization. The overall performance is slightly
worse than with the Redis backend. The Advert backend currently has some
limitations which will be discussed later. While DIANE shows a higher startup
overhead, which is particularly observable for smaller numbers of WU, the
runtime increase only slightly (about 10\,sec) when going from 64 to 2048 WUs. A
reason for this behavior is that DIANE can aggregate SUs. At the investigated 
scales, the single manager does not represent a bottleneck. Further, the TROY 
manager introduces another hierarchy-level which can further enhance the overall
scalability.
...
the Redis c\&c sub-system show a linear scalability up to 128 cores achieving a 
throughput of up to 4 WUs/sec. DIANE shows in particular for lower core counts a 
longer runtime. With higher core counts both DIANE and BigJob with ZeroMQ c\&c 
sub-system show a greater increase of the overall runtime. This increase can be 
mainly attributed to single central manager in the client-server architecture.
As in the last experiment, the Advert c\&c sub-system showed a significantly 
lower performance.
...
service implementation.  The used advert service is based on
...
connection is not optimal. \jhanote{It is not clear why REDIS and
  ZeroMQ are not susceptible to access over WAN delays/latency?}
...
application. To manage the workflow we use the DARE~\cite{dare-tg11-gateways}
framework which builds upon TROY. On the LONI~\cite{loni} cluster Oliver we
utilize a total of 16 cores distributed across four nodes. We make use of two
BigJob implementations: TROY-BigJob and TROY-DIANE. We define eight WUs for
BFAST. We run the experiments in three different setups: (i) TROY-BigJob only,
(ii) TROY-DIANE only and (iii) both TROY-BigJob and TROY-DIANE. In case (iii) on
