...
implementation of the P* model using the SAGA API. \alnote{Should we
  briefly mention the TROY API?} The specific pilot-job and pilot-data
implementations in TROY are referred to as BigJob and
BigData. \jhanote{Previous section needs fixing to make it consistent
  with new structure of Section 3} As we will discuss, consistent with
...
for different backends.\jhanote{Have we not gone to multiple instances
  of the Pilot-Job for TROY and not multiple instances of BigJob?}
Thus, we posit that the TROY API can be used for most, if not all
pilot-job frameworks. \jhanote{Not sure what this means, ``can be
  used''} Before validating this claim, in \S{IV} of this paper, we
...
provided by the P* model. \jhanote{Not the distinction between
...
implementation, \jhanote{I propose to change pilot-job implementation
  to pilot-job framework} can be given a well defined API via the TROY
API. To further substantiate the impact of TROY, we will demonstrate
...
\emph{scheduling}.  Note that scheduling can combine resource requests
...
of RMs can be used to manage sub-slices of a resource.  
%
...
\jhanote{Lets discuss the above paragraph}
...
\item \textbf{Work Unit (WU):} \alnote{Are we submitting to the PJ
    framework or Pilot-Manager?} \jhanote{I don't think we should talk
    about a PJ framework.. for amongst other reasons it has not been
    defined. Even in SU, we have PJ Framework... maybe should also
    change?} \alnote{ok, what term do we want to use if we refer to the 
	overall thing e.g. in the SU section? Pilot-Job?} A WU is a resource 
	request which encapsulates a self-contained piece of work that is 
	submitted to the Pilot-Manager. The PM has the flexibility 
	to combine and schedule WUs, e.\,g.\ it can determine when a WU is run
	and what as well as how many resources it will receive.
...
\begin{figure}[t]
	...
	} \jhanote{Once we have defined BigJob and BigData, there is no need to use
          BJ-SAGA and BD-SAGA!!}}
