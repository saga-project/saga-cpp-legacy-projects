...
\emph{Other Terms:}
\emph{ \#~Resource:} An entity which provides a capability.  For example,
a compute resource provides computing capabilities: it can execute
compute jobs.  
%
\emph{ \#~Resource Slice:} A physical or logical part (i.e.  subset) of a
Resource.
%
\emph{ \#~Resource-Manager (RM):} A system component which manages the
access and utilization of a Resource.  In particular, the RM
assigns slices of the resource to individual resource requests -- that
process is called \emph{scheduling}.  Note that scheduling can combine
resource requests and map them onto the same resource slice, and can
also break resource requests into smaller parts, and assign those
individually.  A hierarchy of RMs can be used to manage sub-slices of 
a resource.  
%
...
% example, a Grid meta scheduler may constring a job description (i.e. a
...
% resources or resource slices. \textbf{Note} that a scheduler can
% potentially break up an incoming resource request into multiple
% smaller resource requests, or can combine multiple resource requests
% into larger ones. 
