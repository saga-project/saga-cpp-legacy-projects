...
(section~\ref{sec:pilot-model}).  
It consists of the TROY API, a runtime
...
data transfer and distributed coordination.
SAGA~\cite{saga_url,saga_gfd90} provides a simple, POSIX-style API to
the most common grid functions (e.\,g.\ file, job management and
distributed coordination) at a sufficiently high-level of abstraction
so as to be independent of the diverse and dynamic grid environments.
The TROY API itself was also designed to be similar to SAGA in
...
Further, we aimed for a minimally, complete API exposing the least
details necessary.  
...
scheduling. The TROY is available at~\cite{troy}.
...
\begin{figure*}[t]
    ...
    \caption{\textbf{BigData Architecture:} The BD Manager is exposing the
      ...
      movements. \amnote{I don't see the mentioned adaptor layer in
      the figure!}\upp\upp}
...
is integrated into the TROY runtime environment via a adaptor layer
...
provided. Various other backends support resource elasticity natively,
e.\,g.\ DIANE and Condor, but can be combined into a BJ pool
nevertheless.
...
efficiently support the compute- and data-related aspects of dynamic
execution.
