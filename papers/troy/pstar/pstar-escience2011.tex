...
BigJob Manager is responsible for the orchestration and scheduling of pilots.
represented by the BigJob Agent. For submission of the pilots, BigJob-SAGA
...
responsible for gathering local information and for executing tasks (SUs) on the
local resource. The SAGA Advert Service and API are used for communication
between manager and agent. The Advert Service exposes a shared data space that
can be accessed by the manager and agent, which follow a push/pull communication
pattern, i.\,e.\ the manager pushes a SU to the Adverts Service while the agents
periodically pull for new data. Results and state updates are pushed back from
...
(a so called sub-job). Binding takes places at submission (early binding). For
