...
\jhanote{(i) definition of pilot-job and US very similar. (ii)
  highlight relation between UW/US, (iii) symmetry: UW for PJ, DU for
  PD, (iv) make explicit that Pilot-Data is the analogue of US, (v)
  shouldn't US cover data.. (vi) address in the conclusion that they
  will merge, but we have shown/named them different for
  motivation.(vii) US--> SU, UW --> WU.}
...
\item \textbf{Unit of Scheduling (US):} US are the entities 
  that get scheduled to the resource via a PJ, and to which UWs are assigned
...
responsible for mapping one or more UWs to a US.
...
specifies the capabilities of the resources required in a UW
...
having defined the P* Model, we propose to extend it with data so as to facilitate
...
relationships between Data Unit (DU) as well as DUs and UWs. These relationships are
referred to as
\emph{affinities}.
...
UWs (8) concurrently. To run these 8 UWs concurrently with BJ-DIANE implementation we have to start 8 different 
