...
We performed tests demonstrate the capabilities of TROY API running eight Bfast matching step tasks.
Three different type of experiments were run
(i) Bigjob-SAGA backend will be used to run all the eight tasks.
(ii) Bigjob-Diane backend will be used to run all the eight tasks.
(iii)Both bigjob SAGA and Bigjob Diane backend are used to run 4 tasks each.

 The machine used in this experiments was Oliver one of  the LONI machine.
 Oliver has 4 cores per each node. So, for all these three experiments the 4 nodes
will be used to run 8 tasks since each  Bfast task uses 2 cores.

In the  Bigjob-DIANE backend Multinode submitter was not used/not implemented yet.
So it submits job request for each node seperately and there is also some overhead involved in
startup time of UOW  with Bigjob-DIANE since DIANE is installed separately for every agent/node request. 
...
As far as I understand the naming in diane is different agents. diane-agent  and diane-agent's-worker(subagent)
