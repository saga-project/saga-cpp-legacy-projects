...
\item\textbf{Pilot-Job Abstraction:} A PJ is a container for multiple
  ...
  mechanism to decouple unit of works (the compute ``tasks'') from being 
  hard-coded to a specific “resource'' or delay the binding. \alnote{ the 
  "thing" that is sent out to \ref{resource}} \label{pilot-agent}
...
\item Conceptual agent/ pilot job / placeholder job: user space - attempt to decouple workload from resource prov. and workload

\item \textbf{Local Resource Manager (LRM)} is the gateway to local resources. 
This can be e.\,g.\ a PBS/Torque or WMS service. \alnote{not a element of P* possibly merge with R - move to impl. section}
...
\item \textbf{Resource:} A storage/compute resource that has a common
  ...
  LRM.\label{resource} \alnote{move to impl. section}
...
pilot-job framework are internally coordinated.
     		\begin{itemize}
				...
				what resource. In the \textbf{hierarchical/multi-Level} 
				...
				\item \textbf{Push vs Pull Model:} We define the terms push and 
				pull based upon the determinism of the binding. If
...
\textbf{Communication:} Different communication 
...
\textbf{Data management:} Managing input and output files can be 
...
The \textbf{End-user abstraction} is the external API (respectively 
	interface) that is exposed by the P*-framework to the application.

\textbf{UW Binding} describes the assignment of a UW to a pilot-job.
	\begin{itemize}
    ...
    %       \end{itemize}
    % \end{itemize}
\textbf{UW Scheduling:} Task scheduling describes 1) the process of  
...
\textbf{Resource Types:} Pilot-jobs can be run on different 
...
\textbf{Application Types} describe the characteristics of the 
...
\subsubsection{Non-Functional Characteristics}
...
\textbf{External Coordination (Exposed to API)} describes the 
  ...
  component, such 
	 ...
	 (\ref{application}).
		...
		\textbf{External Communication (Exposed to API)} described the 
     communication 
...
The API we have designed is similar to SAGA in appearance and
philosophy: it re-uses many of the well defined (and standardized)
semantics and we attempted to keep it simple in the amount of details
it exposes.
