...
\begin{abstract}
  ...
  ability to utilize resources dynamically, and not just statically.
  Pilot-Jobs have been notable in their ability to support dynamic
  resource utilization -- both in the number of applications that use
  them, as well as in the scope of usage and number of CI that support
  them.  In spite of broad uptake, however, there does not exist a
...
\item \textbf{Pilot-Job (PJ):} The PJ (also referred to as pilot) is
  ...
  Commonly, the PJ utilizes an agent to manage the set of resources 
...
\item \textbf{Scheduling Unit (SU):} SU are the entities 
  that get scheduled to the resource via a PJ, and to which WUs are assigned
...
is the manager who is responsible for managing pilot-jobs and WUs. After a WU is 
assigned to the manager it binds it to a SU and schedule the SU to an available 
resource. \upp\upp}
...
\begin{figure*}[t]
    ...
    \caption{\textbf{BigData Architecture:} The PD-Manager exposes the
      TROY Pilot-Data API. \jhanote{``is exposing'' -- wrong tense}
      Application can create group of files and assign files to
      storage. The PD manager tracks files and physical locations in
