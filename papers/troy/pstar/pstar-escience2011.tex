...
hard-coded to a specific ``resource'' or at least to delay that binding. 
Commonly, PJ frameworks consist of the following elements:
...
environments. As shown in Figure~\ref{fig:figures_pstar_troy} TROY is based on
\jhanote{need a better expression. ``located on top'' is too colloquial} \alnote{changed, maybe someone has a more fancy word?}
the SAGA API and runtime environment, which is utilized for job management, data
transfer and distributed coordination. The TROY API maintains the SAGA {\it Look
\& Feel} and was designed to be similar to SAGA in appearance \jhanote{isn't
appearance covered in look-and-feel?} \alnote{Look \& Feel incl. the appearance,
but also some non-functional classes such as context} and philosophy: it re-uses
many of the well defined (and standardized) semantics and syntax of the File,
Job and Advert API. Further, we aimed for a minimally, complete API exposing the
least details necessary. \jhanote{can we call it minimally
  complete?}
...
enables the concurrent usage of multiple PJ
