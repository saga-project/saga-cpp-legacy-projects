...
\begin{abstract}
  ...
  viz., dyanmically, and not just pre-execution, viz., statically.
%  resources dynamically, and not just statically.
...
\jhanote{(i) definition of pilot-job and SU very similar. (ii)
  highlight relation between WU/SU, (iii) symmetry: WU for PJ, DU for
  PD, (iv) make explicit that Pilot-Data is the analogue of SU, (v)
  shouldn't SU cover data.. (vi) address in the conclusion that they
  will merge, but we have shown/named them different for
  motivation.(vii) US--> SU, WU --> WU.}
...
\begin{compactitem}
\item \textbf{Pilot-Job (PJ):} The PJ (also referred to as pilot) is the entity that actually gets submitted and scheduled onto a resource using whatever mechanism that is provided by the resource. Commonly, the PJ utilizes an agent to manage the set of resources allocated to it. Depending on the context, PJ also refers to the concept of utilizing a placeholder job to which at a later step the user's workload is submitted to as well as to the PJ abstraction.
...
\item \textbf{Work Unit (WU):} A work unit is the workload that
  ...
  a resource indirectly via a PJ.  The PJ framework in turn has
  flexibility in determining when and how-many resources the WU will
  receive.

\item \textbf{Scheduling Unit (SU):} SUs are the entities that get scheduled to the resource via a PJ. They represent one or more WUs that are combined inside the PJ framework. Note that that this can either be a simple grouping or a smarter aggregation of WUs (e.g. two sets of parameters merged into one).
...
\textbf{Binding} defines how and at what time the assignment of a WU
to a PJ is done.  For example, a WU can be bound to a PJ either before
the PJ has in turn been scheduled (early binding), whereas late
binding can occur if the WU is bound after the PJ has been assigned
\jhanote{should this assigned really be scheduled?} to a resource.
Depending upon the type of binding supported, the workload
\jhanote{what is worload? WU?} is either specified completely or just
independent of a (specific) resource.  \alnote{Question: Is the time
  of binding independent of wether the PJ has resources or not? Late
  binding can also occur if a PJ has resources.} \jhanote{Sorry, but
  the question is unclear to me ..lets discuss} Furthermore, the time
of binding influences the point at which the binding decision is made,
e.\,g.\ in the case of early binding, the decision could be made by
the application, while in late binding mode the decision is often made
...
\textbf{Scheduling:} Scheduling describes the process of assigning a SU to
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{P*-Model: Elements and Interactions:} The core of
      ...
      pilot-jobs and WUs. After a WU is assigned to the manager it
...
associating a PD to a PS the data is actually moved to the physical location
...
the defined policies, affinities, available sowetatic \alnote{what
  does this word mean?} \jhanote{it means citatewos :)} and dynamic
...
\alnote{changed, maybe someone has a more fancy word?}  the SAGA API
and runtime environment, which is utilized for job management, data
transfer and distributed coordination. The TROY API maintains the SAGA
{\it Look \& Feel} and was designed to be similar to SAGA in
appearance \jhanote{isn't appearance covered in look-and-feel?}
\alnote{Look \& Feel incl. the appearance, but also some
  non-functional classes such as context} \jhanote{can we then use
  descriptive/long-form: ``appearance and non-functional classes''}
...
environment and different PJ implementations referred to as
BigJob. The TROY API is based on the artifacts defined by the P* model
%\jhanote{Is framework here P* ?}\alnote{yes}
...
\begin{figure*}[t]
    ...
    \caption{\textbf{BigData Architecture:} The PD-Manager exposes the
      TROY Pilot-Data API.  Application can create group of files and
      assign files to storage. The PD manager tracks files and
      physical locations in the data catalog. The data scheduler
      optimizes the co-locations of files and compute (in conjunction
      with BJ). The transfer manager initiates and monitors the actual
      data movements. \upp\upp}
...
agent. The Advert Service exposes a shared data space that can be accessed by
the manager and agent, which follow a push/pull communication pattern, i.\,e.\
the manager pushes a SU to the Adverts Service while the agents periodically
pull for new data. Results and state updates are pushed back from the agent to
the manager in the same way. The binding between WU and SU \& PJ takes places at
submission (early binding). Currently, only a simple binding scheme exists,
i.\,e.\ each WU (a task) is mapped to a single SU (a so called sub-job). For
...
multiple BJs. Each BigJob manages a particular resources. An extensible
...
\alnote{SWIFT eval: no standard resource abstraction (SAGA),
  proprietary language (not Python), TODO: check how coasters work! 1
  coaster == 1 Condor-G job?}
...
\jhanote{Can we add some structure to these *other* PJ.. this will be
  ...
  paper by virtue of validation of the P*-model} \alnote{which are the
  minimal P* elements and characteristics we should discuss here?}
\jhanote{Andre L: Is this still a valid/live comment/question? If not,
  should we close}
...
the LONI~\cite{loni}  \jhanote{reference} resource Oliver and a total of 16 cores
...
spawning of one worker agent per WU that must be executed in
parallel. \jhanote{What is executed in parallel to what?} \smnote{In the experiments we run all the BFAST 
WUs (8) concurrently. To run these 8 WUs concurrently with BJ-DIANE implementation we have to start 8 different 
DIANE agents since a DIANE agent can run only one task at a time}The TROY API
