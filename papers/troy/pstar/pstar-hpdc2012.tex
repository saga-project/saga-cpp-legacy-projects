...
In this section we present a Pilot-API that exposes the P* Model  
(\S\ref{sec:pilot-model}) to an application.
...
The Pilot-API is consistent with the SAGA~\cite{saga_url,ogf-gfd-90} API. 
...
independent of the diverse and dynamic grid environments. The Pilot-API
...
semantics and syntax. A further design consideration of the Pilot-API was a
...
Figure~\ref{fig:figures_troy_flow} shows the interactions between the P*
elements. The Pilot-API decouples workload management and resource scheduling by
exposing two separate services: The \texttt{PilotJobService} (PJS) and
\texttt{WorkUnitService} (WUS). The PJS serves as a factory for instantiating
