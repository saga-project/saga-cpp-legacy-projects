...
interoperability as a consequence.  The situation is reminiscent of the
...
\noindent$\bullet$ \textbf{\computeunit \ (\cu):} A \cu \ encapsulates a self-contained
...
\noindent$\bullet$ \textbf{Scheduling Unit (SU):} SUs are the units of scheduling
...
There exist two ways toward system federation:\alnote{would interoperability 
more suitable?} (A) deep integration of
...
Pilot-API) is a semantically weaker approach than (A), it does
...
\up
\subsection{Understanding Coordination in PJ Frameworks}
...
\up
\subsection{Characterizing PJ Frameworks on Production
Infrastructure}
...
infrastructures EGI and OSG (configurations (B.iii) and (B.iv)), these file are
...
\subsection{Understanding PJ Framework Interoperability\up}
\label{sec:experiment-interop}

We define three interoperability configurations: (C.i) SAGA-based
interoperability, i.\,e.\ the usage of BJ in conjunction with different SAGA
adaptors and infrastructures and (C.ii) PJ framework interoperability, i.\,e.\
the usage of different PJ frameworks via the Pilot-API. For C.i we run BigJob
concurrently on FutureGrid::India and XSEDE::Kraken. For B.ii we concurrently
run BigJob and DIANE on FutureGrid::India and EGI as well as Condor and BigJob
on OSG and XSEDE::QueenBee. For all scenarios, we run the same BFAST 
application described in \S\ref{sec:fg-xsede-osg-egi} with 64\,\cus on each
...
Finally, scenario (ii) (right \jhanote{should this be the middle bar
  or the right bar?} bar in
figure~\ref{fig:perf_interop_128-bfast-interop}) demonstrates that two
PJ frameworks can be utilized concurrently using the Pilot-API. Files
are pre-staged for FutureGrid; they need to be staged for the
DIANE/EGI \cus. The performance in this scenario is significantly
better than in the DIANE only case for 128\,\cus (see
...
communication is conducted via a Redis instance deployed on FG.  In
scenario (ii) the BJ manager is deployed on EGI; thus, for each \cu
several cross-atlantic roundtrips are necessary.  \jhanote{Don't
  discuss third bar?}
...
\section{P* as a Model for Pilot-Data\up}
...
\section{Discussion and Future Work \up}
