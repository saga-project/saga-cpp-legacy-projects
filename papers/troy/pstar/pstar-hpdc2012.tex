...
workloads} -- in this case a genome sequencing application -- on
...
multiple production infrastructures using the Pilot-API -- see
...
%
Both the tuple space and the zmq client-server \cc sub-systems can be
...
FutureGrid~\cite{fg}. To evaluate overall performance, throughput ans
...
genome sequencing application -- using three different PJ frameworks
...
PFlop Cray XT5 / 9,408 nodes / 112,896 cores / Torque); FutureGrid:
India (Linux Cluster / 108 nodes / 864 cores / PBS); \note{EGI
details}; OSG: Condor pool (via the \textit{Engage} VO, Glide-In WMS,
...
The investigated workload consts of of 128 \cus. Each \cu executes a
...
configurations: (i) BigJob/XSEDE, (ii) BigJob/FutureGrid, (iii)
BigJob/OSG, (iv) DIANE/EGI.  The Pilot-API provides a unified way to
access the different native PJ capabilities for all three PJ
frameworks, i.\,e.\ BigJob, DIANE and Condor.  \amnote{what are 'read
files'?}\amnote{ I know this has been discussed, but anyway: we say
that the pilot API allows access to BJ, Diance and Condor, but the
isted configurations do not mention Condor.  This needs to be
...
SAGA-Torque adaptor to access Kraken/XSEDE. For OSG, BJ utilizes the
SAGA-Condor adaptor that interfaces directly to OSG's dynamic
GlideIn-WMS resource pool.  Further, we utilize DIANE on EGI.
\msnote{more on EGI later ...} On the XSEDE, Kraken is used with up to
512 cores concurrently.\amnote{why is the corecount mentioned for
Kraken, and not the others?}

Each BFAST \cu requires 1 core; In each scenario, 128 \cus are defined
and submitted to the Compute Unit Service. Depending on the resource,
a different number of cores can be reserved to each BFAST \cu, e.\,g.\
on Kraken 4 cores are reserved for each \cu. Each \cu is associated
with a set of input files which is pre-staged in scenario (i) and
(ii). For the HTC infrastructures: EGI and OSG (scenario (iii) and
...
Kraken 4 cores are reserved for each \cu"???}\amnote{(i), (ii) etc.
are not defined above -- needs fixing.}
...
include both pilot-external, i.\,e.\ waiting times at the RM system,
as well as pilot-internal waiting times.
...
are shared by all jobs running on these machines -- the performance of
BFAST thus particularly degrades on larger machines, such as Kraken,
where a potentially large number of jobs access the filesystem
concurrently (the runtime on Kraken is about 30\,\% slower than on
India).  On EGI and OSG, the BFAST \cu performs best: on OSG about two
times faster than on Kraken. This can mainly be attributed to the use
of local disc storage on EGI and OSG resources.
