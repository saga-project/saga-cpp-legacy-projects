...
Even where meaningful abstractions exist, the challenges of providing
these abstractions in an extensible, reliable and scalable manner are
formiddable.  The lack of appropriate implementations has in fact
resulted in “one-off” solutions that address challenges in a highly
customized and non-reusable manner, and are, in general, expensive to
develop and manage in terms of the time and effort involved.  The lack
of readily available, reusable and extensible implementations of
abstractions that can support multiple applications and usable on
different infrastructure further reinforces the fragmentation....
...
successful abstractions in distributed computing that have been widely
used. %are those provided by pilot-jobs.
\emph{Pilot-jobs (PJ)} provide an effective and simple approach for
decoupling workload management and resource assignment/scheduling.
Pilot-jobs enable this decoupling typically in user-space without the
policy-level complexity of implementing advanced reservations.  The
pilot-job abstraction is also a promising route to address additional,
as well as specific requirements of distributed scientific
...
$T>0$. Distributed CI almost by definition is comprised of a set of
resources that is fluctuating -- growing, shrinking, changing in load
and capability; this is in contrast to a static resource utilization
model traditionally a characteristic of parallel and cluster
computing.  The ability to utilize a dynamic resource pool is thus an
important attribute of any application that needs to utilize
distributed CI efficiently.  However, even for traditional
...
execution is widespread for computational science
applications. ... Distributed applications that have been able to
break the coupling between workload management and resource
