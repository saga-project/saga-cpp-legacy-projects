...
SAGA-Torque adaptor to access Kraken/XSEDE. On OSG, we use SAGA
and the SAGA-Condor adaptor to interface directly with OSG's dynamic 
Glide-in-WMS resource pool. Further, we utilize DIANE on EGI. %\msnote{more on EGI later ...} 
...
\begin{figure}[t]
 ...
 \caption{\textbf{PJ Framework Performance on XSEDE, FutureGrid, EGI and 
  ...
  repeated at least 3 times. Both EGI and OSG require the staging of all input 
  files. Also, queuing times on EGI are longer than on other infrastructures. \up\up}
...
framewors via a uniform API, with multiple PJ frameworks over
distributed production cyberinfrastructure; it also enables the
concurrent use of multiple PJ frameworks, thus providing
