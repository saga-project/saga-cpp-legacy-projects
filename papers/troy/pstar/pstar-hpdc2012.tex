...
We thus introduce the Pilot-API, as abstract interface to
systems implementing the P* Model.
...
Pilot-API:
%
Workflow capabilities and engines are typically tied to specific tools
and infrastructure (e.\,g.\ DAGMan-Condor) and require the adaption of
the application to the workflow engine as opposed to the other way
around. Even though many workflow systems exist (with significant
duplicated effort), they provide limited means for extensibility and
interoperability.  

An analysis of the situation points to many reasons; we are not naive
enough to suggest that it is only due a single factor, but we assert
that an important contributing fact is the lack of an early community
agreement on the right abstractions upon which to construct workflows.
Had those interface abstractions been available, it seems very likely
that many/most workflow engines would have utilized them (or parts
thereof), instead of proprietary solutions.  That would not
immediately allow workflow implementations to interoperate, but would
make the semantic mapping between them significantly simpler, thus
supporting the very notion of interoperation.

The impact of missing interface abstractions on the workflow world can
be seen through the consequences of their absence: in spite of
...
else, providing post-facto justification of its importance\footnote{To
be fair: we are not sure if a generic model and/or a  generic workflow
API are achievable {\it on a useful level} -- we think, nevertherless,
that our discussion is valid.}
...
applications, the Pilot-API expose/provides effective abstractions
that can hide the environmental complexity, supplement the
incompleteness and lack-of-extensibility of many tools used, and
...
to work on a multiple systems: defining the API as the 'smallest
common denominator' is often too simplifying and misses large numbers
of 'edge' use cases; defining the API as the 'greatest common factor'
clutters the API with non-portable semantics, each only used by few
use cases, thus making the API too complex~\cite{leaky_abstractions}.  The
...
frameworks and provides a common access layer to them via a consistent API;
it also enables the concurrent use of multiple PJ frameworks, 
...
% .  Building on the P* Model, the Pilot-API's design
% enables the easy exchange of PJ implementations, and also 


% The Pilot-API functions as common access layer for different PJ
% frameworks, thus
...
unified implementation of P* (e.g., BigData analogous to BigJob) in
