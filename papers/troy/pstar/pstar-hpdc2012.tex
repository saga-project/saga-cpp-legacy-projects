...
BigJob (BJ)~\cite{bigjob_web,saga_bigjob_condor_cloud} is a SAGA-based
pilot-job implementation. BJ supports a wide range of application
types, and is usable over a broad range of infrastructures, i.\,e.\ it
is general-purpose and extensible. In addition, there are specific BJ
flavors for cloud resources such as Amazon EC2 and Microsoft Azure
that are capable of managing set of VMs, as well as a BJ with a
Condor-G/Glide-in based backend. \alnote{these flavors are primarily
  defined by the SAGA adaptor mechanism.}

\jhanote{we'll have to unfortunately descripe saga briefly in order to
  make saga-based clear/obvious}
...
architecture of BJ \jhanote{and its mapping to P*?}. BJ utilizes a M/W
coordination model: The BigJob-Manager is responsible for the
orchestration of pilots, for the binding of \cus \note{OW says:
  binding to what?}  and for the scheduling of SUs. For submission of
the pilots, BigJob relies on the SAGA Job API, and thus can be used in
conjunction with different SAGA adaptors, e.\,g.\ the Globus, the PBS,
the Condor(-G) and the Amazon Web Service adaptor.  \note{OW says:
  Better: relies on the job submission capabilites of SAGA, which
  include Globus, PBS, Condor, ...}  Each pilot initializes a so
called BJ-agent. The agent is responsible for gathering local
information and for executing tasks (SUs) on its local resource. The
SAGA Advert Service is used for communication between manager and
agent. The Advert Service (AS) exposes a shared data space that can be
accessed by manager and agent, which use the AS to realize a push/pull
...
supports alternative c\&c systems, e.\,g.\ Redis~\cite{redis} and
ZeroMQ~\cite{zmq}.
...
application -- using different PJ frameworks: BigJob, DIANE, Condor-G
and infrastructures: XSEDE, FutureGrid, EGI and OSG.  For each of the
frameworks we provide an implementation of the Pilot-API. For the
experiments we rely both on the native and Pilot-API. \jhanote{The
...
matching process, which is used to find potential sequence alignments. The scenario requires about 6\,GB input data: 1.7\,GB for the reference genome and index files and 5.4 GB read files (32 files a 1.68\,GB). We run
...
Finally scenario (iv) \jhanote{should this scenario number be
  different?} demonstrates that two PJ frameworks can be utilized
