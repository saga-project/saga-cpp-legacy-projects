...
\begin{abstract} 
 ...
 % Distributed cyberinfrastructures (CI) and applications require the
  ...
  %\alnote{the following sentence is just a fragment}
...
% Also, maybe interoperability isn't what is desired, but reusability
% and or ease of customization.  Talk about analogy with workflow
...
% \pmnote{ analyzation of tools present, but validation, i dont think we did, since we dont  have any experiments} 
...
combined and aggregated to optimize throughput and response times. Commonly,
...
%   definitely not the case for Condor and not really for BigJob. DIANE
...
FutureGrid (FG), it is generally also usable in other environments. This
...
resource. \cus are referred to as sub-jobs, and internally map
1:1 to \sus.
...
% Also, we will support  as well as clouds and in hybrid grid/cloud 
% environments~\cite{bigjob_cloudcom10,saga_bigjob_condor_cloud}. 
...
used. Instead, we focus on demonstrating interoperability via the
...
XSEDE~\cite{xsede}, FutureGrid (FG)~\cite{fg}, EGI~\cite{egi} and
...
possibility of network congestion on single links. As indication,
typical runs of 128 CUs on OSG end up being scheduled on 10+ sites
...
of the Pilot-API.  All scenarios run the same BFAST
application as in \S\ref{sec:fg-xsede-osg-egi}: 64\,\cus
per infrastructure, totaling 128\,\cus.
...
% does this mean for transport}\alnote{refined we general ly talk about
...
% Similarly, BigJob could be used with
...
% \alnote{We now include Venus-C in 2 places on rather high level. I
% think mention it once would be sufficient. Should we keep the mention of
% Venus-C to the discussion section?}
...
%provides a common access layer to different PJ frameworks via a uniform
%API; 
