...
occur at several levels, e.\,g.\ both inside and outside the PJ framework.
...
\jhanote{everything after 4th row - Candidate for compaction}\upp
}\up\up}
...
The Pilot-API has been deployed to support production-scale science on
production infrastructure~\cite{bigjob_web} (and as demonstrated by
the emphasis on production infrastructure in our experimentation).  In
...
multiple production infrastructures. However, attention to several
deployment issues is required to realize the full power of the
P*/Pilot-API.  For example, in spite of a common Pilot-API, each PJ
framework has a rather different usage modality; this is reflective of
the fact that typically, PJ frameworks ``evolve in'' and are ``native
to'' specific infrastructure, e.g., Condor-G/Glide-in is the native PJ
framework of the OSG, and its use is heavily coupled with
infrastructure specific to OSG, such as WMS.  Additionally, our
experience with fragile grid middleware and infrastructures has taught
us to appreciate the significance of the ease of deployment and the
need for robustness of tools and software on heterogeneous distributed
infrastructures.
