...
In order to provide a means for end users to utilize a scientific application with scalable heterogeneous distributed computing resources, we develop the Distributed Adaptive Runtime Environment (DARE) framework\cite{dareurl}.  As shown in Fig.~\ref{fig:dare-arch}, the framework comprises the access layer, the services layer, and the resource/provisioning layer.  Particularly, the core parts of the services layer and the resource/provisioning layer are powered by SAGA and SAGA-based Pilot-Job abstraction, SAGA-BigJob\cite{saga-ccgrid10,saga-royalsoc,saga-web,jha2009developing,ecmls10, ecmls11}.  The access layer is built upon the open source web application framework, pylons\cite{pylonsurl}.  The combination of the open source web application technology and the underlying application management system is the main design strategy behind the DARE framework for the development of a lightweight, extensible, full-fledged distributed computing science gateway quickly and effectively\cite{pylonsurl}. 
...
The architecture of DARE framework (see Fig.~\ref{fig:dare-arch}) assumes two-tier or three-tier installation scenarios that differ in the access layer such that the former does not need a remotely accessing client.  The core of the access layer is the web server, a pylons web application built with a Model-View-Controller architecture.  The lower application management system layers, composed of the application layer, the dare layer, and SAGA layer, are in fact plugged into the Controller, resulting in a very flexible and agile gateway architecture.  The middleware and resource layers are composed of the SAGA adaptors and back-end scalable distributed systems.  
...
\textit{Access Layer} The Access Layer contains many components mainly dealing with the interactions with a user.  Therefore, it implements a User interface (UI), authentication process, and database keeping information about users and jobs submitted by users.  In fact, pylons-based web server can be accessed by web pages or a remote client, but at this moment, web UI is the only mode for a user access.  Command Line Interface (CLI) is also supported when two-tier structure is preferred.  

The MVC architecture constituting pylons web application framework is very effective and advantageous in many ways in spite of its lightweight nature. For example, database management and connection are very useful to facilitate authentication steps and job/data creation and management.     

\textit{Services Layer}
This layer is composed of sub category layers such as the application layer, the DARE layer, and the SAGA layer.  The application layer is the only layer requiring a developer to build their own.  As a target application differs in an application usage mode along with the consideration of execution patterns closely associated with different distributed resource characteristics, it is a developer's responsibility for building this layer with their own scientific logics.  However, SAGA and SAGA-BigJob mitigate such efforts required for a developer with simple but powerful ways.  For example, by varying SAGA-BigJob configuration with a minimal effort, different execution usages of target computations suitable for pleasingly parallel, loosely coupled, and strongly coupled applications, are quickly implemented.  As presented in this work with four different gateways of four different life-science applications that vary in application usage modes as well as different parallelization strategies, once a developer figures out computational requirements for a target application, it is trivial to develop a optimized runtime environment for the target application executing in distributed resources in a short development cycle.   
...
Importantly, the data movement should be considered when the application layer is developed because the movement of data sets including input, output, temporary files, type of file will be changing from application, and as we have shown recently, in the case of data-intensive application, distributed date management is critical for successful gateway development.  At this moment, GridFTP is used as a primary protocol for data transfer, and Table~\ref{table:NGS-Distributed-file} summarizes transfer times that were associated with data analytics for DARE-NGS with a model system, human genome.
