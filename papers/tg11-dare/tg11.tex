...
\small
\begin{tabular}{|c|c|c|c|} 
  \hline Science  & Target  & Conventional Application  & Distributed Application 
  \\
  Domain & Application(s) & Usage Mode & Usage Mode \\ \hline \hline 
  
  Molecular   &  \texttt{NAMD} &  MPI-based  & Ensemble-based multiple  \\
   Dynamics  &  & single execution & (independent/loosely-coupled) execution \\ \hline
  RNA Folding  & \texttt{SFOLD}, & single node   & multiple tasks with \\
  Prediction   & \texttt{RNAFold} & serial execution &   distributed resources \\ \hline
  NGS data     &  \texttt{BFAST} & memory-intensive  & data-intensive\\ 
     analytics  &  &  single node execution   &  distributed resources \\ \hline
  Virtual Screening   & \texttt{Autodock} &  many tasks   & many tasks \\
  (Docking) &  &  with a cluster  & with multiple resources \\ \hline
