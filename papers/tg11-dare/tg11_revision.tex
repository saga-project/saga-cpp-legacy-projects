...
\begin{abstract}
  ...
  provided by the TeraGrid/XD, and in particular a very successful model for
  ...
  capacity of distributed cyberinfrastructure such as TeraGrid/XD,
...
dependent on high-throughput.  However the need of large-scale
distributed parallel executions for other areas of
...
production cyberinfrastructure, such as the TG, have not been
...
It is interesting to note~\footnote{Based upon shared
  ...
  Katz}: (i) Molecular Biosciences represented 25\% of TG (NU)
cycles used in Q1-2011.  (ii) Based on gateway usage in first quarter
of 20011, 3-4 of $\approx$ 20 TG gateways currently are
biological/life science, which in turn account for about $\approx$ 35\%
...
clearly imply that although a significant fraction of TG cycles
...
gateways to access TG resources.  Given the overall success and
...
Additionally, TG Science gateways have witnessed
...
resources (see the table~\ref{table:TG-sg}).  A primary reason is the additional, not insignificant
...
supports the distributed application design objectives categorized by
the term IDEAS\cite{ideas}.  In this paper we
establish how these capabilities can be generalized and provided via
...
TG/XD. We use DARE as the underlying component for four
...
Size  (GB)    	             &127&	9	&3.81\\
 \hline                       
LW to QB (s)   & 1123 to 1814 & 79 to 128 & 33 to 54 \\
  \hline
LW to Ranger(s) & 5521 to  21166  & 375 to 1500  & 158 to 635\\
   \hline
   
\end{tabular}
\caption{File transfer time with GridFTP between a local workstation (LW), QueenBee (QB), and Ranger with the case of Human Genome index files and exome data. The high speed between LW (cyder located at LSU) to QB was because of the spatial locality of the machines and LONI network which runs through LSU.   }
...
%In Fig.~\ref{fig:dare-rfold-result}, the result compares two different configurations of BigJob size. 
