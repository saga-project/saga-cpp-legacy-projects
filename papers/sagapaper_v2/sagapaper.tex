...
applications.  We posit, however, that a well-defined, stable general
purpose API is one critical element of the solution, as that exposes
the basic distributed capabilities to serve as building blocks for
...
{\it The objective of SAGA, the 'Simple API for Grid Applications is
  ...
  cyberinfrastructure ecosystem.} SAGA can provide effective
abstractions that can hide the environmental complexity, supplement
the incompleteness and lack-of-extensibility of many tools used whilst
promoting interoperability as first-class design objective. Thus we
argue that SAGA is a critical component of the national DCI such as
XSEDE.

While the API's name suggests its string ties to Grid based DCIs, it
is in fact a general purpose API for distributed -- which is
historically rooted in the Grid community.
...
\section{SAGA: Architecture and Implementation} \todo{Ole}
