...
\label{sec:api}
 ...
 section~\ref{sec:designobjectives}.  This section describes the the
 general structure and scope of the API, and demonstrates how it
 facilitates the implementation of distributed applications,
 ...
 and distributed operations semantics.
 ...
 \subsection{General Distributed Programming Practices}
  ...
  patterns.  Obviously, the SAGA API is expected to build upon that,
  or at least to nicely interact with those practices and patterns.
  Among those patterns are

    - async ops\\
    - event based programming\\
    - multithreading\\
    - separation of semantics and communication (transport versus
      protocol layer)\\

  In order to separate the API design considerations in respect to
  these patterns from the actual semantic scope of the API, the SAGA
  API specification consists of two parts.  First, there is a so
  ...
  asynchronous notifications are rendered, how error conditions from
  different backends are uniformly managed, how the SAGA API behaves
  in multi-threaded environments, etc.

 
 \subsection{Semantic Scope of the SAGA API}
  ...
  \begin{table}[ht!]
   \begin{footnotesize}
   \B{Semantic Scope of the SAGA API}\\
     \begin{tabular}{@{}p{2.00cm} p{3.80cm} p{9.00cm}@{}}
      
      \toprule
      {\bf Package} & {\bf Related APIs} & {\bf Scope} \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
\section{Conceptual Architecture} \todo{AM (layer 1), Ole (layer 2, 3)}
