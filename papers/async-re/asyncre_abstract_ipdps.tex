...
The total simulation time can be divided into three components - (i) the actual runtime of NAMD at each generation, (ii) the time to make the exchanges at each generation and (iii) the time required to launch the NAMD jobs after each exchange step. It is important to note here that we are excluding the start-up time for launching the BigJobs and the queue-wait time.
...
The total simulation time can be divided into three components - (i) the actual runtime of NAMD, (ii) the time required to make the exchanges and (iii) the time required to launch the NAMD jobs after the each exchange. Here too, we exclude the start-up time for launching the BigJobs and the queue-wait time. It is important to note here that while in the synchronous RE, everything occurs in a serial fashion, in the asynchronous RE, the NAMD runtime, the time to make the exchanges and the time to restart the NAMD jobs all overlap.
...
\includegraphics[width=1\textwidth]{figures/synchronous.pdf}
%\label{fig:async:b}
