...
In asynchronous (centralized) RE, it can be seen from the table1\ref{}
that $T_W$ is still the biggest component. $T_X$ also has slightly
increased. But since the RE Manager makes the exchanges serially, even
without the large waiting times seen in the synchronous RE, we see an
increase in the time to completion. Again, by changing/substituting
$N_X$ in the eqn2.2\ref{} along with values for asynchronous
(centralized) RE mentioned in the table1\ref{} we can arrive at the
results seen in the graph1\ref{}.
