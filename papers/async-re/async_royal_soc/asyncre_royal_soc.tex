...
\begin{table}
	...
	\hline
	\hline
	$T_{W}$        &2.6\,s &1.1\,s &2.0\,s\\
    % \hline
    % \hspace{2mm}$T_{w}$ &1.8\,s &NA (included in $T_r$)&0.73\,s\\ 
    % \hline
    % \hspace{2mm}$T_{r}$ &0.8 s&1.02 s&1.3\,s\\
	\hline\hline
	$T_{X}$        &0.6\,s &0.8\,s &8.68\,s\\
	\hline
	\hspace{2mm}$T_{f}$        &0\,s   &0.2\,s &7.04\,s\\
	\hline
	\hspace{2mm}$T_{ex}$       &0.4\,s &0.4\,s &0.64\,s\\
	\hline
    \hspace{2mm}$T_{coord}$    &0.2\,s &0.2\,s    &NA (included in $T_W$)\\
	\hline
	\hline
	$\mathbf{T_{C}}$        &\textbf{990\,s} &\textbf{798\,s}    &\textbf{653\,s}\\
	\hline
    \end{tabular}
    \caption{Average values of terms in Eqn~\ref{eq:totaltime} for the three RE algorithms. $T_{MD}$=time replica takes to complete 500 timesteps; $T_W$=time waiting for other replicas/time waiting to be restarted; $T_X$=time to make a pairwise exchange; $T_f$=time to find/lock a partner; $T_{ex}$=time to write/transfer files; $T_{coord}$=updating states after exchange; $T_C$=total time to completion. }
        \alnote{Should we generally add $T_{coord}$ to $T_{w}$ to avoid confusion?} \athotanote{we can do that.. should I?}
        \alnote{Can we put in 0 s instead of NA?} \athotanote{yes..}
...
of synchronous and asynchronous RE, we are not concerned by the
...
\alnote{We are very imprecise when using units in equations. Also I
  ...
  easier to parse)} \jhanote{Yes, I agree on both accounts} \athotanote{s or sec?} 
...
post-processing of each replica run, i.\,e.\ updating the state,
marking nodes free, the stage-out of the output file, is done sequentially. 
The longer of both times -- in this case the post-processing time -- 
determines the overall time spent waiting for other replicas. 
\alnote{I added 0.92\,s bigjob postproc + 0.4\,s for stageout together} \athotanote{we are losing around 15 seconds with the approximations. so, i'm making 1.3=1.4}
On average, it takes 1.4\,s to process a replica that has completed 
running. In an ensemble of 32 replicas the delay between the termination 
of the first and last replica adds up to 44.8\,s, i.\,e.\ for one replica 
pair (there are in total 16 pairs in a 32 replica ensemble): $T_{W} =
2.8$\,s.
...
\alnote{\textbf{included stage-out time into $T_W$ above - just so that you guys can
follow my steps (comment and paragraph below can be deleted):}
...
files. That costs 0.4 s per replica and 0.8 s for a
pair. Thus, $T_W = 1.84 + 0.8=2.64$\,s.}
...
%   restarts}
On average $T_{coord}$ amounts to $0.1$\,s per replica, i.\,e.\  $0.2$\,s per
replica pair. In total $T_{X}$ is: $0+0.4+0.2=0.6$ s.
...
\begin{eqnarray}
  T=  {1 \over p} \times {[ {(71\times {128\over 16}})+ (0.6 + 2.8)\times 128]} = {1 \over p} \times 1003.2 \,s
\label{eq:sync}
\end{eqnarray}
...
\jhanote{Why is seconds within math mode sometime, and outside math
  ...
  invalidate the model completely}% \athotanote{yes. i saw this. in the  other two cases, substituting $N_X$ in the equation satisfies the  all the experiments in section 5. but in decentralized, $T_f$  changes with $N_R$. as to why its 603 and not closer to 640: i think  its something to do the time to find a replica ($T_f$).}
...
search through $N_R/2$ replicas before it finds a partner. 
The exchanges are made by the RE-Manager and only one exchange takes 
place at a time ($\eta$ is 1). \alnote{Not sure whether this view
helps us here. I am aware of the sequential part at the central master,
but couldn't that be considered quasi-parallel? Also, the same limitation
would apply to the synchronous case. An option would be to introduce
a $\eta_{MD}$ and $\eta_{coord}$ in equation~\ref{eq:totaltime}: one for the MD part and
one for the coordination part.}
% \alnote{I 
...
An advert query for a replica state takes 0.01\,s. In the 32 replica
scenario the RE-Manager must request in average 16 other replica states
before it finds a partner; thus, the $T_f$ is in total $0.2$\,s. Both
...
% $({0.2})\times 2=0.4\,s$. $T_{coord}$ is the time it takes to
...
waiting involved, which is mainly caused by the single-threaded 
BigJob implementation, which leads to some delays at the agent.
In particular, we observed a delay of new sub-jobs in cases where 
the BigJob agent was busy processing other sub-jobs after their 
termination. In this scenario the post-processing of a sub-job 
requires with 1.1\,s about 0.2\,s more than in the synchronous
...
\athotanote{well...}
In the worst case, the first replica pair that is restarted has
to wait for the processing of the other remaining 30 replicas: 
$1.1\,s \times 30 = 33\,s$. This corresponds to $T_{W}$ of 1.1 per
replica.\alnote{I still have some issues with that formula i) we don't model
anything related to the central master ii) I am not sure whether the per replica
view is correct. What is a reasonable $\eta$ (resp. $\eta_{MD}$ and $\eta_{coord}$) 
for async RE?}
...
\alnote{\textbf{I propose to take the rest of this paragraph out and not go into the details of $T_r$ and $T_w$:} It should be noted that the timelines of the RE-Manager and the BigJob 
agent run concurrently. Also, the time RE-Manager
waits for the next replica to  become available, $T_w$ is included in $T_r$. This is because we already included the time each replica waits at the BigJob agent before it is restarted ($T_r$). This would be the same time a replica waits for the next replica to become available. Thus, $T_W$ is 1.1 s. }
...
\begin{eqnarray}
T=  {1 \over p} \times {[ {(71\times {128\over 16}}) + (0.8 + 1.1)\times 128]} = {1 \over p} \times 811.2~s
\label{eq:cent}
\end{eqnarray}
...
replica. The replica is restarted after every exchange. Therefore, $T_r$, which is recurring is 0.3 seconds. \alnote{Why does it take longer than in case I/II? 
\textbf{TODO Abhinav:} separate $T_{coord}$ out of $T_r$} \athotanote{done}
...
s. It then posts both these values to the advert server. These are the associated costs: $0.1+0.2\times2+0.1\times2=0.7$ s.  Thus,
$T_W$ is 0.3 + 0.7=2 s. %\alnote{Where is the 0.7 sec coming from?} \athotanote{fixed. is it okay?}


On average the replica-agent scans the list of replicas twice
before it finds a potential partner for an exchange. \alnote{Yesterday, it
scanned through 15 replicas; in the decent case it scans 16 replica; now
it scans 64 replicas. Why is that?} \athotanote{discussed in call.}
% \alnote{Why 15?
...
Each query is 0.1 s, thus $T_f$ is $0.1\times64=6.4$ s. The process of
exchanging the states and temperatures via the advert server and writing
a new configuration file takes, $T_{ex} = 0.1\times 4 +0.2 = 0.6$
s (And even in the other scenario, where the replica in question is 
...
update the state in the advert server, which is 0.1 s.\alnote{In the other cases the state must be updated in the
...
the decentralised version, each query is shown as 0.1 s. This is due to the fact that each replica-agent stores its replica's data (state, energy, temperature) in a unique directory and changing the directory after each query causes the action to take longer.
...
Substituting the above values in equation~\ref{eq:totaltime}, we get:
\begin{eqnarray}
T=  {1 \over p} \times {{(71\times {128\over 16}}) + {(6.8+2)\times 128\over 16}} = {1 \over p} \times 638~s
\label{eq:decent}
\end{eqnarray}
...
{\it Experiments: } We repeated each of the synchronous and
asynchronous experiments with 4, 8, 16, 32, 64, 128 and 256 replicas
making 16, 32, 64, 128, 256, 512 and 1024 exchanges, respectively, on
one machine. Each replica is configured to run 500 time-steps and is
allocated 16 processors. The experiments consisting up to 128 replicas
are done on {\it QueenBee}, while the experiments with 256 replicas
are done on \emph{Ranger} (this is because \emph{QueenBee} only
allocates a maximum of 2048 processors per job request). But we have
normalised the data from {\it Ranger} to compare with results from
\emph{QueenBee}.  It should be noted that the ratio between the number
of replicas and attempted exchanges is kept constant, so that it
allows for comparison between each of these experiments.
...
%(i) total coordination time taken (at every generation)
% (ii) hetereogeneity due to different nodes of a machine. We do not
% have any quantitative information. Just mention it as a factor; 
% do not discusss.

% We see the biggest increase in $T_C$ in the synchronous RE due to
% various reasons: as described in section~\ref{sec:impl_sync_re},
%$T_{w}$, i.\,e.\ 
...
finish. As can be seen from Table~\ref{}, this is a major component of
the $T_C$.  As $N_R$ increases, the number of exchanges at a given
exchange step increases; consequently the total coordination time at
each exchange step increases. \jhanote{Now QUANTIFY THIS AND MAKE
CONNECTION WITH THE DATA IN FIG ..}

We have explained in section 4.1 the total time for 32 replicas. If $N_R$ is 64 and $N_X$ is 256, from equation4.1\ref{}, the new coordination time is $(0.62+2.64) \times 256$. That is the change we see from 32 replicas to 64 replicas in the graph1\ref{}. We have verified this for different number of replicas and found it to be consistent.
...
replicas.  If $N_R$ is 64 and $N_X$ is 256, from equation4.1\ref{},
...
$T_f$, has two components: a search and reverify stage, which 
must both be successful before an exchange is carried out.
...
% In this implementation, the reason for increasing the $T_C$ is that
...
s. % Only $T_f$ changes with the number of replicas.
