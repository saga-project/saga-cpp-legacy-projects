...
manner when the number of replicas in the ensemble is high, so as to avoid having all the replicas starting at the first
replica, which causes contention and failed exchanges. But for for smaller number of replicas, a simple search over all the replicas also works. If a replica
...
\begin{table}
	...
	\hline
	\hspace{2mm}$T_{w}$ &1.84\,seconds &NA (included in $T_f$)&0.73\,second\\ 
	\hline
...
waits for the next replica to  become available, $T_w$ is included in $T_f$.
...
In this case too, we implement a random search of replicas by each replica agent when the total number of replicas in the ensemble is 128 or more, which reduces contention. It should be noted here that in the centralised version of
