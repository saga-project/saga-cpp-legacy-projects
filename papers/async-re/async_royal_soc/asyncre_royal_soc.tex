...
As explained in \S~\ref{sec:repex-approach}
...
the agent; \alnote{which is also not really the case since sub-jobs
...
determines the overall time spent waiting ($T_s$) for other replicas.
In this case the post-processing time is the larger, and on average,
it takes $1.4\,s$"} thus, $T_r$ is 0 for synchronous RE.
...
before exchanges, i.e., the former has one, the latter don't. However,
the lack of a synchronisation barrier leads to a more involved
implementation to dynamically pair replicas. So although, $T_s = 0$
for asynchronous formulations, this comes at the cost of a higher
$T_f$.
...
is the typical time to set/get a value to/from the advert server).\alnote{Should we argue 
with $\frac{N_R}{2}$*4 (in worst case) as previously described or is
this too complex?}
...
As in the decentralised case, the centralised, asynchronous RE algorithm 
does not require synchronization between {\it all} replicas in order to 
transition a replica-pair from \texttt{running} to \texttt{done} state 
and $T_s = 0$ by definition. Using a centralised implementation the time
to find an exchange partner ($T_f$) can be reduced; however, this comes
at a tradeoff that there is some contention at the master. Also, we observed
some delays at the BigJob agents during the startup of the replicas sub-jobs mainly
due to the fact that the BigJob agent is single-threaded and thus, is busy
...
For 32 replicas, that would be 35.2 seconds. Therefore, each replica pair 
waits $35.2/16=2.2$ s at BigJob agent before being restarted. But, 
we observed that - as the experiment progresses - the BigJob agent 
would be processing smaller groups of replicas and starting smaller 
groups of replicas. On average replica pairs after an exchange 
actually wait 2.2/2=1.1 s to be restarted by the BigJob agent. 
\alnote{Shouldn't we use the average number of sub-jobs that
a new job has to wait for as an argument?}
Therefore, the value of $T_W$ is the same as $T_r$.  
\alnote{the last sentence can be
...
one exchange takes place at a time. \jhanote{Abhinav: Does not explain
  ...
  explanation or just refer to the result: 0.2\,s?} \athotanote{explained} An advert query for a replica
...
is in total $0.1\times16=0.2\,s$. Both $T_{ex}$ and $T_{mgmt}$ are same as in the
...
running the scale-up experiments mentioned. \alnote{Some 
repetition to intro of sec 5} The ratio of the number
...
% We see the biggest increase in $T$ in the synchronous RE due to
...
coordination time is $(0.6+2.8) \times 256 = 870.4$.  The new coordination
time is consistent with the difference in $T$ for 32 and 64 replicas
in the Figure~\ref{fig:scaleup}. \alnote{Figure~\ref{fig:scaleup} does 
not show solely the coordination component} A similar analysis was performed for
different replica and exchange counts and the values obtained are in
agreement with the empirical data in Figure~\ref{fig:scaleup}.
...
exchanges serially. Therefore, we still see  an increase in the time to
completion. Again, using Equation~\ref{eq:cent} for 32 replicas as a basis: 
Increasing $N_R$ to 64 and $N_X$ to 256 leads to a new coordination time of 
$(0.8+1.1) \times 256 = 486.4\,s$.\alnote{How large is the increase in experiment?} 
That is the change we see from 32 replicas 
to 64 replicas in Figure~\ref{fig:scaleup}. We have verified this for 
...
For the random access case,\alnote{I think in sec 4 we only talk about the
random case now} we find $T_f$ is $2 \times N_R \times
0.1$ (where $0.1$ is the typical time to set/get a value to/from the
advert server).  For example, in Equation~\ref{eq:decent} we see that for 64 replicas, $T_f$ is
...
\alnote{Next two sentences: How much does this really impact the decentralised case?
Also, we defined $T_s$ as 0}
...
\begin{figure}%
\centering
\subfigure[8 replicas]{\includegraphics[width=0.8\textwidth]{../data/cent_scaleout.pdf}}\qquad
...
scale-up analysis.}
{\it Results:} The results obtained are shown in Figure~\ref{fig:scaleout}. 
The time-to-completion $T$ increases moderately or remains constant with  
higher number of machines, which indicates that the scale-out
behaviour is quite good. As discussed in section~\ref{sec:re_impl}, 
the asynchronous (centralised) RE algorithm does not involve any 
synchronization cost ($T_s$). Thus, the main contributor of this light
increase is $T_X$ and the higher costs for remote file transfers 
...
be for 1 BigJob. % Therefore, we can say that if ${N_R \over N_{BJ}
...
%LONI~\citep{LONI_web}.
Results shown in figures~\ref{fig:scaleup} and~\ref{fig:scaleout} indicate the good scale-up and scale-out properties on different production level infrastructure, such as, the Teragrid and LONI.
