...
$T_W$ is comprised of the synchronisation time spent by a replica waiting for other
replicas to complete running ($T_s$), the time spent waiting to be restarted
...
\begin{table}
	...
	\hline
    \hspace{2mm}$T_{coord}$    &0.2\,s &0.2\,s  &0.2\,s\\
	\hline
	\hline
...
the post-processing time -- determines the overall time spent waiting ($T_s$)
for other replicas.  On average, it takes $1.4\,s$ for the BigJob agent to process a
...
in a 32 replica ensemble): $T_{s} = 2.8\,s$. $T_r$ is 0 for synchronous RE, as the BigJob agent starts working only after the RE Manager completes all exchanges. There is now waiting time at the agent. Thus, $T_W$ is 2.8 s per replica pair.
