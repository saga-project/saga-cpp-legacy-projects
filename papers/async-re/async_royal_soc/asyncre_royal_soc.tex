...
\jhanote{this is an implementation detail. Abhinav: IIUC we restart in
  both cases -- successful and unsuccessful exchanges. Non?} In the
case of an unsuccessful exchange, the replicas are restarted without
exchanging the configuration.


In the synchronous (traditional) model of the RE algorithm, depending
upon on the number of replicas ${N_R}$, ${N_R \over 2}$ pairs of
replicas are created. When \emph{all} the replicas in the ensemble
reach a pre-determined state (e.g. the MD simulation completes a
pre-determined number of
steps), % a decision as to whether to exchange temperatures
%between previously
