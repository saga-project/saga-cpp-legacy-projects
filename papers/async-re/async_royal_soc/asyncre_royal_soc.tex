...
\author[Thota, Luckow, Jha]{
  Abhinav Thota$^{1,2}$, Andr\'e Luckow$^{1}$, Shantenu Jha$^{1,2,3}$,\\
...
step($T_{R}$). Steps (iii) to (vi) repeat after each exchange. We only include steps (i) 
and (ii) for completeness, which occur only at the beginning of the simulation.
They are not necessary for our calculations. We consider the start time as the time 
...
\begin{figure}[t]
      \centering
          ...
          \caption{\footnotesize \bf SAGA/BigJob Architecture
              }
      \label{fig:bigjob}
\end{figure}
...
\begin{figure}[t]
      \centering
          ...
          \caption{\footnotesize \bf Control Flow: Synchronous Replica Exchange
              }
      \label{fig:sync}
\end{figure}
