...
spent waiting for other replicas. On average, it takes 0.92\,seconds
...
%\athotanote{we can remove the sentence about 0.3/10 seconds, what do you think?}
...
take ($T_r$) 1.3 seconds. \alnote{Why does it take longer than in case I/II?} 
...
update the state in the advert server, which in this case is done while restarting the replicas by the replica-agent, is included in $T_r$. \alnote{In the other cases the state must be updated in the
  ...
  Thus, $T_{X}$ is $7.04+0.64= 8.68$ seconds.
...
In asynchronous (centralized) RE, since the RE Manager makes the exchanges serially, we see an
increase in the time to completion. Again, we explain in eqn 4.2 the total time for 32 replicas.  If $N_R$ is 64 and $N_X$ is 256, from equation4.1\ref{}, the new coordination time is $(0.78+1.02) \times 256$. That is the change we see from 32 replicas to 64 replicas in the graph1\ref{}. We have verified this for different number of replicas and found it to be consistent.
...
$T_X$ is very large when compared to other cases. But, making the
exchanges in a decentralised manner, instead of having the RE-Manager
make the exchanges serially makes all the difference. In this
implementation, the reason for increasing the $T_C$ is that $T_f$
increases with increasing $N_R$. The replicas contend with each other
to lock a partner for exchange, but this only gets worse with
increasing number of replicas. 

Typically, when searching serially, we observed that the replica-agent finds a partner after looping over the all the replicas between 2 and 2.5 times. That is, $T_f$ is $2 \times N_R \times 0.11$. And when searching randomly, it makes between 1 and 1.5 times $N_R$ calls to find a partner. Up to 32 replicas, we implemented a serial search and from 64 to 256 replicas we implemented a random search. For example, in eqn4.3 we see that for 32 replicas, $T_f$ is approximately $2 \times 32 \times 0.11 = 7.04$ seconds. For 64 replicas, $T_f$ is approximately $1.5 \times 64 \times 0.11 = 10.56$ seconds. Only $T_f$ changes with the number of replicas.
