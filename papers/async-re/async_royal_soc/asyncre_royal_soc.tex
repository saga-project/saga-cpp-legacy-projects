...
completion. Again, we explain in Equation~\ref{eq:cent} the total time for 32
replicas.  If $N_R$ is 64 and $N_X$ is 256, from equation~\ref{eq:cent},
the new coordination time is $(0.8+1.1) \times 256 = 486.4$ s. That is the
change we see from 32 replicas to 64 replicas in figure~\ref{fig:scaleup}. We
...
Figure~\ref{fig:scaleout}.  As discussed, in the asynchronous
(centralised) RE, there is no synchronization cost.  From
Figure~\ref{fig:scaleout}, we see that $T$ is approximately constant
