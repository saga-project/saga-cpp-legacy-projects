...
($T_{QW}$), and the product of the following components and the number of exchange steps($N_{E}$): (iii) the actual runtime of NAMD at each generation ($T_{MD}$), (iv) time spent waiting 
...
In a heterogeneous environment, $T_{MD}$ might not be a constant. In that case, we could estimate the number of times the replicas might restart on different machines and calculate the total number of restarts required to complete the exchanges.  
...
%\label{fig:async:b}
\caption{\small  In the centralized asynchronous RE (Figure 1(a)) all the replicas are managed by the master, where as in the decentralized asynchronous RE version(Figure 1(b)), for  each replica there is a replica agent which individually manages the replica.} \athotanote{Do we need this figure?} 
\label{fig:cent}
%\vspace{-1em}
\end{figure}
