...
\alnote{What value for $T_{X}$ do we assume?} \athotanote{I just let it be $T_X$ because we did not introduce $T_X$ in this section yet.}
...
become available, and that component $T_w$ is included in $T_f$.\alnote{That is somehow imprecise now! We should only
include it in one component.} {\athotanote{this $T_w$ is not $T_W$. it is only part of $T_W$(time waiting for the next replica to become available. therefore, $T_W$ is still left with $T_r$(time waiting to be restarted at the bigjob agent }
Thus $T_W$ is 1.02 seconds. \alnote{Abhinav: could you please update table 1 with those numbers?} \athotanote{the numbers are already in place}
