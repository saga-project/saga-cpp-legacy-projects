...
the nodefile \jhanote{what is nodefile? I know what it is but use the
  general description of what is happening here} is passed to the
...
\begin{table}
	...
	\hline
	\hspace{2mm}$T_{w}$ &1.84\,seconds &NA (included in $T_r$)&0.73\,second\\ 
	\hline
	\hspace{2mm}$T_{r}$ &0.8 seconds&1.02 seconds&1.3\,second\\
	\hline\hline
	$T_{X}$        &0.62\,seconds &0.78\,seconds &8.68\,seconds\\
	\hline
	\hspace{2mm}$T_{f}$        &0\,seconds   &0.16\,seconds &7.04\,seconds \alnote{we need to analyze this mis-match}\\
	\hline
...
for a pair of replicas, $T_w$ is 29.44/16=1.84 seconds.  The
RE-Manager marks the replicas which finish running as done and
...
includes updating configuration files and transferring them to their
respective directories; it takes 0.2 seconds to write and copy a file
locally. For a pair of replicas involved in the exchange, $T_{ex} =
...
$T_{coord}$ is the time required by the RE-Manager to post a pair of
replica descriptions to the advert server, to be restarted by the
BigJob agent. % \jhanote{Abhinav - is the previous sentence correct?
