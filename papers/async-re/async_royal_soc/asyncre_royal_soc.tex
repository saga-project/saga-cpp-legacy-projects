...
\jhanote{Can the following be eliminated or moved to someplace later}
...
replicas. \jhanote{Ths last sentence is incomplete and incorrect
  grammatically. Please fix. Does it need to be merged with the
  previous sentence?} Because, almost every time, the RE Manager
...
\alnote{What is the component that is actually spent waiting for the
  ...
  consistency!} \jhanote{Previous paragraph has to be simplified.  Too
...
before any exchanges are attempted $T_F$ is 0 seconds.  $T_{ex}$
...
file locally. $T_{ex} = 0.2 \times 2=0.4$ seconds. \jhanote{EXPLAIN
  the factor of two!}

\jhanote{Should it be $T_{coord}$ or $T_{COORD}$ ? Consistency}
$T_{coord}$ is the time required by the RE-Manager to restart a pair
of replicas. \jhanote{Abhinav - is the previous sentence correct?
  ``resubmitting a pair of replicas for restarting'' was too vague!}
On average $T_{coord}$ amounts to $0.11\,sec \times 2 =
