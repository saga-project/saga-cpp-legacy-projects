...
\begin{abstract}{Replica-Exchange, SAGA, Large-Scale, Production}
  ...
  the number of replicas and typical number of cores per replica, on
...
% \alnote{Not sure whether this are unprecedented
%     scales - see Tom's comment...}
% \alnote{\textbf{This sentence is in my opinion redundant to the previous
%   one:}  We use this framework to implement two different RE
%   algorithms in two different ways.}
...
important design objective of distributed applications -- both
logically distributed and physically distributed ones.  It enables
...
number of replicas, which can then be directly and concurrently
...
However, in this work, we will equat synchronous exchange with 
...
In the decentralised implementation of asynchronous RE algorithm, the
replicas are managed and exchanged individually by replica-agents. If
it finds the replica has completed its run, it updates the replica
state at the advert service, which takes $\approx$ $0.1\,s$.  The
replica-agent then retrieves the energy and temperature from the
output file, each of which takes $0.2\,s$, and posts both these values
to the advert service.  The total time for these operations is thus:
$0.1+0.2\times2+0.1\times2=0.7\,s$. \jhanote{Please ``Define'' what
  this operation is? What part of the equation ?} \athotanote{fixed..?} This $0.7\,s$ is a part of $T_{find}$, which has other components - as explained in the following sentences.  \jhanote{I find the
...
{\it Experiments: } To understand the scaling behaviour of the different RE
formulations, we analyse $T$ for 4, 8, 16, 32, 64, 128 and 256 replicas,
...
-- terms $T_{EX}$ and $T_W$ in Equation~\ref{eq:totaltime}. The increase,
...
unless you explain what we mean by algorithmic and implementation} We explain
the reasons for this behaviour - algorithmic and implementation - in the
following paragraphs. } \athotanote{added preceding sentences}
...
{\it Results:} We analyse the performance of the asynchronous (centralised) RE
implementation using a different number of distributed resources (1, 2 and 4
respectively) while keeping the ensemble size, i.\,e.\ the number of replicas,
constant. The model developed in section~\ref{sec:repex-approach} still holds
and after factoring in the performance advantage due to the additional
BigJob-Agents, the analysis provided in~\ref{sec:re_impl}(c)
%\alnote{there is no section 4.3. please use refs} 
can be used to understand the behaviour. In a distributed scenario,
$T_{MD}$ can be different on different machines. Also, depending on
the physical location and other network related aspects, $T_{EX}$
can be effected. 

The resources used in this experiment are mostly homogeneous
in performance; thus, $T_{MD}$ is not effected. Because $T_W$ is 0\,s in
asynchronous RE, the main component that influences $T$ is
$T_{EX}$, which is comprised of $T_{find}$, $T_{file}$ and
$T_{state}$. Finding exchange partners and bookkeeping involves as described
several communications via the advert service and some local
processing. Although the resources are homogenous and all belong to LONI, they
are geographically distributed. This has a minor impact on the latency 
observed in the communication with the advert service: From {\it
QueenBee}, {\it Eric} and {\it Oliver} the creation of an advert entry takes
0.01\,s, from {\it Louie} it requires 0.02\,s. As the variation in these
times is very small, the impact on $T$ can be neglected. We observed that
$T_{file}$ -- i.\,e.\ the time to transfer the configuration file to a
replica -- has a bigger impact on ${T}$.
% The time to retrieve a value from the advert service varies
% by one-hundredth of a second from one machine to another.
\jhanote{distinguish ``case'' from ``configuration''..} \athotanote{changed
case to configuration when seemed appropriate} 

\alnote{explain high fluctuation for $N_R$=32 and \#machines=1} \athotanote{tried}
The results obtained from the scale-out experiments are plotted in
Figure~\ref{fig:scaleout_cent} (It should first be noted that the scale of
y-axis is much smaller than in Figure~\ref{fig:scaleup}). The variations in 
$T$ are small, ranging from 10 to 20 seconds. For 8 replicas on 1 machine
conducting 32 exchanges, $T$ is $570$\,s. If these replicas are distributed
equally over 2 machines, $T$ increases slightly to $586$\,s. In total the 
observed overheads are quite small and well acceptable (with at worst 5\,\% of 
the overall experiment runtime). As explained earlier, the main source ofthese 
overhead are the necessary remote
file transfers. Each remote file transfer takes 0.38\,s while a local transfer
requires only 0.01\,s, i.\,e.\ 0.37\,s less. In the 8 replicas, 32 exchanges
and 2 machines configuration, half of the replicas are located on a different
resource than the RE-Manager, i.\,e.\ the input file must be transferred to
the remote resource in theses cases.
% In each exchange 2 files are transferred, one for each replica. 
%\jhanote{This needs attention..} \athotanote{fixed}  
In average per exchange involving two replicas, one of the necessary input 
...
\alnote{This paragraph really needs attention or should be removed all
together!} We do not have an accurate explanation for the behaviour of the 16
and 32 replica configurations. It is possible that managing larger number of
replicas effects the performance more than the number of machines used. In the
32 replica configuration, we see larger fluctuation in $T$ on 1 machine than
on 2 and 4 machines. But again the experiments were repeated only 5 times and
the fluctuations are in the range of 10 to 20\,s. As we mentioned earlier, the
big-jobs became active within 30\,s of each other. This also adds to the
fluctuations. We will need to repeat these experiments more number of time and
at larger scales to analyse their behaviour accurately. Nonetheless, from
these experiments, we can say that our RE Framework is capable of scaling-out
without acutely effecting the performance. \alnote{can you be a little more
precise and explain e.g. why for the 32 replica scenario the time improves
going from 2 to4 machines?}
\athotanote{ changed explanation}
%Some of the data points on the graph do not behave in the model that we predicted. For example, in the 16 replica configuration, going from 1 to 2 machines, $T$ only changes by 4 s, when it should have moved by 23 s. But the fluctuations are of the same order (seconds), and can easily be explained by fluctuations in the performance of remote operations.
...
%Thus, the loose coordination between replicas at the exchange stage helps to provide the flexibility to use multiple resources, which would not be possible if there was tighter coordination between replicas at the exchange stage. 
Our model and data from
...
obviously a comparison with other RE implementations will also be
performed.
