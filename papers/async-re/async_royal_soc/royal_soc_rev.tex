...
scheme~\citep{metropolis:1087}. The term {$N_R \over 2$} denotes the number of exchanges that can occur after each run of the ensemble of replicas. $N_X \over {N_R \over 2}$ denotes the number of such runs that are needed to complete $N_X$ exchanges. For example, if $N_R$ is 4 and $N_X$ is 16, 2 exchanges are possible after an ensemble run and 8 such runs are required to complete 16 exchanges. 
After the exchange, the replicas are restarted with the new temperatures. \athotanote{reviewers asked to explain metropolis, its role, why temperature needs to be exchanged and/or provide reference. is the explanation we give here sufficient?}  However, any RE production run will
...
T_{find} + T_{file}+T_{state}$); $T_{state}$ arises due do different
reasons -- which may be related to
implementation % Not suprisingly, the
% management costs also differ for
of the different RE algorithms; $T_W$ is comprised of the synchronisation
...
Finally, $\eta$ is the number of
independent exchange events occurring concurrently; for $N_R$
replicas, this is typically $\frac{N_R}{2}$. %$N_R \over 2$.
In a distributed scenario, $T_{MD}$ might be different on different machines and therefore, $T_W$ could be effected. Also, depending on the physical location and other network related aspects, $T_{EX}$ could also be effected. \athotanote{is this ok or should i add anything else?} 
...
\alnote{This Gather/Scatter comparison reads a little artificial.}
In contrast to parameter-sweep applications, communication in RE is not scatter-gather in nature. There is periodic coordination in RE, but in scatter/gather applications the coordination occurs only initially or finally. \athotanote{please verify preceding statement} \alnote{Gathering involves synchronisation.} 
...
Figure~\ref{fig:coordination}(a) \jhanote{Move later: which can be
  ...
  implementations}.  A replica can be in one of these three states:
...
\alnote{paragraph needs attention} It should be noted that even in the
decentralised implementation, the advert service is still
centralised. Only the decision-making and replica-management is
decentralised. We did not observe any increase in latency when
accessing the advert service with respect to increasing number of
connections.  \alnote{we did observe a severe slowdown!!}
\athotanote{at first I though it was due to the large number of
  connections. but it was actually because in the decentralised case
  each replica has a different directory. so, changing the directory
  and retrieving the states was taking longer.} But the communication
times do vary depending on the physical distribution. Also,
maintaining data consistency is not an issue because it is a
centralised system.  \jhanote{What is ``it''?}Two write actions cannot
be carried out simultaneously on a single value.  It should also be
noted that the exchanges are non-deterministic in nature because the
replicas available for exchange are found in a random order and then
the Metropolis scheme~\citep{metropolis:1087} is used to match the
replicas.
...
working of the RE framework, as well as a basic characterization of
...
the output file, is done sequentially. Post-processing of the ensemble of replicas takes longer than the delay between the start-up of first and last replicas. Thus, post-processing determines the overall time spent waiting ($T_W$) for other replicas.
