...
can be effected. The resources used in this experiment are mostly homogeneous
...
processing.  Although the resources are homogenous and all belong to LONI, they are geographically distributed. % and the advert service,
%along with the other resources is also on the same network, 
\alnote{No sure whether I would consider a Grid as LONI as the SAME network} \athotanote{check}
While this effects the communication times to the advert server, we believe that the component that most effects the performance is $T_{file}$ -- which is
...
0.02 s and from {\it Oliver}, it takes 0.01 s. As the variation in communication times to advert server from different machines is small, we don't believe that it significantly effects $T$. \alnote{How many repeats?
...
\alnote{explain high fluctuation for $N_R$=32 and \#machines=1}
The results obtained from the scale-out experiments are plotted in
 Figure~\ref{fig:scaleout_cent}. In Figure~\ref{fig:scaleout_cent}, it should first be noted that the scale of y-axis is much smaller than in Figure~\ref{fig:scaleup}. The variations in $T$ are small, ranging from 10 to 20 seconds. For 8 replicas on 1 machine conducting
32 exchanges, $T$ is $570$\,s. When 8 replicas are distributed equally over 2
machines, $T$ increases slightly to $586$\,s (see 
Figure~\ref{fig:scaleout_cent}). Each remote file transfer takes 0.38\,s while 
a local transfer requires only 0.01\,s, i.\,e.\ 0.37\,s less. 
In the 8 replicas, 32 exchanges and 2 machines configuration, for half the
exchanges, files must be transferred to a remote machine. In each exchange 2
files are transferred, one for each replica. \jhanote{This needs attention..}
\athotanote{fixed} \alnote{I still don't understand why 1 file per replica
must be transfered to a remote machine.} In sum, for 32 exchanges, 32 remote
...
to 2 machines in the 8 replica configuration. We do not have an accurate explanation for the behaviour of the 16 and 32 replica configurations. It is possible that managing larger number of replicas effects the performance more than the number of machines used. In the 32 replica configuration, we see larger fluctuation in $T$ on 1 machine than on 2 and 4 machines. But again the experiments were repeated only 5 times and the fluctuations are in the range of 10 to 20 seconds. As we mentioned earlier, the big-jobs became active within 30 seconds of each other. This also adds to the fluctuations. We will need to repeat these experiments more number of time and at larger scales to analyse their behaviour accurately. Nonetheless, from these experiments, we can say that our RE Framework is capable of scaling-out without acutely effecting the performance.       \alnote{can you be a little more precise and explain e.g. why for the 32 replica scenario the time improves going from 2 to 4 machines?} \athotanote{ changed explanation}%Some of the data points on the graph do not behave in the model that we predicted. For example, in the 16 replica configuration, going from 1 to 2 machines, $T$ only changes by 4 s, when it should have moved by 23 s. But the fluctuations are of the same order (seconds), and can easily be explained by fluctuations in the performance of remote operations.
