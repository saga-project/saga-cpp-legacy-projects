...
\athotanote{unaddressed comments from the reviewers:  1. The paper would be improved if the work were better motivated by an example application near the start of the paper (for example, the MD simulation that is used later in the paper). 2. There needs to be an explanation of why the temperature needs to be exchanged between replicas and what the role of the Metropolis algorithm is.
...
where some synchronisation may also be required.
...
performed over a distributed environment.
5. What is the "Metropolis scheme" -- give reference or explain.
...
7. In section 3b(ii) -- what is "MD"? - we explain this earlier as molecular dynamics in parenthesis.. should we do that again?
...
9. When multiple replicas are concurrently searching for a partner to exchange with it is not clear how this is done safely. Even with reverification non-deterministic behavior could still arise if care is not taken.} non deterministic behavior is not possible because of a single advert server but incorrect behavior is possible. we need to discuss this.
...
implementations (Woods et al. 2005) \athotanote{we dont cite woods et al in the bib!?}is influenced and constrained by
...
$T_{mgmt}$ arises due do different
...
\begin{figure}%
\centering
\subfigure[Central]{\includegraphics[width=0.35\textwidth]{../figures/central_AL.pdf}}\qquad
\subfigure[Decentral]{\includegraphics[width=0.35\textwidth]{../figures/decentral_AL.pdf}}\\
...
%    \hspace{2mm}$T_{r}$ &0 s&0.7\,s &1.1\,s\\
	\hline\hline
	$T_{X}$        &0.6\,s &7.9\,s &1.9\,s\\
	\hline
	\hspace{2mm}$T_{f}$        &0\,s   &6.4\,s &1.3\,s\\
	\hline
	\hspace{2mm}$T_{ex}$       &0.4\,s &0.6\,s &0.4\,s\\
	\hline
    \hspace{2mm}$T_{mgmt}$    &0.2\,s &0.9\,s  &0.2\,s\\
	\hline
	\hline
	$\mathbf{T}$        &\textbf{1003\,s} &\textbf{631\,s}    &\textbf{811\,s}\\
	\hline
    \end{tabular}
    \caption{Average values of terms in Eqn~\ref{eq:totaltime} for the three RE algorithms. $T_{MD}$=time replica takes to complete 500 timesteps; $T_W$=time waiting for other replicas and to be restarted; $T_s$=synchronisation time; $T_r$=time waiting to be restarted; $T_X$=time to make a pairwise exchange; $T_f$=time to find/lock a partner; $T_{ex}$=time to write/transfer files; $T_{mgmt}$=updating states after exchange; $T$=total time-to-completion.} \athotanote{we need to find better names/symbols}
...
thus $0.1+0.2\times2+0.1\times2=0.7\,s$, so $T_mgmt = 0.7\,s$.  Note that
here $T_mgmt$ involves pre-exchange management at the replica-agent and not the 
...
configuration file takes, $T_{ex} = 0.1\times 4 +0.2 = 0.6\,s$. % (And
...
server, which is $0.2\,s$+$0.7\,s$ = 0.9\,s.  Thus, $T_{X}$ is $6.4+0.6 +0.9=
...
\caption{\textbf{asynchronous - decentralised.}}
\label{fig:scaleout}
\end{figure}
...
{\it Experiments:} To evaluate the performance of the different algorithms when scaling out across many machines, we
...
{\it Results:} The time to completion increases moderately from 1 machine to 2 and 4 machines. The increase in time to completion can be caused by three factors: (a) increased synchronization costs, (b) additional time taken for remote file staging, and (c) higher communication costs to contact the advert server. But the machines we used are similar in nature and the NAMD runtimes are almost close to each other. Therefore, that can be ruled out. Also we do not observe any difference in the communication times with the advert server. Therefore, the only factor effecting the $T$ is the additional time to stage the configuration files. The data supports this observation.


\subsubsection{Scale out - Asynchronous (decentralised)}
{\it Results:} The time to completion almost stays constant from 1 machine to 2 and 4 machines. There is no remote file staging involved in this implementation and as mentioned earlier, we do not observe any difference in the communications times with the advert server. 
