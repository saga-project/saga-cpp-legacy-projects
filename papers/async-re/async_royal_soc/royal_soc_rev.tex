...
scale-up and scale-out experiments we find that even though implementation
details are important, the asynchronous RE algorithms have
better scaling properties than the synchronous algorithms.
...
The difference in the coordination cost ($T_{EX} \times {N_X \over
  \eta}$) between a $N_R$ value of 64 and 32 is 45\,s, which accounts
...
asynchronous centralised case is only weakly dependent on $N_R$;
...
\alnote{explain high fluctuation for $N_R$=32 and \#machines=1} The
results obtained from the scale-out experiments are plotted in
Figure~\ref{fig:scaleout_cent} (It should first be noted that the
scale of y-axis is much smaller than in Figure~\ref{fig:scaleup}). The
variations in $T$ are small, ranging from 10 to 20 seconds. For 8
replicas on 1 machine conducting 32 exchanges, $T$ is $570$\,s. If
these replicas are distributed equally over 2 machines, $T$ increases
slightly to $586$\,s. In total the observed overheads are quite small
and well acceptable (with at worst 5\,\% of the overall experiment
runtime). As explained earlier, the main source ofthese overhead are
the necessary remote file transfers. Each remote file transfer takes
0.38\,s while a local transfer requires only 0.01\,s, i.\,e.\ 0.37\,s
less. In the 8 replicas, 32 exchanges and 2 machines configuration,
half of the replicas are located on a different resource than the
RE-Manager, i.\,e.\ the input file must be transferred to the remote
resource in theses cases.
...
\alnote{This paragraph really needs attention or should be removed all
  together!} We do not have an accurate explanation for the behaviour
of the 16 and 32 replica configurations. It is possible that managing
larger number of replicas effects the performance more than the number
of machines used. In the 32 replica configuration, we see larger
fluctuation in $T$ on 1 machine than on 2 and 4 machines. But again
the experiments were repeated only 5 times and the fluctuations are in
the range of 10 to 20\,s. As we mentioned earlier, the big-jobs became
active within 30\,s of each other. This also adds to the
fluctuations. \jhanote{Fix this to explain why if the error is less
  than 30 seconds, it is within tolerance and why this pops up at
  large replica counts and not elsewhere} We will need to repeat these
experiments more number of time and at larger scales to analyse their
behaviour accurately. Nonetheless, from these experiments, we can say
that our RE Framework is capable of scaling-out without acutely
effecting the performance. % \alnote{can you be a little more precise
%   and explain e.g. why for the 32 replica scenario the time improves
%   going from 2 to4 machines?}  \athotanote{ changed explanation}
