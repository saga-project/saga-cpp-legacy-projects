...
\begin{abstract}{Replica-Exchange, SAGA, Large-Scale, Production}  
  ...
  amenable to using distributed resources. We develop a framework
  for RE which supports different replica pairing (synchronous vs. 
  asynchronous) and coordination  mechanisms (centralised vs. 
  decentralised) and which can use a range of production cyberinfrastructure
  concurrently.  \alnote{\textbf{This sentence is in my opinion redundant to the previous
  one:}  We use this framework to implement two different RE
  algorithms in two different ways.}  We characterise the performance 
  of both RE algorithms at unprecedented scales (when considering the number of replicas and the number of processors per replica in RE mode)\athotanote{check} on production distributed
  infrastructure. \alnote{Not sure whether this are unprecedented scales - 
  see Tom's comment...}
  %We scale-up, increasing the number of replicas and scale-out, 
  %increasing the number of machines. 
  We find that the advantages of the asynchronous 
  formulations help outperform the synchronous formulations, in particular with 
  higher number of replicas. \athotanote{added preceding sentences}\alnote{refined}
...
dynamics to binding affinity calculations. A replica is a simulation at a well defined temperature.\athotanote{check placement.} The design and development
...
development, runtime environment and the underlying infrastructure is
an important design objective of distributed applications -- both
logically distributed and physically distributed
ones. %and distributed execution environments.
It enables applications to be flexible (across infrastructure),
extensible (to new methods of communication and coordination) and
scalable. \athotanote{refined}
...
pilot-job implementation -- SAGA BigJob~\citep{saga_bigjob_condor_cloud} 
-- to support the efficient execution of batch job ensembles
without the necessity to queue each individual job. The pilot-job provides 
a container for a set of sub-jobs, which can then be directly executed via 
the pilot-job. A major advantage of this approach is
that  waiting times at the resource manager, which usually
significantly contribute to the overall time-to-completion, are avoided.
\athotanote{added pilot job explanation. remove if thought not needed.}
...
coordination of the replicas. We also assume that the resources, network and communication times are
homogeneous. \jhanote{resources are more than just processors} \athotanote{check} If an
ensemble contains $N_R$ replicas, and the total number of pairwise
exchanges is defined to be $N_X$ and the runtime of a replica to
complete a defined number of time steps which is defined to be
$T_{MD}$, then the total time-to-completion of an experiment $T$ would
be:
\begin{eqnarray}
...
be lower if exchanges cannot be conducted concurrently. It can be seen from Equation~\ref{eq:totaltime} that as $T_{MD}$ increases the cost of coordination becomes less relevant. \athotanote{check}

% this is typically $\frac{N_R}{2}$,
% and $N_X \over {N_R \over 2}$ is the number of ensemble runs needed to complete 
% $N_X$ exchanges.

% The last component $T_W$ is the synchronisation
% time spent by a replica waiting for other replicas to complete
% running.
...
${N_R}$, a {\it fixed} number (${N_R \over 2}$) of replica pairs are
created. When \emph{all} the replicas in the ensemble reach a
pre-determined state (e.g. the Molecular Dynamics (MD) simulation
completes a pre-determined number of steps), an exchange of
temperatures between the predetermined paired replicas is attempted
...
\jhanote{It will be good to define and distinguish Replica, sub-job,
  e.g., A replica is a simulation at well defined temperature.} \athotanote{ i am not sure what to do about this. I defined replica and sub-job in the introduction}
...
state. \jhanote{We've
  ...
  defintion.} \athotanote{fixed} In contrast to parameter sweep applications in which
coordination commonly only occurs at the beginning and end -- a
pattern commonly referred to as scatter-gather -- RE requires periodic
coordination. In the synchronous RE formulation coordination occurs at
defined intervals.
...
synchronisation involved. % only includes the time
...
to support different RE algorithmic formulations. 
%  and enables us to
...
The RE Framework implements synchronous and asynchronous RE. The RE Framework contains the BigJob-Manager, the RE-Manager and the Replica-Agent. The BigJob-Manager is common for both synchronous and asynchronous RE. The RE-Manager's functionality varies according to the implementation - centralised or decentralised. A Replica-Agent is a manager for one replica and every replica has a Replica-Agent in the decentralised implementation.\athotanote{i am helping here?}
...
\alnote{please consistently use hyphen in RE-Manager, BigJob-Manager,
  ... Also, SAGA can be removed in front of BigJob Manager.}  For each
BigJob request from the RE-Manager (1 in Figure~\ref{fig:bigjob}), the BigJob-Manager submits a request to the resource manager (2 in
Figure~\ref{fig:bigjob}). When the BigJob becomes active, the
BigJob-Manager starts the job (3 in Figure~\ref{fig:bigjob}). For each
new sub-job, the RE-Manager submits the sub-job description to the BigJob-Manager (4 in Figure~\ref{fig:bigjob}), and an advert entry storing the description of the
sub-job is created by the BigJob-Manager (5 in
Figure~\ref{fig:bigjob}). The BigJob-Agent periodically polls for new
jobs (6 in Figure~\ref{fig:bigjob}). If a new job is found and
resources are available, the BigJob-Agent runs the job (7 in
Figure~\ref{fig:bigjob}). \athotanote{improved description} If not,
the job is queued. It is possible that there are more sub-jobs than
the big-job can accommodate. Further, the agent continuously monitors
the running sub-jobs and updates the sub-job states in the advert
service. Once a sub-job finishes running, the compute resources are
freed and marked as available.
%%%%% FIGURE %%%%%
\begin{figure}[t]
      \centering
          \includegraphics[scale=0.65]{../figures/Bigjob_arch.pdf}
          \caption{\footnotesize RE-Manager and BigJob-Manager
...
big-jobs on multiple resources and manage them from one location. %when any one of the big-jobs becomes active. 
\jhanote{last sentence needs attention} \athotanote{not sure what to do.}
...
As we will discuss in Section~\ref{sec:re_impl}, the synchronous and asynchronous RE
...
Figure~\ref{fig:coordination}(a). %\jhanote{Move later: which can be
  %used to understand both the asynchronous and synchronous RE
  %implementations}.  
A replica can be in one of these three states: (i) \texttt{new}
(submitted but not started), (ii) \texttt{running} and (iii)
\texttt{done}.  Once the big-job(s) is active and replicas are
\texttt{running}, the RE-Manager constantly queries the BigJob
Manager for the latest replica states.  When the RE-Manager finds a
...
the BigJob-Manager restarts them. The RE-Manager keeps count of
...
% used only to support the asynchronous RE.
...
a replica is not found available, the RE-Manager queries the 
...
% \alnote{see comment about BJ impl. detail above}.  The RE-Manager
% counts the successful exchanges. This process is repeated till all
...
\begin{figure}%
\centering
\subfigure[Centralised]{\includegraphics[width=0.47\textwidth]{../figures/central_AL.pdf}}\qquad
\subfigure[Decentralised]{\includegraphics[width=0.47\textwidth]{../figures/decentral_AL.pdf}}\\
...
increasing number of connections when accessing the advert service .  % \alnote{we did observe a severe
%   slowdown!!}
% \athotanote{at first I though it was due to the large
%   number of connections. but it was actually because in the
%   decentralised case each replica has a different directory. so,
%   changing the directory and retrieving the states was taking longer.}
...
examined in~\cite{Luckow:2008fp}. It is to be noted that in actual biological science simulations, the number of time-steps for which the MD codes are run is far higher than 500. And in such cases, the cost of coordination becomes less relevant. \athotanote{check}
...
available here ~\citep{async-re_url}.  %\athotanote{is the placement
%  okay?}%\jhanote{This needs to be moved to an appropriate place}
...
% \alnote{What is the other part of $T_{find}$?} \athotanote{in the next para.. 
% I will try to improve explanation}  
...
\alnote{This is not reflected in our model.} \athotanote{fixed}
...
% The actual difference observed in $T$ from 32 to 64 replicas is 366 s. 
% \jhanote{Abhinav: Please provide value of the difference in T here please} 
%\athotanote{mentioned} 
...
The difference in the coordination cost ($T_{EX} \times {N_X \over \eta}$) between a $N_R$ value of 64 and 32 is 45\,s, which accounts 
for the bulk of the increase in overall $T$. \alnote{if I do the numbers: 32*2*0.1=6.4s and
not 45s?} \athotanote{corrected explanation}
...
%The asynchronous RE algorithm itself possesses no inherent 
...
distributed resources to evaluate the performance of the centralised
implementation of asynchronous RE. \jhanote{Need a consistent and
  simple way of referring to the three cases/implementation/ etc!?}
We used the following TeraGrid and/or LONI resources: \emph{QueenBee,
  Eric, Louie} and \emph{Oliver}. The experiments are conducted with
8, 16 and 32 replicas that make 32, 64 and 128 exchanges
respectively. These exchanges are repeated on 1, 2 and 4 machines
while distributing equal number of replicas on each machine. It is
important to note that all experiments are conducted using four
big-jobs, irrespective of the number of machines used because varying
the number of big-jobs, or the ratio of replicas per big-job effects
the overall performance.  Another important point to note is that only
the experimental runs where all the four
big-jobs %\alnote{when referring to the pilot job, BigJob should be consistently spelled as big-job}
...
analysis provided
in~\ref{sec:re_impl}(c) %\alnote{there is no section 4.3. please use refs}
...
0.01 s, from {\it Eric}, it takes 0.01 s, from {\it Louie}, it takes
0.02 s and from {\it Oliver}, it takes 0.01 s.  \alnote{How many repeats?
Why is Eric faster than QB? Can we reduce the number of decimals to 2?} \athotanote{reduced to 2 decimals. repeated many times but data collected during a short period of time. my guess is that the performance depend on the distance of the advert server from the machine and load. don't know what else I can say here.}

\jhanote{distinguish ``case'' from ``configuration''..} \athotanote{changed case to configuration when seemed appropriate} The scale-out
behaviour can be seen in Figure~\ref{fig:scaleout_cent}.  For 8
replicas on 1 machine conducting 32 exchanges, $T$ is 570 s. When 8
replicas are distributed equally over 2 machines, $T$ is 586 s (see
...
exchanges on 2 machines configuration, for half the exchanges, files are
transferred to the remote machine. In each exchange 2 files are
transferred, one for each replica. \jhanote{This needs attention..} \athotanote{fixed}
Thus for 32 exchanges, 32 remote-transfers occur. Thus, the cumulative
$T_{file}$ due to remote transfers is $0.37 \times 32=12\,s$. Within
error bars, this is the difference we see in the
Figure~\ref{fig:scaleout_cent} when moving from 1 to 2 machines in the
8 replica case. The 16 and 32 replica cases shown in the graph can be
analysed in similar manner.  Some of the data points on the graph do
not behave in the model that we
predicted. % \alnote{can you give an example?}
For example, in the 16 replica configuration, going from 1 to 2 machines, $T$
...
%behaviour is quite good. 
 \jhanote{We are inviting trouble with such a
  qualititative remark} \athotanote{removed}
%As discussed in section~\ref{sec:re_impl}, the asynchronous (centralised) RE algorithm does not involve any synchronisation cost ($T_W$).
Thus, the loose coordination between replicas at the exchange stage
helps to provide the flexibility to use multiple resources, which
would not be possible if there was tighter coordination between
replicas at the exchange stage. The data from
Figure~\ref{fig:scaleout_cent} might only suggest additional file
transfer costs, but in a typical distributed scenario, with machines
on different networks, physical locations, cost of distributed
exchanges and heterogeneity may also play a role in effecting the
performance. We intend to examine these factors in the future;
obviously a comparision with other RE implementations will also be
performed.
