...
equally over 2 machines, $T$ increases slightly to $586$\,s. 
Each remote file transfer takes 0.38\,s while
a local transfer requires only 0.01\,s, i.\,e.\ 0.37\,s less. In the 8
replicas, 32 exchanges and 2 machines configuration, half of the replicas
are located on a different resource than the RE-Manager, i.\,e.\ the input 
file must be transferred to the remote resource in theses cases. 
% In each exchange 2 files are transferred, one for each replica. \jhanote{This needs attention..} \athotanote{fixed}  
In average for 32 exchanges, 32 remote transfers are necessary. Thus, the
cumulative $T_{file}$ due to remote
