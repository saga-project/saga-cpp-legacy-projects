...
development and the underlying infrastructure is an important design
...
communication and coordination) and scalable. \athotanote{refined}
...
replica pairing/exchanging ($T_{state}$). $T_{state}$ arises due to
...
waiting for \emph{all} replicas to finish running before 
...
$N_R$ increases, the number of exchanges at a given exchange step increases;
...
to contentions at the central process that coordinates the exchange. Further,
some implementation details, such as the fact that the current implementation
of the coordinator process is single-threaded, also effect $T_W$.
...
service).  From Equation~\ref{eq:decent} it can be derived
that for 64 replicas, $T_{find}$ is approximately $0.7$+ $2 \times 64 \times
0.1 = 13.5$\,s. \alnote{Where do the 0.7 come from? $T_{file}$?} 
