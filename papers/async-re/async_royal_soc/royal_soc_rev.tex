...
replicas that are MPI jobs. In this section we outline the architecture,
implementation and the basic performance of our RE framework when used to
implement the different formulations. The RE Framework implements synchronous
and asynchronous RE. The RE Framework consists of the RE-Manager and the SAGA
BigJob framework, which is used to efficiently execute the ensemble jobs. The
BigJob framework is common for both synchronous and asynchronous RE. The
RE-Manager's functionality varies in the different implementations --
...
\begin{figure}[t]
      \centering
          ...
          \caption{\footnotesize The RE Framework: The framework consists of
...
started and put directly in the \texttt{running} state. The additional states 
in the decentralised implementation are needed to implement and maintain
...
service is still centralised. Only the replica-management and exchange
coordination are decentralised. We did not observe any increase in latency
with respect to the increasing number of connections when accessing the advert
service. % \alnote{we did observe a severe
%   slowdown!!}
