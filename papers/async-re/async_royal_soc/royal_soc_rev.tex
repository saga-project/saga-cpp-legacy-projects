...
y-axis is much smaller than in Figure~\ref{fig:scaleup}). The variations in $T$
are small, ranging from 10 to 20 seconds. For 8 replicas on 1 machine
conducting 32 exchanges, $T$ is $570$\,s. When 8 replicas are distributed
equally over 2 machines, $T$ increases slightly to $586$\,s (see
Figure~\ref{fig:scaleout_cent}). Each remote file transfer takes 0.38\,s while
a local transfer requires only 0.01\,s, i.\,e.\ 0.37\,s less. In the 8
replicas, 32 exchanges and 2 machines configuration, for half the exchanges,
files must be transferred to a remote machine. In each exchange 2 files are
transferred, one for each replica. \jhanote{This needs attention..}
\athotanote{fixed} \alnote{I still don't understand why 1 file per replica
must be transferred to a remote machine.} In sum, for 32 exchanges, 32 remote
transfers are necessary. Thus, the cumulative $T_{file}$ due to remote
transfers is approximately $0.37 \times 32=12\,s$. Within error bars, this is
the difference we see in the Figure~\ref{fig:scaleout_cent} when moving from 1
to 2 machines in the 8 replica configuration. We do not have an accurate
explanation for the behaviour of the 16 and 32 replica configurations. It is
possible that managing larger number of replicas effects the performance more
than the number of machines used. In the 32 replica configuration, we see
larger fluctuation in $T$ on 1 machine than on 2 and 4 machines. But again the
experiments were repeated only 5 times and the fluctuations are in the range
of 10 to 20 seconds. As we mentioned earlier, the big-jobs became active
within 30 seconds of each other. This also adds to the fluctuations. We will
need to repeat these experiments more number of time and at larger scales to
analyse their behaviour accurately. Nonetheless, from these experiments, we
can say that our RE Framework is capable of scaling-out without acutely
effecting the performance. \alnote{can you be a little more precise and
explain e.g. why for the 32 replica scenario the time improves going from 2 
to4 machines?} \athotanote{ changed explanation}
