...
available at:
\url{https://svn.cct.lsu.edu/repos/saga-projects/applications/REMDgManager/src/main.py}~\citep{async-re_url}.
...
$0.1+0.2\times2+0.1\times2=0.7\,s$. \jhanote{Please ``Define'' what
  this operation is? What part of the equation ?}  \jhanote{I find the
  following sentence: out of place. Please place appropriately} Note
that here $T_{find}$ involves pre-exchange management at the
replica-agent and not the BigJob-Agent (like the centralised case).
...
delays at the BigJob-Agent during the startup of the replicas sub-jobs
\jhanote{replicas or sub-jobs? I think sub-jobs!}  mainly due to the
...
processing.  Since the resources are homogenous and the advert services,
along with the other resources is also on the same network, \alnote{No sure
whether I would consider a Grid as LONI as the SAME network} the 
component that mainly effects the performance is $T_{file}$ -- which is
...
case to configuration when seemed appropriate} The scale-out behaviour can be
seen in Figure~\ref{fig:scaleout_cent}. For 8 replicas on 1 machine conducting
...
to 2 machines in the 8 replica case. The 16 and 32 replica cases shown in the 
graph can be analysed in similar manner. \alnote{can you be a little more precise and explain e.g. why for the 32 replica scenario the time improves going from 2 to 4 machines?} Some of the data points on the graph
do not behave in the model that we predicted. For example, in the 16 replica
configuration, going from 1 to 2 machines, $T$ only changes by 4 s, when it
should have moved by 23 s. But the fluctuations are of the same order
(seconds), and can easily be explained by fluctuations in the performance of
remote operations.
