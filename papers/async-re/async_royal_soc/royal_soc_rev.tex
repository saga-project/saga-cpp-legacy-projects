...
\alnote{reads a little bumpy} 
  ...
  coordination (coordination of replicas and exchanges) mechanisms, that can use a range of production
  cyberinfrastructure concurrently.  We use this framework to
  implement three different %formulations of the 
  RE algorithms.  We
...
enables the use of a range of infrastructure. %i.e., it supports a scheme that does not depend on a static set of resources that are pre-defined at the time of workload submission. 
For example, we can submit multiple BigJobs on multiple resources and can start our sub-jobs when any one of them becomes active.

Figure ~\ref{fig:bigjob} shows the architecture of the RE Manager and SAGA BigJob Manager framework. The SAGA-BigJob framework consists of three components: (i) the SAGA-BigJob Manager, (ii) the
...
components of the RE and SAGA-BigJob framework. The 
RE-Manager uses the SAGA BigJob-Manager; the actual tasks that the RE Manager
performs depend on the RE algorithm and the implementation being investigated. The RE-Manager supports two different replica management mechanisms:  centralised and decentralised. In the former, the RE-Manager manages all the replicas and performs the exchanges and in the latter, each replica is managed individually by a {\it replica-agent}.
...
We implemented both the synchronous and asynchronous RE using the centralised implementation but only the latter using the decentralised implementation as we believe that the asynchronous RE is better suited for decentralised implementation. We will explain next how the centralised and decentralised replica management mechanisms are implemented.
...
the asynchronous and synchronous RE implementations.
...
The implementation of the asynchronous RE-Manager, is
...
exchanges, whenever the asynchronous RE-Manager finds a
...
\alnote{we did observe a severe slowdown!!} \athotanote{at first I though it was due to the large number of connections. but it was actually because in the decentralised case each replica has a different directory. so, changing the directory and retrieving the states was taking longer.} But the 
