...
\begin{abstract}{Replica-Exchange, SAGA, Large-Scale, Production}  
  ...
  for RE which supports different replica pairing (synchronous vs. asynchronous) and coordination
  mechanisms (centralised vs. decentralised) and which can use a range of production cyberinfrastructure
  concurrently.  We use this framework to implement two different RE
  algorithms in two different ways.  We characterise the performance of the different RE
  algorithms at unprecedented scales on production distributed
  infrastructure. We scale-up, increasing the number of replicas and scale-out, increasing the number of machines. We find that even with a simple, non-optimised prototype of the
RE Framework, the advantages of asynchronous formulations soon outweigh the synchronous formulations, i.e., as the number of replicas increases the
performance of asynchronous RE beats that of the synchronous RE. \athotanote{added preceding sentences}
...
new job is found and resources are available, the BigJob-Agent runs the job. 
...
for 32 replicas in section~\ref{sec:re_impl}(\ref{subsec:sync}); we use that analysis as the basis to
understand the scale-up behaviour of the three cases. We can see the behaviour of different RE implementations in Figure~\ref{fig:scaleup}. That the change in $T$ as $N_R$ increases is exponential for synchronous and asynchronous (centralised) RE and linear for asynchronous (decentralised) RE denotes the scale-up properties of the three implementations. We explain the reasons for this behaviour - algorithmic and implementation - in the following paragraphs. \athotanote{added preceding sentences}
...
coordination time at each exchange step increases.  For
$N_R = 64$ and $N_X = 256$, using Equation~\ref{eq:totaltime}, the
...
Figure~\ref{fig:scaleup}. While this is a single threaded implementation, the cost of synchronisation ($T_W$) is unavoidable even in a decentralised implementation. Thus, $T_W$, which is algorithmic, is the major cause for the increase in $T$. \athotanote{added preceding sentence} % The actual difference observed in $T$ from 32 to 64 replicas is 366 s. \jhanote{Abhinav: Please provide value of
...
64. % The actual difference observed in $T$ from 32 to 64 replicas is 18 s.\jhanote{Abhinav: Please provide value of the
%   difference in T here please again}.\athotanote{done}
The variation in $T$ for other values of $N_R$ can be
accounted for by changing values of $T_{find}$. As this is an asynchronous algorithm, $T_W$ is 0. The reason we give for a linear increase in $T$ ad $N_R$ increases is $T_{find}$, which is an implementation detail. We find that the asynchronous RE algorithm itself does not limit the performance. \athotanote{added preceding sentence} 
...
we predicted. % \alnote{can you give an example?}
