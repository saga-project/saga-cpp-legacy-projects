...
analysis provided in~\ref{sec:re_impl}(c) %\alnote{there is no section  4.3. please use refs}
 can be used to understand the behaviour. In a
...
the replica. The time to retrieve a value from the advert server varies by one-hundredth of a second from one machine to another. From {\it QueenBee}, it takes 0.007 s, from {\it Eric}, it takes 0.005 s, from {\it Louie}, it takes 0.024 s and from {\it Oliver}, it takes 0.016 s.

In the case of 8 replicas on 1 machine conducting 32
...
predicted.\alnote{can you give an example?} For example, in the 16 replica case, going from 1 to 2 machines, $T$ only changes by 4 s, when it should have moved by 23 s. But the inconsistencies
