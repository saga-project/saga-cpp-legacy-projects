...
\caption{\textbf{synchronous.}\alnote{Please insert proper caption}}
\label{fig:scaleout}
\end{figure}
...
\caption{\textbf{asynchronous - decentralised.} \alnote{Please insert proper caption}}
\label{fig:scaleout}
\end{figure}
...
{\it Results:} The time to completion increases moderately from 
1 machine to 2 and 4 machines. The increase in time to completion 
can be caused by three factors: (a) increased synchronization costs, 
...
in nature and the NAMD runtimes are almost close to each other. Therefore, 
that can be ruled out. Also we do not observe any difference in the 
communication times with the advert server. Therefore, the only factor 
effecting the $T$ is the additional time to stage the configuration files. 
...
{\it Results:} The time to completion almost stays constant from 1 machine to 2 and 4 machines. 
There is no remote file staging involved in this implementation and as mentioned earlier, 
we do not observe any difference in the communications times with the advert server. 
...
does not involve any synchronisation cost ($T_s$). 
