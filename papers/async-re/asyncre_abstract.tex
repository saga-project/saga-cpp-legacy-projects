...
\begin{figure}
\centering
\includegraphics[scale=0.23]{figures/combined_graph.pdf}
\caption{\small The graph shows the completion times of synchronous and asynchronous RE simulations across 1, 2 and 3 machines, respectively. In each case, four 64 core BigJobs were launched, distributed across 1, 2 or 3 machines and were assigned 16 replicas. The size of each replica is 16 cores.}
\label{fig:graph}
\vspace{-1em}
\end{figure}
...
%LONI~\cite{LONI_web}.
Preliminary results, shown in the graph(Figure~\ref{fig:graph}), indicate the two most important advantages of asynchronous RE and SAGA/BigJob over traditional RE: (a) the reduced time to completion when running on more than one machine, and (b) the overwhelming reduction in the the time to completion compared to synchronous RE.
