...
asynchronous replica exchange (Case II), the master launches one or more BigJobs and submits each replica to its respective BigJob. The master now monitors the replicas. As and when it finds a replica in done state, the master starts searching for an appropriate partner from any other replicas in done state. The decision as to make the exchange or not is made using
...
to their respective BigJob(s) and goes back to monitoring the replicas. 
...
We configured both the Cases I \& II to run a parallel NAMD simulation with 16 replicas sampling a temperature between 300 and 450 K. Four 64 core BigJobs are distributed across to 1, 2 or 4 machines equally and each replica uses 16 MPI processes and runs 500 time steps between exchange attempts. The metric used is the time to complete 64 attempted exchanges. %Both cases were run over 1, 2 and 4 machines, where the BigJobs are distributed equally across the resources. 
