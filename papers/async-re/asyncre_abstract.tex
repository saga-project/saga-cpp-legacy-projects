...
A major limitation of this model is that the replicas are paired and the exchanges can only take place between the paired replicas.
This limits the group of replicas which are available for exchange. This causes the replicas to wait for their partners when there could be other replicas probably ready to exchange. %Moreover, the replicas are attached to their partners, 
