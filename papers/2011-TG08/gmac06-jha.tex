...
\terms{Theory, Performance, Reliability}
% \begin{keywords}
...
However, there are class of applications for which the individual job run-time characteristics are inherently difficult to predict and plan for, but can benefit in multiple ways from utilizing distributed resources concurrently. For these applications the resource requirement is dynamic and unpredictable; interestingly, the resource requirements and utilization might possibly be dependent upon both the execution trajectory and underlying infrastructure. %Such applications are hard to develop and deploy.
It is difficult to define an effective static scheduling strategy.
% that will be effective throughout the execution of a complete applications; hence static resource mapping is not an option.  
...
In this section, we discuss the experiments performed, the design decisions and motivation behind these experiments. % As alluded to Grids are dynamic and in principle, can provide an effective infrastructure for applications with time-varying and hard-to-predict resource requirements.
...
% and compare it to \tc when using that machine over a few weeks. 
...
The proposed FAUST heart-beat monitor consists of a client-side service that is called from the actual simulation and a server-side service that is called from Lazarus. The client-side service, a library function, will update the SAGA advert database entry of that particular simulation with vital information and statistics, including the current iteration, the amount of memory allocated, the convergence rate etc... The current design allows for the application to register a key-value pair of entries to the advert database, making it flexible and application independent. The server-side service is called by Lazarus to check on the status of running simulations, check that allocated memory is within tolerance, the convergence rate is acceptable and the time per iteration does not vary greatly between simulations. 
...
\BIBentryALTinterwordspacing
 [Online]. Available: \url{http://www.realitygrid.org/}
\BIBentrySTDinterwordspacing

\bibitem{gat}
G.~Allen, K.~Davis, T.~Goodale, A.~Hutanu, H.~Kaiser, T.~Kielmann, A.~Merzky,
  R.~van Nieuwpoort, A.~Reinefeld, F.~Schintke, T.~Schott, E.~Seidel, and
  B.~Ullmer, ``The grid application toolkit: Toward generic and easy
  application programming interfaces for the grid,'' \emph{Proceedings of the
  IEEE}, vol.~93, no.~3, pp. 534--550, March 2005.

\bibitem{cog}
\BIBentryALTinterwordspacing
 [Online]. Available: \url{http://wiki.cogkit.org/index.php/Main_Page}
\BIBentrySTDinterwordspacing

\bibitem{bangerth}
W.~Bangerth, H.~Klie, V.~Matossian, M.~Parashar, and M.~F. Wheeler, ``An
  autonomic reservoir framework for the stochastic optimization of well
  placement,'' \emph{Cluster Computing}, vol.~8, no.~4, pp. 255--269, 2005.

\bibitem{accord}
Accord: A Programming Framework for Autonomic Applications,” H. Liu and M.
  Parashar, IEEE Transactions on Systems, Man and Cybernetics, Special Issue on
  Engineering Autonomic Systems, Editors: R. Sterritt and T. Bapty, IEEE Press,
  Vol. 36, No 3, pp. 341 – 352, 2006. (PDF).

\bibitem{DataAssim}
G.~Evensen, \emph{Data Assimilation: The Ensemble Kalman Filter}.\hskip 1em
  plus 0.5em minus 0.4em\relax Secaucus, NJ, USA: Springer-Verlag New York,
  Inc., 2006.
...
\bibitem{escience07}
\BIBentryALTinterwordspacing
S.~Jha, H.~Kaiser, Y.~El~Khamra, and O.~Weidner, ``Design and implementation of
  network performance aware applications using saga and cactus,'' in
  \emph{Accepted for 3rd IEEE Conference on eScience2007 and Grid Computing,
  Bangalore, India.}, 2007. [Online]. Available:
  \url{http://saga.cct.lsu.edu/publications/saga_cactus_escience.pdf}
\BIBentrySTDinterwordspacing

\bibitem{ogf_web}
\BIBentryALTinterwordspacing
{Open Grid Forum}. [Online]. Available: \url{http://www.ogf.org/}
\BIBentrySTDinterwordspacing

\bibitem{drmaa_url}
{Distributed Resource Management Application API}, {http://drmaa.org}.

\bibitem{gridrpc_url}
{http://forge.ogf.org/sf/projects/gridrpc-wg}.

\bibitem{saga_web}
{SAGA - A Simple API for Grid Applications}, http://saga.cct.lsu.edu/.

\bibitem{saga_royalsoc}
A.~Luckow, S.~Jha, J.~Kim, A.~Merzky, and B.~Schnor, ``Adaptive distributed
  replica-exchange simulations,'' \emph{Theme Issue of the Philosophical
  Transactions of the Royal Society A: Crossing Boundaries: Computational
  Science, E-Science and Global E-Infrastructure Proceedings of the UK
  e-Science All Hands Meeting 2008}, vol. 367, 2009.

\bibitem{faust_url}
\BIBentryALTinterwordspacing
{Famework for Adaptive Ubquitious Scalable Tasks (FAUST)}. [Online]. Available:
  \url{{http://saga.cct.lsu.edu/index.php?option=com_content&task=view&id=98&I%
temid=174}}
\BIBentrySTDinterwordspacing

\bibitem{cactus_web}
\BIBentryALTinterwordspacing
{Cactus Framework}. [Online]. Available: \url{{http://www.cactuscode.org}}
\BIBentrySTDinterwordspacing

\bibitem{bqp}
J.~Brevik, D.~Nurmi, and R.~Wolski, ``Predicting bounds on queuing delay for
  batch-scheduled parallel machines,'' in \emph{PPoPP '06: Proceedings of the
  eleventh ACM SIGPLAN symposium on Principles and practice of parallel
  programming}.\hskip 1em plus 0.5em minus 0.4em\relax New York, NY, USA: ACM,
  2006, pp. 110--118.

\bibitem{bqp_url}
\BIBentryALTinterwordspacing
{Batch Queue Predictor}. [Online]. Available:
  \url{{http://spinner.cs.ucsb.edu/batchq/invbqueue.php}}
\BIBentrySTDinterwordspacing

\bibitem{PETSc}
S.~Balay, K.~Buschelman, W.~D. Gropp, D.~Kaushik, M.~G. Knepley, L.~C. McInnes,
  B.~F. Smith, and H.~Zhang, ``{PETSc} {W}eb page,'' 2001,
  http://www.mcs.anl.gov/petsc.

\bibitem{gridsat03}
W.~Chrabakh and R.~Wolski, ``{GridSAT: A Chaff-based Distributed SAT Solver for
  the Grid},'' \emph{SuperComputing}, vol.~00, p.~37, 2003.

\bibitem{majority_voting}
{F. J. Seinstra and J. M. Geusebroek}, {Color-Based Object Recognition on a
  Grid, Proceedings of the 9th European Conference on Computer Vision (ECCV),
  Graz, Austria,2006, Springer-Verlag.}
