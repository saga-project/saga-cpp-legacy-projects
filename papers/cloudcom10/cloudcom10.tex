...
implement a defined entry point, \jhanote{This is a bit unclear to the
  reader: What is an 'entry point' here..? What is important about an
  application 'must soley implement' it?} which is then called by
...
Agent that represents the pilot-job. \jhanote{Andre, the third
  component, or at least (iii) is missing!}
...
the Azure-based BigJob. Similarl to the grid BigJob BM, the Azure BM
...
sub-job terminates, the master \jhanote{what is the master -- is it
  with respect to ``roles'' or with respect to ``workers'' i.e
  ensemble members?} can collect results via the BigJob API.  NAMD is
...
average startup times of the different pilot-job backends
\jhanote{should we use BigJob backend in lieu of pilot-job backend
  here?} for grids, Condor pools and clouds. Each experiment was
...
respectively or to the AQS service \jhanote{please check}, and the
...
\begin{figure}[t]
    ...
    \caption{\textbf{NAMD Runtimes on Different Resource Types: } The
          ...
          HPC resources as QB and Poseidon. \jhanote{this figure is
            not referenced from/in the text anywhere!} \up}
...
instances are connected using 10 Gbps Ethernet.
...
at the master \jhanote{I think the master here is infrastructure level
  and not application level? Need clarification please} will
increase. As seen in
Figure~\ref{fig:performance_repex_scaleout_vmsizes} however, this
overhead for upto 32 replicas is {\it at maximum} a few percent of the
overall runtime. \jhanote{please check}
\begin{figure}[ht]
    \centering
...
shorter the overall runtime.  However, the efficiency for this problem
set drops to $<0.4$ when going from the 4 to 8 cores. \jhanote{Can you
  elaborate here. For example, we hven't defined efficiency... The
  reader will find it difficult to see this point}
