...
In Luckow et\,al.~\cite{repex_ptrsb} although an efficient scale-out of RE
...
the BigJob API. The BigJob API is a SAGA-style API that utilizes common
SAGA idioms and classes (e.\,g.\ saga.job.description) for managing 
pilot-jobs and for starting sub-jobs. 

\jhanote{Andre: Please check this makes sense?} \alnote{make sense and is correct. Just a note: the API is SAGA style and actually uses some SAGA classes, e.g. saga.job.description. But I think this does not need to go here.}
Initially, the BM launches the requested number of worker roles % (also referred to
% as VM) 
...
time for the LONI resource Poseidon. \jhanote{which coincidentally was
  very lightly loaded during the course of these experiments.} \alnote{agree,
...
LONI/TeraGrid.  Each FutureGrid VM provides 2 virtual cores and
...
scenario was executed in 26\,minutes (see~\cite{repex_ptrsb} for
details).
...
development considerably, in particular compared to the efforts
...
%     NSF-OCI 0710874.
    ...
    the research theme. ``Distributed Programming Abstractions''.  We
    thank Joohyun Kim (CCT) for assistance with the RNA models. We
    thank FutureGrid and Microsoft for providing the resources for the
    experiments. }
