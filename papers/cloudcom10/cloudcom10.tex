...
in \S~\ref{sec:bigjob-saga}. In \S~\ref{sec:pilot-bj} the pilot-job 
concept and the BigJob framework is introduced. 
...
ensemble of batch jobs on grids without the necessity to queue each individual
...
\emph{BigJob} is a SAGA-based pilot-job implementation~\cite{10.1109/CCGRID.2010.91}. 
...
heterogeneous backend, e.\,g.\ grids, Condor pools and EC2-based and Azure clouds, 
...
\begin{figure}[htbp]
    ...
    \caption{BigJob Sub-Job Submission: The startup time of the Azure
    BigJob could be improved in comparison to the traditional Grid
    BigJob mainly by relying on the AQS abstraction, which removes
    the need to repeatedly poll the advert service.}
