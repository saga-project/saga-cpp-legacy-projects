...
While closely coupled applications are still not well supported in
many cloud environments, loosely coupled, pleasingly parallel
applications can usually be well supported in clouds.  An interesting
example of such applications is those based upon the replica–exchange
algorithm (RE)~\cite{hansmann}. RE simulations are used to understand
physical phenomena—ranging from protein folding dynamics to binding
affinity calculations. The RE method involves the concurrent execution
of multiple similar simulations—the replicas. The coupling between the
replicas occurs via periodic exchange attempts between paired
replicas.  The exchange is typically infrequent compared with the
run-time of each replica, and is small in terms of communication
bandwidth requirements. Thus, RE is prima facie a perfect algorithm to
...
were TeraGrid resources. We utilize the SAGA BigJob framework to
...
EC2-style clouds (such as the ScienceCloud and FutureGrid).
...
platform. \jhanote{Given that we're not using SAGA BigJob here, should
  we call this an extension of the earlier work?}  We analyze the
suitability of the different abstraction provided by the Azure
...
provided it. The SAGA Bigjob framework that provides the basis for the
Azure-based replica-exchange application is presented in
section~\ref{sec:bigjob-saga}.  A significant issue in cloud computing
is performance. We provide an in-depth performance analysis in
...
% system.
REMD is a commonly used approach to enhance the sampling and
...
extending the BigJob framework to utlize the native abstractions
