...
#
# Description:
#   C-shell script for setting up the
...
#if ( $VO_GEANT4_SW_DIR ) then
#export VO_GEANT4_SW_DIR=/afs/cern.ch/project/gd/apps/geant4
#endif
...
#TEMPORARY FIX: TO BE CHECKED WITH ALBERTO
#Linking Brachy ...
#/afs/cern.ch/project/gd/apps/geant4/dirInstallations/diriAIDA/lib/libiAIDA.a(SymbolManager.o)(.text+0x70e): In function `iAIDA::AIDA_Tuple_native::SymbolManager::makeAndLoadEvaluator(std::basic_string<char, std::char_traits<char>, std::allocator<char> > const&, std::basic_string<char, std::char_traits<char>, std::allocator<char> > const&)':
#/users/ribon/dirInstallations/dirPI/myiAIDA/iAIDA-1.0.9/src/SymbolManager.cpp:111: warning: Using 'dlopen' in statically linked applications requires at runtime the shared libraries from the glibc version used for linking
