...
[
  ...
  if test "x$want_sqlite3" = "xyes"; then
    ...
    for tmp_path in $tmp_location $SQLITE3_LOCATION /usr /usr/local /opt $packages; do
      
      AC_MSG_CHECKING(for sqlite3 in $tmp_path)

      have_something=`ls $tmp_path/lib/libsqlite3* 2>/dev/null`
      ...
      SQLITE3_LDFLAGS="-L$tmp_path/lib/ -lsqlite3"
      ...
      if test "x$link_sqlite3" = "xno"; then
        ...
        if test -e "$tmp_path/lib/libsqlite3.a"; then
          SQLITE3_S_LIBS="$tmp_path/lib/libsqlite3.a"
