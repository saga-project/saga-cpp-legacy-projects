...
{
   "PIX",  /* -1  Pixel WCS */
   "LIN",  /*  0  Linear projection */
   "AZP",  /*  1  Zenithal/Azimuthal Perspective */
   "SZP",  /*  2  Zenithal/Azimuthal Perspective */
   "TAN",  /*  3  Gnomonic = Tangent Plane */
   "SIN",  /*  4  Orthographic/synthesis */
   "STG",  /*  5  Stereographic */
   "ARC",  /*  6  Zenithal/azimuthal equidistant */
   "ZPN",  /*  7  Zenithal/azimuthal PolyNomial */
   "ZEA",  /*  8  Zenithal/azimuthal Equal Area */
   "AIR",  /*  9  Airy */
   "CYP",  /* 10  CYlindrical Perspective */
   "CAR",  /* 11  Cartesian */
   "MER",  /* 12  Mercator */
   "CEA",  /* 13  Cylindrical Equal Area */
   "COP",  /* 14  Conic PerSpective (COP) */
   "COD",  /* 15  COnic equiDistant */
   "COE",  /* 16  COnic Equal area */
   "COO",  /* 17  COnic Orthomorphic */
   "BON",  /* 18  Bonne */
   "PCO",  /* 19  Polyconic */
   "SFL",  /* 20  Sanson-Flamsteed (GLobal Sinusoidal) */
   "PAR",  /* 21  Parabolic */
   "AIT",  /* 22  Hammer-Aitoff */
   "MOL",  /* 23  Mollweide */
   "CSC",  /* 24  COBE quadrilateralized Spherical Cube */
   "QSC",  /* 25  Quadrilateralized Spherical Cube */
   "TSC",  /* 26  Tangential Spherical Cube */
   "NCP",  /* 27  Special case of SIN */
   "GLS",  /* 28  Same as SFL */
   "DSS",  /* 29  Digitized Sky Survey plate solution */
   "PLT",  /* 30  Plate fit polynomials (SAO) */
   "TNX"   /* 31  Gnomonic = Tangent Plane (NOAO with corrections) */
};
...
int main(int argc, char **argv)
{
   ...
   for(ifiles=0; ifiles<ntables; ++ifiles)
   {
	...
	strcpy(csysStr, keyval);
	 ...
	 else if(datatype == WCS)
	 {
	    ...
	    if(debugLevel >= 4)
	    {
	       printf("Projection: %s (%d)\n\n", proj[input.wcs->prjcode+1], input.wcs->prjcode);
	       fflush(stdout);
	    }

	    if(input.wcs->prjcode == WCS_PIX
	    || input.wcs->prjcode == WCS_LIN)
	    {
	       printf("[struct stat=\"ERROR\", msg=\"%s projection is not valid for sky reprojection.\"]\n", proj[input.wcs->prjcode+1]);
	       fflush(stdout);

	       exit(0);
	    }

	    if(input.wcs->prjcode == WCS_COP
	    || input.wcs->prjcode == WCS_COE
	    || input.wcs->prjcode == WCS_COD
	    || input.wcs->prjcode == WCS_COO
	    || input.wcs->prjcode == WCS_BON
	    || input.wcs->prjcode == WCS_ZPN)
	    {
	       printf("[struct stat=\"ERROR\", msg=\"%s projection may require parameters not carried in the table. Use four corners mode (e.g. 'mImgtbl -c ...') instead.\"]\n", proj[input.wcs->prjcode+1]);
	       fflush(stdout);
   ...
   || zmax - zmin > 1)
   {
      fprintf(fstatus, "[struct stat=\"WARNING\", msg=\"All-sky data\"]\n");
      fflush(stdout);
      exit(0);
   }
