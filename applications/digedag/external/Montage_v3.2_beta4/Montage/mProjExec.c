...
int main(int argc, char **argv)
{
   ...
   if(tryAltOut)
   {
      sprintf(cmd, "mTANHdr %s %s/altout.hdr", template, projdir);
   ...
   while(1)
   {
    ...
    if (row_number % MPI_size == MPI_rank) {
	 ...
	 sprintf(cmd, "mGetHdr %s %s %s/orig.hdr", hdustr, infile, projdir);
	 ...
	 sprintf(cmd, "mTANHdr %s/orig.hdr %s/altin.hdr",
	    projdir, projdir);
	 ...
	 if(!wcsMatch)
	    ...
	    sprintf(cmd, "mProjectPP %s %s -i %s/altin.hdr -o %s/altout.hdr %s %s %s",
	       scaleStr, hdustr, projdir, projdir, infile, outfile, template);

	 else if(inp2p == COMPUTED  && outp2p == INTRINSIC)
	    sprintf(cmd, "mProjectPP %s %s -i %s/altin.hdr %s %s %s",
	       scaleStr, hdustr, projdir, infile, outfile, template);

	 else if(inp2p == INTRINSIC && outp2p == COMPUTED )
	    sprintf(cmd, "mProjectPP %s %s -o %s/altout.hdr %s %s %s",
	       scaleStr, hdustr, projdir, infile, outfile, template);
	 ...
	 if(!wcsMatch)
	    ...
	    sprintf(cmd, "mProjectPP %s %s -b \"%s\" -i %s/altin.hdr -o %s/altout.hdr %s %s %s",
	       scaleStr, hdustr, border, projdir, projdir, infile, outfile, template);

	 else if(inp2p == COMPUTED  && outp2p == INTRINSIC)
	    sprintf(cmd, "mProjectPP %s %s -b \"%s\" -i %s/altin.hdr %s %s %s",
	       scaleStr, hdustr, border, projdir, infile, outfile, template);

	 else if(inp2p == INTRINSIC && outp2p == COMPUTED )
	    sprintf(cmd, "mProjectPP %s %s -b \"%s\" -o %s/altout.hdr %s %s %s",
	       scaleStr, hdustr, border, projdir, infile, outfile, template);
