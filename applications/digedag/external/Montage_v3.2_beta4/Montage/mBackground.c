...
int main(int argc, char **argv)
{
   ...
   if (argc < 5) 
   {
      printf ("[struct stat=\"ERROR\", msg=\"Usage: mBackground [-d level] [-n(o-areas)] [-s statusfile] in.fits out.fits A B C | mBackground [-t] [-d level] [-n(o-areas)] [-s statusfile] in.fits out.fits images.tbl corrfile.tbl\"]\n");
      exit(1);
   }
   ...
   if(!tableDriven)
   {
      if (argc != 6) 
      {
	 printf ("[struct stat=\"ERROR\", msg=\"Usage (parameter mode): mBackground [-d level] [-n(o-areas)] [-s statusfile] in.fits out.fits A B C\"]\n");
	 exit(1);
      }
   ...
   else
   {
      ...
      if (argc != 5) 
      {
	 printf ("[struct stat=\"ERROR\", msg=\"Usage (table mode): mBackground [-t] [-d level] [-n(o-areas)] [-s statusfile] in.fits out.fits images.tbl corrfile.tbl\"]\n");
	 exit(1);
      }
