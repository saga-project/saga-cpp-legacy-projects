...
int main(int argc, char **argv)
{
   ...
   while ((c = getopt(argc, argv, "d:s:f")) != EOF) 
   {
      switch (c) 
      {
         case 'd':
         ...
         default:
            printf("[struct stat=\"ERROR\", msg=\"Usage: %s [-f(ixed-size)] [-d level] [-s statusfile] in.fits out.fits factor\"]\n", argv[0]);
            exit(1);
            break;
      }
   }
   ...
   if (argc - optind < 3) 
   {
      fprintf(fstatus, "[struct stat=\"ERROR\", msg=\"Usage: %s [-f(ixed-size)] [-d level] [-s statusfile] in.fits out.fits factor\"]\n", argv[0]);
      exit(1);
   }
