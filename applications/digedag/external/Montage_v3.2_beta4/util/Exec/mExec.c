...
int main(int argc, char **argv, char **envp)
{
   ...
   && wcs->prjcode != WCS_ARC)
   {
      ...
      else
      {
         ...
         if(debug >= 2)
         {
            fprintf(fdebug, "Using distorted TAN on output: max error = %-g\n", maxerror);
            fflush(fdebug);
         }
   ...
   while(1)
   {
      ...
      && strcmp(wcsin->ptype, "ARC") != 0)
      {
         ...
         else
         {
            ...
            if(debug)
            {
               fprintf(fdebug, "Using distorted TAN on input: max error = %-g\n", maxerror);
               fflush(fdebug);
            }
   ...
   if(baseCount > 1)
   {
	 ...
	 if(levelOnly)
      ...
      if(debug >= 1)
      {
         fprintf(fdebug, "TIME: mDiff/mFitplane  %6d (%d diffs)\n", (int)(currtime - lasttime), count);
         fflush(fdebug);
      }
