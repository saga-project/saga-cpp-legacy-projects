...
{
   int       i, j, k, jj, ipix, index, itemp, nowcs;
   int       saveHist, restoreHist, showCompass, noAirglow;
   int       istart, iend, nx;
   ...
   char      saveHistFile   [1024];
   char      restoreHistFile[1024];
   char      line           [1024];
   ...
   if(argc < 2)
   {
      printf("[struct stat=\"ERROR\", msg=\"Usage: %s [-d] [-nowcs] [-compass] [-t(rue-color) power] [-s statusfile] [-ho savehistfile] [-hi restorehistfile] [-ct color-table] -gray in.fits minrange maxrange [logpower/gaussian] -red red.fits rminrange rmaxrange [rlogpower/gaussian] -green green.fits gminrange gmaxrange [glogpower/gaussian] -blue blue.fits bminrange bmaxrange [blogpower/gaussian] -out out.jpg\"]\n", argv[0]);
      exit(1);
   }
   ...
   for(i=0; i<argc; ++i)
   {
      ...
      /* Kludge to suppress green airglow for 2MASS */

      else if(strcmp(argv[i], "-airglow") == 0)
         noAirglow = 1;
	 ...
	 i+=6;
      ...
      else if(strcmp(argv[i], "-ho") == 0)
      {
         if(i+1 >= argc)
         {
            printf ("[struct stat=\"ERROR\", msg=\"Too few arguments following -ho flag\"]\n");
            fflush(stdout);
            exit(1);
         }

	 saveHist = 1;
         strcpy(saveHistFile, argv[i+1]);
      ...
      else if(strcmp(argv[i], "-hi") == 0)
      {
         if(i+1 >= argc)
         {
            printf ("[struct stat=\"ERROR\", msg=\"Too few arguments following -hi flag\"]\n");
            fflush(stdout);
            exit(1);
         }

         restoreHist = 1;
         strcpy(restoreHistFile, argv[i+1]);
   ...
   if(isRGB)
   {
	...
	cd22   = -cd22;
	 ...
	 fflush(stdout);
      ...
      /* If another histogram is to be read in, replace the data values. */
      /* The calculated specs above are left alone.                      */

      if(restoreHist)
      {
	 fhist = fopen(restoreHistFile, "r");

	 if(fhist == (FILE *)NULL)
	 {
	    fprintf(fstatus, "[struct stat=\"ERROR\", msg=\"Cannot open histogram file [%s].\"]\n", restoreHistFile);
	    exit(1);
	 }
	 ...
	 for(i=0; i<256; ++i)
	 {
	    ...
	    sscanf(line, "%lf %lf %lf %lf",
	       &graydataval[i], &bluedataval[i], &greendataval[i], &reddataval[i]);
	 }
	 ...
	 /* Look up pixel color values */
	    ...
	    /* Special correction for 2MASS airglow problem: */
	    /* Don't let green be brighter than red or blue  */
   ...
   if(saveHist)
   {
	 ...
	 exit(1);
      ...
      fprintf(fhist, "|%14s|%14s|%14s|%14s|\n",
	 "graydataval", "bluedataval", "greendataval", "reddataval");

      for(i=0; i<256; ++i)
	 fprintf(fhist, " %14.6e %14.6e %14.6e %14.6e \n",
	    graydataval[i], bluedataval[i], greendataval[i], reddataval[i]);
...
void createColorTable(int itable)
{
    ...
    */
    ...
    /*
    int red_tbl6[]   = {  0,  64, 128, 192, 255};
    int grn_tbl6[]   = {  0,   0, 128, 192, 255};
    int blue_tbl6[]  = {  0,   0,   0,   0,   0};
    */

    int red_tbl6[]   = {  0,  32, 128, 192, 255};
    int grn_tbl6[]   = {  0,   0, 128, 192, 255};
    int blue_tbl6[]  = {  0,   0,   0, 128, 255};
	      ...
	      int imnaxis1, int imnaxis2)
{
   ...
   /* MIN: Determine what type of      */
   /* range string we are dealing with */

   nbin = NBIN - 1;
   
   strcpy(valstr, minstr);
   ...
   if(ptr >= valstr+strlen(valstr))
      ptr  = valstr+strlen(valstr);
   ...
   *ptr = '\0';
   ...
   if(strcmp(valstr, "min") == 0)
   {
      mintype = PERCENTILE;

      minval = 0.;
   }
   else if(strcmp(valstr, "med") == 0)
   {
      mintype = PERCENTILE;

      minval = 50.;
   }
   else if(valstr[strlen(valstr)-1] == '%')
   {
      mintype = PERCENTILE;

      valstr[strlen(valstr)-1] = '\0';

      minval = strtod(valstr, &end);

      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"min percentile value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   else if(valstr[strlen(valstr)-1] == 's')
   {
      mintype = SIGMA;
      ...
      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"min sigma value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   else
   {
      ...
      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"min value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   ...
   /* MAX: Determine what type of      */
   /* value string we are dealing with */
   
   strcpy(valstr, maxstr);
   ...
   *ptr = '\0';
   ...
   else if(valstr[strlen(valstr)-1] == '%')
   {
      ...
      maxval = strtod(valstr, &end);

      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"max percentile value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   else if(valstr[strlen(valstr)-1] == 's')
   {
      ...
      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"max sigma value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   else
   {
      ...
      if(end < valstr + strlen(valstr))
      {
	 printf("[struct stat=\"ERROR\", msg=\"max value \"%s\" is not a real number\"]\n", valstr);
	 fflush(stdout);
	 exit(1);
      }
   }
   ...
   /* BETA: Determine what type of      */
   /* value string we are dealing with  */
   
   if(type == ASINH)
   {
      strcpy(valstr, betastr);
	 ...
	 betaval = strtod(valstr, &end);

	 if(end < valstr + strlen(valstr))
	 {
	    printf("[struct stat=\"ERROR\", msg=\"beta percentile value \"%s\" is not a real number\"]\n", valstr);
	    fflush(stdout);
	    exit(1);
	 }
      }
	 ...
	 if(end < valstr + strlen(valstr))
	 {
	    printf("[struct stat=\"ERROR\", msg=\"beta sigma value \"%s\" is not a real number\"]\n", valstr);
	    fflush(stdout);
	    exit(1);
	 }
      }
	 ...
	 if(end < valstr + strlen(valstr))
	 {
	    printf("[struct stat=\"ERROR\", msg=\"beta value \"%s\" is not a real number\"]\n", valstr);
	    fflush(stdout);
	    exit(1);
	 }
      }
