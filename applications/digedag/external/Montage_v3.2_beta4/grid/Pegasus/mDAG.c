...
int main(int argc, char **argv)
{
   ...
   int     ifname, icntr1, icntr2, iplusname, iminusname, iscale;
  ...
  if(path)
      sprintf(cmd, "%s/bin/mArchiveList %s %s \"%s\" %s %s %s/images.tbl",
         path, survey, band, mosaicCenter, mosaicWidth, mosaicHeight, workdir);
      else 
         sprintf(cmd, "mArchiveList %s %s \"%s\" %s %s %s/images.tbl",
            survey, band, mosaicCenter, mosaicWidth, mosaicHeight, workdir);
   ...
   if(path)
      sprintf(cmd, "%s/bin/mDAGTbls %s/images.tbl %s/region.hdr %s/rimages.tbl %s/pimages.tbl %s/cimages.tbl",
         path, workdir, workdir, workdir, workdir, workdir);
      else 
         sprintf(cmd, "mDAGTbls %s/images.tbl %s/region.hdr %s/rimages.tbl %s/pimages.tbl %s/cimages.tbl",
   ...
   fprintf(fdag, "<adag xmlns=\"http://www.griphyn.org/chimera/DAX\"\n");
   fprintf(fdag, "      xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n");
   fprintf(fdag, "      xsi:schemaLocation=\"http://www.griphyn.org/chimera/DAX http://www.griphyn.org/chimera/dax-1.8.xsd\"\n");
   fprintf(fdag, "      count=\"1\" index=\"0\" name=\"montage\">\n\n");
   fflush(fdag);
   ...
   fprintf(ffit, "| cntr1 | cntr2 |       stat         |\n");
   fprintf(ffit, "|  int  |  int  |       char         |\n");
   ...
   fprintf(fcache, "statfile_%s.tbl ipac_cluster\n%s/%s/statfile.tbl\n", idstr, workurlbase, workdir);
   ...
   if(numTiles > 1)
   {
	 ...
	 for(j=0; j<numTilesY; ++j)
	 {
	    ...
	    fprintf(fdag, "\n");
            ...
            fprintf(fcache, "region_%d_%d_%s.hdr ipac_cluster\n%s/%s/region_%d_%d.hdr\n",   i, j, idstr, workurlbase, workdir, i, j);
            fprintf(fcache, "cimages_%d_%d_%s.tbl ipac_cluster\n%s/%s/cimages_%d_%d.tbl\n", i, j, idstr, workurlbase, workdir, i, j);

         }
      }

      fprintf(fdag, "  <filename file=\"slist_%s.tbl\" link=\"input\"/>\n", idstr);
      fprintf(fdag, "  <filename file=\"simages_%s.tbl\" link=\"input\"/>\n", idstr);
      fprintf(fdag, "\n");
      fflush(fdag);

      fprintf(fcache, "slist_%s.tbl ipac_cluster\n%s/%s/slist.tbl\n", idstr, workurlbase, workdir);
   ...
   fprintf(fcache, "region_%s.hdr ipac_cluster\n%s/%s/region.hdr\n", idstr, workurlbase, workdir);

   if(numTiles > 1)
      fprintf(fcache, "shrunken_%s.hdr ipac_cluster\n%s/%s/shrunken.hdr\n", idstr, workurlbase, workdir);

   fprintf(fcache, "pimages_%s.tbl ipac_cluster\n%s/%s/pimages.tbl\n", idstr, workurlbase, workdir);
   fprintf(fcache, "cimages_%s.tbl ipac_cluster\n%s/%s/cimages.tbl\n", idstr, workurlbase, workdir);
   fflush(fcache);
   ...
   while(1)
   {
	 ...
	 fprintf(fdag,"    <argument>\n      -X\n      <filename file=\"%s.fits\"/>\n      <filename file=\"p%s.fits\"/>\n      <filename file=\"region_%s.hdr\"/>\n    </argument>\n\n", fname, fname, idstr);
      ...
      fprintf(fdag,"    <uses file=\"%s.fits\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", fname);

      sprintf(key, "ID%06d", id);
      sprintf(val, "%s.fits",  fname);
      HT_add_entry(depends, key, val);

      fprintf(fcache,"%s.fits ipac_cluster\n%s/%s.fits\n",fname,urlbase,fname);

      fprintf(fdag,"    <uses file=\"p%s.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", fname);

      sprintf(key, "p%s.fits", fname);
      sprintf(val, "ID%06d", id);
      HT_add_entry(depends, key, val);

      fprintf(fdag,"    <uses file=\"p%s_area.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", fname);


      fprintf(fdag,"    <uses file=\"region_%s.hdr\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
   ...
   while(1)
   {
      ...
      fprintf(fdag, "    <argument>\n      -s <filename file=\"fit.%03d.%03d.txt\"/>\n      <filename file=\"p%s.fits\"/>\n      <filename file=\"p%s.fits\"/>\n      <filename file=\"diff.%03d.%03d.fits\"/>\n      <filename file=\"region_%s.hdr\"/>\n    </argument>\n\n", 
         cntr1, cntr2, plusname, minusname, cntr1, cntr2, idstr);

      fprintf(fdag, "    <uses file=\"fit.%03d.%03d.txt\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n",
         cntr1, cntr2);

      sprintf(key, "fit.%03d.%03d.txt",  cntr1, cntr2);
      sprintf(val, "ID%06d", id);
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"p%s.fits\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n",
         plusname);
      ...
      fprintf(fdag, "    <uses file=\"p%s_area.fits\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
         plusname);
      ...
      fprintf(fdag, "    <uses file=\"diff.%03d.%03d.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", cntr1, cntr2);

      sprintf(key, "diff.%03d.%03d.fits",  cntr1, cntr2);
      sprintf(val, "ID%06d", id);
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"diff.%03d.%03d_area.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", cntr1, cntr2);
   ...
   fprintf(fdag, "    <uses file=\"statfile_%s.tbl\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

   sprintf(key, "ID%06d", id);
   sprintf(val, "statfile.tbl");
   HT_add_entry(depends, key, val);

   fprintf(fdag, "    <uses file=\"fits.tbl\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n");
   ...
   while(1)
   {
      ...
      fprintf(fdag, "    <uses file=\"fit.%03d.%03d.txt\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
         cntr1, cntr2);

      sprintf(key, "ID%06d", id);
      sprintf(val, "fit.%03d.%03d.txt", cntr1, cntr2);
      HT_add_entry(depends, key, val);
   }
   ...
   fprintf(fdag, "    <uses file=\"pimages_%s.tbl\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

   sprintf(key, "ID%06d", id);
   sprintf(val, "pimages.tbl");
   HT_add_entry(depends, key, val);

   fprintf(fdag, "    <uses file=\"fits.tbl\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n");

   sprintf(key, "ID%06d", id);
   sprintf(val, "fits.tbl");
   HT_add_entry(depends, key, val);

   fprintf(fdag, "    <uses file=\"corrections.tbl\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n");
   ...
   while(1)
   {
      ...
      fprintf(fdag, "    <uses file=\"corrections.tbl\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n");

      sprintf(key, "ID%06d", id);
      sprintf(val, "corrections.tbl");
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"c%s.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
         fname);
      ...
      fprintf(fdag, "    <uses file=\"c%s_area.fits\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
         fname);
         ...
         for(j=0; j<numTilesY; ++j)
	 {
	    ...
	    ++cntr;
	       ...
	       i, j, idstr, i, j);
            ...
            fprintf(fdag, "    <uses file=\"cimages_%d_%d_%s.tbl\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\" temporaryHint=\"tmp\"/>\n",
	       i, j, idstr);
	    ...
	    HT_add_entry(depends, key, val);
            ...
            fprintf(fdag, "    <uses file=\"newcimages.tbl\" link=\"output\" dontRegister=\"true\" dontTransfer=\"true\"/>\n");
	    ...
	    while(1)
	    {
		  ...
		  break;
	       ...
	       fprintf(fdag, "    <uses file=\"%s.fits\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
	          fname);
   ...
   else
   {
	 ...
	 id, level, cntr);
      ...
      fprintf(fdag, "    <uses file=\"cimages_%s.tbl\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\" temporaryHint=\"tmp\"/>\n", idstr);
         ...
         for(j=0; j<numTilesY; ++j)
	 {
	    ...
	    ++cntr;
	       ...
	       i, j, i, j, idstr, i, j, idstr);
            ...
            fprintf(fdag, "    <uses file=\"newcimages_%d_%d.tbl\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n",
	       i, j);
	    ...
	    HT_add_entry(depends, key, val);
            ...
            fprintf(fdag, "    <uses file=\"region_%d_%d_%s.hdr\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n",
	       i, j, idstr);
	    ...
	    HT_add_entry(depends, key, val);
            ...
            fprintf(fdag, "    <uses file=\"tile_%d_%d_%s.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n",
	       i, j, idstr);
	    ...
	    HT_add_entry(depends, key, val);
            ...
            fprintf(fdag, "    <uses file=\"tile_%d_%d_%s_area.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n",
	       i, j, idstr);
	    ...
	    while(1)
	    {
		  ...
		  break;
	       ...
	       fprintf(fdag, "    <uses file=\"%s_area.fits\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
	          fname);
	    }
   ...
   else
   {
	 ...
	 id, level, cntr);
      ...
      fprintf(fdag, "    <uses file=\"newcimages.tbl\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n");
      ...
      fprintf(fdag, "    <uses file=\"mosaic_%s.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

      sprintf(key, "mosaic.fits");
      sprintf(val, "ID%06d", id);
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"mosaic_%s_area.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
   ...
   if(numTiles > 1)
   {
         ...
         for(j=0; j<numTilesY; ++j)
	 {
	    ...
	    ++cntr;
            ...
            fprintf(fdag, "    <uses file=\"tile_%d_%d_%s.fits\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n",
	       i, j, idstr);
	    ...
	    HT_add_entry(depends, key, val);
            ...
            fprintf(fdag, "    <uses file=\"shrunken_%d_%d_%s.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", 
	       i, j, idstr);
   ...
   else
   {
      ...
      fprintf(fdag, "    <uses file=\"mosaic_%s.fits\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

      sprintf(key, "ID%06d", id);
      sprintf(val, "mosaic.fits");
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"shrunken_%s.fits\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
   ...
   if(numTiles > 1)
   {
      ...
      fprintf(fdag, "    <uses file=\"slist_%s.tbl\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

      sprintf(key, "ID%06d", id);
      sprintf(val, "slist.tbl");
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"simages_%s.tbl\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
      ...
      for(i=0; i<numTilesX; ++i)
      {
         for(j=0; j<numTilesY; ++j)
	 {
	    fprintf(fdag, "    <uses file=\"shrunken_%d_%d_%s.fits\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", 
	       i, j, idstr);
      ...
      fprintf(fdag, "    <uses file=\"simages_%s.tbl\" link=\"input\" dontRegister=\"true\" dontTransfer=\"true\" temporaryHint=\"tmp\"/>\n", idstr);

      sprintf(key, "ID%06d", id);
      sprintf(val, "simages.tbl");
      HT_add_entry(depends, key, val);

      fprintf(fdag, "    <uses file=\"shrunken_%s.hdr\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
   ...
   fprintf(fdag, "    <uses file=\"shrunken_%s.fits\" link=\"input\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);

   sprintf(key, "ID%06d", id);
   sprintf(val, "shrunken.fits");
   HT_add_entry(depends, key, val);

   fprintf(fdag, "    <uses file=\"shrunken_%s.jpg\" link=\"output\" dontRegister=\"false\" dontTransfer=\"false\"/>\n", idstr);
