...
#ifndef MR_PARSE_COMMAND_HPP
#define MR_PARSE_COMMAND_HPP
...
bool parseCommand(int argCount, char *argList[], po::variables_map& vm) {
   po::options_description desc_cmdline ("Usage: "+std::string(MR_MASTER_EXE_NAME)+" [options]");
   try {
      ...
      if (vm.count("version")) {
         std::cout << std::endl << MR_MASTER_VERSION_FULL << std::endl;
         return false;
      }
      if (!vm.count("config")) {
