...
void DistributedJobRunner::Initialize(const std::string& config_file_path) {
 ...
 // uuid_ = std::string("MapReduce-eb6e444e-2004-49bc-852f-76551e7a8a87"); 
...
LOG_DEBUG << "Use specified " << output_prefix;
...
void DistributedJobRunner::createNewSession_(void) {
  ...
  try {
     ...
     //address.store_object<std::string>(serverURL_);
   ...
   try {
      while (binaryListIT != binaryList.end()) {
         ...
         && hostListIT->hostOS   == binaryListIT->targetOS) {
            ...
            env.push_back("SAGA_LOCATION=/home/smaddi2/saga152");
            env.push_back("SAGA_VERBOSE=0");
            env.push_back("LD_LIBRARY_PATH=/home/smaddi2/fix_mapreduce/source/mapreduce/:/usr/local/lib:/home/smaddi2/saga152//lib:/opt/bioscope/bioscope/lib/local:/usr/local/lib:/share/apps/bioscope/lib/local:");
            ...
            for ( int i = 0; i < JOBS_PER_SERVICE; i++ )
            {
              ...
              if (state != saga::job::Running) {
                ...
                std::string mss;
                mss =agentJob.get_job_id ();
                log->write(mss , MR_LOGLEVEL_INFO);
...
/*
        ...
        while (hostListIT != hostList.end()) {
               saga::job::service js(hostListIT->rmURL);
               saga::job::job     j = js.get_job (jobs_[successCounter].get_job_id());
               j.cancel ();
         }
