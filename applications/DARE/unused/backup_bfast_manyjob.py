...
if __name__ == "__main__":
   ...
   #parse dare conf file
    ...
    try:
        ...
        resource_list = []
        resource_list.append( {"gram_url" : gram_url, "walltime": walltime , "number_cores" : job_cores, "allocation" : allocation, "queue" : queue, "re_agent": re_agent, "working_directory": work_dir})
        ...
        for i in range(1, int(number_of_jobs)+1):
            ...
            jd.spmd_variation = "single"
            jd.arguments = ["match -f %s/%s.fa -A 1 -r %s/reads.%s.%s.fastq -n 8 -T %s > %s/bfast.matches.file.%s.%s.bmf"%( bfast_ref_genome_dir, refgnome, bfast_reads_dir, bfast_uuid, i, bfast_tmp_dir, bfast_matches_dir,refgnome,i)] 
            jd.working_directory = work_dir  
        ...
        for i in range(1, int(number_of_jobs)+1):
            ...
            jd.number_of_processes = "8"
            jd.spmd_variation = "single"
            jd.arguments = ["localalign -f %s/%s.fa -A 1 -m %s/bfast.matches.file.%s.%s.bmf > %s/bfast.aligned.file.%s.%s.baf"%(bfast_ref_genome_dir, refgnome, bfast_matches_dir,refgnome,i, bfast_localalign_dir,refgnome,i)]
            jd.working_directory = work_dir
        ...
        for i in range(1, int(number_of_jobs)+1):
            ...
            jd.number_of_processes = "8"
            jd.spmd_variation = "single"
            jd.arguments = ["postprocess -f %s/%s.fa -A 1 -i %s/bfast.aligned.file.%s.%s.baf > %s/bfast.reported.file.%s.%s.sam "%(bfast_ref_genome_dir, refgnome,bfast_localalign_dir,refgnome,i,bfast_reported_dir,refgnome,i)]
            jd.working_directory = work_dir
