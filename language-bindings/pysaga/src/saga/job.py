...
class JobDescription(Object,Attributes):
    ...
    type:  Vector String B{(need update)}
    value: -
    ...
    type:  String
    value: -
    notes: - the SPMD JSDL extension defines the value to be an URI. For simplicity, SAGA allows
    ...
    type:  Bool
    value: 'False'
    notes: - this implies that stdio streams will stay connected to the submitter after job submission, and during job execution.
           - if an implementation cannot handle interactive jobs, and this attribute is present, and 'True', the job creation MUST
             throw an 'IncorrectParameter' error with a descriptive error message.
    ...
    notes: - semantics as specified by JSDL
     ...
     type: String
     value: 'Default'
     notes: - can have the Values 'True', 'False', and 'Default'
     ...
     type:  Int
     value: -
     notes: - Could be viewed as a desired job start time, but that is up to the resource manager.
      ...
      type: Int
      value: -
      notes: - intended to provide hints to the scheduler for scheduling policies.
             - available in JSDL, DRMAA
             - semantics as defined in JSDL
      ...
      type:  String
      value: -
      notes: - While SAGA itself does not define the semantics of a "queue", many backend systems can make use of this attribute.
             - not supported by JSDL
      ...
      type: Vector String
      value: -
      notes: - format: URI (e.g. fax:+123456789, sms:+123456789, mailto:joe@doe.net).
             - available in DRMAA
             - not supported by JSDL    
    """
    ...
    def __init__(self):
        ...
        Raises: NotImplemented
                NoSuccess
        Note: a JobDescription is not associated with a Session, but can be used for JobServices from different sessions.
        """
...
class JobService(Object, Async):
    ...
    def __init__(self, session, url = ""):
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
        Notes: - 'url' defaults to an empty string - in that case, the implementation must perform a  resource discovery, 
                 or fall back to a fixed value, or find a valid rm contact in any other way. If that is not possible,
                 a BadParameter exception MUST be thrown, and MUST indicate that a rm contact string is needed.
               - if the rm identified by the url cannot be contacted (i.e. does not exist), a BadParameter exception is thrown.
    ...
    def create_job(self, jd):
        ...
        Pre Condition: - jd has an 'Executable' attribute.
        Post Condition: - job is in 'New' state
                        - jd is deep copied (no state is shared after method invocation)
                        - 'Owner' of the job is the id of the context used for creating the job.
        Raises: NotImplemented
                BadParameter
                PermissionDenied
                AuthorizationFailed
                ...
                Timeout
                NoSuccess
        Notes:  - calling run() on the job will submit it to the resource, and advance its state.
                - if the job description does not have a valid 'Executable' attribute, a BadParameter exception is thrown.
                - if the job description contains values which are outside of the allowed range, or cannot be parsed, 
                  or are otherwise invalid and not usable for creating a job instance, a BadParameter exception is thrown, 
    ...
    def run_job(self, commandline, host = ""):
        ...
        job: a job object representing the submitted job instance
        Post Condition: - job is in 'Running', 'Done' or 'Failed' state.
                        - 'Owner' of the job is the id of the context used for creating the job.
        Raises: NotImplemented
                BadParameter
                PermissionDenied
                ...
                Timeout
                NoSuccess
        Notes: - This is a convenience routine built on the create_job method, and is intended to simplify
                  the steps of creating a job_description, creating and running the job, and then
                  querying the standard I/O streams.
                - the I/O handles have to be passed to the call as references, in most languages, as calls
           ...
           - the job is guaranteed to run on the given host, or not at all.
           - the method is exactly equivalent to the sequence of (1) creation of a job_description with 'Executable' set to the values from the
             ...
             throw any of the exceptions which can occur in this sequence, with the semantics defined in
             the detailed description of the methods used in this sequence. No other exception are to be expected.
           - if 'host' is an empty string (the default), the implementation MUST choose an arbitrary host for execution.
           - stdin, stdout and stderr are guaranteed to contain/provide the complete standard I/O
    ...
    def list(self):
        ...
        Permissions: Query on jobs identified by the returned ids
        Raises: NotImplemented
                PermissionDenied
                AuthorizationFailed
                AuthenticationFailed
                Timeout
                NoSuccess
        Notes: - which jobs are viewable by the calling user context, and how long a resource manager keeps job information, are both implementation dependent.
    ...
    def get_job (self, job_id):
        ...
        Pre Condition: Job identified by job_id is managed by the job_service.
        Permissions: Query on the job.
        Raises: NotImplemented
                BadParameter
                DoesNotExist
                PermissionDenied
                ...
                Timeout
                NoSuccess
        Notes: - in general, only a job_service representing the resource manager which submitted the job may be
    ...
    def get_self (self):
        ...
        Pre Condition: the application is managed by the job_service.
        Post Condition: - job_self is, by definition, in 'Running' state.
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
        Notes: - in general, only a job_service representing the resource manager which started the application
                 which now calls get_self() can successfully return a job_self instance. However, other
                 job_services may succeed as well.
               - if a job_service cannot handle the calling job as a job_self instance, a 'NoSuccess' exception
...
class Job(Task, Attributes, Permissions): # Async inherited from Permissions
    """
    Attributes:
    name: JobID
    ...
    def get_job_description(self):
        ...
        Post Condition: jd is deep copied (no state is shared after method invocation)
        Permission: Query
        Raises: NotImplemented
                DoesNotExist
                PermissionDenied
                ...
                Timeout
                NoSuccess
        Notes:    - There are cases when the JobDescription is not available. This may include cases when
              ...
              persist the information). In that case, a 'DoesNotExist' exception is thrown, with a descriptive error message.
        """
    ...
    def get_stdin(self):
        ...
        Pre Condition: the job is interactive.
        Post Condition: the jobs standard input stream is available  at stdin.
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
     Notes:    - if the preconditions are met, but the standard input stream is not available for some
                 reason, a 'DoesNotExist' exception is thrown.
               - the stream MUST be valid until the job reaches a final state. If it is, for some reason,
                 disconnected earlier, a language typical error message is thrown (e.g. EBADF could be
                 returned on writes on that stream in C).
               - if the job is not interactive, e.g. it was  submitted with the 'Interactive' attribute set
                 to 'False', an 'IncorrectState' exception is thrown.
               - if the job is not in 'New' state, it is not guaranteed that the job did not receive other
    ...
    def get_stdout(self):
        ...
        Pre Condition: the job is interactive.
        Post Condition: the jobs standard output stream is available from stdout.
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
        Notes: - if the preconditions are met, but the standard output stream is not available for some
              reason, a 'DoesNotExist' exception is thrown.
            - the stream MUST be valid until the job reaches a final state. If it is, for some reason,
              disconnected earlier, a language typical error message is thrown (e.g. EBADF could be
            ...
            - if the job is not interactive, e.g. it was submitted with the 'Interactive' attribute set
              to 'False', an 'IncorrectState' exception is thrown.
            - if the job is not in 'New' state, it is not guaranteed that the job did write data on
    ...
    def get_stderr(self):
        ...
        Pre Condition: the job is interactive.
        Post Condition: the jobs standard error stream is available from stderr.
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
        Notes: - if the preconditions are met, but the standard error stream is not available for some
              reason, a 'DoesNotExist' exception is thrown.
            - the stream MUST be valid until the job reaches a final state. If it is, for some reason,
              disconnected earlier, a language typical error message is thrown (e.g. EBADF could be
              returned on reads on that stream in C).
    ...
    def suspend(self):
        ...
        Pre Condition: the job is in 'Running' state.
        Post Condition: the job is in 'Suspended' state.
        Permissions: Exec (job can be controlled).
        Raises: NotImplemented
                IncorrectState
                PermissionDenied
                AuthorizationFailed
    ...
    def resume(self):
        ...
        Pre Condition:the job is in 'Suspended' state.
        ...
        Throws: NotImplemented
                ...
                Timeout
                NoSuccess
        Note: if the job is not in 'Suspended' state, an 'IncorrectState' exception is thrown.
    ...
    def checkpoint(self):
        ...
        Throws: NotImplemented
                ...
                Timeout
                NoSuccess
       Notes:    - The semantics of checkpoint(), and the actions taken to initiate a checkpoint, are resource
              manager specific. In particular, the implementation or backend can trigger either
              a system level or an application level
            - if the job is not in 'Running' state, an 'IncorrectState' exception is thrown.
    ...
    def migrate(self, jd):
        ...
        Pre Condition: - the job is in 'Running' or 'Suspended' state.
        Post Condition: - the job keeps its state.
                        - jd is deep copied (no state is shared after method invocation)
                        - the job reflects the attributes specified in the job_description.
        ...
        Raises: NotImplemented
                ...
                Timeout
                NoSuccess
        Notes: - jd might indicate new resource requirements, for example.
               - the action of migration might change the job identifier within the resource manager.
               - ideally, the submitted job description was obtained by get_job_description(), and then
               ...
               - the method can call the same exceptions as the submit_job() and run() methods, in
    ...
    def signal(self, signum):
        ...
        Pre Condition:  - job is in 'Running' or 'Suspended' state.
        Post Condition: - the signal was delivered to the job.
        Permission:  Exec (job can be controlled).
        Throws: NotImplemented
                BadParameter
                IncorrectState
                ...
                Timeout
                NoSuccess
        Notes:  - there is no guarantee that the signal number specified is valid for the operating system
                  on the execution host where the job is running, or that the signal can be delivered.
                - if the signal number is not supported by the backend, a BadParameter exception is thrown.
                - if the job is not in 'Running' or 'Suspended' state, an 'IncorrectState' exception is thrown.
