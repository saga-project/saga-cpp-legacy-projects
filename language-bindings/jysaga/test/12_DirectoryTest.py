...
def checkObjectMethods(o):
    try:
        ...
        print "!!! get_id(): ", str(e.__class__),str(e)
    try:
        print "=== get_type:    "+ str(o.get_type() ) + \
            ", ObjectType.DIRECTORY is " +str(ObjectType.DIRECTORY)
    except Exception, e:
        print "!!! get_type:", str(e.__class__),str(e) 
    try:   
    ...
    try:    
        ...
        print "!!! clone(): " + str(e.__class__),str(e) 
    try:
        url = URL("/tmp")
        clone.is_dir(url)
    except NullPointerException, e:
        print "!!! clone.is_dir(): " + str(e.__class__),str(e)
...
print "========================================================================"
print "== Test of Directory                                                  =="
print "========================================================================"

task = None
temp_filename = "/tmp/11_DirectoryTest.py.temp"
temp_dir = "/tmp"
temp_dirname = "/tmp/11_DirectoryDIRECTORY"
temp_dirname_child = "/tmp/11_DirectoryDIRECTORY/child"
temp_filename_link = "/tmp/11_DirectoryTest.py.link"
...
try:
    ...
    os.mkdir(temp_dirname_child)
except:
    import java.io.File
    f = java.io.File(temp_dirname)
    f.mkdir() 
    f = java.io.File(temp_dirname_child)
    f.mkdir() 

       
direc = Directory(URL(temp_dirname))  

       
#__init__(self, name, session='default', flags=512)
#initialize the Directory object     

url_tempfile = URL(temp_filename)
temp = direc.get_size(url_tempfile)
...
temp = direc.is_file(url_tempfile)
if temp != True and temp != False:
    print "!!! is_file does not return True or False."
else:
    print "=== is_file:",temp
...
url_tempdir_child = URL(temp_dirname_child)
direc2 = direc.open_dir(url_tempdir_child)
if not isinstance(direc2, Directory):
    print "!!! open_dir does not return a Directory"
else:
    print "=== open_dir"
    
temp = direc2.is_file(url_tempfile)
if temp != True and temp != False:
    print "!!! is_file does not return True or False."
else:
    print "=== is_file on child:",temp
...
url_filename = URL(temp_filename)
aFile = direc.open(url_filename)
if not isinstance(aFile, File):
    print "!!! open does not return a File"
else:
    print "=== open"
temp = aFile.get_size()
if type(temp) is not int:
    print "!!! get_size on File does not return an int."
else:
    print "=== get_size of opened File:",temp
...
print "=== Inherited from NSDirectory ========================================="
url_parent = URL(temp_dir)
url_child = URL(temp_dirname)
nsd = Directory(url_parent)
...
sess = Session(False)
nse1 = Directory(url)
nse2 = Directory(url,sess)
nse3 = Directory(url, sess, Flags.CREATE)
