...
#TODO: if application managed buffer: update internal buffer between calls using the buffer
    def __init__(self, size = -1, data = None, **impl):
        ...
        Initialize an I/O buffer.
            ...
            - B{Call format: Buffer( size )}
                - B{Postcondition:}
                    - if "data" is not specified and size > 0, the buffer memory 
                        is allocated by the implementation.
                    - if "data" is not specified, the buffer memory is managed 
    ...
    def get_size(self):
        ...
        @Note: if the buffer was created with negative size with the second call 
            format of the __init__(), or the size was set to a negative value 
            with set_size(), this method returns '-1' if the buffer was not yet 
            used for an I/O operation.
        @note: if the buffer was used for a successfull I/O operation where data 
    ...
    def get_data(self):
        ...
        @return: buffer data to retrieve. Type depends on what type was used to 
            ...
            it returns a string.
        @rtype: char array, list or string
        @Raise NotImplemented:
        @raise DoesNotExist:
        @raise IncorrectState:
        ...
        try:
            if self.managedByImp is True:
                ...
                if type(self.applicationBuf) is list or\
                   type(self.applicationBuf) is array:
                    self.update_data()
                    return self.applicationBuf
                else:
                    raise NoSuccess, "self.applicationBuf is not a array or a "\
                                    +"list. Internal inconsistincy."
        except org.ogf.saga.error.SagaException, e:
           raise self.convertException(e)
...
#TODO: Use java-byte/C++-signed char, or unsigned char??
    ...
    def close(self, timeout = -0.0):
        ...
        @type timeout: float
        @PostCondition: any operation on the object other than close() or 
            __del__ will cause an 'IncorrectState' exception.
        @raise NotImplemented:
        @note: if the current data memory is managed by the implementation, it 
            is freed.
        @note: if the current data memory is managed by the application, it is 
            not accessed anymore by the implementation after this method 
            returns.
        @note: if close() is implicitly called in  __del__(), it will never 
            raise an exception.
        ...
        if type(timeout) is not float and type(timeout) is not int:
            raise BadParameter, "Parameter timout is wrong type. Type: "\
                                 + str(type(timeout))
        try:
            if timeout > 0:
    ...
    def clone(self):
        ...
        @Note: that method is overloaded by all classes which implement 
            L{Object}, and returns a deep copy of the respective class type.
