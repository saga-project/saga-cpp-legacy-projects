...
\subsection*{Unit I (15 min)} \textbf{A minimal SAGA application.} Every application that uses just a single SAGA call is considered a SAGA application, since it triggers the whole stack. SAGA is not a framework. It doesn’t impose a specific programming model or way to use it  (like, e.g. MPI). Look at it as a TOOLBOX for distributed computing.
...
SAGA_LIB          = my_saga_based_library 
...
\item All of these exist in the full SAGA installation (Remember: http://saga.cct.lsu.edu is your source of information for all things SAGA).
