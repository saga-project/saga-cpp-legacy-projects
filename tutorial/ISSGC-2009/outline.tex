...
{\it Don't forget to set the value of ``XX''}. The server also hosts a SAGA advert DB (PostgreSQL). You will use it in one of the exercises to store some data. To make sure that you don't mess with each other's data, we created directories for each participant - make sure that you can access yours:
...
\begin{verbatim}
http://issgc-server-01.polytech.unice.fr/saga/
\end{verbatim}
...
\subsection*{Unit III } \textbf{Running a SAGA application.} 
...
in case something goes wrong. If set, SAGA will print detailed debug informations.
...
// line arguments (it's /bin/echo we're launching!). The master job (this one)
...
/tmp/student-package/sourcecode/depending_jobs ssh://user@134.59.140.163 ssh://user@134.59.140.164
\end{verbatim}}
...
// To make it more interesting we now use /usr/bin/bc to do some 
// calculations, where the result of the previous calculation is used as
...
\noindent Not surprisingly the code snippet above is independent of any application specific details and focusses on the assignment of workloads to workers, execution and then retrival. This specific approach adopted here relies heavily on the use of the Advert Service.  
...
\item All of these exist in the full SAGA installation (http://saga.cct.lsu.edu is your source of information for all things SAGA).  https://svn.cct.lsu.edu/repos/saga-projects/tutorial/ISSGC-2009/student-package/
