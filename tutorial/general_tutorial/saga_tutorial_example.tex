...
\subsubsection*{Unit II (15 min)} \textbf{Compiling and linking.} Like
...
required libraries can be found by the loader. The easiest way to do
that is to set the LD\_LIBRARY\_PATH (DYLD\_LIBRARY\_PATH on Mac OS)
if it isn't already set. E.g.:

%\jhanote{Fix the LD\_LIBRARY\_PATH below. Make Generic}

\begin{verbatim}
export LD_LIBRARY_PATH=/opt/saga-1.5.3-pre/lib:$LD_LIBRARY_PATH
\end{verbatim}

Furthermore, SAGA needs to know where its configuration files are and
where to find its middleware adaptors. E.g.: 

\begin{verbatim}
export SAGA_LOCATION=/opt/saga-1.5.3-issgc/
\end{verbatim}
...
Another environment variable that might come in handy is
\texttt{SAGA\_VERBOSE} in case something goes wrong. If set, SAGA will
print detailed debug information. E.g.:
...
This application is executed using the following command in the
sub-directory student-package/sourcecode from where you checked out
the code from SVN.
...
computational resource. There are multiple reasons this might be
required (see lecture notes). Here we will demonstrate this capability
using the \textbf{Hello distributed world!} example discussed in Unit
IV.  Instead of launching three jobs on three machines, we will launch
one job on one machine, which will then launch itself on another
machine, which in turn will do so onto yet another machine.

\jhanote{Andre: This needs updating...}
...
systems. For more details, see the code at the following:

\jhanote{Sharath: Please update and provide a link to the MapReduce
  documentationa here (ideally in PDF)}
